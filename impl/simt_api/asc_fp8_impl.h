...
#warning "asc_fp8_impl.h is an internal header file and must not be used directly. Functions or variables defined in this file maybe removed in the future. Please use "asc_fp8.h" and use public functions or variables defined in interface header files."
#endif

#if (__NPU_ARCH__ == 3101) || (__NPU_ARCH__ == 5102)

__SIMT_DEVICE_FUNCTIONS_DECL__ inline hifloat8x2_t __float22hif82_rna(const float2 x) {
    return __cvt_hifloat8x2_t<ROUND::A, RoundingSaturation::RS_DISABLE_VALUE>(x);
}

__SIMT_DEVICE_FUNCTIONS_DECL__ inline hifloat8x2_t __float22hif82_rna_sat(const float2 x) {
    return __cvt_hifloat8x2_t<ROUND::A, RoundingSaturation::RS_ENABLE_VALUE>(x);
}
...
__SIMT_DEVICE_FUNCTIONS_DECL__ inline float2 __hif822float2_rn_sat(const hifloat8x2_t x) {
    return __cvt_float2<ROUND::R, RoundingSaturation::RS_ENABLE_VALUE>(x);
}
...
__SIMT_DEVICE_FUNCTIONS_DECL__ inline float2 __hif822float2_rz_sat(const hifloat8x2_t x) {
    return __cvt_float2<ROUND::Z, RoundingSaturation::RS_ENABLE_VALUE>(x);
}
...
__SIMT_DEVICE_FUNCTIONS_DECL__ inline float2 __hif822float2_rd_sat(const hifloat8x2_t x) {
    return __cvt_float2<ROUND::F, RoundingSaturation::RS_ENABLE_VALUE>(x);
}
...
__SIMT_DEVICE_FUNCTIONS_DECL__ inline float2 __hif822float2_ru_sat(const hifloat8x2_t x) {
    return __cvt_float2<ROUND::C, RoundingSaturation::RS_ENABLE_VALUE>(x);
}
...
__SIMT_DEVICE_FUNCTIONS_DECL__ inline float2 __hif822float2_rna_sat(const hifloat8x2_t x) {
    return __cvt_float2<ROUND::A, RoundingSaturation::RS_ENABLE_VALUE>(x);
}
...
__SIMT_DEVICE_FUNCTIONS_DECL__ inline half2 __hif822half2_rz(const hifloat8x2_t x) {
    return __cvt_half2<ROUND::Z, RoundingSaturation::RS_DISABLE_VALUE>(x);
}
...
__asc_cvt_float2_to_fp8x2(const float2 x, const __asc_saturation_t saturate,
    ...
    if (saturate == __ASC_NOSAT) {
        if (fp8_interpretation == __ASC_E4M3) {
            float8_e4m3x2_t tmp = __cvt_float8_e4m3x2_t<ROUND::R, RoundingSaturation::RS_DISABLE_VALUE>(x);
            res = (__asc_fp8x2_storage_t)tmp.x;
            res = res | (__asc_fp8x2_storage_t(tmp.y) << 8U);
        } else {
            float8_e5m2x2_t tmp = __cvt_float8_e5m2x2_t<ROUND::R, RoundingSaturation::RS_DISABLE_VALUE>(x);
            res = (__asc_fp8x2_storage_t)tmp.x;
            res = res | (__asc_fp8x2_storage_t(tmp.y) << 8U);
        }
    } else {
        if (fp8_interpretation == __ASC_E4M3) {
            float8_e4m3x2_t tmp = __cvt_float8_e4m3x2_t<ROUND::R, RoundingSaturation::RS_ENABLE_VALUE>(x);
            res = (__asc_fp8x2_storage_t)tmp.x;
            res = res | (__asc_fp8x2_storage_t(tmp.y) << 8U);
        } else {
            float8_e5m2x2_t tmp = __cvt_float8_e5m2x2_t<ROUND::R, RoundingSaturation::RS_ENABLE_VALUE>(x);
            res = (__asc_fp8x2_storage_t)tmp.x;
            res = res | (__asc_fp8x2_storage_t(tmp.y) << 8U);
        }
    }
