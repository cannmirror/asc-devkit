...
__aicore__ inline void ReduceAnyImpl(const LocalTensor<T> &dst, const LocalTensor<T> &src, const LocalTensor<uint8_t> &sharedTmpBuffer,
    const uint32_t srcShape[], bool srcInnerPad)
{
    ...
    if constexpr (std::is_same_v<pattern, Pattern::Reduce::AR>) {
        ...
        ReduceRAImpl<T, MicroAPI::RegTraitNumOne,
            MicroAPI::Max<T, MicroAPI::MaskMergeMode::ZEROING, MicroAPI::RegTensor<T>>, isReuseSource>(
