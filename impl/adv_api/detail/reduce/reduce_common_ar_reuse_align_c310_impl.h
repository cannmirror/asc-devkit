...
__aicore__ inline void ReduceARCastfoldZero(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, uint32_t dimA, uint32_t dimR,
...
__aicore__ inline void ReduceARfoldOneToThree(MicroAPI::RegTensor<T, Trait>& vreg0,
...
__aicore__ inline void ReduceAROverVLImpl(__ubuf__ T *dstAddr, __ubuf__ T *srcAddr, __ubuf__ T *tmpAddr, uint32_t dimA, uint32_t dimR)
{
    ...
    uint16_t needInplaceReduceAdd = 0;

    if constexpr (isReuseSource) {
        if (needInplaceAdd == 1 && dimR < 2 * vlSize) {
            needInplaceAdd = 0;
            needInplaceReduceAdd = 1;
            foldZero = 0;
        }
    }
    ...
    __VEC_SCOPE__
    {
        ...
        if constexpr (isReuseSource) {
            ...
            for (uint16_t i = 0; i < needInplaceReduceAdd; i++) {
                uint32_t sreg0 = tailR;
                MicroAPI::RegTensor<T, Trait> vregTmp;
                mask = MicroAPI::UpdateMask<T, Trait>(sreg0);
                for (uint16_t loopA = 0; loopA < static_cast<uint16_t>(dimA); loopA++) {
                    DataCopy(vregMain, srcAddr + loopA * dimRAxis);
                    DataCopy(vregTail, srcAddr + loopA * dimRAxis + mainR);
                    Binaryfunc(vregTmp, vregMain, vregTail, mask);
                    Select(vregTmp, vregTmp, vregMain, mask);
                    ReduceARfoldOneToThree<T, Trait, Binaryfunc, Reducefunc>(vregTmp, vregMain, fullMask);
                    DataCopyUnAlign((__ubuf__ T *&)dstAddr, vregMain, uDst, 1);
                }
                MicroAPI::DataCopyUnAlignPost((__ubuf__ T *&)dstAddr, uDst, 0);
            }
        } else {
