...
{
    static_assert(SupportType<SrcType, int8_t>(),
        "This AscendAntiQuant only support int8 input dtype");
    static_assert(SupportType<OutputDataType, half>(),
        "This AscendAntiQuant only support f16 output dtype");
    ...
    const LocalTensor<scaleT> &offsetTensor, const AscendAntiQuantParam &para)
{
    ...
    static_assert(SupportType<dstT, half, float>(),
        "AscendAntiQuant only support half/float output dtype");
    static_assert(SupportType<scaleT, half, float>(),
        "AscendAntiQuant only support half/float scale/offset dtype");
    ...
    if constexpr (policy == AscendAntiQuantPolicy::PER_TOKEN) {
        static_assert(SupportType<srcT, int8_t>(),
            "AscendAntiQuant PerToken only support int8_t input dtype");
        ...
        static_assert(
            SupportType<srcT, int8_t>(),
            "AscendAntiQuant PerGroup only support int8_t input dtype");
        static_assert(
