 ...
 * \brief
 */
#ifndef DETAIL_MATH_CUMSUM_CUMSUM_COMMON_C310_IMPL_H
#define DETAIL_MATH_CUMSUM_CUMSUM_COMMON_C310_IMPL_H
...
__aicore__ inline void CumSumCopyWithCast(const LocalTensor<U> &dstTensor, const LocalTensor<T> &srcTensor, const uint32_t outter, const uint32_t inner)
{
...
template <typename T>
__aicore__ inline void CumSumCopyOut(const LocalTensor<T> &dstTensor, const LocalTensor<T> &srcTensor, const uint32_t dstOutter, const uint32_t srcInner)
{
    __local_mem__ T *src = (__local_mem__ T *)srcTensor.GetPhyAddr();
...
__aicore__ inline void TransposeCommonGather(__local_mem__ D *dstAddr, __local_mem__ T *srcAddr,
    ...
    uint32_t forLoop1, uint32_t forLoop2, uint32_t srcStride1, uint32_t srcStride2)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t j = 0; j < static_cast<uint16_t>(forLoop1); j++) {
            ...
            for (uint16_t k = 0; k < static_cast<uint16_t>(mainLoop); k++) {
                DataCopyGather(srcReg, srcAddr + j * srcStride1 + k * vlSize * srcStride2, indexReg, mainMask);
                MicroAPI::Interleave((MicroAPI::RegTensor<uint16_t> &)castReg,
                    (MicroAPI::RegTensor<uint16_t> &)tmpReg,
                    (MicroAPI::RegTensor<uint16_t> &)srcReg,
                    (MicroAPI::RegTensor<uint16_t> &)zeroReg);
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t j = 0; j < static_cast<uint16_t>(forLoop1); j++) {
            ...
            for (uint16_t k = 0; k < static_cast<uint16_t>(mainLoop); k++) {
                ...
                MicroAPI::DeInterleave((MicroAPI::RegTensor<uint16_t> &)castReg,
                        (MicroAPI::RegTensor<uint16_t> &)tmpReg,
                        (MicroAPI::RegTensor<uint16_t> &)vreg,
                        (MicroAPI::RegTensor<uint16_t> &)zeroReg);
...
__aicore__ inline void TransposeAB(const LocalTensor<D> &dstTensor, const LocalTensor<T> &srcTensor,
    uint32_t outter, uint32_t inner)
{
    ...
    TransposeCommonGather<D, T, MicroAPI::RegTraitNumOne, vlSize>(
        (__local_mem__ D *)dstTensor.GetPhyAddr(), (__local_mem__ T *)srcTensor.GetPhyAddr(), inner,
        outter, srcStride1, srcStride2);
}
} // namespace Internal
...
#endif  // DETAIL_MATH_CUMSUM_CUMSUM_COMMON_C310_IMPL_H
