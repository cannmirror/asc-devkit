...
__aicore__ inline decltype(auto) MakeTileLayout(
    const Coord& coord, const LayoutType& layout, const TileShape& tileShape)
{
    using OriginShape = Std::remove_cvref_t<decltype(layout.Shape())>;
    if constexpr (
        nesting_depth_v<TileShape> == nesting_depth_v<OriginShape> &&
...
__aicore__ inline decltype(auto) MakeTileLayout(
    const Coord& coord, const LayoutType& layout, const LocalTensor<TensorType>& tileTensor)
{
