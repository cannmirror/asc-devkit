...
static constexpr bool VerifyingMmadTemplate = (IsTileTensorV<T> && IsTileTensorV<U> && IsTileTensorV<S>);

template <typename T, typename U, typename S, typename V>
static constexpr bool VerifyingMmadWithBiasTemplate =
    (IsTileTensorV<T> && IsTileTensorV<U> && IsTileTensorV<S> && IsTileTensorV<V>);

template <const MmadTrait& trait = DEFAULT_MMAD_TRAIT, typename T, typename U, typename S, typename Params>
__aicore__ inline typename Std::enable_if<VerifyingMmadTemplate<T, U, S>, void>::type Mmad(
    const T& dst, const U& fm, const S& filter, const Params& params)
{
    constexpr Hardware dstPos = GetHardPos<T>();
    constexpr Hardware fmPos = GetHardPos<U>();
    constexpr Hardware filterPos = GetHardPos<S>();
    using Tensor2Tensor =
        typename MmadTensor2Tensor<dstPos, fmPos, filterPos, Hardware::MAX, CURRENT_ARCH_VERSION>::type;
...
__aicore__ inline typename Std::enable_if<VerifyingMmadWithBiasTemplate<T, U, S, V>, void>::type Mmad(
