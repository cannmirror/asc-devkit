        ...
        uint8_t cacheMode, bool reluEn, uint8_t unitFlag, bool isChannelSplit, bool nz2ndEn, bool nz2dnEn)
    {
        ...
        if constexpr (CURRENT_ARCH_VERSION == ArchVersion::V3510) {
            copy_matrix_cc_to_gm(
                dst, src, 0, nSize, mSize, dstStride, srcStride, cacheMode, 0, unitFlag,
...
private:
    ...
    __aicore__ inline static void CopyMatrixCcToUb(
        ...
        uint8_t dualDstCtl, bool reluEn, uint8_t unitFlag, bool subBlockId, bool nz2ndEn, bool nz2dnEn)
    {
        if ASCEND_IS_AIV {
            return;
        }
        if constexpr (CURRENT_ARCH_VERSION == ArchVersion::V3510) {
            copy_matrix_cc_to_ub(
                dst, src, 0, nSize, mSize, dstStride, srcStride, dualDstCtl, subBlockId, 0, unitFlag,
                static_cast<uint64_t>(quantPre), reluEn, false, nz2ndEn, static_cast<uint64_t>(QuantMode_post::NoConv),
...
private:
    ...
    template <typename T>
    __aicore__ inline static void SetParamsToRegister(
