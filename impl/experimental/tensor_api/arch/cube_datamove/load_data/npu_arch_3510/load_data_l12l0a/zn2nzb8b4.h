...
private:
        ...
        int16_t srcStride, uint16_t dstStride)
    {
        ...
        for (uint16_t idx = 0; idx < mLoop; ++idx) {
            ...
            LoadCbufToCaS43510::LoadData<trait>(
        ...
        int16_t srcStride, uint16_t dstStride)
    {
        ...
        for (uint16_t idx = 0; idx < mLoop; ++idx) {
            auto sliceDst = dst(MakeCoord(MakeCoord(0, 0), MakeCoord(0, idx)));
            LoadCbufToCa3510::LoadData<trait>(
                sliceDst, src, mStartPosition, kStartPosition, mStep, kStep, srcStride, dstStride);
    ...
    __aicore__ inline static void LoadDataImpl(const T& dst, const U& src)
    {
        ...
        auto srcStride =
            GetEleFromLayout<decltype(srcLayout), AttrInfo::STRIDE, AttrInfo::ROW, 1>(srcLayout) / STRIDE_UNIT;
        auto dstStride =
            GetEleFromLayout<decltype(dstLayout), AttrInfo::STRIDE, AttrInfo::COLUMN, 1>(dstLayout) / STRIDE_UNIT;
