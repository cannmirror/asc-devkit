...
__aicore__ inline typename Std::enable_if<VerifyingDataCopyTemplate<T, U>, void>::type DataCopy(
    const T& dst, const U& src)
{
    constexpr Hardware dstTPos = GetHardPos<T>();
    constexpr Hardware srcTPos = GetHardPos<U>();
    using Tensor2Tensor = typename DataCopyTensor2Tensor<dstTPos, srcTPos, CURRENT_ARCH_VERSION>::type;
...
__aicore__ inline typename Std::enable_if<VerifyingDataCopyTemplateWithCoord<T, U, Coord>, void>::type DataCopy(
    const T& dst, const U& src, const Coord& coord)
{
