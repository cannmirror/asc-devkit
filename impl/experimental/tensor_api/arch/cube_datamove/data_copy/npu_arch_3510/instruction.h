        ...
        uint8_t rightPaddingCnt, uint8_t cacheMode, uint64_t srcStride, uint32_t dstStride)
    {
        ...
        if constexpr (CURRENT_ARCH_VERSION == ArchVersion::V3510) {
            copy_gm_to_cbuf_align_v2(
                dst, src, 0, blockCount, blockLen, leftPaddingCnt, rightPaddingCnt, true, cacheMode, srcStride,
                dstStride);
        }
    }
};
        ...
        uint64_t loop4SrcStride, bool enableSmallC0)
    {
        ...
        if constexpr (CURRENT_ARCH_VERSION == ArchVersion::V3510) {
            ...
            copy_gm_to_cbuf_multi_nd2nz(
                dst, src, 0, loop1SrcStride, cacheMode, nValue, dValue, loop4SrcStride, enableSmallC0);
        }
    }
};
...
private:
    ...
    __aicore__ inline static void CopyL12FB(
        uint64_t dst, __cbuf__ T* src, uint16_t blockCount, uint16_t blockLen, uint16_t srcStride, uint16_t dstStride)
    {
        if ASCEND_IS_AIV {
            return;
        }
