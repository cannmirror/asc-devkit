...
struct FixpipeInfoParams {
    ...
    __aicore__ inline FixpipeInfoParams(const FixpipeParams<T>& intriParams, const uint8_t srcByteSize, const uint8_t dstByteSize)
    {
        // 存放tiling信息
...
__aicore__ inline void FixpipeL0C2L1Impl(__cbuf__ T* dst, __cc__ U* src, const FixpipeParams<U>& intriParams)
{
    if constexpr(std::is_same<T, int8_t>::value || std::is_same<T, int16_t>::value ||
        ...
        copy_matrix_cc_to_cbuf((__cbuf__ T*)dst, (__cc__ U*)src, 0,
            ...
            intriParams.eltwiseAntiqEnable, intriParams.loopEnhanceMergeEnable, intriParams.c0PadEnable,
            intriParams.postWinoEnable, intriParams.broadCastEnable, intriParams.nz2dnEnable);
    } else {
        copy_matrix_cc_to_cbuf_s4((__cbuf__ T*)dst, (__cc__ U*)src, 0,
            intriParams.nSize, intriParams.mSize, intriParams.dstStride,
            intriParams.srcStride, intriParams.preClipReluMode, intriParams.unitFlag,
            intriParams.quantParams.preQuantMode, intriParams.preReluMode, intriParams.channelSplitEnable,
            intriParams.nz2ndEnable, intriParams.quantParams.postQuantMode, intriParams.postReluMode,
...
__aicore__ inline void FixpipeL0C2GMImpl(__gm__ T* dst, __cc__ U* src, const FixpipeParams<U>& intriParams)
{
    if constexpr(std::is_same<T, int8_t>::value || std::is_same<T, int16_t>::value ||
        std::is_same<T, int32_t>::value) {
        copy_matrix_cc_to_gm((__gm__ T*)dst, (__cc__ U*)src, 0,
            intriParams.nSize, intriParams.mSize, intriParams.dstStride,
            intriParams.srcStride, intriParams.preClipReluMode, intriParams.unitFlag,
            intriParams.quantParams.preQuantMode, intriParams.preReluMode, intriParams.channelSplitEnable,
            intriParams.nz2ndEnable, intriParams.quantParams.postQuantMode, intriParams.postReluMode,
        ...
        copy_matrix_cc_to_gm_s4((__gm__ void*)dst, (__cc__ U*)src, 0,
            intriParams.nSize, intriParams.mSize, intriParams.dstStride,
            intriParams.srcStride, intriParams.preClipReluMode, intriParams.unitFlag,
            intriParams.quantParams.preQuantMode, intriParams.preReluMode, intriParams.channelSplitEnable,
            intriParams.nz2ndEnable, intriParams.quantParams.postQuantMode, intriParams.postReluMode,
...
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "Fixpipe not unsupported L0C to UB on this version!"); });
}
    ...
    const FixpipeParams<U>& intriParams)
{
    if constexpr ((!IsSameType<U, int32_t>::value) && (!IsSameType<U, half>::value)) {
        ...
        if (dstHWPos == Hardware::UB) {
            FixpipeL0C2UBImpl((__ubuf__ T*)dst.GetPhyAddr(), (__cc__ U*)src.GetPhyAddr(), intriParams);
        } else {
            FixpipeL0C2L1Impl((__cbuf__ T*)dst.GetPhyAddr(), (__cc__ U*)src.GetPhyAddr(), intriParams);
        }
    }
}
    ...
    uint16_t nIterIndex)
{
    ...
    if (GetBit(fixpipeInfo.quantCfg, QUANTPRE_SCALE_VECTOR_CFGBIT)) {
        ...
        // L1 -> FB
        uint16_t fbufBurstLen = deqDataSize / 64;  // burst length，单位64B
    ...
    if (GetBit(fixpipeInfo.quantCfg, PRERELU_SCALE_VECTOR_CFGBIT)) {
        ...
        uint32_t deqValueOffset = nIterIndex * fixpipeInfo.tiling.nSize;
        // L1 -> FB
        uint16_t fbufBurstLen = reluDataSize / 64;  // burst length，单位64B
        copy_cbuf_to_fbuf(deqTensorTempBuf, fixpipeInfo.cbufWorkspace + cbufOffset + deqValueOffset, 1, fbufBurstLen, 0, 0);
        AscendCUtils::FreeTemporaryFbBuffer<uint64_t>(deqTensorTempBuf);
    ...
    if (GetBit(fixpipeInfo.quantCfg, POSTREELU_SCALE_VECTOR_CFGBIT)) {
        ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "Fixpipe buffer not support ReLU_POST on this version!"); });
    }

    if (GetBit(fixpipeInfo.quantCfg, ELTWISEANTIQ_SCALE_VECTOR_CFGBIT)) {
        uint16_t antiqDataSize = DivCeil(calNSize * sizeof(uint32_t), 64) * 64;  // 搬移对齐到64，数据类型写死u32
        __fbuf__ uint64_t* deqTensorTempBuf =
            AscendCUtils::GetTemporaryFbBufferAddr<uint64_t>(frontAddrU32 + (AntiqAddr << 16), antiqDataSize / sizeof(uint32_t));
        uint32_t deqValueOffset = nIterIndex * fixpipeInfo.tiling.nSize;
        // L1 -> FB
        uint16_t fbufBurstLen = antiqDataSize / 64; // burst length，单位64B
        copy_cbuf_to_fbuf(deqTensorTempBuf, fixpipeInfo.cbufWorkspace + cbufOffset + deqValueOffset, 1, fbufBurstLen, 0, 0);
        AscendCUtils::FreeTemporaryFbBuffer<uint64_t>(deqTensorTempBuf);
        fpcAddr = fpcAddr | (((uint64_t)deqTensorTempBuf & 0xFFFF) >> (uint64_t)6) << 32;  // 右移6位，地址也in unit of 64B
        cbufOffset += fixpipeInfo.totalN * sizeof(uint32_t) / 8;
    }
    set_fpc(fpcAddr);
}
...
__aicore__ inline void FixpipeL0C2GMImplN(const GlobalTensor<T> &dst, const LocalTensor<U> &src,
    const FixpipeInfoParams<U>& fixpipeInfo, const FixpipeParams<U> &intriParams, uint16_t calNSize, uint16_t nIterIndex)
{
    ...
    FixpipeL0C2GMImpl((__gm__ T*)(dst.GetPhyAddr() + dstOffset), (__cc__ U*)(src.GetPhyAddr() + srcOffset),
    ...
    const FixpipeInfoParams<U>& fixpipeInfo, const FixpipeParams<U> &intriParams, uint16_t calNSize, uint16_t nIterIndex)
{
    ...
    uint32_t srcOffset = nIterIndex * fixpipeInfo.srcOffset;
    uint32_t dstOffset = nIterIndex * fixpipeInfo.dstOffset;
    FixpipeL0C2UBImpl((__ubuf__ T*)(dst.GetPhyAddr() + dstOffset), (__cc__ U*)(src.GetPhyAddr() + srcOffset),
        intriParams, fixpipeInfo);
}
    ...
    const FixpipeInfoParams<U>& fixpipeInfo, const FixpipeParams<U> &intriParams, uint16_t calNSize, uint16_t nIterIndex)
{
    ...
    uint32_t srcOffset = nIterIndex * fixpipeInfo.srcOffset;
    uint32_t dstOffset = nIterIndex * fixpipeInfo.dstOffset;
    FixpipeL0C2L1Impl((__cbuf__ T*)(dst.GetPhyAddr() + dstOffset), (__cc__ U*)(src.GetPhyAddr() + srcOffset),
        intriParams, fixpipeInfo);
}
    ...
    const LocalTensor<uint64_t> &cbufWorkspace, const FixpipeParams<U> &intriParams)
{
    if constexpr ((!IsSameType<U, int32_t>::value)&&(!IsSameType<U, half>::value)) {
        ...
        if (fixpipeInfo.tiling.tailNSize > 0) {
            FixpipeL0C2GMImplN(dst, src, fixpipeInfo, intriParams, fixpipeInfo.tiling.tailNSize, fixpipeInfo.tiling.nIterNum);
        }
    }
}
    ...
    const LocalTensor<uint64_t>& cbufWorkspace, const FixpipeParams<U>& intriParams)
{
    if constexpr ((!IsSameType<U, int32_t>::value) && (!IsSameType<U, half>::value)) {
        ...
        if (dstHWPos == Hardware::UB) {
            ...
            // 尾块多搬一次
            if (fixpipeInfo.tiling.tailNSize > 0) {
                FixpipeL0C2UBImplN(dst, src, fixpipeInfo, intriParams, fixpipeInfo.tiling.tailNSize, fixpipeInfo.tiling.nIterNum);
            }
        } else {
            // 整块正常搬移
            for (uint16_t i = 0; i < fixpipeInfo.tiling.nIterNum; ++i) {
            ...
            // 尾块多搬一次
            if (fixpipeInfo.tiling.tailNSize > 0) {
                FixpipeL0C2L1ImplN(dst, src, fixpipeInfo, intriParams, fixpipeInfo.tiling.tailNSize, fixpipeInfo.tiling.nIterNum);
            }
        }
    }
}
...
template <typename T, typename U, const FixpipeConfig &config>
__aicore__ inline void FixpipeL0C2L1Impl(__cbuf__ T* dst, __cc__ U* src, __cbuf__ uint64_t* cbufWorkspace,
...
template <typename T, typename U, const FixpipeConfig &config>
__aicore__ inline void FixpipeL0C2GMImpl(__gm__ T* dst, __cc__ U* src, __cbuf__ uint64_t* cbufWorkspace,
    const FixpipeParamsV220 &intriParams)
{
    ASCENDC_ASSERT(false,
        { KERNEL_LOG(KERNEL_ERROR, "unsupported fixpipe"); });
