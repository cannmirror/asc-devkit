 ...
 * \brief AscendC l310 eff support reduce api.
 */
...
#define CONTINUOUS_MODE_REDUCE_VF(reducefunc, vregType, pltType, dstStrideOffset) \
        ASCENDC_ASSERT((false), {                                                 \
                KERNEL_LOG(KERNEL_ERROR,                                          \
                    " vector calculate is not support on this version!");             \
            })
...
{
    ASCENDC_ASSERT((false), "GetReduceMaxMinCount is not supported on this version");
}
...
{
    ASCENDC_ASSERT((false), "GetAccVal is not supported on this version");
    return 0;
}
} // namespace AscendC
