 ...
 * \brief AscendC l310 eff support common api.
 */
...
__aicore__ inline void SoftSyncAllImpl(__gm__ int32_t* gmWorkspaceAddr, __ubuf__ int32_t* ubWorkspaceAddr,
    const int usedCores)
{
    ASCENDC_ASSERT((false), { KERNEL_LOG(KERNEL_ERROR, "unsupported software syncAll in this version"); });
}

template<bool isAIVOnly = true>
__aicore__ inline void SyncAllImpl()
{
    ASCENDC_ASSERT((false), { KERNEL_LOG(KERNEL_ERROR, "unsupported hardware syncAll in this version"); });
}
...
{
    ASCENDC_ASSERT((false), "GetICachePreloadStatusImpl is not supported on this version");
    return 0;
}
...
{
    ASSERT(0 && "unsupported software syncAll in this version");
}
} // namespace AscendC
