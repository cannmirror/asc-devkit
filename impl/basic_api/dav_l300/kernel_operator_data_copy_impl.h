    ...
    const uint32_t blockLen, const uint32_t srcStride, const uint32_t dstStride)
{
    ...
    copy_ubuf_to_gm_align((__gm__ T*)dst, (__ubuf__ T*)src, 0, blockCount, burstLength, srcStride300,
        dstStride300);
}
...
__aicore__ inline void DataCopyUB2L1ND2NZImpl(__cbuf__ T* dst, __ubuf__ T* src, const Nd2NzParams& intriParams)
{
    ...
    ASCENDC_DEBUG_ASSERT(((srcDValue * sizeof(T)) % BYTE_32_ALIGN == 0), "dValue should be 32B aligned \n");
    ASCENDC_DEBUG_ASSERT(((dstNzMatrixStride * sizeof(T)) % BYTE_32_ALIGN == 0), "dValue should be 32B aligned \n");
