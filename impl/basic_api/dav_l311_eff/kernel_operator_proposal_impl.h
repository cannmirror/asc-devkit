...
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported Vbitsort on this version"); });
}
...
__aicore__ inline void Vmrgsort4Cal(__ubuf__ T* dst, __ubuf__ T* addrArray[MRG_SORT_ELEMENT_LEN], uint64_t src1,
    uint64_t config)
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported Vmrgsort on this version"); });
}

template <typename T>
__aicore__ inline void VconcatCal(__ubuf__ T* dst, __ubuf__ T* src, const ProposalIntriParams& intriParams)
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported VCONCAT on this version"); });
}

template <typename T>
__aicore__ inline void VextractCal(__ubuf__ T* dst, __ubuf__ T* src, const ProposalIntriParams& intriParams)
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "unsupported VEXTRACT on this version"); });
}
...
{
    ASCENDC_ASSERT((false), "GetMrgSortResult is not supported on ascend310b1");
}
} // namespace AscendC
