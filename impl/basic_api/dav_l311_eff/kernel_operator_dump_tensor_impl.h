...
__aicore__ inline void InitDumpImpl(bool mixFlag, uint32_t gmLen)
{
    ...
    *(__gm__ uint8_t *)((__gm__ uint8_t *)blockInfoStart + DUMP_META_CORE_TYPE_POS) = static_cast<uint8_t>(AscendC::AIC);
...
__aicore__ inline FixpipeInfoParams<half> GetFixpipeParamImpl(uint32_t dumpSize)
{
    ...
    FixpipeInfoParams<half> fixpipeInfo(fixpipeParams, sizeof(half));
...
__aicore__ inline void DumpTensorLocal2GMImpl(const LocalTensor<T>& src, uint32_t desc, uint32_t dumpSize)
{
    ...
    *((__gm__ uint32_t *)ptr->dumpAddr + DUMP_MESSAGE_HEAD_RSV_POS) = 0;
    ...
    if (srcHWPos == Hardware::UB) {
        ...
        ASCENDC_ASSERT((false), { KERNEL_LOG(KERNEL_ERROR, "dump tensor  unsupport dump tensor from l0c"); });
    }
    PipeBarrier<PIPE_ALL>();
...
__aicore__ inline void DumpTensorGM2GMImpl(const GlobalTensor<T>& src, uint32_t desc, uint32_t dumpSize)
{
    ...
    PipeBarrier<PIPE_ALL>();
    InitTmpTensor(tmp, (uint8_t)QuePosition::A1);
