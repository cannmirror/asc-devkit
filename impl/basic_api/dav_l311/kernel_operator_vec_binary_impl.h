...
__aicore__ inline void MulAddDst(S &dstReg, V &srcReg0, V &srcReg1, MicroAPI::MaskReg &mask)
{
    ...
    const int32_t &count)
{
    ...
    if constexpr (sizeof(T) == 8) {
        ...
        const uint16_t repeatTimes = static_cast<uint16_t>(CeilDivision(count, repeatStride));
        __VEC_SCOPE__
        {
...
template <typename T, typename U>
__aicore__ inline void SubRelu(U &dstReg, U &srcReg0, U &srcReg1, MicroAPI::MaskReg &mask)
{
    ...
    const int32_t &count)
{
    ...
    if constexpr (sizeof(T) == 8) {
        ...
        constexpr uint16_t numPerRep = VECTOR_REG_WIDTH / sizeof(T);
        const uint16_t repeatTimes = static_cast<uint16_t>(CeilDivision(count, numPerRep));
        __VEC_SCOPE__
        {
