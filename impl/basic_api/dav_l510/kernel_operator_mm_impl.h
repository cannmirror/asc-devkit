...
#define FIXP_REG_ELT_OP_SUB (0b010)
...
__aicore__ inline void LoadData2DL12L0BCal(__cb__ T* dst, __cbuf__ T* src, const LoadData2DParamsV2& loadDataParam)
{
    ...
    if constexpr (sizeof(T) > 2) {
        ...
        load_cbuf_to_cb((__cb__ T *)dst, (__cbuf__ T *)src, loadDataParam.mStartPosition, loadDataParam.kStartPosition, loadDataParam.mStep,
...
__aicore__ inline void matmul(
    __cbuf__ void* c, __cbuf__ T* a, __cb__ U* b, __cbuf__ int32_t* bias, uint64_t bt_addr, uint64_t config)
{
    if constexpr ((IsSameType<T, int8_t>::value && IsSameType<U, int8_t>::value) ||
        (IsSameType<T, int16_t>::value && IsSameType<U, int8_t>::value) ||
        (IsSameType<T, half>::value && IsSameType<U, half>::value) ||
        (IsSameType<T, float8_e4m3_t>::value && IsSameType<U, float8_e4m3_t>::value) ||
        (IsSameType<T, float8_e4m3_t>::value && IsSameType<U, int8_t>::value)) {

        matmul_to_cbuf(c, a, b, bias, bt_addr, config);
    } else if constexpr (IsSameType<T, float8_e4m3_t>::value) {
        matmul_to_cbuf_e4m3s4(c, a, (__cb__ void*)b, bias, bt_addr, config);
    } else if constexpr (IsSameType<T, int8_t>::value) {
        matmul_to_cbuf_s8s4(c, a, (__cb__ void*)b, bias, bt_addr, config);
    } else {
        matmul_to_cbuf_s4s4(c, (__cbuf__ void*)a, (__cb__ void*)b, bias, bt_addr, config);
    }
}
    ...
    const MmadFixParams& mmadFixParams)
{
    ...
    config |= (((uint64_t)mmadFixParams.actPostEnable & 0x7) << 15); // new
    ...
    config |= (((uint64_t)mmadFixParams.postProcessBandwithCtrl & 0x1) << 39);
    config |= (((uint64_t)mmadFixParams.n & 0xffff) << 48);
    matmul((__cbuf__ void*)c, a, b, (__cbuf__  int32_t *)get_imm(0), 0, config); // 待修改，适配新的数据类型
}
 ...
 * ************************************************************************************************* */
template <typename T, typename U, typename S>
__aicore__ inline void ConvFixCal(__cbuf__ T* conv_dst, __cbuf__ U* feature_map, __cb__ S* weight,
    const ConvFixParams& convFixParams)
{
    uint64_t config = 0;
    config |= (((uint64_t)convFixParams.biasMatrixInitVal & 0x1) << 0);
    config |= (((uint64_t)convFixParams.biasMatrixBroadcast & 0x1) << 1);
    config |= (((uint64_t)convFixParams.preQuantMode & 0x1f) << 2);
    config |= (((uint64_t)convFixParams.preReluMode & 0x7) << 7);
    config |= (((uint64_t)convFixParams.postQuantMode & 0x1f) << 10);
    config |= (((uint64_t)convFixParams.actPostEnable & 0x7) << 15); // new
    config |= (((uint64_t)convFixParams.preClipReluMode & 0x1) << 18);
    config |= (((uint64_t)convFixParams.eltwiseOp & 0x7) << 20);
    config |= (((uint64_t)convFixParams.eltwiseAntiqEnable & 0x1) << 23);
    config |= (((uint64_t)convFixParams.eltwiseBroadcastEnable & 0x1) << 24);
    config |= (((uint64_t)convFixParams.lsbMask & 0x7) << 28);
    config |= (((uint64_t)convFixParams.dualSrcEnable & 0x1) << 34);
    config |= (((uint64_t)convFixParams.compactEnable & 0x1) << 35);
    config |= (((uint64_t)convFixParams.dependEnable & 0x1) << 36);
    config |= (((uint64_t)convFixParams.instrId & 0x3) << 37);
    config |= (((uint64_t)convFixParams.postProcessBandwithCtrl & 0x1) << 39);
    config |= (((uint64_t)convFixParams.n & 0xffff) << 48);
    convolution(conv_dst, feature_map, weight, (__cbuf__  int32_t *)get_imm(0), (uint64_t)0, config);
}

template <typename T, typename U, typename S>
__aicore__ inline void ConvFixCal(__cbuf__ T* conv_dst, __cbuf__ U* feature_map, __cb__ S* weight,
    uint64_t bias, const ConvFixParams& convFixParams)
{
    uint64_t config = 0;
    config |= (((uint64_t)convFixParams.biasMatrixInitVal & 0x1) << 0);
    config |= (((uint64_t)convFixParams.biasMatrixBroadcast & 0x1) << 1);
    ...
    if (convFixParams.biasMatrixBroadcast) {
        // enable bias broadcast, bias store in C2
        convolution(conv_dst, feature_map, weight, (__cbuf__  int32_t *)get_imm(0), bias, config);
    } else {
        // disable bias broadcast, bias store in C1
        convolution(conv_dst, feature_map, weight, (__cbuf__  int32_t*)bias, (uint64_t)0, config);
    }
}
 ...
 * ************************************************************************************************* */
template <typename T, typename U>
__aicore__ inline void PostProcCal(__cbuf__ T* postproc_dst, __cbuf__ U* feature_map, uint64_t config)
{
    postproc((__cbuf__ void*)postproc_dst, feature_map, config);
}
...
__aicore__ inline void Load3DSetPaddingCal(const LoadDataPaddingParam<T> &param)
{
    ...
    if constexpr (sizeof(T) == B16_BYTE_SIZE) {
        config = (uint16_t)GetScalarBitcodeValue((T)param.padValue);
    } else if constexpr (sizeof(T) == B8_BYTE_SIZE) {
        config = ((((uint16_t)param.padValue) & 0xFF) << 8) | ((uint16_t)param.padValue & 0xFF);
    } else {
        config = (((uint16_t)param.padValue & 0xFF) << 12) | (((uint16_t)param.padValue & 0xFF) << 8) |
            (((uint16_t)param.padValue & 0xFF) << 4) | (uint16_t(param.padValue) & 0xFF);
    }
    config |= (uint64_t)param.quantSch << 32;
    set_m_padding(config);
}
...
__aicore__ inline void Load3DSetPaddingCal(const T padValue)
{
    ...
    if constexpr (sizeof(T) == B16_BYTE_SIZE) {
        config = (uint16_t)GetScalarBitcodeValue((T)padValue);
    } else if constexpr (sizeof(T) == B8_BYTE_SIZE) {
        config = ((((uint16_t)padValue) & 0xFF) << 8) | ((uint16_t)padValue & 0xFF);
    } else {
        config = (((uint16_t)padValue & 0xFF) << 12) | (((uint16_t)padValue & 0xFF) << 8) |
            (((uint16_t)padValue & 0xFF) << 4) | ((uint16_t)padValue & 0xFF);
    }
    set_m_padding(config);
}
