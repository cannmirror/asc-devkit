 ...
 * \brief AscendC l310 support vector ternary scalar api.
 */
                                    ...
                                    const int32_t& count)
{
    __VEC_SCOPE__ {
        ...
        Duplicate(zero_vreg, (half)0, full_preg);
        uint32_t sreg = (uint32_t)count;
        int32_t dataNumPerRepeat = VECTOR_REG_WIDTH / sizeof(float);
        uint16_t repeatTime = CeilDivision(count, dataNumPerRepeat);
        for (uint16_t i = 0; i < (uint16_t)repeatTime; ++i) {
            preg = CreatePredicate<float>(sreg);
            DataCopy(src_vreg, src, i * dataNumPerRepeat);
            Interleave(src_vreg, tmp_vreg, src_vreg, zero_vreg);
            Cast<float, half, Mode::ZEROING, PartMode::EVEN>(cvt_vreg, src_vreg, preg);
            DataCopy(dst_vreg, dst, i * dataNumPerRepeat);
            Axpy(dst_vreg, cvt_vreg, (float)scalarValue, preg);
            DataCopy(dst, dst_vreg, i * dataNumPerRepeat, preg);
        }
    }
}
