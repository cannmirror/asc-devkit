...
__aicore__ inline void CubeResGroupHandle<CubeMsgType>::AssignQueue(uint8_t queueIdIn)
{
    ...
    if ASCEND_IS_AIV {
        ...
        ASCENDC_DEBUG_ASSERT((aivSubgroupID < aivNumForCurAic),
            KERNEL_LOG(KERNEL_ERROR, "aivSubgroupID (%u) must be less than aivNumForCurAic (%u)",
            aivSubgroupID,
            aivNumForCurAic));
...
__aicore__ inline void CubeResGroupHandle<CubeMsgType>::SetSkipMsg(uint8_t skipCnt)
{
    if ASCEND_IS_AIC {
        ...
        ASCENDC_DEBUG_ASSERT((aivWork < aivNumForCurAic),
            KERNEL_LOG(KERNEL_ERROR, "aivWork + skipCnt(%u) is %u, it cannot be larger than aivNumForCurAic (%u)",
            skipCnt,
            aivWork,
            aivNumForCurAic));
        msgPos += skipCnt;
...
{
    ASCENDC_DEBUG_ASSERT(
        (sizeof(CubeMsgType) % 64 == 0), KERNEL_LOG(KERNEL_ERROR, "CubeMsgType Size is %u must be aligned to 64B.", sizeof(CubeMsgType)));
