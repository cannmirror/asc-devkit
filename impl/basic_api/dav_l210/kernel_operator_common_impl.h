 ...
 * \brief AscendC l210 support common api.
 */
...
__aicore__ inline void SetSysWorkspace(GM_ADDR workspace)
{
    ...
    ASCENDC_ASSERT((workspace != nullptr) && "workspace can not be nullptr");
#else
...
__aicore__ inline void SoftSyncAllImpl(__gm__ int32_t* gmWorkspaceAddr, __ubuf__ int32_t* ubWorkspaceAddr,
    const int usedCores)
{
    ASCENDC_ASSERT((false) && "unsupported software syncAll on current device");
}

template<bool isAIVOnly = true>
__aicore__ inline void SyncAllImpl()
{
    ASCENDC_ASSERT((false) && "unsupported hardware syncAll on current device");
}

template <pipe_t pipe>
__aicore__ inline void NotifyEventImpl(uint16_t flagId)
{
    ASCENDC_ASSERT((false), "NotifyEvent is not supported on this device!");
}

template <uint8_t modeId, pipe_t pipe>
__aicore__ inline void WaitEventImpl(uint16_t flagId)
{
    ASCENDC_ASSERT((false), "WaitEvent is not supported on this device!");
}

template <atomic_type_t type, atomic_op_t op>
__aicore__ inline void SetStoreAtomicConfigImpl()
{
    ASCENDC_ASSERT((false), "SetStoreAtomicConfig is not supported on this device!");
}

__aicore__ inline int64_t GetStoreAtomicConfigImpl()
{
    ASCENDC_ASSERT((false), "GetStoreAtomicConfig is not supported on this device!");
    return 0;
}
...
__aicore__ inline void DataCachePreloadImpl(const GlobalTensor<uint64_t> &src, const T cacheOffset)
{
    if constexpr ((IsSameType<T, int16_t>::value) || (IsSameType<T, int64_t>::value)) {
        ...
        ASCENDC_ASSERT((false), "current data type is not supported in this device");
    }
}

__aicore__ inline int64_t GetICachePreloadStatusImpl()
{
    ASCENDC_ASSERT((false), "unsupport GetICachePreloadStatus in this device");
    return 0;
}
...
{
    ASCENDC_ASSERT((false), "unsupport this enableBit in this device");
}
} // namespace AscendC
