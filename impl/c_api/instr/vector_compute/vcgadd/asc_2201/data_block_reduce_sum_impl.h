 ...
 * \brief
 */
#ifndef IMPL_INSTR_VECTOR_COMPUTE_VCADD_ASC_2201_DATA_BLOCK_REDUCE_SUM_IMPL_H
#define IMPL_INSTR_VECTOR_COMPUTE_VCADD_ASC_2201_DATA_BLOCK_REDUCE_SUM_IMPL_H
...
template <typename T>
__aicore__ void data_block_reduce_sum_impl(__ubuf__ T* dst, __ubuf__ T* src, const asc_reduce_config& config)
{
    vcgadd(dst, src, static_cast<uint8_t>(config.repeat), static_cast<uint16_t>(config.dst_repeat_stride),
           static_cast<uint16_t>(config.src0_stride), static_cast<uint16_t>(config.src1_stride));
}
...
__aicore__ void data_block_reduce_sum_impl(__ubuf__ T* dst, __ubuf__ T* src, uint32_t count)
{
    ...
    data_block_reduce_sum_impl<T>(dst, src, CAPI_DEFAULT_REDUCE_CFG);
    set_mask_norm();
}
