 ...
 * \brief
 */
#ifndef IMPL_INSTR_VECTOR_COMPUTE_VCMIN_ASC_2201_DATA_BLOCK_REDUCE_MIN_IMPL_H
#define IMPL_INSTR_VECTOR_COMPUTE_VCMIN_ASC_2201_DATA_BLOCK_REDUCE_MIN_IMPL_H
...
template <typename T>
__aicore__ void data_block_reduce_min_impl(__ubuf__ T* dst, __ubuf__ T* src, const asc_reduce_config& config)
{
...
__aicore__ void data_block_reduce_min_impl(__ubuf__ T* dst, __ubuf__ T* src, uint32_t count)
{
    ...
    data_block_reduce_min_impl<T>(dst, src, CAPI_DEFAULT_REDUCE_CFG);
    set_mask_norm();
}
