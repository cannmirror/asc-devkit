...
__aicore__ inline void AddsImpl(RegT &dstReg, RegT &srcReg0, ScalarT scalar, MaskReg &mask)
{
    ...
    static_assert(SupportType<ScalarT, uint8_t, int8_t, uint16_t, int16_t, uint32_t, int32_t, half, float, bfloat16_t>(),
...
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "Rounds api is not supported on current device!"); });
}
