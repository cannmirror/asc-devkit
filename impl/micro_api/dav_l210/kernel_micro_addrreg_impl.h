...
namespace AscendC {
namespace MicroAPI {
// 注：Kirin9000s和后面版本的vag里的系数次序是相反的。
template <typename T>
__aicore__ inline AddrReg CreateAddrRegImpl(uint16_t index0, uint16_t stride0)
{
...
#endif
    if constexpr (sizeof(T) == 1) {
        return vag_b8(stride1, stride0);
    } else if constexpr (sizeof(T) == 2) {
        return vag_b16(stride1, stride0);
    }
    return vag_b32(stride1, stride0);
}

template <typename T>
__aicore__ inline AddrReg CreateAddrRegImpl(
    uint16_t index0, uint16_t stride0, uint16_t index1, uint16_t stride1, uint16_t index2, uint16_t stride2)
{
    static_assert(sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4,
        "CreateAddrReg only support type b8/b16/b32 on current device");
#if defined(__CCE_KT_TEST__) && __CCE_KT_TEST__ == 1
...
#endif
    if constexpr (sizeof(T) == 1) {
        return vag_b8(stride2, stride1, stride0);
    } else if constexpr (sizeof(T) == 2) {
        return vag_b16(stride2, stride1, stride0);
    }
    return vag_b32(stride2, stride1, stride0);
}
...
#endif
    if constexpr (sizeof(T) == 1) {
        return vag_b8(stride3, stride2, stride1, stride0);
    } else if constexpr (sizeof(T) == 2) {
        return vag_b16(stride3, stride2, stride1, stride0);
    }
    return vag_b32(stride3, stride2, stride1, stride0);
}
} // namespace MicroAPI
}  // namespace AscendC
