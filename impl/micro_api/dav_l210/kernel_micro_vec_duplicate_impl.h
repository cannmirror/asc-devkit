...
__aicore__ inline void DuplicateImpl(RegT &dstReg, T1 scalarValue)
{
    ...
    static_assert(SupportType<ActualT, int8_t, uint8_t, uint16_t, int16_t, uint32_t, int32_t, half, float>(),
        "current data type is not supported on current device!");
    vbr(dstReg, scalarValue);
}
...
__aicore__ inline void DuplicateImpl(RegT &dstReg, T1 scalarValue, MaskReg &mask)
{
    ...
    vdup(dstReg, scalarValue, mask, modeValue);
}
