 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_ATAN_ATAN_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_ATAN_ATAN_C310_IMPL_H
...
namespace AtanInternal {
constexpr MicroAPI::CastTrait atanCastTraitF162F32 = {
    MicroAPI::RegLayout::ZERO, MicroAPI::SatMode::UNKNOWN, MicroAPI::MaskMergeMode::ZEROING, RoundMode::UNKNOWN};
constexpr MicroAPI::CastTrait atanCastTraitF322F16 = {
...
template <typename T>
__aicore__ inline void Sign(MicroAPI::RegTensor<T>& dstReg, MicroAPI::RegTensor<T>& srcReg,
    MicroAPI::RegTensor<T>& denominator, MicroAPI::MaskReg preg)
{
    constexpr float kFpMax = sizeof(T) == sizeof(float) ? AtanInternal::ATAN_FP32_MAX : AtanInternal::ATAN_FP16_MAX;
    constexpr float kFpMin = sizeof(T) == sizeof(float) ? AtanInternal::ATAN_FP32_MIN : AtanInternal::ATAN_FP16_MIN;
...
// 1/(k*2+1)
__aicore__ inline void TaylorExpand(MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcReg,
    MicroAPI::RegTensor<float>& squareReg, const uint16_t expandLevel, MicroAPI::MaskReg preg)
{
    ...
    uint16_t COUNT_SIX = expandLevel == AtanInternal::TAYLOR_COUNT_SIX ? 1 : 0;
...
// (x-y)/(1+xy)
__aicore__ inline void AtanTransform(MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcReg,
    ...
    MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& castReg, MicroAPI::MaskReg preg)
{
    ...
    MicroAPI::Mins(clipReg, castReg, AtanInternal::MAX_INPUT_VALUE, preg);
    MicroAPI::Maxs(clipReg, clipReg, AtanInternal::MIN_INPUT_VALUE, preg);
    MicroAPI::Abs(absReg, clipReg, preg);
    // 1. x in (0, tan(pi/8))
    TaylorExpand(dstReg, absReg, squareReg, AtanInternal::TAYLOR_COUNT_FOUR, preg);
    ...
    TaylorExpand(tmpReg2, tmpReg, squareReg, AtanInternal::TAYLOR_COUNT_FOUR, preg);
    ...
    TaylorExpand(tmpReg, tmpReg2, squareReg, AtanInternal::TAYLOR_COUNT_SIX, preg);
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repeatTimes; ++i) {
            if constexpr (sizeof(T) == sizeof(half)) {
                ...
                MicroAPI::Cast<float, T, AtanInternal::atanCastTraitF162F32>(castReg, srcReg, preg);
            } else {
                MicroAPI::DataCopy(castReg, srcUb + i * B32_DATA_NUM_PER_REPEAT);
            }
            AtanCompute(dstReg, castReg, preg);
            if constexpr (sizeof(T) == sizeof(half)) {
                MicroAPI::Cast<T, float, AtanInternal::atanCastTraitF322F16>(srcReg, dstReg, preg);
    ...
    const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const uint32_t calCount)
{
    ...
    if constexpr (config.algo == defaultAtanConfig.algo) {
        ...
        AtanTaylorImpl(dstTensor, srcTensor, calCount);
    } else {
        ...
        AtanPolynomialImpl(dstTensor, srcTensor, calCount);
    }
}
...
#endif // AICORE_ADV_API_DETAIL_MATH_ATAN_ATAN_C310_IMPL_H