 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_COS_COS_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_COS_COS_C310_IMPL_H
...
constexpr MicroAPI::CastTrait cosCastTraitF16F32 = {
    MicroAPI::RegLayout::ZERO, MicroAPI::SatMode::UNKNOWN, MicroAPI::MaskMergeMode::ZEROING};
constexpr MicroAPI::CastTrait cosCastTraitF32F16 = {
...
__aicore__ inline void CosPolymonial(__ubuf__ T* dst, __ubuf__ T* src, uint32_t calCount, uint16_t repeat)
{
    ...
    for (uint16_t i = 0; i < repeat; i++) {
        ...
        if constexpr (std::is_same<T, half>::value) {
            ...
            MicroAPI::Cast<float, half, cosCastTraitF16F32>(srcTmp, srcReg, mask);
            CosPolynomialApproximation(dstTmp, srcTmp, xTmp, round, kpi, mask);
            MicroAPI::Cast<half, float, cosCastTraitF32F16>(dstReg, dstTmp, mask);
...
__aicore__ inline void CosPolynomialImpl(__ubuf__ T* dst, __ubuf__ T* src, uint32_t calCount)
{
    ...
    VF_CALL<CosPolymonial<T>>(dst, src, calCount, repeat);
}
} // namespace Cos
} // namespace MicroAPI
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t calCount)
{
    ...
    if constexpr (config.algo == CosAlgo::POLYNOMIAL_APPROXIMATION) {
        MicroAPI::Cos::CosPolynomialImpl(
            (__ubuf__ T*)dstTensor.GetPhyAddr(), (__ubuf__ T*)srcTensor.GetPhyAddr(), calCount);
...
#endif // AICORE_ADV_API_DETAIL_MATH_COS_COS_C310_IMPL_H
