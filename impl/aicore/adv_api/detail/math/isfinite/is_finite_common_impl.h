...
#ifndef AICORE_ADV_API_DETAIL_MATH_ISFINITE_IS_FINITE_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_ISFINITE_IS_FINITE_COMMON_IMPL_H
...
__aicore__ inline void IsFiniteImpl(const LocalTensor<U>& dst, const LocalTensor<T>& src, uint32_t calCount)
{
    ...
    static_assert(SupportType<T, float, half, bfloat16_t>(), "IsFinite do not support this type on current device");

    static_assert(
        SupportType<U, bool, float, half, bfloat16_t>(), "IsFinite do not support this output type on current device");
        ...
        for (uint16_t i = 0; i < repeatTimes; ++i) {
            ...
            if constexpr (IsSameType<T, float>::value) {
                MicroAPI::CompareScalar<uint32_t, CMPMODE::EQ>(
                    cmpMaskPINF, (MicroAPI::RegTensor<uint32_t>&)vSrcReg0, INF, preg);
                MicroAPI::CompareScalar<uint32_t, CMPMODE::EQ>(
                    cmpMaskNINF, (MicroAPI::RegTensor<uint32_t>&)vSrcReg0, NEG_INF, preg);
            } else if constexpr (IsSameType<T, half>::value) {
                MicroAPI::CompareScalar<uint16_t, CMPMODE::EQ>(
                    cmpMaskPINF, (MicroAPI::RegTensor<uint16_t>&)vSrcReg0, HALF_INF, preg);
                MicroAPI::CompareScalar<uint16_t, CMPMODE::EQ>(
                    cmpMaskNINF, (MicroAPI::RegTensor<uint16_t>&)vSrcReg0, HALF_NEG_INF, preg);
            } else if constexpr (IsSameType<T, bfloat16_t>::value) {
                MicroAPI::CompareScalar<uint16_t, CMPMODE::EQ>(
                    cmpMaskPINF, (MicroAPI::RegTensor<uint16_t>&)vSrcReg0, B_HALF_INF, preg);
                MicroAPI::CompareScalar<uint16_t, CMPMODE::EQ>(
                    cmpMaskNINF, (MicroAPI::RegTensor<uint16_t>&)vSrcReg0, B_HALF_NEG_INF, preg);
            }
            ...
            if constexpr (IsSameType<U, bool>::value) {
                if constexpr (IsSameType<T, float>::value) {
                    ...
                    MicroAPI::Select((MicroAPI::RegTensor<int8_t, MicroAPI::RegTraitNumOne>&)vDstReg0,
                        (MicroAPI::RegTensor<int8_t, MicroAPI::RegTraitNumOne>&)vReg0,
                        (MicroAPI::RegTensor<int8_t, MicroAPI::RegTraitNumOne>&)vReg1, cmpMaskReg);
