 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_CLAMP_CLAMP_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_CLAMP_CLAMP_COMMON_IMPL_H
...
template <typename T>
__aicore__ inline void GetMainParams(const uint32_t calCount, ClampParams& params, const uint32_t sharedTmpBufferSize)
{
...
template <typename T>
__aicore__ inline void GetTailParams(const uint32_t calcTail, ClampParams& params)
{
    if constexpr (sizeof(T) == sizeof(uint16_t)) {
...
{
    ClampParams params;
    ClampCompute<T, isReuseSource>(
        dstTensor, srcTensor, sharedTmpBuffer, scalar, calCount, CLAMPMODE::CLAMP_MAX, params);
}
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const T scalar, const uint32_t calCount)
{
    ClampParams params;
    ClampCompute<T, isReuseSource>(
        dstTensor, srcTensor, sharedTmpBuffer, scalar, calCount, CLAMPMODE::CLAMP_MIN, params);
}
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_MATH_CLAMP_CLAMP_COMMON_IMPL_H