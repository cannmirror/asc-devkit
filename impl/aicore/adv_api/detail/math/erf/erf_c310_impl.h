 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_ERF_ERF_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_ERF_ERF_C310_IMPL_H

#include "kernel_tensor.h"
#include "kernel_pop_stack_buffer.h"
#include "math/erf_utils.h"
...
__aicore__ inline void ErfPadeCompute(
    MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcReg, MicroAPI::MaskReg& mask)
{
...
__aicore__ inline void ErfSubsectionCompute(
    MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcReg, MicroAPI::MaskReg& mask)
{
    ...
    MicroAPI::Select(
        tmpF32Reg1, (MicroAPI::RegTensor<float>&)tmpU32Reg, (MicroAPI::RegTensor<float>&)tmpU32Reg1, cmpMask);
    MicroAPI::Duplicate(tmpU32Reg, ERF_P1[1], mask);
    MicroAPI::Duplicate(tmpU32Reg1, ERF_P2[1], mask);
    MicroAPI::Select(
        tmpF32Reg2, (MicroAPI::RegTensor<float>&)tmpU32Reg, (MicroAPI::RegTensor<float>&)tmpU32Reg1, cmpMask);
    ...
    MicroAPI::Select(tmpF32Reg2, (MicroAPI::RegTensor<float>&)tmpU32Reg, (MicroAPI::RegTensor<float>&)tmpU32Reg1,
        cmpMask);                                              // tmpF32Reg2: f11
    ...
    for (uint16_t i = 0; i < repeatTimes; ++i) {
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            ...
            MicroAPI::DataCopy<T, MicroAPI::StoreDist::DIST_PACK_B32>(
                dstUb + i * B32_DATA_NUM_PER_REPEAT, srcReg, mask);
        } else {
...
#endif // AICORE_ADV_API_DETAIL_MATH_ERF_ERF_C310_IMPL_H
