 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_LOG_LOG_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_LOG_LOG_C310_IMPL_H
...
namespace AscendC {
template <typename T>
__aicore__ inline void LogImpl(__ubuf__ T* dst, __ubuf__ T* src, const uint32_t& calCount)
{
    static_assert(
...
template <typename T>
__aicore__ inline void LogXImpl(__ubuf__ T* dst, __ubuf__ T* src, const uint32_t& calCount, const float LnXRec)
{
    static_assert(
        (std::is_same_v<T, half> || std::is_same_v<T, float>), "current data type is not supported on current device!");
...
#endif // AICORE_ADV_API_DETAIL_MATH_LOG_LOG_C310_IMPL_H
