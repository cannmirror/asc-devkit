 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_LOG_LOG_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_LOG_LOG_COMMON_IMPL_H
    ...
    const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const LocalTensor<uint8_t>& tmpTensor)
{
    ...
    // src->tmp
    Cast<float, T, false>(tmpTensor.ReinterpretCast<float>(), srcTensor, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
    ...
    PipeBarrier<PIPE_V>();
    Muls<float, false>(tmpTensor.ReinterpretCast<float>(), tmpTensor.ReinterpretCast<float>(),
        static_cast<float>(Ln2Reciprocal), MASK_PLACEHOLDER, 1, unaryParams);
    PipeBarrier<PIPE_V>();

    // tmp->dst
    Cast<T, float, false>(dstTensor, tmpTensor.ReinterpretCast<float>(), RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
...
#endif
#endif // AICORE_ADV_API_DETAIL_MATH_LOG_LOG_COMMON_IMPL_H
