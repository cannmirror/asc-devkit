 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_CUMSUM_CUMSUM_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_CUMSUM_CUMSUM_COMMON_IMPL_H
    ...
    LocalTensor<T> tempBuffer, const CumSumInfo& cumSumInfo)
{
    ...
    Cast<float, T, false>(floatTempBuffer, tempBuffer, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
    ...
    LocalTensor<float> tempBuffer, const CumSumInfo& cumSumInfo)
{
    ...
    for (uint32_t row = 1; row < cumSumInfo.inner; ++row) {
        Add<float, false>(tempBuffer[addOffset], tempBuffer[addOffset - alignOutter], tempBuffer[addOffset],
    ...
    LocalTensor<uint8_t>& sharedTmpBuffer, const CumSumInfo& cumSumInfo)
{
    if constexpr (sizeof(T) == sizeof(half)) {
        ...
        Cast<T, float, false>(dstTensor, tmpBuffer, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
            {1, 1, HALF_DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE});
        PipeBarrier<PIPE_V>();
        ...
        const BinaryRepeatParams binaryParams;
        for (uint32_t row = 1; row < cumSumInfo.outter; ++row) {
            Add<T, false>(dstTensor[row * cumSumInfo.inner], dstTensor[(row - 1) * cumSumInfo.inner],
...
#endif // AICORE_ADV_API_DETAIL_MATH_CUMSUM_CUMSUM_COMMON_IMPL_H
