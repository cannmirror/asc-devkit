 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_LGAMMA_LGAMMA_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_LGAMMA_LGAMMA_COMMON_IMPL_H
    ...
    LGammaParams& params, const uint32_t splitSize)
{
    ...
    // float-->half
    Cast<half, float, false>(dstTensor, params.tmp1, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
    ...
    const LocalTensor<float>& dst, const LocalTensor<float>& src, LGammaParams& params)
{
    ...
    // for nan
    SetVectorMask<float>(0, ConstCeil(params.splitSize, sizeof(uint16_t) * ONE_BYTE_BIT_SIZE));
    Not<uint16_t, false>(params.mask.ReinterpretCast<uint16_t>(), params.tmpMask2.ReinterpretCast<uint16_t>(),
        MASK_PLACEHOLDER, 1, params.unaryParams);
    Not<uint16_t, false>(params.tmpMask1.ReinterpretCast<uint16_t>(), params.tmpMask3.ReinterpretCast<uint16_t>(),
        MASK_PLACEHOLDER, 1, params.unaryParams);
    PipeBarrier<PIPE_V>();
    ...
    const LocalTensor<uint8_t>& tmp, const uint32_t calCount)
{
    ...
    for (uint32_t i = 0U; i < loopCount; ++i) { LgammaComputeImpl(dst[i * splitSize], src[i * splitSize], params); }
    if (calcTail > 0) {
...
#endif
#endif // AICORE_ADV_API_DETAIL_MATH_LGAMMA_LGAMMA_COMMON_IMPL_H
