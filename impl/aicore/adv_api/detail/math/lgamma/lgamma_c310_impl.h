 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_LGAMMA_LGAMMA_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_LGAMMA_LGAMMA_C310_IMPL_H
...
struct MulAddsParams {
    ...
    __aicore__ constexpr MulAddsParams(const float r0In, const float r1In, const float r2In, const float r3In,
        const float r4In, const float r5In, const float r6In, const float r7In, const float r8In, const float r9In) :
        r0(r0In),
        r1(r1In), r2(r2In), r3(r3In), r4(r4In), r5(r5In), r6(r6In), r7(r7In), r8(r8In), r9(r9In)
    {}
};
...
__aicore__ inline void LGamma0715(
    MicroAPI::RegTensor<float>& resReg, MicroAPI::RegTensor<float>& tmpReg, MicroAPI::MaskReg mask)
{
...
__aicore__ inline void LGamma153(
    MicroAPI::RegTensor<float>& resReg, MicroAPI::RegTensor<float>& tmpReg, MicroAPI::MaskReg mask)
{
...
__aicore__ inline void LGammaPositive(MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcReg,
    ...
    MicroAPI::MaskReg cmpMaskReg1, MicroAPI::MaskReg cmpMaskReg2, MicroAPI::MaskReg cmpMaskReg, MicroAPI::MaskReg mask)
{
    NotNumUnion notNum;
    notNum.i = F32_INF;
    ...
    MicroAPI::MaskReg cmpMaskReg2, MicroAPI::MaskReg cmpMaskReg, MicroAPI::MaskReg posMaskReg, MicroAPI::MaskReg mask)
{
    ...
    LGammaPositive(
        dstReg, srcReg, resReg, tmpReg, aReg, bReg, cReg, oneReg, cmpMaskReg1, cmpMaskReg2, cmpMaskReg, mask);

    // Cal the result for x < 0, select to dst
    LGammaNegative(dstReg, srcReg, resReg, tmpReg, aReg, bReg, cReg, infReg, oneS32Reg, cmpMaskReg1, cmpMaskReg2,
        cmpMaskReg, mask);
...
__aicore__ inline void LgammaComputeImpl(
    __local_mem__ T* dstUb, __local_mem__ T* srcUb, uint32_t sreg, uint16_t repeatTimes)
{
    ...
    for (uint16_t i = 0; i < repeatTimes; ++i) {
        ...
        if constexpr (IsSameType<T, half>::value) {
            ...
            LgammaComputeFloat(dstReg, castReg, resReg, tmpReg, aReg, bReg, cReg, oneReg, infReg, oneS32Reg,
                cmpMaskReg1, cmpMaskReg2, cmpMaskReg, posMaskReg, mask);
...
#endif // AICORE_ADV_API_DETAIL_MATH_LGAMMA_LGAMMA_C310_IMPL_H
