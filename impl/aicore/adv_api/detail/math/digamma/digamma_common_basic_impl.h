 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_DIGAMMA_DIGAMMA_COMMON_BASIC_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_DIGAMMA_DIGAMMA_COMMON_BASIC_IMPL_H
    ...
    const LocalTensor<uint8_t>& mask, const LocalTensor<float>& src, DigammaParams& params)
{
    ...
    SetVectorMask<float>(0, ConstCeil(params.splitSize, sizeof(uint16_t) * ONE_BYTE_BIT_SIZE));
    Not<uint16_t, false>(params.mask1.ReinterpretCast<uint16_t>(), params.mask1.ReinterpretCast<uint16_t>(),
        MASK_PLACEHOLDER, 1, params.unaryParams);
    Not<uint16_t, false>(params.mask2.ReinterpretCast<uint16_t>(), params.mask2.ReinterpretCast<uint16_t>(),
        MASK_PLACEHOLDER, 1, params.unaryParams);
    PipeBarrier<PIPE_V>();
    And<uint16_t, false>(mask.ReinterpretCast<uint16_t>(), params.mask1.ReinterpretCast<uint16_t>(),
        params.mask2.ReinterpretCast<uint16_t>(), MASK_PLACEHOLDER, 1, params.binaryParams);
...
#endif // AICORE_ADV_API_DETAIL_MATH_DIGAMMA_DIGAMMA_COMMON_BASIC_IMPL_H