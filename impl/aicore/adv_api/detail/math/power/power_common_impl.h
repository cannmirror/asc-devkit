 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_POWER_POWER_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_POWER_POWER_COMMON_IMPL_H
    ...
    const LocalTensor<float>& srcTensor1, const LocalTensor<uint8_t>& stackTensor, uint32_t calCount)
{
    ...
    for (uint32_t i = 0; i < loopCount; ++i) {
        PipeBarrier<PIPE_V>();
        CommonPowerF(dstTensor[i * splitSize], srcTensor0[i * splitSize], srcTensor1[i * splitSize], tmpScalar,
            powerParam, splitSize);
    }
    if (calcTail > 0) {
    ...
    const float& scalarValue, const LocalTensor<uint8_t>& stackTensor, uint32_t calCount)
{
    ...
    if (calcTail > 0) {
        SetVectorMask<float>(0, calcTail);
        CommonPowerF(dstTensor[loopCount * splitSize], srcTensor0[loopCount * splitSize], stackSrc1, tmpScalar,
            powerParam, calcTail);
    }
}
    ...
    const LocalTensor<float>& srcTensor1, const LocalTensor<uint8_t>& stackTensor, uint32_t calCount)
{
    ...
    if (calcTail > 0) {
        SetVectorMask<float>(0, calcTail);
        CommonPowerF(dstTensor[loopCount * splitSize], stackSrc0, srcTensor1[loopCount * splitSize], tmpScalar,
            powerParam, calcTail);
    }
}
...
#endif // AICORE_ADV_API_DETAIL_MATH_POWER_POWER_COMMON_IMPL_H