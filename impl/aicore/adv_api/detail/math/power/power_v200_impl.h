 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_POWER_POWER_V200_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_POWER_POWER_V200_IMPL_H
...
__aicore__ inline void GrepSignBit(const LocalTensor<uint8_t>& dst, const LocalTensor<float>& src,
    const LocalTensor<float>& tmpTensor1, const LocalTensor<float>& tmpTensor2, const UnaryRepeatParams& unaryParam,
    ...
    const UnaryRepeatParams& unaryParam, const BinaryRepeatParams& binaryParam, const uint32_t calCount)
{
    ...
    Duplicate<uint32_t, false>(tmpTensor.ReinterpretCast<uint32_t>(), lastZero, MASK_PLACEHOLDER, 1, DEFAULT_BLK_STRIDE,
    ...
    Cast<float, int32_t, false>(tmpTensor.ReinterpretCast<float>(), tmpTensor.ReinterpretCast<int32_t>(),
...
#endif // AICORE_ADV_API_DETAIL_MATH_POWER_POWER_V200_IMPL_H
