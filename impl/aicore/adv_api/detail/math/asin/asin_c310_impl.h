 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_ASIN_ASIN_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_ASIN_ASIN_C310_IMPL_H
#include "kernel_tensor.h"
#include "../math_constant_util.h"
#include "../../common/check.h"

namespace AscendC {
namespace AsinInternal {
constexpr MicroAPI::CastTrait ASIN_CAST_TRAIT_NONE = {
    MicroAPI::RegLayout::ZERO, MicroAPI::SatMode::SAT, MicroAPI::MaskMergeMode::ZEROING, RoundMode::CAST_NONE};

constexpr MicroAPI::CastTrait ASIN_CAST_TRAIT_FLOOR = {
    MicroAPI::RegLayout::ZERO, MicroAPI::SatMode::SAT, MicroAPI::MaskMergeMode::ZEROING, RoundMode::CAST_FLOOR};

constexpr MicroAPI::CastTrait ASIN_CAST_TRAIT_RINT = {
...
__aicore__ inline void AsinComputeVFF32(
    __local_mem__ T* dstUb, __local_mem__ T* srcUb, uint32_t calSize, uint16_t repeatTimes, uint16_t stride)
{
...
template <typename T, bool convertToAcos = false>
__aicore__ inline void AsinComputeVFF16(
    __local_mem__ T* dstUb, __local_mem__ T* srcUb, uint32_t calSize, uint16_t repeatTimes, uint16_t stride)
{
    MicroAPI::MaskReg mask;
...
__aicore__ inline void AsinCompute(const LocalTensor<T>& dst, const LocalTensor<T>& src, uint32_t calSize)
{
    ...
    if constexpr (IsSameType<T, half>::value) {
        VF_CALL<AsinInternal::AsinComputeVFF16<T, convertToAcos>>(dstUb, srcUb, calSize, repeatTimes, stride);
    } else if (IsSameType<T, float>::value) {
        VF_CALL<AsinInternal::AsinComputeVFF32<T, convertToAcos>>(dstUb, srcUb, calSize, repeatTimes, stride);
    }
}
...
#endif // AICORE_ADV_API_DETAIL_MATH_ASIN_ASIN_C310_IMPL_H
