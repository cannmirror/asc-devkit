 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_PHILOX_PHILOX_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_PHILOX_PHILOX_C310_IMPL_H
namespace AscendC {
    ...
    MicroAPI::RegTensor<uint32_t>& tmpCtr3, MicroAPI::MaskReg& pg)
{
    if constexpr (DstUnalign) {
        ...
        MicroAPI::DataCopyUnAlign<uint32_t, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
...
*/
__aicore__ inline void PhiloxRandomIndexCal(
    ...
    const PhiloxCounter& philoxCounter, const PhiloxRandomParams& params)
{
    ...
    if (params.row == 1 || params.stride == params.column) {
        ...
        if (params.column / PhiloxInternal::PHILOX_ONCE_COUNTER_NUM * sizeof(uint32_t) % ONE_BLOCK_SIZE == 0) {
            ...
            VF_CALL<PhiloxRandomMultiRowNoFuse<Rounds, T, false>>(
                dstUbStart, philoxKey, philoxCounter, params, strideCounterOneRow, mainIter, tailCount, hasTail);
        } else {
            // unalign pattern. each process element: column / PHILOX_ONCE_COUNTER_NUM
            VF_CALL<PhiloxRandomMultiRowNoFuse<Rounds, T, true>>(
                dstUbStart, philoxKey, philoxCounter, params, strideCounterOneRow, mainIter, tailCount, hasTail);
        }
    }
}
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_MATH_PHILOX_PHILOX_C310_IMPL_H
