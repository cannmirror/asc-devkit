 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_XOR_XOR_V300_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_XOR_XOR_V300_IMPL_H
    ...
    const LocalTensor<T>& src1Tensor, const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t calCount)
{
    ...
    if (tail != 0) {
        XorCalcSimplified(dstTensor[loopCount * stackSize], src0Tensor[loopCount * stackSize],
            src1Tensor[loopCount * stackSize], sharedTmpBuffer.ReinterpretCast<T>(), tail);
    }
}
} // namespace AscendC
#endif
