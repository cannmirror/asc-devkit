 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATH_SINCOS_SINCOS_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_MATH_SINCOS_SINCOS_C310_IMPL_H
...
constexpr MicroAPI::CastTrait castTraitI64F32 = {
    MicroAPI::RegLayout::ZERO, MicroAPI::SatMode::NO_SAT, MicroAPI::MaskMergeMode::ZEROING, RoundMode::CAST_ROUND};
constexpr MicroAPI::CastTrait castTraitF32I64 = {
    MicroAPI::RegLayout::UNKNOWN, MicroAPI::SatMode::NO_SAT, MicroAPI::MaskMergeMode::ZEROING, RoundMode::CAST_ROUND};
constexpr MicroAPI::CastTrait castTraitI32F32 = {
    MicroAPI::RegLayout::UNKNOWN, MicroAPI::SatMode::NO_SAT, MicroAPI::MaskMergeMode::ZEROING, RoundMode::CAST_ROUND};

template <typename T, typename U>
__aicore__ inline void AndScalar(
    MicroAPI::RegTensor<T>& dstReg, MicroAPI::RegTensor<U>& srcReg, T val, MicroAPI::MaskReg& mask)
{
...
__aicore__ inline void FMaf(
    MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcReg, float scalarValue, MicroAPI::MaskReg& mask)
{
...
__aicore__ inline void FMaf(MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcReg1, float scalarValue,
    float scalarValue2, MicroAPI::MaskReg& mask)
{
...
__aicore__ inline void ReinterpretedU32ToFloatAndCastToU32(
    MicroAPI::RegTensor<uint32_t>& dstReg, MicroAPI::RegTensor<uint32_t>& srcReg, MicroAPI::MaskReg& mask)
{
    ...
    MicroAPI::RegTensor<uint32_t>& regHi, MicroAPI::MaskReg& mask)
{
    ...
    MicroAPI::Mull(
        (MicroAPI::RegTensor<uint32_t>&)regPLow, (MicroAPI::RegTensor<uint32_t>&)regPHigh, regIa, regLo, mask);

    // step 13: p = (unsigned long long int)ia * mid + (p >> 32);
    MicroAPI::Mull((MicroAPI::RegTensor<uint32_t>&)tmpU64Reg.reg[0], (MicroAPI::RegTensor<uint32_t>&)tmpU64Reg.reg[1],
        regIa, regMid, mask);
    ...
    MicroAPI::MaskReg& mask)
{
    ...
    // step 15: q = (int)(p >> 62);
    MicroAPI::ShiftRights((MicroAPI::RegTensor<uint32_t>&)regQ, (MicroAPI::RegTensor<uint32_t>&)regPHigh,
        (int16_t)(Q_SHIFT_BITS - B32_BITS), mask);

    // step 16: p = p & 0x3fffffffffffffffULL;
    MicroAPI::Duplicate(tmpU32Reg, P_AND_COEFF1 >> B32_BITS, mask);
    MicroAPI::And((MicroAPI::RegTensor<uint32_t>&)regPHigh, (MicroAPI::RegTensor<uint32_t>&)regPHigh, tmpU32Reg, mask);
    ...
    MicroAPI::Select(
        (MicroAPI::RegTensor<uint32_t>&)regPHigh, tmpU32Reg, (MicroAPI::RegTensor<uint32_t>&)regPHigh, tmpMask);
...
__aicore__ inline void TrigRedSlowpathFComputeP(MicroAPI::RegTensor<uint32_t>& regPHigh,
    MicroAPI::RegTensor<uint32_t>& regPLow, MicroAPI::RegTensor<float>& srcReg,
...
__aicore__ inline void TrigRedSlowpathFComputeRI(MicroAPI::RegTensor<float>& dstRegR,
    MicroAPI::RegTensor<int32_t>& dstRegI, MicroAPI::RegTensor<uint32_t>& regPHigh,
    MicroAPI::RegTensor<uint32_t>& regPLow, MicroAPI::RegTensor<float>& srcReg, MicroAPI::MaskReg& mask)
{
...
__aicore__ inline void CosfPoly(
    MicroAPI::RegTensor<float>& dstReg, MicroAPI::RegTensor<float>& srcRegS, MicroAPI::MaskReg& mask)
{
...
__aicore__ inline void TrigRedFComputeRI(MicroAPI::MaskReg& tmpMask, MicroAPI::RegTensor<float>& dstRegR,
    MicroAPI::RegTensor<int32_t>& dstRegI, MicroAPI::RegTensor<uint32_t>& regPHigh,
    MicroAPI::RegTensor<uint32_t>& regPLow, MicroAPI::RegTensor<float>& srcRegA, MicroAPI::MaskReg& mask)
{
    constexpr float A_ABS_COEFF = 3.1415926535f * 0.25f;
    MicroAPI::RegTensor<float> tmpF32Reg;
    MicroAPI::RegTensor<int32_t> tmpI32Reg;
    ...
    __ubuf__ int32_t*& tmpBufferI, const uint32_t alignCount)
{
    ...
    for (uint16_t i = 0; i < 6; ++i) { tmpBuffer[i] = oneOverPiF[i]; }
...
template <typename T>
__aicore__ inline void TrigRedFPreProcessImpl(
    __ubuf__ float* tmpBufferR, __ubuf__ int32_t* tmpBufferI, __ubuf__ T* src, uint32_t calCount, uint16_t repeatTimes)
{
    ...
    for (uint16_t i = 0; i < repeatTimes; i++) {
        ...
        MicroAPI::DataCopy<T, MicroAPI::LoadDist::DIST_UNPACK_B16>(srcReg, src + i * B32_DATA_NUM_PER_REPEAT);
        MicroAPI::Cast<float, T, SinCosImpl::castTraitF16F32>(castReg, srcReg, mask);
        TrigRedFPreporcessForHalf(regR, regI, castReg, mask);

        MicroAPI::DataCopy(tmpBufferR + i * B32_DATA_NUM_PER_REPEAT, regR, mask);
        MicroAPI::DataCopy(tmpBufferI + i * B32_DATA_NUM_PER_REPEAT, regI, mask);
    }
}
    ...
    __ubuf__ T* src, __ubuf__ uint32_t* tmpBuffer, uint32_t calCount, uint16_t repeatTimes)
{
    ...
    MicroAPI::DataCopy(oneOverPiFReg, tmpBuffer);

    for (uint16_t i = 0; i < repeatTimes; i++) {
        MicroAPI::MaskReg mask = MicroAPI::UpdateMask<float>(calCount);
        MicroAPI::DataCopy(castReg, src + i * B32_DATA_NUM_PER_REPEAT);
        // a = a * 0.0f + a
        MicroAPI::Duplicate(tmpF32Reg, 0.0f, mask);
        ...
        MicroAPI::DataCopy((__ubuf__ float*)tmpBufferRegPHigh + i * B32_DATA_NUM_PER_REPEAT, regR, mask);
        MicroAPI::DataCopy(tmpBufferRegPLow + i * B32_DATA_NUM_PER_REPEAT, regI, mask);

        TrigRedFComputeP(selectMask, regPHigh, regPLow, castReg, oneOverPiFReg, mask);

        MicroAPI::DataCopy(tmpBufferRegPHigh + i * B32_DATA_NUM_PER_REPEAT, regPHigh, selectMask);
        MicroAPI::DataCopy(tmpBufferRegPLow + i * B32_DATA_NUM_PER_REPEAT, regPLow, selectMask);
    }
}
    ...
    __ubuf__ T* src, __ubuf__ uint32_t* tmpBuffer, uint32_t calCount, uint16_t repeatTimes)
{
    ...
    for (uint16_t i = 0; i < repeatTimes; i++) {
        MicroAPI::MaskReg mask = MicroAPI::UpdateMask<float>(calCount);
        MicroAPI::DataCopy(castReg, src + i * B32_DATA_NUM_PER_REPEAT);
        MicroAPI::DataCopy(regPHigh, tmpBufferRegPHigh + i * B32_DATA_NUM_PER_REPEAT);
        MicroAPI::DataCopy(regPLow, tmpBufferRegPLow + i * B32_DATA_NUM_PER_REPEAT);
        // a = a * 0.0f + a
        MicroAPI::Duplicate(tmpF32Reg, 0.0f, mask);
        MicroAPI::FusedMulDstAdd(castReg, tmpF32Reg, castReg, mask);
        ...
        MicroAPI::DataCopy((__ubuf__ float*)tmpBufferRegPHigh + i * B32_DATA_NUM_PER_REPEAT, regR, selectMask);
        MicroAPI::DataCopy((__ubuf__ int32_t*)tmpBufferRegPLow + i * B32_DATA_NUM_PER_REPEAT, regI, selectMask);
    }
}

template <typename T, int mode = 0>
__aicore__ inline void SCFCoreImpl(
    __ubuf__ T* dst, __ubuf__ float* tmpBufferR, __ubuf__ int32_t* tmpBufferI, uint32_t calCount, uint16_t repeatTimes)
{
    ...
    for (uint16_t i = 0; i < repeatTimes; i++) {
        MicroAPI::MaskReg mask = MicroAPI::UpdateMask<float>(calCount);
        MicroAPI::DataCopy(regR, tmpBufferR + i * B32_DATA_NUM_PER_REPEAT);
        MicroAPI::DataCopy(regI, tmpBufferI + i * B32_DATA_NUM_PER_REPEAT);
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            ...
            MicroAPI::DataCopy<T, MicroAPI::StoreDist::DIST_PACK_B32>(dst + i * B32_DATA_NUM_PER_REPEAT, srcReg, mask);
        } else {
            MicroAPI::DataCopy(dst + i * B32_DATA_NUM_PER_REPEAT, dstReg, mask);
        }
    }
}

template <typename T>
__aicore__ inline void SCFCoreImpl(__ubuf__ T* dstSin, __ubuf__ T* dstCos, __ubuf__ float* tmpBufferR,
    __ubuf__ int32_t* tmpBufferI, uint32_t calCount, uint16_t repeatTimes)
{
    MicroAPI::RegTensor<T> srcReg;
    MicroAPI::RegTensor<int32_t> regI;
    MicroAPI::RegTensor<float> regR;
    ...
    for (uint16_t i = 0; i < repeatTimes; i++) {
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            ...
            MicroAPI::DataCopy<T, MicroAPI::StoreDist::DIST_PACK_B32>(
                dstSin + i * B32_DATA_NUM_PER_REPEAT, srcReg, mask);
            MicroAPI::Cast<T, float, SinCosImpl::castTraitF32F16>(srcReg, dstRegCos, mask);
            MicroAPI::DataCopy<T, MicroAPI::StoreDist::DIST_PACK_B32>(
                dstCos + i * B32_DATA_NUM_PER_REPEAT, srcReg, mask);
        } else {
            MicroAPI::DataCopy(dstSin + i * B32_DATA_NUM_PER_REPEAT, dstRegSin, mask);
            MicroAPI::DataCopy(dstCos + i * B32_DATA_NUM_PER_REPEAT, dstRegCos, mask);
        }
    }
}
} // namespace SinCosImpl
...
__aicore__ inline void SinCosRadianReductionImpl(
    __ubuf__ T* dstSin, __ubuf__ T* dstCos, __ubuf__ T* src, __ubuf__ uint32_t* tmpBuffer, uint32_t calCount)
{
    static_assert(
        (std::is_same_v<T, half> || std::is_same_v<T, float>), "current data type is not supported on current device!");
    constexpr uint32_t oneRepSize = GetVecLen() / sizeof(float);
    ...
    VF_CALL<SinCosImpl::SCFCoreImpl<T>>(dstSin, dstCos, tmpBufferR, tmpBufferI, calCount, repeatTimes);
}

} // namespace AscendC

#endif // AICORE_ADV_API_DETAIL_MATH_SINCOS_SINCOS_C310_IMPL_H
