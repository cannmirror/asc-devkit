 ...
 */
#ifndef AICORE_ADV_API_DETAIL_SELECT_SELECTWITHBYTESMASK_SELECTWITHBYTESMASK_V200_IMPL_H
#define AICORE_ADV_API_DETAIL_SELECT_SELECTWITHBYTESMASK_SELECTWITHBYTESMASK_V200_IMPL_H
    ...
    const SelectWithBytesMaskShapeInfo& info, const uint32_t tmpBufferOffset, const uint32_t loopSize)
{
    if (info.srcLastAxis == info.maskLastAxis) {
        SelectWithBytesMaskLoopImpl<T, U, reverse>(
            dst, src0, src1, mask, sharedTmpBuffer[tmpBufferOffset], loopSize, src0.GetSize(), 0, 0);
    } else {
...
#endif // AICORE_ADV_API_DETAIL_SELECT_SELECTWITHBYTESMASK_SELECTWITHBYTESMASK_V200_IMPL_H
