 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_NORMALIZATION_NORMALIZE_NORMALIZE_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_NORMALIZATION_NORMALIZE_NORMALIZE_COMMON_IMPL_H
...
{
    // cast FP16 -> FP32
    Cast<float, U, false>(castRes, src, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
    ...
    const LocalTensor<T>& srcX, const NormalizeTmpTensor<float>& tmpTensor, const NormalizePara& para, const uint32_t N)
{
    ...
    if constexpr (IsSameType<T, float>::value) {
        Adds<float, false>(tmpTensor.tempTensorA, srcX, (float)0, MASK_PLACEHOLDER, 1,
            {1, 1, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE});
    } else {
        Cast<float, T, false>(tmpTensor.tempTensorA, srcX, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
            {1, 1, DEFAULT_REPEAT_STRIDE, HALF_DEFAULT_REPEAT_STRIDE});
    }
    PipeBarrier<PIPE_V>();
}
    ...
    const UnaryRepeatParams& unaryParams, const BinaryRepeatParams& binaryParams, const uint32_t N)
{
    ...
    if constexpr (IsSameType<T, float>::value) {
        ...
        Cast<T, float, false>(output, srcxFP32, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
...
{
    CHECK_FUNC_HIGHLEVEL_API(Normalize, (U, T, isReuseSource, config),
...
#endif // AICORE_ADV_API_DETAIL_NORMALIZATION_NORMALIZE_NORMALIZE_COMMON_IMPL_H
