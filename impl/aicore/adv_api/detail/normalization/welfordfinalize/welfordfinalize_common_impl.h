 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_NORMALIZATION_WELFORDFINALIZE_WELFORDFINALIZE_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_NORMALIZATION_WELFORDFINALIZE_WELFORDFINALIZE_COMMON_IMPL_H
...
__aicore__ inline void welfordFinalizeOutputPre(const LocalTensor<float>& outputMean,
    const LocalTensor<float>& outputVariance, const LocalTensor<float>& tempOutputMean,
    const LocalTensor<float>& tempOutputVariance)
{
...
__aicore__ inline void WelfordFinalizeExeVariance(const LocalTensor<float>& inputMean,
    const LocalTensor<float>& inputVariance, const LocalTensor<float>& outputVariance,
    const LocalTensor<int32_t>& counts, const WelfordFinalizeTmpTensors<float>& tempTensors,
...
__aicore__ inline void GetWelfordFinalizeOutputMeanWithTail(const LocalTensor<float>& inputMean,
    const LocalTensor<float>& tempTensorCal, const LocalTensor<float>& tempoutputMean, const WelfordFinalizePara& para,
    const LocalTensor<float>& outputMean, const uint32_t computeLength, const uint32_t offset)
{
    const UnaryRepeatParams unaryParams;
    SetMaskCount();
    SetVectorMask<float, MaskMode::COUNTER>(0, computeLength);
...
__aicore__ inline void GetWelfordFinalizeOutputMeanNoTail(const LocalTensor<float>& inputMean,
    const LocalTensor<float>& tempoutputMean, const LocalTensor<float>& tempTensorCal, const WelfordFinalizePara& para,
    const LocalTensor<float>& outputMean, const uint32_t computeLength)
{
    SetMaskCount();
    const UnaryRepeatParams unaryParams;
    SetVectorMask<float, MaskMode::COUNTER>(0, computeLength);
...
__aicore__ inline void GetWelfordFinalizeOutputVarianceWithTail(const LocalTensor<float>& inputMean,
    const LocalTensor<float>& inputVariance, const LocalTensor<float>& tempoutputMean,
    const LocalTensor<float>& tempTensorCal, const WelfordFinalizePara& para, const LocalTensor<float>& outputVariance,
...
__aicore__ inline void GetWelfordFinalizeOutputVarianceNoTail(const LocalTensor<float>& inputMean,
    const LocalTensor<float>& inputVariance, const LocalTensor<float>& tempoutputMean,
    const LocalTensor<float>& tempTensorCal, const WelfordFinalizePara& para, const LocalTensor<float>& outputVariance,
...
__aicore__ inline void WelfordFinalizeExeVariance(const LocalTensor<float>& inputMean,
    const LocalTensor<float>& inputVariance, const LocalTensor<float>& outputVariance,
    WelfordFinalizeTmpTensors<float>& tempTensors, const WelfordFinalizePara& para, const uint32_t computeLength,
    const uint32_t offset)
{
    LocalTensor<float> tempMean = tempTensors.tempMean;
    LocalTensor<float> tempVariance = tempTensors.tempVariance;
    ...
    const WelfordFinalizePara& para, WelfordFinalizeTilingData& tiling)
{
    ...
    for (uint32_t i = 0; i < tiling.round; i++) {
        ...
        PipeBarrier<PIPE_V>();
        WelfordFinalizeExeVariance<isReuseSource>(inputMean[offset], inputVariance[offset],
            tempOutputVariance[outOffset], tempTensors, para, tiling.computeLength, offset);
        offset += tiling.computeLength;
        outOffset++;
        if (outOffset == B32_LEN) {
            SetMaskNorm();
            WholeReduceSum(tempOutputVariance, tempOutputVariance, B32_LEN, 1, DEFAULT_BLK_STRIDE, DEFAULT_BLK_STRIDE,
                outOffset >> 0x3);
            PipeBarrier<PIPE_V>();
            outOffset = 1;
        }
    ...
    if (tiling.tail > 0) {
        ...
        PipeBarrier<PIPE_V>();
        WelfordFinalizeExeVariance<isReuseSource>(inputMean[offset], inputVariance[offset],
            tempOutputVariance[outOffset], tempTensors, para, tiling.tail, offset);
        outOffset++;
    }
    SetMaskNorm();
    WholeReduceSum(
    ...
    WelfordFinalizeTilingData& tiling)
{
    ...
    for (uint32_t i = 0; i < tiling.round; i++) {
        ...
        PipeBarrier<PIPE_V>();
        WelfordFinalizeExeVariance<isReuseSource>(inputMean[offset], inputVariance[offset],
            tempOutputVariance[outOffset], counts[offset], tempTensors, para, tiling.computeLength);
        offset += tiling.computeLength;
        outOffset++;
        if (outOffset == B32_LEN) {
            SetMaskNorm();
    ...
    if (tiling.tail > 0) {
        ...
        PipeBarrier<PIPE_V>();
        WelfordFinalizeExeVariance<isReuseSource>(inputMean[offset], inputVariance[offset],
            tempOutputVariance[outOffset], counts[offset], tempTensors, para, tiling.tail);
        outOffset++;
    }
    SetMaskNorm();
    WholeReduceSum(
    ...
    const WelfordFinalizePara& para, WelfordFinalizeTmpTensors<float>& tempTensors, WelfordFinalizeTilingData& tiling)
{
    ...
    ASCENDC_ASSERT((stackBuffer.GetSize() >= (minTmpSize + minTmpOutSize)), {
        KERNEL_LOG(
            KERNEL_ERROR, "sharedTmpBuffer size must >= %d Bytes!", (minTmpSize + minTmpOutSize) * sizeof(float));
    });
...
__aicore__ inline void welfordFinalizeCommonCheck(const LocalTensor<float>& inputMean,
    const LocalTensor<float>& inputVariance, const LocalTensor<float>& outputMean,
    const LocalTensor<float>& outputVariance, const WelfordFinalizePara& para)
{
    ...
    ASCENDC_ASSERT((inputVariance.GetSize() >= para.abLength), {
        KERNEL_LOG(KERNEL_ERROR,
            "Failed to check the size of inputVariance, size is %u, which should be para.abLength!",
            inputVariance.GetSize());
    });
    ASCENDC_ASSERT((outputMean.GetSize() >= OUTPUT_SIZE), {
        KERNEL_LOG(KERNEL_ERROR,
            "Failed to check the size of outputMean, outputMean is %u, which should be larger than 8!",
            outputMean.GetSize());
    });
...
{
    CHECK_FUNC_HIGHLEVEL_API(WelfordFinalize, (isReuseSource),
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const WelfordFinalizePara& para)
{
    ...
    ASCENDC_ASSERT((counts.GetSize() >= para.abLength), {
        KERNEL_LOG(KERNEL_ERROR,
            "Failed to check the size of counts, size is %u, which should be larger than para.abLength!",
            counts.GetSize());
    });
    welfordFinalizeCommonCheck(inputMean, inputVariance, outputMean, outputVariance, para);
...
#endif // AICORE_ADV_API_DETAIL_NORMALIZATION_WELFORDFINALIZE_WELFORDFINALIZE_COMMON_IMPL_H
