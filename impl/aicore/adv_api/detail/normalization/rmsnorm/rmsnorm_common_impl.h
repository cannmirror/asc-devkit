...
#ifndef AICORE_ADV_API_DETAIL_NORMALIZATION_RMSNORM_RMSNORM_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_NORMALIZATION_RMSNORM_RMSNORM_COMMON_IMPL_H
    ...
    const uint32_t bsLength, const uint32_t hLength, const uint32_t originalHLength)
{
    for (uint32_t i = 0; i < bsLength; i++) {
        ...
        while (totalNum > 1) {
            ...
            RepeatReduceSum<float, false>(dstTmp, srcTmp, 1, MASK_PLACEHOLDER, DEFAULT_BLK_STRIDE, DEFAULT_BLK_STRIDE,
...
__aicore__ inline void RmsNormCompute(const LocalTensor<T>& dst, const LocalTensor<T>& src, const LocalTensor<T>& gamma,
    const T epsilon, const RmsNormTiling& tiling, RmsNormParams& params)
{
    UnaryRepeatParams unaryParams;
...
#endif
#endif // AICORE_ADV_API_DETAIL_NORMALIZATION_RMSNORM_RMSNORM_COMMON_IMPL_H