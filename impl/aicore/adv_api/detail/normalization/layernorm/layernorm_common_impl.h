 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_NORMALIZATION_LAYERNORM_LAYERNORM_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_NORMALIZATION_LAYERNORM_LAYERNORM_COMMON_IMPL_H
    ...
    const LocalTensor<T>& beta, const LocalTensor<uint8_t>& sharedTmpBuffer, const T epsilon, LayerNormTiling& tiling)
{
    ...
    CHECK_FUNC_HIGHLEVEL_API(LayerNorm, (T, isReuseSource),
...
{
    CHECK_FUNC_HIGHLEVEL_API(LayerNorm, (U, T, isReuseSource, config),
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const WelfordUpdateParam& para)
{
    ...
    for (uint32_t i = 0; i < round; ++i) {
        WelfordUpdateCompute<isReuseSource>(outMean[offset], outVar[offset], src[offset], inMean[offset], inVar[offset],
            sharedTmpBuffer, para, tmpNum, unaryParams, binaryParams);
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const WelfordUpdateParam& para)
{
    ...
    ASCENDC_ASSERT((para.abComputeLength > 0), {
        KERNEL_LOG(KERNEL_ERROR, "Failed to check para.abComputeLength, para.abComputeLength should be greater than 0.",
            para.abComputeLength, para.abLength);
    });
...
#endif // AICORE_ADV_API_DETAIL_NORMALIZATION_LAYERNORM_LAYERNORM_COMMON_IMPL_H
