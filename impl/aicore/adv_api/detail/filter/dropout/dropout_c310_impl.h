 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_FILTER_DROPOUT_DROPOUT_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_FILTER_DROPOUT_DROPOUT_C310_IMPL_H
...
__aicore__ inline void DropOutBitModeFP32Main(__local_mem__ float* dstUb, __local_mem__ float* srcUb,
    ...
    uint32_t repeatElm, uint16_t loopH, uint32_t srcLastAxis, uint32_t maskLastAxis)
{
    ...
    for (uint16_t i = 0; i < static_cast<uint16_t>(newRepeatTimes); ++i) {
        MicroAPI::DataCopy<uint32_t, MicroAPI::MaskDist::DIST_US>(
            tmpMask0, (__ubuf__ uint32_t*)maskUb + loopH * (maskLastAxis >> 2) + i * 4);
        MicroAPI::MaskInterleave<uint16_t>(selMask0, selMask1, tmpMask0, tmpMask1);
        maskReg = MicroAPI::UpdateMask<float>(sreg);
        MicroAPI::DataCopy<float>(src0Reg, srcUb + loopH * srcLastAxis + i * 2 * repeatElm);
        MicroAPI::Select(dst0Reg, src0Reg, scalarReg, selMask0);
        MicroAPI::Mul(dst0Reg, dst0Reg, vDivValueReg, selMask0);
        MicroAPI::DataCopy<float>(dstUb + loopH * srcLastAxis + i * 2 * repeatElm, dst0Reg, maskReg);
        MicroAPI::DataCopy<float>(src1Reg, srcUb + loopH * srcLastAxis + (i * 2 + 1) * repeatElm);
        MicroAPI::Select(dst1Reg, src1Reg, scalarReg, selMask1);
        MicroAPI::Mul(dst1Reg, dst1Reg, vDivValueReg, selMask1);
        MicroAPI::DataCopy<float>(dstUb + loopH * srcLastAxis + (i * 2 + 1) * repeatElm, dst1Reg, maskReg);
    }
}
    ...
    __local_mem__ uint8_t* maskUb, const T divValue, const uint32_t dataSize)
{
    ...
    if constexpr (sizeof(T) == 4) {
        DropOutBitModeFP32Main(dstUb, srcUb, maskUb, vDivValueReg, sreg, newRepeatTimes, repeatElm, 0, 0, 0);
        MicroAPI::MaskReg maskReg;
    ...
    const uint32_t dataSize)
{
    ...
    VF_CALL<VFDropOutBitModeCalc<T>>(dstUb, srcUb, maskUb, divValue, dataSize);
}
    ...
    __local_mem__ uint8_t* maskUb, const T divValue, const DropOutShapeInfo& info)
{
    ...
    for (uint16_t loopH = 0; loopH < static_cast<uint16_t>(info.firstAxis); ++loopH) {
        ...
        if constexpr (sizeof(T) == 4) {
            DropOutBitModeFP32Main(dstUb, srcUb, maskUb, vDivValueReg, width, newRepeatTimes, repeatElm, loopH,
    ...
    const DropOutShapeInfo& info)
{
    ...
    VF_CALL<VFDropOutBitModeCalcInfo<T>>(dstUb, srcUb, maskUb, divValue, info);
}
    ...
    const uint32_t dataSize)
{
    ...
    VF_CALL<VFDropOutByteModeCalc<T>>(dstUb, srcUb, maskUb, divValue, dataSize);
}
    ...
    const DropOutShapeInfo& info)
{
    ...
    VF_CALL<VFDropOutByteModeCalcInfo<T>>(dstUb, srcUb, maskUb, divValue, info);
}
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_FILTER_DROPOUT_DROPOUT_C310_IMPL_H
