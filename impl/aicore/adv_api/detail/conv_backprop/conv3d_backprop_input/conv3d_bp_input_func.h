...
#ifndef AICORE_ADV_API_DETAIL_CONV_BACKPROP_CONV3D_BACKPROP_INPUT_CONV3D_BP_INPUT_FUNC_H
#define AICORE_ADV_API_DETAIL_CONV_BACKPROP_CONV3D_BACKPROP_INPUT_CONV3D_BP_INPUT_FUNC_H
...
static __aicore__ inline void UpdateIdxAndStep(Intf* self)
{
    ...
    if constexpr (Intf::conv3dConfig.enableKernelSplit) {
        self->ctx.curHoIdx_ =
            self->ctx.curHoStartIdx_
            + self->ctx.curML0Idx_ * self->ctx.tiling_->baseM / self->ctx.splitWi_ * self->ctx.tiling_->strideH;
    } else {
        self->ctx.curHoIdx_ =
...
struct IterateAll {
    ...
    {
        while (self->template Iterate<sync>()) { self->template GetTensorC<sync>(output, enAtomic); }
