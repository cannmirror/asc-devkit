...
template <class Intf>
__aicore__ inline void LoadToB2V1(Intf* self, const AscendC::LocalTensor<typename Intf::SrcT>& l1B1Matrix,
    uint32_t kPos, AscendC::LocalTensor<typename Intf::SrcT>& l0b)
{
    // 转置逆序
    ...
    AscendC::LocalTensor<typename Intf::L0cT>& useC1Buf, QuantMode_t quantMode)
{
    ...
    if (self->ctx.hwI_ <= UINT32_MAX) {
        AscendC::DataCopyCO12DstParams dataCopyParams(static_cast<uint16_t>(self->ctx.baseUseN_), // nSize
        ...
        for (uint16_t i = 0; i < blockCnt; i++) {
            AscendC::DataCopyCO12DstParams dataCopyParams(static_cast<uint16_t>(self->ctx.tiling_->c0), // nSize
                static_cast<uint16_t>(self->ctx.baseUseM_),                                             // mSize
    ...
    AscendC::LocalTensor<typename Intf::L0cT>& useC1Buf)
{
    ...
    if constexpr ((std::is_same<typename Intf::SrcT, bfloat16_t>::value)
                  || (std::is_same<typename Intf::SrcT, half>::value)) {
        AscendC::FixpipeParams<typename Intf::L0cT> fixpipeParams(static_cast<uint16_t>(Ceil(self->ctx.baseUseN_, 16)),
