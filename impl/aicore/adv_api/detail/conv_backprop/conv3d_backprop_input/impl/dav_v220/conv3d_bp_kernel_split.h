...
#ifndef AICORE_ADV_API_DETAIL_CONV_BACKPROP_CONV3D_BACKPROP_INPUT_IMPL_DAV_V220_CONV3D_BP_KERNEL_SPLIT_H
#define AICORE_ADV_API_DETAIL_CONV_BACKPROP_CONV3D_BACKPROP_INPUT_IMPL_DAV_V220_CONV3D_BP_KERNEL_SPLIT_H
...
template <class Intf>
static __aicore__ inline void LoadToB2ForKernelSplit(Intf* self,
    const AscendC::LocalTensor<typename Intf::SrcT>& l1B1Matrix, uint32_t kRepeat, uint32_t kBlockC0Num,
    ...
    AscendC::LocalTensor<typename Intf::L0cT>& useC1Buf, QuantMode_t fixPipQuantMode)
{
    ...
    int64_t outOneComputeDstOffset =
        static_cast<int64_t>(self->ctx.curML0Idx_) % self->ctx.mIter_ * self->ctx.tiling_->baseM
            * self->ctx.tiling_->strideH * self->ctx.tiling_->strideW * self->ctx.tiling_->c0
        +                                                                              // M方向偏移
