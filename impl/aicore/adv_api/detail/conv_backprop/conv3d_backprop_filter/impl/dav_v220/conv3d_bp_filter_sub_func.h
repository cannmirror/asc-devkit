...
#ifndef AICORE_ADV_API_DETAIL_CONV_BACKPROP_CONV3D_BACKPROP_FILTER_IMPL_DAV_V220_CONV3D_BP_FILTER_SUB_FUNC_H
#define AICORE_ADV_API_DETAIL_CONV_BACKPROP_CONV3D_BACKPROP_FILTER_IMPL_DAV_V220_CONV3D_BP_FILTER_SUB_FUNC_H
...
static __aicore__ inline void LoadL12L0a(Intf* self, const AscendC::LocalTensor<typename Intf::SrcT>& l1AMatrix,
    uint32_t kPos, AscendC::LocalTensor<typename Intf::SrcT>& l0a, const Out2L1ScalarParams& params, uint64_t kaStepIdx)
{
    ...
    Intf* self, bool cachePosA1, uint64_t kaIdx, const Out2L1ScalarParams& params, bool isLoadA1, uint64_t kaStepIdx)
{
    ...
    if (params.isLoad2L1A) {
        ...
        if constexpr (AscendC::IsSameType<typename Intf::SrcT, float>::value) {
            if (blockCount & 0x1) {
                InitConstValue(useA1Buf[blockCount * dataCopyParams.blockLen * AscendC::ONE_BLK_SIZE / sizeof(float)],
        ...
        if (blockLen <= MAX_BLOCK_LEN && srcStride <= MAX_16BITS_STRIDE) {
            ...
            for (uint32_t idx = 0; idx < loop; ++idx) {
                ...
                if constexpr (Intf::Config::xType::format
                              == ConvCommonApi::ConvFormat::NCDHW) { // Transdata merge x input
...
template <class Intf>
static __aicore__ inline void LoadL0c2Gm(Intf* self, const AscendC::GlobalTensor<typename Intf::DstT>& output,
    uint8_t enAtomic = 0, bool enSequentialWrite = false)
{
    ...
    if constexpr (Intf::Config::dType::format == ConvCommonApi::ConvFormat::FRACTAL_Z_3D) {
        if (!enSequentialWrite) {
            ...
            AscendC::FixpipeParamsV220 fixpipeParams(static_cast<uint16_t>(self->ctx.baseUseN_),
                static_cast<uint16_t>(self->ctx.baseUseM_),
            ...
            AscendC::FixpipeParamsV220 fixpipeParams(static_cast<uint16_t>(self->ctx.baseUseN_),
                static_cast<uint16_t>(self->ctx.tiling_->baseM),
