 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_KFC_KFC_REGISTER_OBJ_H
#define AICORE_ADV_API_DETAIL_KFC_KFC_REGISTER_OBJ_H
...
#define REGIST_CUBE_OBJ(tpipe, workspace, ...)                                                                         \
    ...
    if ASCEND_IS_AIC {                                                                                                 \
        ...
        if constexpr (!asEnableMixDualMaster) {                                                                        \
            while (server.isRun()) { server.Run(__VA_ARGS__); };                                                       \
            server.Quit();                                                                                             \
            return;                                                                                                    \
        }                                                                                                              \
    }                                                                                                                  \
    KfcCommClient __kfcClient__(workspace, AscendC::GetSubBlockIdx(), enableHardPollKfc, asEnableMixDualMaster);       \
    if ASCEND_IS_AIV {                                                                                                 \
        if constexpr (!asEnableMixDualMaster) {                                                                        \
