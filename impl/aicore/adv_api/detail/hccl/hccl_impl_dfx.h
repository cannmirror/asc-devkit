...
#ifndef AICORE_ADV_API_DETAIL_HCCL_HCCL_IMPL_DFX_H
#define AICORE_ADV_API_DETAIL_HCCL_HCCL_IMPL_DFX_H
namespace AscendC {
...
public:
...
#ifdef __MSTX_DFX_REPORT__
    __aicore__ inline DfxScopeGuard(HcclApiOperType operType, uint32_t rankDim, uint8_t repeat, uint64_t dataCnt,
        HcclDataType dataType, GM_ADDR src, uint64_t srcStride, GM_ADDR dst, uint64_t dstStride) :
        operType_(operType)
    {
        PrintTimeStamp(static_cast<uint32_t>(operType_));
        ...
        struct DfxRecordInfo {
            ...
            int32_t rankDim;
            int32_t flagId;
        } info = {reinterpret_cast<uint64_t>(src), reinterpret_cast<uint64_t>(dst), dataCnt, dataCnt,
            (srcStride == 0U ? dataCnt : srcStride), (dstStride == 0U ? dataCnt : dstStride), 1U, 1U,
            SIZE_TABLE[dataType], SIZE_TABLE[dataType], static_cast<uint32_t>(repeat), static_cast<int32_t>(rankDim)};
        switch (operType_) {
    ...
    __aicore__ inline DfxScopeGuard(HcclApiOperType operType, uint32_t rankDim, uint8_t repeat, GM_ADDR src,
        void* sendCounts, void* sDispls, HcclDataType srcDataType, GM_ADDR dst, void* recvCounts, void* rDispls,
        ...
        operType_(operType)
    {
        ...
        struct DfxRecordInfo {
            ...
            int32_t rankDim;
            int32_t flagId;
        } info = {reinterpret_cast<uint64_t>(src), reinterpret_cast<uint64_t>(dst), static_cast<uint64_t*>(sendCounts),
            static_cast<uint64_t*>(recvCounts), static_cast<uint64_t*>(sDispls), static_cast<uint64_t*>(rDispls), 1U,
            1U, SIZE_TABLE[srcDataType], SIZE_TABLE[dstDataType], static_cast<uint32_t>(repeat),
            static_cast<int32_t>(rankDim), 0};
...
#ifdef __MSTX_DFX_REPORT__
    static constexpr size_t SIZE_TABLE[HcclDataType::HCCL_DATA_TYPE_RESERVED] = {sizeof(int8_t), sizeof(int16_t),
        sizeof(int32_t), 2U, sizeof(float), sizeof(int64_t), sizeof(uint64_t), sizeof(uint8_t), sizeof(uint16_t),
        sizeof(uint32_t), 8U, 2U, 16U};
    static constexpr int64_t WORKING_BLOCK_IDX = 0;
#endif
};
} // namespace AscendC

#endif // AICORE_ADV_API_DETAIL_HCCL_HCCL_IMPL_DFX_H
