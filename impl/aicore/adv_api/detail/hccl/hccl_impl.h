 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_HCCL_HCCL_IMPL_H
#define AICORE_ADV_API_DETAIL_HCCL_HCCL_IMPL_H
...
{
    DFX_MAKE_GUARD(HcclApiOperType::ALL_GATHER_PREPARE, GetRankDim(), repeat, sendCount, dataType, sendBuf, 0U, recvBuf,
        strideCount);
...
{
    DFX_MAKE_GUARD(HcclApiOperType::ALL_TO_ALL_PREPARE, GetRankDim(), repeat, dataCount, dataType, sendBuf, strideCount,
        recvBuf, strideCount);
...
template <HcclServerType serverType, const auto& config>
template <bool commit>
__aicore__ inline HcclHandle Hccl<serverType, config>::AlltoAllV(GM_ADDR sendBuf, void* sendCounts, void* sdispls,
    HcclDataType sendType, GM_ADDR recvBuf, void* recvCounts, void* rdispls, HcclDataType recvType, uint8_t repeat)
{
    DFX_MAKE_GUARD(HcclApiOperType::ALL_TO_ALL_V_PREPARE, GetRankDim(), repeat, sendBuf, sendCounts, sdispls, sendType,
...
template <HcclServerType serverType, const auto& config>
template <bool commit>
__aicore__ inline HcclHandle Hccl<serverType, config>::ReduceScatter(GM_ADDR sendBuf, GM_ADDR recvBuf,
    uint64_t recvCount, HcclDataType dataType, HcclReduceOp op, uint64_t strideCount, uint8_t repeat)
{
    DFX_MAKE_GUARD(HcclApiOperType::REDUCE_SCATTER_PREPARE, GetRankDim(), repeat, recvCount, dataType, sendBuf,
        strideCount, recvBuf, 0U);
...
template <HcclServerType serverType, const auto& config>
__aicore__ inline bool Hccl<serverType, config>::SetReduceDataTypeAbility(
...
#endif // AICORE_ADV_API_DETAIL_HCCL_HCCL_IMPL_H
