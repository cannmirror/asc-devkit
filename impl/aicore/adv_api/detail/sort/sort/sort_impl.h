 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_SORT_SORT_SORT_IMPL_H
#define AICORE_ADV_API_DETAIL_SORT_SORT_SORT_IMPL_H

#include "../sort_common_utils.h"

namespace AscendC {
enum class SortType { RADIX_SORT, MERGE_SORT };
struct SortConfig {
...
namespace MicroAPI {
namespace internal {
__aicore__ inline void ConvertRegToWithShift(
    __local_mem__ uint32_t*& input, RegTensor<uint8_t>& dst, const int16_t offset)
{
...
__aicore__ inline void ConvertRegToWithShift(
    __local_mem__ uint64_t*& input, RegTensor<uint8_t>& dst, const int16_t offset)
{
...
__aicore__ inline void ConvertRegToWithShift(
    __local_mem__ uint16_t*& input, RegTensor<uint8_t>& dst, const int16_t offset)
{
...
__aicore__ inline void ConvertRegToWithShift(
    __local_mem__ uint8_t*& input, RegTensor<uint8_t>& dst, const int16_t offset)
{
...
__aicore__ inline void PreProcess(__local_mem__ U* src, __local_mem__ U* dst, uint32_t count)
{
...
template <typename T>
__aicore__ inline void GetExclusiveSum(__local_mem__ T* srcValue, __local_mem__ uint8_t* tmpSrc,
...
__aicore__ inline void EightBitsSort(__local_mem__ uint8_t* srcValueU8, __local_mem__ uint8_t* srcValueU8Back,
    __local_mem__ uint8_t* inputIndexAddr, __local_mem__ uint16_t* inputIndexU16Addr, uint32_t count)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t j = 0; j < repeatTime; ++j) {
            ...
            DataCopy<uint8_t, PostLiteral::POST_MODE_UPDATE>(
        ...
        for (uint16_t i = 0; i < 4; ++i) {
            ...
            for (uint16_t j = 0; j < repeatTime; ++j) {
                ...
                DataCopy<uint8_t, PostLiteral::POST_MODE_UPDATE>(srcReg, (__local_mem__ uint8_t*&)srcAddr, GetVecLen());
                DataCopy<uint8_t, PostLiteral::POST_MODE_UPDATE>(
                    indexReg, (__local_mem__ uint8_t*&)indexAddr, GetVecLen());
            ...
            for (uint16_t j = 0; j < repeatTime; ++j) {
                ...
                DataCopy<uint8_t, PostLiteral::POST_MODE_UPDATE>(
                    srcReg, (__local_mem__ uint8_t*&)tmpSrcAddr, GetVecLen());
                DataCopy<uint8_t, PostLiteral::POST_MODE_UPDATE>(
                    tmpIndexReg, (__local_mem__ uint8_t*&)indexAddr, GetVecLen());
                vselr(selectData, srcReg, tmpIndexReg);
        ...
        for (uint16_t j = 0; j < repeatTime; ++j) {
            ...
            DataCopy<uint8_t, PostLiteral::POST_MODE_UPDATE>(
                sortedRes, (__local_mem__ uint8_t*&)tmpIndexLoad, GetVecLen());
            // zero extend to u16
...
__aicore__ inline void LocalSort(__local_mem__ uint8_t* srcU8, __local_mem__ uint16_t* tmpIndexU16,
    __local_mem__ uint16_t* exclusiveSumOrigin, __local_mem__ uint32_t* sortedLocalIndex, uint32_t count)
{
    ...
    __VEC_SCOPE__
    {
        ...
        DataCopy<uint16_t, PostLiteral::POST_MODE_UPDATE>(
            bucketOffset0, (__local_mem__ uint16_t*&)exclusiveSumOrigin, GetVecLen() / sizeof(uint16_t));
        DataCopy<uint16_t, PostLiteral::POST_MODE_UPDATE>(
            bucketOffset1, (__local_mem__ uint16_t*&)exclusiveSumOrigin, GetVecLen() / sizeof(uint16_t));
        ...
        for (uint16_t i = 0; i < repeatTime; i++) {
            ...
            DataCopy<uint16_t, PostLiteral::POST_MODE_UPDATE>(
                indexU16Bucket0, (__local_mem__ uint16_t*&)tmpIndexU16, GetVecLen() / sizeof(uint16_t));
            DataCopy<uint16_t, PostLiteral::POST_MODE_UPDATE>(
                indexU16Bucket1, (__local_mem__ uint16_t*&)tmpIndexU16, GetVecLen() / sizeof(uint16_t));
            ...
            Histograms<uint8_t, uint16_t, HistogramsBinType::BIN0, HistogramsType::FREQUENCY>(
                distrubtionHist0, key, maskRegB8);
            Histograms<uint8_t, uint16_t, HistogramsBinType::BIN1, HistogramsType::FREQUENCY>(
                distrubtionHist1, key, maskRegB8);
            ...
            vselr(bucketOffsetKeyLowbits, bucketOffestLow, key);
            vselr(bucketOffsetKeyHighbits, bucketOffestHigh, key);
            ...
            vselr(localOffsetWriteKeyLowbits, localOffsetLowbits, key);
...
__aicore__ inline void GatherAndStoreB64Elements(RegTensor<uint32_t>& localOffset, MaskReg& maskReg,
...
template <typename T>
__aicore__ inline void UpdateValueAndIndexByLocalIndex(__local_mem__ uint8_t* prevSortedValue,
    ...
    __local_mem__ T* currSortedIndex, uint32_t count)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repeatTime; i++) {
            ...
            DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                localOffset0, (__local_mem__ uint32_t*&)sortedLocalIndex, GetVecLen() / sizeof(uint32_t));
            DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                localOffset1, (__local_mem__ uint32_t*&)sortedLocalIndex, GetVecLen() / sizeof(uint32_t));
            DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                localOffset2, (__local_mem__ uint32_t*&)sortedLocalIndex, GetVecLen() / sizeof(uint32_t));
            DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                localOffset3, (__local_mem__ uint32_t*&)sortedLocalIndex, GetVecLen() / sizeof(uint32_t));
            ...
            if constexpr (sizeof(T) == 8) {
                ...
                DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                    (__local_mem__ uint32_t*&)currSortedIndex, vecIndex0, GetVecLen() / sizeof(uint32_t), maskLowP0);
                DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                    (__local_mem__ uint32_t*&)currSortedIndex, vecIndex1, GetVecLen() / sizeof(uint32_t), maskHighP0);
                DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                    (__local_mem__ uint32_t*&)currSortedIndex, vecIndex2, GetVecLen() / sizeof(uint32_t), maskLowP1);
                DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                    (__local_mem__ uint32_t*&)currSortedIndex, vecIndex3, GetVecLen() / sizeof(uint32_t), maskHighP1);
            }
            DataCopy<uint8_t, PostLiteral::POST_MODE_UPDATE>(
                (__local_mem__ uint8_t*&)currSortedValue, resB8, GetVecLen(), maskReg);
        }
    }
}

template <typename T>
__aicore__ inline void UpdateValueAndIndexByLocalIndex(__local_mem__ uint16_t* prevSortedValue,
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repeatTime; i++) {
            ...
            if constexpr (sizeof(T) == 8) {
                ...
                DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                    (__local_mem__ uint32_t*&)currSortedIndex, indexU32P0, GetVecLen() / sizeof(uint32_t), maskLow);
                DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                    (__local_mem__ uint32_t*&)currSortedIndex, indexU32P1, GetVecLen() / sizeof(uint32_t), maskHigh);
            }
            DataCopy<uint16_t, PostLiteral::POST_MODE_UPDATE>(
                (__local_mem__ uint16_t*&)currSortedValue, indexP0, GetVecLen() / sizeof(uint16_t), maskReg);
        }
    }
}

template <typename T>
__aicore__ inline void UpdateValueAndIndexByLocalIndex(__local_mem__ uint32_t* prevSortedValue,
    ...
    __local_mem__ T* currSortedIndex, uint32_t count)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repeatTime; i++) {
            ...
            DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                localOffset, (__local_mem__ uint32_t*&)sortedLocalIndex, GetVecLen() / sizeof(uint32_t));
            DataCopyGather(index, prevSortedValue, localOffset, maskReg);
            DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                (__local_mem__ uint32_t*&)currSortedValue, index, GetVecLen() / sizeof(uint32_t), maskReg);
            if constexpr (sizeof(T) == 8) {
                ...
                DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                    (__local_mem__ uint32_t*&)currSortedIndex, indexU32, GetVecLen() / sizeof(uint32_t), maskReg);
            }
        }
    }
}
...
template <typename T>
__aicore__ inline void UpdateValueAndIndexByLocalIndex(__local_mem__ uint64_t* prevSortedValue,
...
template <typename T>
__aicore__ inline void UpdateValueByLocalIndex(__local_mem__ T* srcValue, __local_mem__ uint32_t* sortedLocalIndex,
        ...
        for (uint16_t i = 0; i < repeatTime; i++) {
            if constexpr (sizeof(T) == 1) {
                ...
                if constexpr (sizeof(T) == 8) {
                    ...
                    DataCopy<uint32_t, PostLiteral::POST_MODE_UPDATE>(
                        (__local_mem__ uint32_t*&)currSortedValue, indexP0, GetVecLen() / sizeof(uint32_t), maskReg);
                }
            }
        }
    }
...
template <typename T>
__aicore__ inline void SaveBufferTo(__local_mem__ T* srcBuffer, __local_mem__ T* dstBuffer, uint32_t count)
{
    uint16_t repeatTime = DivCeil(count, GetVecLen() / sizeof(T));
    __VEC_SCOPE__
    {
...
__aicore__ inline void SaveBufferTo(
    __local_mem__ uint64_t* srcBuffer, __local_mem__ uint64_t* dstBuffer, uint32_t count)
{
...
template <typename T>
__aicore__ inline void SwapBuffer(__local_mem__ T*& a, __local_mem__ T*& b)
{
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t count)
{
    static_assert(SupportBytes<T, 2, 4, 8>() || SupportType<T, uint8_t, int8_t>(),
        "Advanced Sort API only supports data value with "
    ...
    ArrangeCommonTmpBuffer(
        sharedTmpBuffer, alignCount, totalExclusiveSum, srcU8, tmpIndexU16, sortedLocalIndex, srcU8Copy, tmpIndexU8);
    ...
    if constexpr (sizeof(T) == 1) {
        ...
        SortU8ElementsWithRound(
            prevSortedValue, srcU8, srcU8Copy, totalExclusiveSum, tmpIndexU8, tmpIndexU16, sortedLocalIndex, count, 1);
    ...
    const uint32_t count)
{
    ...
    static_assert(SupportType<U, int32_t, uint32_t, int64_t, uint64_t>(),
        "Advanced Sort API only supports src index with int32_t/uint32_t/int64_t/uint64_t types.");
    // Only for AI Vector Core.
    if ASCEND_IS_AIC {
        return;
    }
...
#endif // AICORE_ADV_API_DETAIL_SORT_SORT_SORT_IMPL_H
