 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_SORT_TOPK_TOPK_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_SORT_TOPK_TOPK_COMMON_IMPL_H
    ...
    const int32_t k, const TopkTiling& tilling, const TopKInfo& topKInfo, const bool isLargest = true)
{
    ...
    ASCENDC_ASSERT((stackTensor.GetSize() / sizeof(T) >= tilling.tmpLocalSize), {
        KERNEL_LOG(KERNEL_ERROR,
            "The stack "
            "buffer is insufficient, TopK api need %d, but only %d exists.",
...
#endif // AICORE_ADV_API_DETAIL_SORT_TOPK_TOPK_COMMON_IMPL_H