 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_SORT_TOPK_TOPK_V220_IMPL_H
#define AICORE_ADV_API_DETAIL_SORT_TOPK_TOPK_V220_IMPL_H
    ...
    const int32_t dstIdx, const int32_t dstOffsetFourBytes, const int outterIdx)
{
    ...
    if constexpr (sizeof(T) == sizeof(float)) {
        ...
        GatherMask<T>(dstValueLocal[dstOffsetFourBytes], tmpLocal[tmpLocalDstOffset], REDUCEV2_MODE_ONE, true,
        ...
        struct GatherMaskParams reducev2Params(DEFAULT_BLK_STRIDE, 1, DEFAULT_REPEAT_STRIDE, DEFAULT_BLK_STRIDE);
        GatherMask<T>(dstValueLocal[dstOffsetTwoBytes], tmpLocal[tmpLocalDstOffset], REDUCEV2_MODE_THREE, true,
    ...
    const TopKInfo& topKInfo)
{
    ...
    GatherMask<float, uint32_t>(tempBuffer, tmpLocal,
        tmpLocal[tilling.topkMrgSrc1MaskSizeOffset + EIGHT].ReinterpretCast<uint32_t>(), true, VREDUCEV2_FOUR_BYTE_MASK,
        reducev2Params2, rsvdCnt);
    PipeBarrier<PIPE_V>();
}
...
#endif // AICORE_ADV_API_DETAIL_SORT_TOPK_TOPK_V220_IMPL_H
