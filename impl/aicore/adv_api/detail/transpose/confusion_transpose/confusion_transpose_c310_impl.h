 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_TRANSPOSE_CONFUSION_TRANSPOSE_CONFUSION_TRANSPOSE_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_TRANSPOSE_CONFUSION_TRANSPOSE_CONFUSION_TRANSPOSE_C310_IMPL_H
    ...
    uint32_t forLoop1, uint32_t forLoop2, uint32_t srcStride0, uint32_t srcStride1, uint32_t srcStride2)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < static_cast<uint16_t>(forLoop0); i++) {
            ...
            for (uint16_t k = 0; k < static_cast<uint16_t>(tailLoop); k++) {
                DataCopyGather(
                    MainVreg, srcAddr + i * srcStride0 + mainLoop * srcStride1 * factor, indexReg5, tailMask);
                MicroAPI::DataCopyUnAlign(((__ubuf__ T*&)hoistDstAddr), MainVreg, ureg0, tail);
            }
            MicroAPI::DataCopyUnAlignPost(((__ubuf__ T*&)hoistDstAddr), ureg0, 0);
        }
...
__aicore__ inline void ConfusionTransposeCopySrcToDst(__ubuf__ T* dstAddr, __ubuf__ T* srcAddr, uint32_t totalCount)
{
    ...
    const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, ConfusionTranspose021Tiling& tiling)
{
    ...
    if ((tiling.dim1 == 1) || (tiling.dim2 == 1)) {
        ...
        ConfusionTransposeCopySrcToDst<T, MicroAPI::RegTraitNumOne, vlSize>(
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repeatTimes; i++) {
            ...
            for (uint16_t j = 0; j < heightLoopNum; j++) {
                ...
                AscendC::MicroAPI::DataCopy<T, MicroAPI::DataCopyMode::DATA_BLOCK_COPY,
                    MicroAPI::PostLiteral::POST_MODE_NORMAL>(vSrcReg0, (__ubuf__ T*&)rowStrideAddr, 1, 0, mask);
                AscendC::MicroAPI::DataCopy<T, MicroAPI::DataCopyMode::DATA_BLOCK_COPY,
                    MicroAPI::PostLiteral::POST_MODE_NORMAL>(vSrcReg1, (__ubuf__ T*&)nerborStrideAddr, 1, 0, mask);
                ...
                AscendC::MicroAPI::DataCopy<T, MicroAPI::DataCopyMode::DATA_BLOCK_COPY,
                    MicroAPI::PostLiteral::POST_MODE_NORMAL>(
        ...
        if (tailWidth) {
            ...
            for (uint16_t j = 0; j < heightLoopNum; j++) {
                ...
                if (tailWidth > regWidthHalf) {
                    ...
                    uint64_t dstNerborStrideAddr =
                        uint64_t(dstAddr + dstOffset + regWidth * heightLoopNum + j * c0Size);
                    AscendC::MicroAPI::DataCopy<T, MicroAPI::DataCopyMode::DATA_BLOCK_COPY,
                        MicroAPI::PostLiteral::POST_MODE_NORMAL>(
...
#endif // AICORE_ADV_API_DETAIL_TRANSPOSE_CONFUSION_TRANSPOSE_CONFUSION_TRANSPOSE_C310_IMPL_H