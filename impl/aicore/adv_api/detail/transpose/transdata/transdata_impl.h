 ...
 */
#ifndef AICORE_ADV_API_DETAIL_TRANSPOSE_TRANSDATA_TRANSDATA_IMPL_H
#define AICORE_ADV_API_DETAIL_TRANSPOSE_TRANSDATA_TRANSDATA_IMPL_H
...
template <typename T>
__aicore__ inline void TransDataImplNcdhwToFractal(const LocalTensor<T>& dst, const LocalTensor<T>& src,
    const LocalTensor<uint8_t>& tmpBuffer, const TransDataTmpParams& param)
{
    ...
    for (int j = 0; j < n1; j++) {
        ...
        for (int32_t i = 0; i < remain; i++) { srcLocalList[i] = currSrcAddr + i * currAxis * sizeof(T); }
        for (int32_t i = remain; i < DEFAULT_TRANSDATA_5HD_LIST; i++) { srcLocalList[i] = tmpBufferAddr; }
    ...
    for (int32_t j = 0; j < c1; j++) {
        ...
        for (int32_t i = 0; i < DEFAULT_TRANSDATA_5HD_LIST; i++) { dstLocalList[i] = currDstAddr + i * c0 * sizeof(T); }
        for (int32_t i = 0; i < remain; i++) { srcLocalList[i] = currSrcAddr + i * currAxis * sizeof(T); }
        for (int32_t i = remain; i < DEFAULT_TRANSDATA_5HD_LIST; i++) { srcLocalList[i] = tmpBufferAddr; }
        TransDataTo5HD<half>(dstLocalList, srcLocalList, transDataParams);
    }
    ...
    for (int32_t i = 0; i < c1; i++) { DataCopy(dst[i * currAxis], tmpDstTensor[i * d * currAxis], copyParams); }
    PipeBarrier<PIPE_V>();
}

// Transdata NCDHW -> NDC1HWC0
template <typename T>
__aicore__ inline void TransDataImplNcdhwTo6Hd(const LocalTensor<T>& dst, const LocalTensor<T>& src,
    const LocalTensor<uint8_t>& tmpBuffer, const TransDataTmpParams& param)
{
    constexpr int32_t c0 = 16;
    constexpr int32_t elePerBlk = ONE_BLK_SIZE / sizeof(T);
    ...
    for (int32_t k = 0; k < n; k++) {
        ...
        for (int32_t j = 0; j < c1; j++) {
            ...
            for (int32_t i = 0; i < remain; i++) { srcLocalList[i] = currSrcAddr + i * axisPadHwd * sizeof(T); }
            for (int32_t i = remain; i < DEFAULT_TRANSDATA_5HD_LIST; i++) { srcLocalList[i] = tmpBufferAddr; }
            TransDataTo5HD<half>(dstLocalList, srcLocalList, transDataParams);
        }
        PipeBarrier<PIPE_V>();
...
template <typename T>
__aicore__ inline void TransDataImpl6HdToNcdhw(const LocalTensor<T>& dst, const LocalTensor<T>& src,
    const LocalTensor<uint8_t>& tmpBuffer, const TransDataTmpParams& param)
{
    const int32_t n = param.n, c = param.c, d = param.d, h = param.h, w = param.w;
    constexpr int32_t c0 = 16;
    ...
    for (int32_t k = 0; k < n; k++) {
        ...
        for (int32_t j = 0; j < c1; j++) {
            ...
            for (int32_t i = 0; i < remain; i++) { dstLocalList[i] = currDstAddr + i * axisPadHwd * sizeof(T); }
...
#endif // AICORE_ADV_API_DETAIL_TRANSPOSE_TRANSDATA_TRANSDATA_IMPL_H