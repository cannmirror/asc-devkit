...
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_GEGLU_GEGLU_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_GEGLU_GEGLU_COMMON_IMPL_H
    ...
    const LocalTensor<T>& srcTensor1, const LocalTensor<uint8_t>& sharedTmpBuffer, uint32_t calCount)
{
    ...
    ASCENDC_ASSERT(ret, {
        KERNEL_LOG(KERNEL_ERROR, "Size of src0: %u is not equal to size of src1: %u", srcTensor0.GetSize(),
            srcTensor1.GetSize());
    });
