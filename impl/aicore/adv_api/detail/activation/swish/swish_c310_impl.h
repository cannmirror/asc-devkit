 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SWISH_SWISH_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SWISH_SWISH_C310_IMPL_H
...
template <typename T>
__aicore__ inline void SwishComputeVF(
    ...
    const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal, const uint32_t count, const T scalarValue)
{
    ...
    VF_CALL<SwishComputeVF<T>>(
        (__ubuf__ T*)dstLocal.GetPhyAddr(), (__ubuf__ T*)srcLocal.GetPhyAddr(), count, scalar, repeatTimes);
}

} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SWISH_SWISH_C310_IMPL_H