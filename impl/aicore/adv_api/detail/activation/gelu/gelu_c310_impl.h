 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_GELU_GELU_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_GELU_GELU_C310_IMPL_H
...
__aicore__ inline void GeluImplVF(__ubuf__ T* dst, __ubuf__ T* src, uint32_t count, const uint16_t repeatTimes)
{
    ...
    constexpr T coefficientsA = 0.044715;
    constexpr T coefficientsB = 1.5957691216057308;
    ...
    for (uint16_t i = 0; i < repeatTimes; ++i) {
        ...
        if constexpr (highPrecision) {
            ...
            MicroAPI::Cast<float, half, GeluInternal::castTraitB16ToB32>(
                srcVreg, (MicroAPI::RegTensor<half>&)srcVreg, mask);
        } else {
        ...
        if constexpr (highPrecision) {
            MicroAPI::Cast<half, float, GeluInternal::castTraitB32ToB16>(
...
template <typename T>
__aicore__ inline void FastGeluCoreAlg(MicroAPI::RegTensor<T>& dstVreg, MicroAPI::RegTensor<T>& srcVreg,
    MicroAPI::MaskReg& mask, MicroAPI::RegTensor<T>& stackVreg)
{
    ...
    const LocalTensor<half>& dstLocal, const LocalTensor<half>& srcLocal, const uint32_t dataSize)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < repeatTimes; ++i) {
            ...
            MicroAPI::Cast<float, half, GeluInternal::castTraitB16ToB32>(srcVregFloat, srcVreg, mask);

            FastGeluCoreAlg<float>(dstVregFloat, srcVregFloat, mask, stackVregFloat);

            MicroAPI::Cast<half, float, GeluInternal::castTraitB32ToB16>(dstVreg, dstVregFloat, mask);
...
template <typename T>
__aicore__ inline void FastGeluV2CoreAlg(MicroAPI::RegTensor<T>& dstVreg, MicroAPI::RegTensor<T>& srcVreg,
    MicroAPI::MaskReg& mask, MicroAPI::RegTensor<T>& stackVregA, MicroAPI::RegTensor<T>& stackVregB,
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t dataSize)
{
    ...
    if constexpr (highPrecision && (IsSameType<T, half>::value)) {
        FastGeluHighPrecisionAlg(dstLocal, srcLocal, dataSize);
    } else {
        FastGeluAlg(dstLocal, srcLocal, dataSize);
    }
}
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t dataSize)
{
    ...
    if constexpr (highPrecision && (IsSameType<T, half>::value)) {
        FastGeluV2HighPrecisionAlg(dstLocal, srcLocal, dataSize);
    } else {
        FastGeluV2Alg(dstLocal, srcLocal, dataSize);
    }
}
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_GELU_GELU_C310_IMPL_H
