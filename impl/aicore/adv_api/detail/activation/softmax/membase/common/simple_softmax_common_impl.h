 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_COMMON_SIMPLE_SOFTMAX_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_COMMON_SIMPLE_SOFTMAX_COMMON_IMPL_H
    ...
    const SoftMaxTiling& tiling, const uint32_t offset1, const uint32_t offset2, const uint32_t curSplitM)
{
    ...
    if constexpr (config.oriSrcM == 0 || config.oriSrcK == 0) {
        ...
        GenericSubNDImpl(tmpBuffer0, tmpBuffer0, tmpBuffer2, curSplitM, splitK, DEFAULT_REPEAT_STRIDE * HALF_FACTOR);
        ...
        Cast(tmpBuffer2, inSumTensor[offset2], RoundMode::CAST_NONE, reduceSize);
        PipeBarrier<PIPE_V>();
        GenericDivNDImpl(tmpBuffer0, tmpBuffer0, tmpBuffer2, curSplitM, splitK, DEFAULT_REPEAT_STRIDE * HALF_FACTOR);
    ...
    const SoftMaxTiling& tiling, const uint32_t offset1, const uint32_t offset2, const uint32_t curSplitM)
{
    ...
    if constexpr (config.oriSrcM == 0 || config.oriSrcK == 0) {
        ...
        GenericSubNDImpl(dst[offset1], src[offset1], inMaxTensor[offset2], curSplitM, splitK, DEFAULT_REPEAT_STRIDE);
        PipeBarrier<PIPE_V>();
        Exp(dst[offset1], dst[offset1], splitSize);
        PipeBarrier<PIPE_V>();
        GenericDivNDImpl(dst[offset1], dst[offset1], inSumTensor[offset2], curSplitM, splitK, DEFAULT_REPEAT_STRIDE);
    }
}
    ...
    const SoftMaxTiling& tiling, const uint32_t offset1, const uint32_t offset2, const uint32_t curSplitM)
{
    ...
    if constexpr (config.oriSrcM == 0 || config.oriSrcK == 0) {
        ...
        Cast(tmpBuffer0, src[offset1], RoundMode::CAST_NONE, splitSize);
        PipeBarrier<PIPE_V>();
        GenericSubNDImpl(tmpBuffer0, tmpBuffer0, inMaxTensor[offset2], curSplitM, splitK, DEFAULT_REPEAT_STRIDE);
        PipeBarrier<PIPE_V>();
        Exp(tmpBuffer0, tmpBuffer0, tiling.splitSize);
        PipeBarrier<PIPE_V>();
        GenericDivNDImpl(tmpBuffer0, tmpBuffer0, inSumTensor[offset2], curSplitM, splitK, DEFAULT_REPEAT_STRIDE);
        PipeBarrier<PIPE_V>();
        Cast(dst[offset1], tmpBuffer0, FLOAT2HALF_ROUND_MODE, splitSize);
    }
}
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_COMMON_SIMPLE_SOFTMAX_COMMON_IMPL_H