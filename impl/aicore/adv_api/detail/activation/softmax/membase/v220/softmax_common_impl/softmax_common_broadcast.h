 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_V220_SOFTMAX_COMMON_IMPL_SOFTMAX_COMMON_BROADCAST_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_V220_SOFTMAX_COMMON_IMPL_SOFTMAX_COMMON_BROADCAST_H
    ...
    const LocalTensor<float>& dst, const LocalTensor<float>& src, const struct ReduceLastND& reduceParam)
{
    ...
    if (tail != 0) { // use duplicate in tail
        ...
        for (uint32_t j = 0; j < tail; j++) { scalarList[j] = src[(range * BRCB_BROADCAST_NUMBER + j)].GetValue(0); }
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_V220_SOFTMAX_COMMON_IMPL_SOFTMAX_COMMON_BROADCAST_H