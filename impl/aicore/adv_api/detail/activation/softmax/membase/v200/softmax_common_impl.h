 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_V200_SOFTMAX_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_V200_SOFTMAX_COMMON_IMPL_H
    ...
    const LocalTensor<T>& dst, const LocalTensor<T>& src, const struct BroadCastLastND& brcParam)
{
    ...
    for (uint32_t i = 0; i < rangeM; i++) { BroadCastLastCompute(dst, src, brcParam, SCALAR_STACK_DEPTH, i); }
    // to compute tail M
    ...
    if (reduceParam.originalSrcK <= elementNumPerRep) {
        ...
        for (uint32_t i = 0; i < range; i++) {
            WholeReduceSum(dst[i * MAX_REPEAT_TIMES * reduceParam.dstK], src[i * MAX_REPEAT_TIMES * reduceParam.srcK],
                mask, MAX_REPEAT_TIMES, reduceParam.dstK, 1, reduceParam.srcK / elementNumPerBlk);
        }
        if (tail != 0) {
            WholeReduceSum(dst[range * MAX_REPEAT_TIMES * reduceParam.dstK],
    ...
    if (reduceMaxParam.originalSrcK <= FLOAT_REPEAT_SIZE) {
        ...
        for (uint32_t i = 0; i < range; i++) {
            WholeReduceMax(dstMax[i * MAX_REPEAT_TIMES * reduceMaxParam.dstK],
                ...
                reduceMaxParam.dstK / HALF_FACTOR, 1, reduceMaxParam.srcK / FLOAT_NUM_PER_BLK);
        }
        if (tail != 0) {
    ...
    const uint32_t& repeat, const uint32_t& brcbCount)
{
    ...
    if (tailM != 0) {
        for (uint32_t j = 0; j < tailM; j++) { scalarList[j] = srcLocal.GetValue(rangeM * SCALAR_STACK_DEPTH + j); }
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_MEMBASE_V200_SOFTMAX_COMMON_IMPL_H
