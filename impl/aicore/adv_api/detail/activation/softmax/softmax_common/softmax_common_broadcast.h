 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_SOFTMAX_COMMON_SOFTMAX_COMMON_BROADCAST_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_SOFTMAX_COMMON_SOFTMAX_COMMON_BROADCAST_H
    ...
    const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal, const uint32_t brcbCount)
{
    ...
    for (uint32_t j = 0; j < SCALAR_STACK_DEPTH; j++) { scalarList[j] = srcLocal.GetValue(j); }
    ...
    const uint32_t& repeat, const uint32_t& brcbCount)
{
    ...
    for (uint32_t i = 0; i < rangeM; i++) {
        ...
        for (uint32_t j = 0; j < SCALAR_STACK_DEPTH; j++) { scalarList[j] = srcLocal.GetValue(offset + j); }
        for (uint32_t k = 0; k < SCALAR_STACK_DEPTH; k++) {
    ...
    if (tailM != 0) {
        ...
        for (uint32_t j = 0; j < tailM; j++) { scalarList[j] = srcLocal.GetValue(offset + j); }
    ...
    const uint32_t& repeat, const uint32_t& brcbCount)
{
    ...
    for (uint32_t i = 0; i < rangeM; i++) {
        offset = i * brcbCount * SCALAR_STACK_DEPTH;
        for (uint32_t j = 0; j < SCALAR_STACK_DEPTH; j++) { scalarList[j] = srcLocal.GetValue(offset + j * brcbCount); }
        for (uint32_t k = 0; k < SCALAR_STACK_DEPTH; k++) {
            Duplicate<float, false>(dstLocal[offset + k * brcbCount], scalarList[k], MASK_PLACEHOLDER,
                DEFAULT_BLK_STRIDE, DEFAULT_BLK_STRIDE, DEFAULT_REPEAT_STRIDE);
        }
    ...
    if (tailM != 0) {
        offset = rangeM * brcbCount * SCALAR_STACK_DEPTH;
        for (uint32_t j = 0; j < tailM; j++) { scalarList[j] = srcLocal.GetValue(offset + j * brcbCount); }
        for (uint32_t k = 0; k < tailM; k++) {
            Duplicate<float, false>(dstLocal[offset + k * brcbCount], scalarList[k], MASK_PLACEHOLDER,
                DEFAULT_BLK_STRIDE, DEFAULT_BLK_STRIDE, DEFAULT_REPEAT_STRIDE);
        }
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_SOFTMAX_COMMON_SOFTMAX_COMMON_BROADCAST_H