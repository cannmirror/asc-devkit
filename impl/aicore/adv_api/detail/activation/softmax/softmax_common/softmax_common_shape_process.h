 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_SOFTMAX_COMMON_SOFTMAX_COMMON_SHAPE_PROCESS_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_SOFTMAX_COMMON_SOFTMAX_COMMON_SHAPE_PROCESS_H
...
__aicore__ inline LastAxisShapeND GetLastAxisShapeND(const ShapeInfo& shapeInfo)
{
    ...
    for (uint32_t i = 0; i < shapeInfo.shapeDim; i++) { calculateSize *= shapeInfo.shape[i]; }
...
__aicore__ inline LastAxisShapeND GetLastAxisOriginShapeND(const ShapeInfo& srcShapeInfo)
{
    uint32_t calculateSize = 1;
    LastAxisShapeND ndinfo;
    for (uint32_t i = 0; i < srcShapeInfo.originalShapeDim; i++) { calculateSize *= srcShapeInfo.originalShape[i]; }
    ...
    const uint32_t reduceK, const LastAxisShapeND& ndinfo, bool isBasicBlock = false)
{
    ...
    if (isBasicBlock && (splitM > SOFTMAX_BASIC_TILE_NUM) && (ndinfo.m % SOFTMAX_BASIC_TILE_NUM == 0)) {
        ...
        while (ndinfo.m % splitM != 0) { splitM -= SOFTMAX_BASIC_TILE_NUM; }
        // max repeat only support 255
        while (splitM * ndinfo.k >= FLOAT_REPEAT_SIZE * DEFAULT_BLOCK_SIZE) { splitM = splitM / HALF_FACTOR; }
    }
    return splitM;
}

};     // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_SOFTMAX_COMMON_SOFTMAX_COMMON_SHAPE_PROCESS_H