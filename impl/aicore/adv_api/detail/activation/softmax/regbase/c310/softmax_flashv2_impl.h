 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_C310_SOFTMAX_FLASHV2_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_C310_SOFTMAX_FLASHV2_IMPL_H
...
__aicore__ inline void SoftmaxFlashV2NZNoUpdateImpl(const LocalTensor<T1>& dstTensor,
    ...
    const LocalTensor<float>& workLocal, const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    ...
    uint16_t offset = CeilDivision(srcM * SOFTMAX_COMPUTE_DIM, FLOAT_REPEAT_SIZE) * FLOAT_REPEAT_SIZE;
    NotNumUnion notNum;
...
template <typename T1, typename T2>
__aicore__ inline void SoftmaxFlashV2NZWithTailNoUpdateImpl(const LocalTensor<T1>& dstTensor,
    const LocalTensor<T2>& expSumTensor, const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor,
    const LocalTensor<float>& workLocal, const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    uint16_t srcM = tiling.srcM;
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < mRepeatTimes; ++i) {
            ...
            MicroAPI::FusedExpSub(tmpVreg, srcVreg, maxVreg, pregOneBlk);
            StoreIfNeedCast<T1>(dstUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock, tmpVreg, pregOneBlk);
            if constexpr (sizeof(T1) == 2) {
                MicroAPI::DataCopy(expUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock, tmpVreg, pregCnt);
            }
        }
    ...
    const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    ...
    uint16_t dtypeBlkStride = dtypeRepStride / DEFAULT_BLK_NUM;
    uint16_t offset = CeilDivision(srcM * SOFTMAX_COMPUTE_DIM, FLOAT_REPEAT_SIZE) * FLOAT_REPEAT_SIZE;
    uint16_t offset1 = CeilDivision(srcM * SOFTMAX_SHAPE_NZ_BASIC_COUNT, FLOAT_REPEAT_SIZE) * FLOAT_REPEAT_SIZE;
    NotNumUnion notNum;
    notNum.i = F32_NEG_INF;
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < e2bRep; ++i) {
            ...
            if constexpr (sizeof(T1) == 2 && sizeof(T2) == 4) {
                MicroAPI::Cast<T1, float, softmaxCastTraitF32F16>(t1Reg, maxVreg, pregCnt);
                MicroAPI::Pack<uint16_t, uint32_t>(
                    (MicroAPI::RegTensor<uint16_t>&)t1Reg, (MicroAPI::RegTensor<uint32_t>&)t1Reg);
...
template <typename T1, typename T2>
__aicore__ inline void SoftmaxFlashV2NZWithTailUpdateImpl(const LocalTensor<T1>& dstTensor,
    const LocalTensor<T2>& expSumTensor, const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor,
    ...
    const LocalTensor<float>& workLocal, const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < mRepeatTimes; ++i) {
            ...
            StoreIfNeedCast<T1>(dstUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock, tmpVreg, pregOneBlk);
            if constexpr (sizeof(T1) == 2) {
                MicroAPI::DataCopy(expUb + i * FLOAT_REPEAT_SIZE + kRepeatTimes * dataBlock, tmpVreg, pregOneBlk);
            }
        }
    ...
    const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    ...
    if constexpr (!isUpdate) {
        ...
        if (originalSrcShape.k != tiling.srcK) {
            SoftmaxFlashV2NZWithTailUpdateImpl<T1, T2>(dstTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
    ...
    const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < srcM; ++i) {
            ...
            if constexpr (sizeof(T1) == 2 && sizeof(T2) == 4) {
                MicroAPI::Cast<T1, float, softmaxCastTraitF32F16>(t1Reg, expMaxVreg, pregOneBlk);
                MicroAPI::Pack<uint16_t, uint32_t>(
                    (MicroAPI::RegTensor<uint16_t>&)t1Reg, (MicroAPI::RegTensor<uint32_t>&)t1Reg);
                MicroAPI::DataCopy<T1, MicroAPI::StoreDist::DIST_INTLV_B16>(
...
template <typename T1, typename T2>
__aicore__ inline void SoftmaxFlashV2NDWithTailUpdateImpl(const LocalTensor<T1>& dstTensor,
    const LocalTensor<T2>& expSumTensor, const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor,
    const LocalTensor<T1>& expMaxTensor, const LocalTensor<T2>& inExpSumTensor, const LocalTensor<T2>& inMaxTensor,
    const LocalTensor<float>& workLocal, const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
...
__aicore__ inline void SoftmaxFlashV2NDNoUpdateImpl(const LocalTensor<T1>& dstTensor,
    const LocalTensor<T2>& expSumTensor, const LocalTensor<T2>& maxTensor, const LocalTensor<T1>& srcTensor,
    const LocalTensor<float>& workLocal, const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    if constexpr (isBasicBlock) {
        SoftMaxGenericNDImpl<T1, T2, true>(
            dstTensor, expSumTensor, maxTensor, srcTensor, workLocal, originalSrcShape, tiling);
    } else {
        if (tiling.srcK == B32_DATA_NUM_PER_BLOCK && IsSameType<T1, float>::value) {
            SingleSoftMaxGenericNDForBlkImpl<T1, T2, true>(
                dstTensor, expSumTensor, maxTensor, srcTensor, workLocal, originalSrcShape, tiling);
        } else if (tiling.srcK == B32_DATA_NUM_PER_BLOCK * 2) {
            SingleSoftMaxGenericNDAlignedWithBlkImpl<T1, T2, true>(
                dstTensor, expSumTensor, maxTensor, srcTensor, workLocal, originalSrcShape, tiling);
        } else if (originalSrcShape.k <= FLOAT_REPEAT_SIZE) {
            SingleSoftMaxGenericNDImpl<T1, T2, true>(
                dstTensor, expSumTensor, maxTensor, srcTensor, workLocal, originalSrcShape, tiling);
        } else if (originalSrcShape.k % FLOAT_REPEAT_SIZE != 0) {
            SoftMaxGenericNDWithTailImpl<T1, T2, true>(
                dstTensor, expSumTensor, maxTensor, srcTensor, workLocal, originalSrcShape, tiling);
        } else {
            SoftMaxGenericNDImpl<T1, T2, true>(
                dstTensor, expSumTensor, maxTensor, srcTensor, workLocal, originalSrcShape, tiling);
    ...
    const LastAxisShapeND& originalSrcShape, const SoftMaxTiling& tiling)
{
    ...
    if constexpr (isBasicBlock && isUpdate) {
        SoftmaxFlashV2NDUpdateImpl<T1, T2>(dstTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor, inExpSumTensor,
            inMaxTensor, workLocal, originalSrcShape, tiling);
    } else {
        if constexpr (!isUpdate) {
            SoftmaxFlashV2NDNoUpdateImpl<T1, T2>(
                dstTensor, expSumTensor, maxTensor, srcTensor, workLocal, originalSrcShape, tiling);
        } else {
            if (originalSrcShape.k % FLOAT_REPEAT_SIZE) {
                SoftmaxFlashV2NDWithTailUpdateImpl<T1, T2>(dstTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
                    inExpSumTensor, inMaxTensor, workLocal, originalSrcShape, tiling);
            } else {
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_C310_SOFTMAX_FLASHV2_IMPL_H