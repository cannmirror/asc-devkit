 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_C310_SOFTMAX_FLASH_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_C310_SOFTMAX_FLASH_IMPL_H

#include "softmax_common_impl.h"

namespace AscendC {
template <typename T1, typename T2>
__aicore__ inline void SoftmaxFlashNDImpl(__local_mem__ T1* dstUb, __local_mem__ T2* sumUb, __local_mem__ T2* maxUb,
    __local_mem__ T1* srcUb, __local_mem__ T1* expMaxUb, __local_mem__ T2* inSumUb, __local_mem__ T2* inMaxUb,
    __local_mem__ float* workUb, __local_mem__ float* tmpUb, const uint16_t srcM, const uint16_t repeatTimes,
    const uint32_t srcK)
{
    ...
    const SoftMaxShapeInfo& softmaxShapeInfo = {})
{
    ...
    if (!isUpdate) {
        ...
        if constexpr (isBasicBlock) {
            VF_CALL<SoftmaxFlashNDImpl<T1, T2>>(
                dstUb, sumUb, maxUb, srcUb, expMaxUb, inSumUb, inMaxUb, workUb, tmpUb, srcM, repeatTimes, srcK);
        } else {
            if (originalSrcShape.k % stride != 0) {
                VF_CALL<SoftmaxFlashNDWithTailImpl<T1, T2>>(dstUb, sumUb, maxUb, srcUb, expMaxUb, inSumUb, inMaxUb,
                    workUb, tmpUb, srcM, repeatTimes, srcK, originK);
            } else {
                VF_CALL<SoftmaxFlashNDImpl<T1, T2>>(
                    dstUb, sumUb, maxUb, srcUb, expMaxUb, inSumUb, inMaxUb, workUb, tmpUb, srcM, repeatTimes, srcK);
            }
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_C310_SOFTMAX_FLASH_IMPL_H
