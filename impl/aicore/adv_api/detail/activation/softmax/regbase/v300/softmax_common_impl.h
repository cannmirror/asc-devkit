 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_V300_SOFTMAX_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_V300_SOFTMAX_COMMON_IMPL_H
    ...
    const LocalTensor<half>& tmpTensor, const struct ReduceLastND& reduceParam)
{
    ...
    if (reduceParam.originalSrcK <= HALF_REPEAT_SIZE) {
        WholeReduceMax(dst, src, reduceParam.originalSrcK, reduceParam.srcM, 1, 1, reduceParam.srcK / HALF_NUM_PER_BLK,
        ...
        WholeReduceMax(dst, tmpTensor, FULL_MASK_LEN, reduceParam.originalSrcM, 1, 1, DEFAULT_REPEAT_STRIDE,
            ReduceOrder::ORDER_ONLY_VALUE);
    }
    ...
    const LocalTensor<float>& tmpTensor, const struct ReduceLastND& reduceParam)
{
    ...
    if (reduceParam.originalSrcK <= FLOAT_REPEAT_SIZE) {
        ...
        // repeat  = srcM,next need broadcast
        WholeReduceMax(dst, tmpTensor, FLOAT_REPEAT_SIZE, reduceParam.originalSrcM, 1, 1, DEFAULT_REPEAT_STRIDE,
            ReduceOrder::ORDER_ONLY_VALUE);
    }
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_REGBASE_V300_SOFTMAX_COMMON_IMPL_H