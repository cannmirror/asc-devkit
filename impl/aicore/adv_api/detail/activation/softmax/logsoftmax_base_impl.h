 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_LOGSOFTMAX_BASE_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_LOGSOFTMAX_BASE_IMPL_H
    ...
    const LocalTensor<float>& dst, const LocalTensor<float>& src, const uint32_t originalSrcM, const uint32_t srcK)
{
    if (srcK < SOFTMAX_SUB_DIV_ROW_COLUMN_SIZE) {
        ...
        for (int j = 0; j < originalSrcM; j++) {
            ...
            PipeBarrier<PIPE_V>();
            Muls<float, false>(dst[j * srcK], src[j * srcK], static_cast<float>(SCALAR_NATURE_LOG_10), MASK_PLACEHOLDER,
    ...
    const uint32_t& splitCount, const ReduceLastND& reduceParam)
{
    ...
    LogSoftMaxGenericNZSubImpl(splitNZBlockCount, tmpBuffer0, tmpBuffer1, splitOffset, lastSplitNZBlockOffset, mask,
        lastBlockMaskLen, splitCount);

    LogSoftMaxGenericNZExpImpl(splitNZBlockCount, tmpBuffer0, tmpBuffer1, splitOffset, lastSplitNZBlockOffset, mask,
        lastBlockMaskLen, splitCount);

    LogSoftMaxGenericNZReduceSumImpl(tmpBuffer0, tmpBuffer1, sumTensor, offset2, splitCount, mask, reduceParam);

    LogSoftMaxGenericNZDivImpl(splitNZBlockCount, tmpBuffer0, tmpBuffer1, splitOffset, lastSplitNZBlockOffset, mask,
        lastBlockMaskLen, splitCount);
    ...
    const LogSoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo = {})
{
...
#endif
    CHECK_FUNC_HIGHLEVEL_API(LogSoftMax, (T, isReuseSource, isDataFormatNZ),
    ...
    SoftMaxTilingFunc(tempBuffer.GetSize(), {srcNDinfo.m, srcNDinfo.k, originalSrcShape.m, srcNDinfo.k}, newTiling,
        sizeof(T), sizeof(T), isDataFormatNZ);
    ...
    if constexpr (isDataFormatNZ) {
        ...
        if (srcNDinfo.k != tiling.srcK || srcNDinfo.m != tiling.srcM) {
            ...
            LogSoftMaxNDImpl(dst, sumTensor, maxTensor, src, tempBuffer, originalSrcShape, tiling);
        }
    }
#endif
}
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SOFTMAX_LOGSOFTMAX_BASE_IMPL_H