 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_REGLU_REGLU_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_REGLU_REGLU_C310_IMPL_H
    ...
    __ubuf__ T* dst, __ubuf__ T* src0, __ubuf__ T* src1, uint32_t count, const uint16_t repeatTimes)
{
    ...
    for (uint16_t i = 0; i < repeatTimes; ++i) {
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            ...
            MicroAPI::Cast<float, T, ReGluInternal::castTraitB16ToB32>(srcVreg0, srcOrigin0, mask);
            MicroAPI::DataCopy<T, MicroAPI::LoadDist::DIST_UNPACK_B16>(srcOrigin1, src1 + i * oneRepElm);
            MicroAPI::Cast<float, T, ReGluInternal::castTraitB16ToB32>(srcVreg1, srcOrigin1, mask);
        } else {
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            MicroAPI::Cast<T, float, ReGluInternal::castTraitB32ToB16>((MicroAPI::RegTensor<T>&)dstVreg, dstVreg, mask);
    ...
    const LocalTensor<T>& srcTensor1, const uint32_t count)
{
    ...
    VF_CALL<ReGluImplVF<T>>((__ubuf__ T*)dstTensor.GetPhyAddr(), (__ubuf__ T*)srcTensor0.GetPhyAddr(),
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_REGLU_REGLU_C310_IMPL_H
