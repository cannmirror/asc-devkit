 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_ACTIVATION_SWIGLU_SWIGLU_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_ACTIVATION_SWIGLU_SWIGLU_C310_IMPL_H
...
template <typename T>
__aicore__ inline void SwiGLUImplVF(__ubuf__ T* dst, __ubuf__ T* src0, __ubuf__ T* src1, const float scalarValue,
    uint32_t count, const uint16_t repeatTimes)
{
    ...
    for (uint16_t i = 0; i < repeatTimes; ++i) {
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            ...
            MicroAPI::Cast<float, half, SwiGLUInternal::castTraitB16ToB32>(srcVreg0, srcOrigin0, mask);
            MicroAPI::DataCopy<half, MicroAPI::LoadDist::DIST_UNPACK_B16>(srcOrigin1, src1 + i * oneRepElm);
            MicroAPI::Cast<float, half, SwiGLUInternal::castTraitB16ToB32>(srcVreg1, srcOrigin1, mask);
        } else {
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            MicroAPI::Cast<half, float, SwiGLUInternal::castTraitB32ToB16>(
    ...
    const LocalTensor<T>& srcTensor1, const float scalarValue, const uint32_t count)
{
    ...
    VF_CALL<SwiGLUImplVF<T>>((__ubuf__ T*)dstTensor.GetPhyAddr(), (__ubuf__ T*)srcTensor0.GetPhyAddr(),
...
#endif // AICORE_ADV_API_DETAIL_ACTIVATION_SWIGLU_SWIGLU_C310_IMPL_H
