 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_KFC_MATMUL_SERVER_AUX_H
#define AICORE_ADV_API_DETAIL_MATMUL_KFC_MATMUL_SERVER_AUX_H
...
template <bool SHARED, class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, const auto& MM_CFG, class MM_CB,
    MATMUL_POLICY_TEMPLATE_OF(MATMUL_POLICY)>
...
private:
    __aicore__ inline void WaitAB()
    {
        if constexpr (GetPhyType(A_TYPE::pos) == Hardware::UB || GetPhyType(B_TYPE::pos) == Hardware::UB
                      || PhyMxScalePosIsUB<A_TYPE>() || PhyMxScalePosIsUB<B_TYPE>()
                      || GetPhyType(BIAS_TYPE::pos) == Hardware::UB) {
