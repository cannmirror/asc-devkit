 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_H
        ...
        for (batchLoop->OuterStart(); !batchLoop->OuterEnd(); batchLoop->OuterNext()) {
            if constexpr (ToMatmulConfig(MM_CFG).isBiasBatch) {
                bias = MATMUL_MODULE(BiasScheduler)
                           ->CopyIn(MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetSingleCoreN(),
            ...
            for (batchLoop->SplitStart(); !batchLoop->SplitEnd(); batchLoop->SplitNext()) {
                MATMUL_MODULE(BatchCopyCubeInA)
                    ->BatchLoad(a1, matrixStrideA, batchLoop->GetOuterIndex(), batchLoop->GetSplitIndex(),
                        batchLoop->GetSplitSize());
                MATMUL_MODULE(BatchCopyCubeInB)
                    ->BatchLoad(b1, matrixStrideB, batchLoop->GetOuterIndex(), batchLoop->GetSplitIndex(),
                        batchLoop->GetSplitSize());
...
private:
    ...
    __aicore__ inline void UpdateOffset(BatchOffsetInfo& batchOffsetInfo, BatchSchedulerContext& ctx)
    {
        ...
        if constexpr (ToMatmulConfig(MM_CFG).isNBatchOut) {
            MATMUL_MODULE(BatchLoop)->SetBatchOutCacheNum(MATMUL_MODULE(BatchLoop)->GetBatchOutCacheNum() + 1);
        }
    }
    ...
    __aicore__ inline void ComputeMultiIter(LocalTensor<TransAT>& a1, LocalTensor<TransBT>& b1,
        LocalTensor<BiasT>& bias, bool enPartialSum, BatchSchedulerContext& ctx)
    {
        ...
        do {
            ...
            MATMUL_MODULE(BiasScheduler)
                ->SplitLoad(bias, ctx.bL0Params.axisL0Len,
                    ctx.offsetBias
        ...
        bool enPartialSum, BatchSchedulerContext& ctx)
    {
        ...
        do {
            ...
            MATMUL_MODULE(LoadToA2)->Prepare(MATMUL_MODULE(MatmulShapeInfo)->IsTransposeA(), ctx.aL0Params.kAxisL1Len,
        ...
        BatchSchedulerContext& ctx, int32_t sL0CInit, int32_t sL0CLast, bool enPartialSum)
    {
        ...
        MATMUL_MODULE(LoadToB2)->Load(b2, b1[ctx.offsetB], ctx.bL0Params.axisL1Len, ctx.bL0Params.kAxisL1Len,
            ctx.bL0Params.axisL0Len, computeK, ctx.bL0Params.axisL1Offset, bKL1Offset,
        ...
        for (int32_t idx = 0; idx < nL0DBLoop; ++idx) {
            ...
            MATMUL_MODULE(MmadCompute)
                ->Compute(MATMUL_MODULE(CubeOutBuffer)
                              ->GetTensor()[idx * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM()
                                            * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseN()],
        ...
        BatchSchedulerContext& ctx, int32_t sL0CInit, int32_t sL0CLast, bool enPartialSum)
    {
        ...
        MATMUL_MODULE(LoadToA2)->Load(a2, a1[ctx.offsetA], ctx.aL0Params.axisL1Len, ctx.aL0Params.kAxisL1Len,
            ctx.aL0Params.axisL0Len, computeK, ctx.aL0Params.axisL1Offset, aKL1Offset,
            MATMUL_MODULE(MatmulShapeInfo)->IsTransposeA());
        ...
        for (int32_t idx = 0; idx < mL0DbLoop; ++idx) {
            ...
            MATMUL_MODULE(LoadToB2)->Load(b2, b1[ctx.offsetB], ctx.bL0Params.axisL1Len, ctx.bL0Params.kAxisL1Len,
                ctx.bL0Params.axisL0Len, computeK, axisL1DbOffset, bKL1Offset,
                MATMUL_MODULE(MatmulShapeInfo)->IsTransposeB());
            ...
            MATMUL_MODULE(BiasScheduler)
                ->SplitLoad(bias, ctx.bL0Params.axisL0Len,
                    ctx.offsetBias
                        + (MATMUL_MODULE(NLoop)->GetOuterIdx() + idx)
                              * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseN());
    ...
    __aicore__ inline void CalcBatchIterateBiasOffsetInfo(BatchOffsetInfo& batchOffsetInfo)
    {
        ...
        if (tiling.GetCLayoutInfoG() == 1 && (tiling.GetALayoutInfoG() != 1 || tiling.GetBLayoutInfoG() != 1)) {
            ...
            ASSERT(
                ...
                && (tiling.GetCLayoutInfoB() != 1
                    || (tiling.GetALayoutInfoB() == 1
                        && tiling.GetBLayoutInfoB() == 1))); // multi axis BRC is not supported.
        ...
        if constexpr (!ToMatmulConfig(MM_CFG).isBiasBatch) {
            ...
            batchOffsetInfo.alignBias =
                CeilAlign(MATMUL_MODULE(MatmulShapeInfo)->template GetSingleCoreN<false, IsBasic(MM_CFG)>(),
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_H
