 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_SINGLE_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_SINGLE_H
...
class BatchScheduler<IMPL, A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, MM_CFG,
    enable_if_t<!MatmulFeatureTrait<MM_CFG>::IsSupportCmatrixInitVal()
...
public:
    ...
    __aicore__ inline void Schedule(const LocalTensor<DstT>& dst, bool enPartialSum, uint8_t enAtomic,
        bool enSequentialWrite, const uint32_t matrixStrideA, const uint32_t matrixStrideB,
        const uint32_t matrixStrideC)
    {
        if constexpr (MatmulFeatureTrait<MM_CFG>::IsNeedUB()) {
    ...
    __aicore__ inline void Schedule(const GlobalTensor<DstT>& dst, bool enPartialSum, uint8_t enAtomic,
        bool enSequentialWrite, const uint32_t matrixStrideA, const uint32_t matrixStrideB,
        const uint32_t matrixStrideC)
    {
        ScheduleImpl(dst, enPartialSum, enAtomic, enSequentialWrite, matrixStrideA, matrixStrideB, matrixStrideC);
...
private:
    ...
    {
        MATMUL_MODULE(Scheduler)->GetResult(
            ubCmatrix[MATMUL_MODULE(BatchLoop)->GetOuterIndex() * MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreM()
                      * MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreN()],
            enAtomic, enSequentialWrite);
        if constexpr (!MatmulFeatureTrait<MM_CFG>::IsNeedUB()) {
            return;
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_SINGLE_H
