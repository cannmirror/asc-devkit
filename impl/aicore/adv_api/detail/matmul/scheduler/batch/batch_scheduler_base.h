 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_BASE_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_BASE_H
...
public:
    ...
    __aicore__ inline void CalcBatchIterateAOffsetInfo(BatchOffsetInfo& batchOffsetInfo)
    {
        ...
        } else if (A_TYPE::layout != LayoutMode::NORMAL && tiling.GetALayoutInfoB() == 1
                   && tiling.GetBLayoutInfoB() != 1) { // BRC for B axis
            ASSERT((tiling.GetBLayoutInfoB() > 0)
    ...
    // Compute related
    __aicore__ inline void MacroCompute(LocalTensor<TransAT>& a1, LocalTensor<TransBT>& b1, BatchSchedulerContext& ctx,
        int32_t sL0CInit, int32_t sL0CLast)
    {
        ...
        MATMUL_MODULE(MmadCompute)
            ->Compute(MATMUL_MODULE(CubeOutBuffer)->GetTensor()[offsetC], a2, b2, ctx.aL0Params.axisL0Len, kL0Len,
...
private:
        ...
        const T& dst, const LocalTensor<L0cT>& co1Local, uint8_t enAtomic, bool enSequentialWrite)
    {
        ...
        if (mLoop->GetL0DBLoopNum() > 1) {
            GetTensorCImpl(dst, co1Local, mLoop->GetOuterIdx(), nLoop->GetOuterIdx(), mLoop->GetBaseShape(),
                nLoop->GetBaseShape(), mLoop->GetBaseBlockShape(), nLoop->GetBaseBlockShape(), enAtomic,
                enSequentialWrite);
            int32_t baseUseM =
                (mLoop->GetOuterIdx() + 2 == mLoop->GetTotalIter()) ? mLoop->GetTail() : tiling.GetBaseM();
            int32_t blockUseM = Ceil(baseUseM, BLOCK_CUBE);
            GetTensorCImpl(dst, co1Local[tiling.GetBaseM() * tiling.GetBaseN()], mLoop->GetOuterIdx() + 1,
                nLoop->GetOuterIdx(), baseUseM, nLoop->GetBaseShape(), blockUseM, nLoop->GetBaseBlockShape(), enAtomic,
                enSequentialWrite);
            ...
            GetTensorCImpl(dst, co1Local[tiling.GetBaseM() * tiling.GetBaseN()], mLoop->GetOuterIdx(),
                nLoop->GetOuterIdx() + 1, mLoop->GetBaseShape(), baseUseN, mLoop->GetBaseBlockShape(), blockUseN,
    ...
    __aicore__ inline void GetTensorCImpl(const GlobalTensor<DstT>& gm, const LocalTensor<L0cT>& co1Local, int curRow,
        int curCol, int32_t baseHeight, int32_t baseWidth, int32_t baseBlockHeight, int32_t baseBlockWidth,
    ...
    template <class T>
    __aicore__ inline void GetTensorCImplCore(const T& dst, const LocalTensor<L0cT>& co1Local, int32_t curRow,
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BATCH_BATCH_SCHEDULER_BASE_H
