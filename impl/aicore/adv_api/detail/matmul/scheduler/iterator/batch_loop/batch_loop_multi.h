...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_ITERATOR_BATCH_LOOP_BATCH_LOOP_MULTI_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_ITERATOR_BATCH_LOOP_BATCH_LOOP_MULTI_H
...
public:
    ...
    __aicore__ inline void SplitStart()
    {
        ...
        const auto& tiling = MATMUL_MODULE(MatmulShapeTiling)->GetTiling();
        ASSERT((batchA_ * tiling.GetSingleCoreM() * tiling.GetSingleCoreK()
...
private:
    ...
    __aicore__ inline void CalcBatchAB(int32_t batchNumLarge, int32_t batchNumLess, int32_t largeMatrixSingleBatchSize,
        int32_t lessMatrixSingleBatchSize)
    {
        ...
        if (oddAndLargeThanL1_) {
            ...
            while (batchNumLess % batchInner != 0 && batchInner > 0) { --batchInner; }
            mainBatchInner_ = batchInner;
    ...
    __aicore__ inline void UpdateBatchNumParams()
    {
        ...
        if (batchOuter_ > 1 && batchA_ == batchB_) {
            ...
            splitSize_ =
                (batchNum_ >= DB_FACTOR) && (batchA_ % DB_FACTOR == 0) && (batchB_ % DB_FACTOR == 0) ? DB_FACTOR : 1;
            splitBatchNum_ = batchNum_ / splitSize_;
        }
    }
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_ITERATOR_BATCH_LOOP_BATCH_LOOP_MULTI_H