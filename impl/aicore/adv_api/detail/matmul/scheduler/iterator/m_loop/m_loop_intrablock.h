...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_ITERATOR_M_LOOP_M_LOOP_INTRABLOCK_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_ITERATOR_M_LOOP_M_LOOP_INTRABLOCK_H
    ...
    __aicore__ inline void SetSingleShape(int32_t singleShape)
    {
        if (MATMUL_MODULE(MatmulSubBlockInfo)->GetSubBlockIdx() == 0) {
            ...
            ASCENDC_ASSERT((v1TotalIter_ > 0), {
                KERNEL_LOG(KERNEL_ERROR, "invalid singleCoreM, v1TotalIter_ is %d , which should be larger than 0",
                    v1TotalIter_);
            });
        }
    }
