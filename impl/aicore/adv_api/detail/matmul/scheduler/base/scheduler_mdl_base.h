 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_MDL_BASE_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_MDL_BASE_H
        ...
        const bool isATranspose, const bool isBTranspose, const bool sL0CInit, const bool sL0CLast)
    {
        ...
        if constexpr (HasSparseIndex<B_TYPE>()) {
            MATMUL_MODULE(MmadCompute)
                ->template Compute<true>(cMatrix, a2, b2, madM, madK, madN, isATranspose, isBTranspose, unitFlag,
        ...
        if constexpr (PhyPosIsL1OrUB<MM_CFG>(B_TYPE::pos)) {
            ...
            if constexpr (!MatmulFeatureTrait<MM_CFG>::IsNeedUB()) {
                if constexpr (IsSameTypeV<TransBT, int8_t>) {
                    ...
                    bL0Params.axisL1Offset = tilingBaseN
    ...
    __aicore__ inline void DoPreloadLoad()
    {
            ...
            if constexpr (ToMatmulConfig(MM_CFG).doMTE2Preload == PRELOAD_M) {
                if (cacheA1Factor_ == 1
                    ...
                    MATMUL_MODULE(CopyCubeInA)
                        ->AsyncLoadData((MATMUL_MODULE(MLoop)->GetInnerIdx()
        ...
        if constexpr (ToMatmulConfig(MM_CFG).doMTE2Preload == PRELOAD_K) {
            ...
            if (cacheB1Factor_ == 1 && !MATMUL_MODULE(KLoop)->IsBKL1FullLoad()
                ...
                MATMUL_MODULE(CopyCubeInB)
                    ->AsyncLoadData(nextKbIdx, MATMUL_MODULE(NLoop)->GetInnerIdx(),
                ...
                // preload B1
                MATMUL_MODULE(CopyCubeInB)
                    ->AsyncLoadData(0, (MATMUL_MODULE(NLoop)->GetInnerIdx() + 1) % MATMUL_MODULE(NLoop)->GetTotalIter(),
