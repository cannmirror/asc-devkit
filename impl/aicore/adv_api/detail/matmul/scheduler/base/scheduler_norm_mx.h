 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_NORM_MX_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_NORM_MX_H
...
private:
    ...
    __aicore__ inline void ComputeOneIter(bool enPartialSum)
    {
        ...
        LocalTensor<ScaleT> scaleA1 = MATMUL_MODULE(CopyCubeInScaleA)
                                          ->LoadData(0, 0, MATMUL_MODULE(MLoop)->GetTileShape(),
                                              Ceil(MATMUL_MODULE(KLoop)->GetTileShapeA(), MX_K_FACTOR));
        LocalTensor<ScaleT> scaleB1 = MATMUL_MODULE(CopyCubeInScaleB)
                                          ->LoadData(0, 0, Ceil(MATMUL_MODULE(KLoop)->GetTileShapeB(), MX_K_FACTOR),
        ...
        CubeCompute(MATMUL_MODULE(CubeOutBuffer)->GetTensor(), a2, b2, aL0Params.axisL0Len, bL0Params.axisL0Len,
            CeilAlign(MATMUL_MODULE(KLoop)->GetTileShapeA(), c0Size_), isATranspose, isBTranspose, enPartialSum,
            !enPartialSum, true);
    ...
    __aicore__ inline void ComputeMultiIter(bool enPartialSum)
    {
        ...
        do {
            ...
            LocalTensor<SrcAT> a1 =
                MATMUL_MODULE(CopyCubeInA)
                    ->LoadData(MATMUL_MODULE(MLoop)->GetInnerIdx(), kOuterIdx, MATMUL_MODULE(MLoop)->GetTileShape(),
                        MATMUL_MODULE(KLoop)->GetTileShapeA());
            LocalTensor<SrcBT> b1 =
                MATMUL_MODULE(CopyCubeInB)
                    ->LoadData(kOuterIdx, MATMUL_MODULE(NLoop)->GetInnerIdx(), MATMUL_MODULE(KLoop)->GetTileShapeB(),
                        MATMUL_MODULE(NLoop)->GetTileShape());
            // scaleA/scaleB
            ...
            LocalTensor<ScaleT> scaleB1 =
                MATMUL_MODULE(CopyCubeInScaleB)
                    ->LoadData(kOuterIdx, MATMUL_MODULE(NLoop)->GetInnerIdx(),
...
private:
    __aicore__ inline void PadZeros(const LocalTensor<SrcAT>& a1, const LocalTensor<SrcBT>& b1)
    {
        if (MATMUL_MODULE(MatmulShapeInfo)->IsTransposeA()) {
            PadZerosOuterAxis(a1, MATMUL_MODULE(KLoop)->GetTileShapeA(), MATMUL_MODULE(MLoop)->GetTileShape(),
        ...
        if (!MATMUL_MODULE(MatmulShapeInfo)->IsTransposeB()) {
            PadZerosOuterAxis(b1, MATMUL_MODULE(KLoop)->GetTileShapeB(), MATMUL_MODULE(NLoop)->GetTileShape(),
                MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseK(),
    ...
    template <typename T>
    __aicore__ inline void PadZerosOuterAxis(
        const LocalTensor<T>& dst, int height, int width, int baseHeight, int baseWidth)
    {
        constexpr int32_t c0Size = AuxGetC0Size<T>();
        if (height < baseHeight) {
            const uint32_t alignBaseHeight = Ceil(height, c0Size) * c0Size;
            auto padTensor = dst.template ReinterpretCast<uint16_t>();
            InitConstValueParams<uint16_t> initConstValueParams;
            initConstValueParams.repeatTimes = Ceil(width, c0Size);
            initConstValueParams.initValue = 0;
            initConstValueParams.dstGap = height;
            initConstValueParams.blockNum = alignBaseHeight - height;
            uint32_t dstOffset = height * BLOCK_CUBE;
            InitConstValue(padTensor[dstOffset], initConstValueParams);
        }
    }

    __aicore__ inline void SplitPrepare(const bool isATranspose, const bool isBTranspose, const bool isScaleATranspose,
        const bool isScaleBTranspose, MxSplitParams& aL0Params, MxSplitParams& bL0Params)
    {
        ...
        MxSplitParams& aL0Params, MxSplitParams& bL0Params, const bool isATranspose, const bool isBTranspose)
    {
        ...
        if (isATranspose) {
            int32_t blockUseK = Ceil(MATMUL_MODULE(KLoop)->GetTileShapeA(), BLOCK_CUBE);
            if constexpr (PhyPosIsL1OrUB<MM_CFG>(A_TYPE::pos)) {
                blockUseK = Ceil(MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreK(), BLOCK_CUBE);
            }
            aL0Params.axisL0Len = Ceil(MATMUL_MODULE(MLoop)->GetTileShape(), c0Size_) * c0Size_;
            aL0Params.kAxisL1Len = Ceil(blockUseK, MX_EVEN_FACTOR) * MX_EVEN_FACTOR * BLOCK_CUBE;
        }
        if (!isBTranspose) {
            int32_t blockUseK = Ceil(MATMUL_MODULE(KLoop)->GetTileShapeB(), BLOCK_CUBE);
            if constexpr (PhyPosIsL1OrUB<MM_CFG>(B_TYPE::pos)) {
                blockUseK = Ceil(MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreK(), BLOCK_CUBE);
            }
            bL0Params.axisL0Len = Ceil(MATMUL_MODULE(NLoop)->GetTileShape(), c0Size_) * c0Size_;
            bL0Params.kAxisL1Len = Ceil(blockUseK, MX_EVEN_FACTOR) * MX_EVEN_FACTOR * BLOCK_CUBE;
        }
        if constexpr (PhyPosIsL1OrUB<MM_CFG>(A_TYPE::scalePosition)) {
            aL0Params.kAuxMatrixL1Offset =
                Ceil(MATMUL_MODULE(KLoop)->GetOuterIdx() * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseK(),
                    MX_K_FACTOR);
        } else {
            if constexpr (!IS_INTRA_BLOCK) {
                aL0Params.kAuxMatrixL1Len = Ceil(MATMUL_MODULE(KLoop)->GetTileBlockShapeA() * c0Size_, MX_K_FACTOR);
            } else {
                aL0Params.kAuxMatrixL1Len =
                    Ceil(MATMUL_MODULE(KLoop)->template GetTileBlockShapeA<true>() * c0Size_, MX_K_FACTOR);
            }
        }
        if constexpr (PhyPosIsL1OrUB<MM_CFG>(B_TYPE::scalePosition)) {
            ...
            if constexpr (!IS_INTRA_BLOCK) {
                bL0Params.kAuxMatrixL1Len = Ceil(MATMUL_MODULE(KLoop)->GetTileBlockShapeB() * c0Size_, MX_K_FACTOR);
            } else {
                bL0Params.kAuxMatrixL1Len =
                    Ceil(MATMUL_MODULE(KLoop)->template GetTileBlockShapeB<true>() * c0Size_, MX_K_FACTOR);
            }
        }
    }
    ...
    __aicore__ inline MxSplitParams InitSplitAParams()
    {
        ...
        if constexpr (PhyPosIsL1OrUB<MM_CFG>(A_TYPE::scalePosition)) {
            aL0Params.kAuxMatrixL1Len =
                Ceil(CeilAlign(MATMUL_MODULE(MatmulShapeInfo)->template GetSingleCoreK<IS_INTRA_BLOCK>(), c0Size_),
                    MX_K_FACTOR);
        ...
        const MxSplitParams& aL0Params, const bool isATranspose, const bool isScaleATranspose)
    {
        ...
        int32_t kL0Len = Ceil(MATMUL_MODULE(KLoop)->GetBaseShape(), c0Size_) * c0Size_;
        // Split
    ...
    __aicore__ inline void CubeCompute(const LocalTensor<L0cT>& cMatrix, const LocalTensor<SrcA2T>& a2,
        ...
        const bool isATranspose, const bool isBTranspose, const bool enPartialSum, const bool sL0CInit,
        const bool sL0CLast)
    {
        ...
        MATMUL_MODULE(MmadCompute)
            ->Compute(cMatrix, a2, b2, madM, madK, madN, isATranspose, isBTranspose, unitFlag, cmatrixSource,
                cmatrixInitVal, isBias);
