 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_NORM_OUTER_PRODUCT_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_NORM_OUTER_PRODUCT_H
...
class MatmulScheduler<IMPL, A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, MM_CFG, POLICY_TYPE,
    enable_if_t<
        (isNormEnableScheduler<A_TYPE,
             MM_CFG> || (A_TYPE::layout == LayoutMode::NORMAL && ToMatmulConfig(MM_CFG).batchMode == BatchMode::SINGLE_LARGE_THAN_L1))
...
private:
        ...
        const bool isBTranspose)
    {
        ...
        for (int dbInner = 0; dbInner < l0aDBLoop; dbInner++) {
            ...
            LocalTensor<TransAT> a1 = MATMUL_MODULE(CopyCubeInA)
                                          ->LoadData(MATMUL_MODULE(MLoop)->GetInnerIdx() + dbInner, kOuterIdx, dbUsedM,
            ...
            CubeCompute(MATMUL_MODULE(CubeOutBuffer)
                            ->GetTensor()[dbInner * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM()
    ...
    __aicore__ inline void ComputeLoopN(const LocalTensor<TransAT>& a1, LocalTensor<BiasT>& bias,
        const SplitParams& aL0Params, const SplitParams& bL0Params, const bool enPartialSum, const int32_t kOuterIdx,
        const bool isATranspose, const bool isBTranspose)
    {
        ...
        for (int dbInner = 0; dbInner < l0bDBLoop; dbInner++) {
            ...
            LocalTensor<TransBT> b1 = MATMUL_MODULE(CopyCubeInB)
                                          ->LoadData(kOuterIdx, MATMUL_MODULE(NLoop)->GetInnerIdx() + dbInner,
        ...
        if constexpr (ToMatmulConfig(MM_CFG).iterateOrder == IterateOrder::ORDER_N) {
            LocalTensor<BiasT> bias =
                MATMUL_MODULE(BiasScheduler)
                    ->CopyIn(MATMUL_MODULE(NLoop)->GetBaseShape(), 1,
    ...
    __aicore__ inline LocalTensor<TransAT> SplitA(const LocalTensor<TransAT>& a1, const SplitParams& aL0Params,
        const int32_t posA, const uint16_t mL1Len, const uint16_t madMLen, const uint16_t kL0Len,
        const bool isATranspose)
    {
        ...
        if (!(MATMUL_MODULE(TBufPoolL0)->template Hit<TPosition::A2>(posA))) {
            ...
            MATMUL_MODULE(LoadToA2)->Load(a2, a1, mL1Len, aL0Params.kAxisL1Len, madMLen, kL0Len, aL0Params.axisL1Offset,
    ...
    __aicore__ inline LocalTensor<TransBT> SplitB(const LocalTensor<TransBT>& b1, const SplitParams& bL0Params,
        const int32_t posB, const uint16_t nL1Len, const uint16_t madNLen, const uint16_t kL0Len,
        const bool isBTranspose)
    {
        LocalTensor<TransBT> b2;
        if (!(MATMUL_MODULE(TBufPoolL0)->template Hit<TPosition::B2>(posB))) {
            ...
            MATMUL_MODULE(LoadToB2)->Load(b2, b1, nL1Len, bL0Params.kAxisL1Len, madNLen, kL0Len, bL0Params.axisL1Offset,
    ...
    __aicore__ inline void GetResultImpl(const GlobalTensor<DstT>& gm, uint8_t enAtomic, bool enSequentialWrite)
    {
        ...
        if (MATMUL_MODULE(NLoop)->GetL0DBLoopNum() > 1) { // Means L0 N db, need to excute twice FixpipeL0CToGm
            ...
            FixpipeL0CToGm(gm, co1Local[co1Offset], MATMUL_MODULE(MLoop)->GetInnerIdx(),
                MATMUL_MODULE(NLoop)->GetInnerIdx() + 1, enAtomic, enSequentialWrite, baseHeight, baseWidth,
            ...
            FixpipeL0CToGm(gm, co1Local[co1Offset], MATMUL_MODULE(MLoop)->GetInnerIdx() + 1,
                MATMUL_MODULE(NLoop)->GetInnerIdx(), enAtomic, enSequentialWrite, baseHeight, baseWidth,
                baseBlockHeight, baseBlockWidth);
        } else {
            FixpipeL0CToGm(gm, co1Local, MATMUL_MODULE(MLoop)->GetInnerIdx(), MATMUL_MODULE(NLoop)->GetInnerIdx(),
                enAtomic, enSequentialWrite, baseHeight, baseWidth, baseBlockHeight, baseBlockWidth);
    ...
    __aicore__ inline void FixpipeL0CToGm(const GlobalTensor<DstT>& gm, const LocalTensor<L0cT>& co1Local, int32_t curM,
        int32_t curN, uint8_t enAtomic, bool enSequentialWrite, int32_t baseHeight, int32_t baseWidth,
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_NORM_OUTER_PRODUCT_H
