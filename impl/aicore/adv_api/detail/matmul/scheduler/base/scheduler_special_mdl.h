 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_SPECIAL_MDL_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_SPECIAL_MDL_H
...
class MatmulScheduler<IMPL, A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, MM_CFG, POLICY_TYPE,
    enable_if_t<DoMatmulSpecialMDL(MM_CFG)>>
...
private:
        ...
        do {
            ...
            if (enSequentialWrite) {
                MATMUL_MODULE(CopyCubeOut)
                    ->template Copy<true>(gm, co1Local[offset], MATMUL_MODULE(MLoop)->GetInnerIdx(),
                ...
                MATMUL_MODULE(CopyCubeOut)
                    ->template Copy<false>(gm, co1Local[offset], MATMUL_MODULE(MLoop)->GetInnerIdx(),
                        MATMUL_MODULE(NLoop)->GetInnerIdx(), MATMUL_MODULE(MLoop)->GetBaseShape(),
                        MATMUL_MODULE(NLoop)->GetBaseShape(), MATMUL_MODULE(MLoop)->GetBaseBlockShape(),
                        MATMUL_MODULE(NLoop)->GetBaseBlockShape());
            }
    ...
    __aicore__ inline void ComputeKDB(const LocalTensor<TransT>& a1, const LocalTensor<TransT>& b1,
        const SplitParams& aL0Params, const SplitParams& bL0Params, const int32_t offset, const bool isATranspose,
        const bool isBTranspose, const bool sL0CInit, const bool sL0CLast)
    {
