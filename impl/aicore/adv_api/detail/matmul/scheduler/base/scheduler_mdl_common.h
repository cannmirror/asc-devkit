 ...
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_MDL_COMMON_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_MDL_COMMON_H
...
private:
    ...
    __aicore__ inline void ComputeKDB(const LocalTensor<TransAT>& a1, const LocalTensor<TransBT>& b1,
        const SplitParams& aL0Params, const SplitParams& bL0Params, const bool isATranspose, const bool isBTranspose,
    ...
    __aicore__ inline void ComputeMDB(const LocalTensor<TransAT>& a1, const LocalTensor<TransBT>& b1,
        LocalTensor<BiasT>& bias, SplitParams& aL0Params, SplitParams& bL0Params, bool isATranspose, bool isBTranspose,
        bool sL0CInit, bool sL0CLast)
    {
        // Split b2
        LocalTensor<TransBT> b2 = BASE_MODULE::SplitB(b1, bL0Params, isBTranspose);
        ...
        for (uint32_t idx = 0; idx < l0aDBLoop; ++idx) {
            ...
            BASE_MODULE::CubeCompute(MATMUL_MODULE(CubeOutBuffer)
    ...
    __aicore__ inline void ComputeNDB(const LocalTensor<TransAT>& a1, const LocalTensor<TransBT>& b1,
        LocalTensor<BiasT>& bias, SplitParams& aL0Params, SplitParams& bL0Params, bool isATranspose, bool isBTranspose,
        bool sL0CInit, bool sL0CLast)
    {
        // Split a2
    ...
    __aicore__ inline void FixpipeL0CToGm(const GlobalTensor<DstT>& gm, const LocalTensor<L0cT>& co1Local, int32_t curM,
        int32_t curN, uint8_t enAtomic, bool enSequentialWrite)
    {
        ...
        if (enSequentialWrite) {
            MATMUL_MODULE(CopyCubeOut)
                ->template Copy<true>(gm, co1Local, curM, curN, MATMUL_MODULE(MLoop)->GetBaseShape(),
                    MATMUL_MODULE(NLoop)->GetBaseShape(), MATMUL_MODULE(MLoop)->GetBaseBlockShape(),
                    MATMUL_MODULE(NLoop)->GetBaseBlockShape());
        } else {
            MATMUL_MODULE(CopyCubeOut)
                ->template Copy<false>(gm, co1Local, curM, curN, MATMUL_MODULE(MLoop)->GetBaseShape(),
                    MATMUL_MODULE(NLoop)->GetBaseShape(), MATMUL_MODULE(MLoop)->GetBaseBlockShape(),
                    MATMUL_MODULE(NLoop)->GetBaseBlockShape());
        }
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_MDL_COMMON_H
