 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_INTRABLOCK_H
#define AICORE_ADV_API_DETAIL_MATMUL_SCHEDULER_BASE_SCHEDULER_INTRABLOCK_H
...
public:
        ...
        const GlobalTensor<DstT>& gm, uint8_t enAtomic = 0, bool enSequentialWrite = false, bool fakeMsg = false)
    {
        ...
        if (fakeMsg) {
            while (BASE_MODULE::ScheduleOnce(false)) { BASE_MODULE::GetResult(gm, enAtomic, enSequentialWrite); }
        } else {
    ...
    __aicore__ inline void Compute()
    {
        ...
        do {
            ...
            LocalTensor<TransT> a1 = MATMUL_MODULE(CopyCubeInA)
                                         ->LoadData(MATMUL_MODULE(MLoop)->template GetOuterIdx<true>(), kOuterIdx,
            ...
            if constexpr (!IS_INTRA_BLOCK) {
                ...
                LocalTensor<BiasT> bias = MATMUL_MODULE(BiasScheduler)
                                              ->CopyIn(MATMUL_MODULE(NLoop)->template GetBaseShape<false>(), 1,
            ...
            MATMUL_MODULE(LoadToA2)->Load(a2, a1, aL0Params.axisL1Len, aL0Params.kAxisL1Len, aL0Params.axisL0Len,
                kL0Len, aL0Params.axisL1Offset, aL0Params.kAxisL1Offset, isATranspose);
            ...
            MATMUL_MODULE(MmadCompute)
                ->Compute(MATMUL_MODULE(CubeOutBuffer)->GetTensor(), a2, b2, aL0Params.axisL0Len, kL0Len,
                    bL0Params.axisL0Len, isATranspose,
                    MATMUL_MODULE(MatmulShapeInfo)->template IsTransposeB<IS_INTRA_BLOCK>(), unitFlag, cmatrixSource,
        ...
        const GlobalTensor<DstT>& gm, uint8_t enAtomic = 0, bool enSequentialWrite = false)
    {
        ...
        if (enSequentialWrite) {
            MATMUL_MODULE(CopyCubeOut)
                ->template Copy<true>(gm, co1Local, MATMUL_MODULE(MLoop)->template GetOuterIdx<true>(),
            ...
            MATMUL_MODULE(CopyCubeOut)
                ->template Copy<false>(gm, co1Local, MATMUL_MODULE(MLoop)->template GetOuterIdx<true>(),
                    MATMUL_MODULE(NLoop)->GetInnerIdx(), MATMUL_MODULE(MLoop)->template GetBaseShape<true>(),
                    MATMUL_MODULE(NLoop)->template GetBaseShape<true>(),
                    MATMUL_MODULE(MLoop)->template GetBaseBlockShape<true>(),
                    MATMUL_MODULE(NLoop)->template GetBaseBlockShape<true>());
