...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_SPLIT_LOAD_TO_L0B_LOAD_TO_L0B_LOAD2DV2_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_SPLIT_LOAD_TO_L0B_LOAD_TO_L0B_LOAD2DV2_H
...
class LoadToL0B<IMPL, INPUT_TYPE, MM_CFG,
    ...
    using L0B_T = typename Conditional<HasScalePosition<INPUT_TYPE>::value,
        typename GetL0DataType<typename INPUT_TYPE::T, true>::Type,
        typename GetL0DataType<typename INPUT_TYPE::T, false>::Type>::type;
...
public:
    ...
    __aicore__ inline void Load(const LocalTensor<L0B_T>& dst, const LocalTensor<B_T>& bMatrix, uint16_t bL1N,
        ...
        uint16_t bAuxL1NOffset = 0) const
    {
        if (isBTranspose) {
            ...
            if constexpr (!HasScalePosition<INPUT_TYPE>::value) {
                ...
                MxTransLoadDataToL0(dst, bMatrix, bL1N, madN, madK, bL1NOffset, bL1KOffset, l1BAuxMatrix, bAuxL1K,
                    bAuxL1KOffset, bAuxL1NOffset);
            }
        } else {
            // Mx should run for MXLoad
            if constexpr (!HasScalePosition<INPUT_TYPE>::value) {
                ...
                MxLoadDataToL0(dst, bMatrix, bL1K, madN, madK, bL1NOffset, bL1KOffset, l1BAuxMatrix, bAuxL1K,
                    bAuxL1KOffset, bAuxL1NOffset);
            }
        }
    }
...
private:
    ...
    __aicore__ inline void LoadDataToL0(const LocalTensor<B_T>& dst, const LocalTensor<B_T>& bMatrix, uint16_t bL1K,
        uint16_t madN, uint16_t madK, uint16_t bL1NOffset, uint16_t bL1KOffset) const
    {
        LoadData2DParamsV2 loadDataParams;
    ...
    __aicore__ inline void MxTransLoadDataToL0(const LocalTensor<L0B_T>& dst, const LocalTensor<B_T>& bMatrix,
        ...
        const LocalTensor<AuxDtype>& l1BAuxMatrix, uint16_t bAuxL1K, uint16_t bAuxL1KOffset,
        uint16_t bAuxL1NOffset) const
    {
    ...
    __aicore__ inline void MxLoadDataToL0(const LocalTensor<L0B_T>& dst, const LocalTensor<B_T>& bMatrix, uint16_t bL1K,
        uint16_t madN, uint16_t madK, uint16_t bL1NOffset, uint16_t bL1KOffset,
        const LocalTensor<AuxDtype>& l1BAuxMatrix, uint16_t bAuxL1K, uint16_t bAuxL1KOffset,
        uint16_t bAuxL1NOffset) const
    {
#if defined(__DAV_C310__) || defined(__DAV_310R6__)
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_SPLIT_LOAD_TO_L0B_LOAD_TO_L0B_LOAD2DV2_H
