...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_SPLIT_LOAD_TO_L0A_LOAD_TO_L0A_LOAD2DV2_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_SPLIT_LOAD_TO_L0A_LOAD_TO_L0A_LOAD2DV2_H
...
class LoadToL0A<IMPL, A_TYPE, MM_CFG,
    ...
    using L0A_T =
        typename Conditional<HasScalePosition<A_TYPE>::value, typename GetL0DataType<typename A_TYPE::T, true>::Type,
...
public:
    ...
    __aicore__ inline void Load(const LocalTensor<L0A_T>& dst, const LocalTensor<A_T>& aMatrix, uint16_t aL1M,
        ...
        uint16_t aAuxL1MOffset = 0) const
    {
        if (isATranspose) {
            ...
            if constexpr (!HasScalePosition<A_TYPE>::value) {
                ...
                MxTransLoadDataToL0(dst, aMatrix, aL1K, madM, madK, aL1MOffset, aL1KOffset, l1AAuxMatrix, aAuxL1K,
                    aAuxL1KOffset, aAuxL1MOffset);
            }
        } else {
            // Mx should run for MXLoad
            if constexpr (!HasScalePosition<A_TYPE>::value) {
                ...
                MxLoadDataToL0(dst, aMatrix, aL1M, madM, madK, aL1MOffset, aL1KOffset, l1AAuxMatrix, aAuxL1K,
                    aAuxL1KOffset, aAuxL1MOffset);
            }
        }
    }
...
private:
    ...
    __aicore__ inline void LoadDataToL0(const LocalTensor<A_T>& dst, const LocalTensor<A_T>& aMatrix, uint16_t aL1M,
        uint16_t madM, uint16_t madK, uint16_t aL1MOffset, uint16_t aL1KOffset) const
    {
        LoadData2DParamsV2 loadDataParams;
    ...
    __aicore__ inline void MxTransLoadDataToL0(const LocalTensor<L0A_T>& dst, const LocalTensor<A_T>& aMatrix,
        ...
        const LocalTensor<AuxDtype>& l1AAuxMatrix, uint16_t aAuxL1K, uint16_t aAuxL1KOffset,
        uint16_t aAuxL1MOffset) const
    {
    ...
    __aicore__ inline void MxLoadDataToL0(const LocalTensor<L0A_T>& dst, const LocalTensor<A_T>& aMatrix, uint16_t aL1M,
        uint16_t madM, uint16_t madK, uint16_t aL1MOffset, uint16_t aL1KOffset,
        const LocalTensor<AuxDtype>& l1AAuxMatrix, uint16_t aAuxL1K, uint16_t aAuxL1KOffset,
        uint16_t aAuxL1MOffset) const
    {
#if defined(__DAV_C310__) || defined(__DAV_310R6__)
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_SPLIT_LOAD_TO_L0A_LOAD_TO_L0A_LOAD2DV2_H
