...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_DATACOPY_WRAPPER_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_DATACOPY_WRAPPER_H
...
public:
    ...
    __aicore__ inline void PadUnalignedToTrans(const LocalTensor<DstT>& trans, const GlobalTensor<DstT>& gm,
        int32_t dstOffset, bool isComputeLineByLine, int32_t baseHeight, int32_t baseWidth, int32_t baseBlockHeight,
    ...
    template <bool enSequentialWrite>
    __aicore__ inline void CopyTrans2GM(const GlobalTensor<DstT>& gm, const LocalTensor<DstT>& trans, int32_t curRow,
        ...
        int32_t dstOffset, int32_t offset, int32_t dstStride, bool isComputeLineByLine, bool isTargetAligned)
    {
        ...
        if constexpr (IsSameType<SrcT, int8_t>::value) {
            ...
            if (isTargetAligned) {
                ...
                if constexpr (IsSameType<DstT, int8_t>::value || IsSameType<DstT, uint8_t>::value) {
                    CopyTrans2GMByVecByLineUnalign<true>(gm[dstOffset], trans, baseHeight, baseWidth, baseBlockHeight,
                        baseBlockWidth, blockLen, newBlockCount, offset);
                } else {
                    CopyTrans2GMByVecByLineUnalign<false>(gm[dstOffset], trans, baseHeight, baseWidth, baseBlockHeight,
                        baseBlockWidth, blockLen, newBlockCount, offset);
                }
            }
        } else {
    ...
    __aicore__ inline void CopyLocal2GMNZ2NDOnTheFlyTail(const GlobalTensor<DstT>& gm,
        const LocalTensor<DstT>& localBuf, int32_t baseHeight, int32_t baseWidth, int32_t iterIdx, int32_t calcWidth,
        const event_t& eventIDMte3ToMte2)
    {
...
private:
    ...
    __aicore__ inline void PadUnalignedToTransWithStride(const LocalTensor<DstT>& trans, const GlobalTensor<DstT>& gm,
    ...
    __aicore__ inline void CopyTrans2GMByVecByLineAlign(const GlobalTensor<DstT>& gm, const LocalTensor<DstT>& trans,
        int32_t baseHeight, int32_t blockLen, int32_t blockCount, int32_t offset)
    {
        int32_t dstOffset = 0;
        int32_t srcOffset = 0;
    ...
    __aicore__ inline void CopyTrans2GMByVecByLineUnalignOneBlock(const GlobalTensor<DstT>& gm,
        const LocalTensor<DstT>& trans, int32_t baseHeight, int32_t baseWidth, int32_t blockLen, int32_t blockCount,
        int32_t offset)
    {
        CopyTrans2GMEnQue();
        ...
        for (int32_t i = 0; i < baseHeight; ++i) {
            ...
            for (int32_t j = 0; j < baseWidth; ++j) { transAligin.SetValue(j, trans.GetValue(transIndex + j)); }
            for (int32_t j = baseWidth; j < blockCount; ++j) { transAligin.SetValue(j, 0); }
        ...
        int32_t blockCount, int32_t offset) -> enable_if_t<isDstB8, void>
    {
        ...
        for (int32_t i = PATTERN_OFFSET; i < PATTERN_SIZE; ++i) { src1Pattern.SetValue(i, 0); }
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_DATACOPY_WRAPPER_H