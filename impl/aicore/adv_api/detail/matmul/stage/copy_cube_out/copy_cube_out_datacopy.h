...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_DATACOPY_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_DATACOPY_H
...
public:
    ...
    __aicore__ inline void Copy(const GlobalTensor<DstT>& gm, const LocalTensor<DstT>& co2Local,
        ...
        int32_t baseBlockHeight, int32_t baseBlockWidth, const ScheduleContext& context = 0)
    {
        CopyOutImpl<enSequentialWrite>(
...
private:
        ...
        int32_t baseBlockWidth)
    {
        ...
        if constexpr (enSequentialWrite) {
            ...
            dataCopyInfo.dstStride =
                (CeilAlign(MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreM(), BLOCK_CUBE) - baseBlockHeight * BLOCK_CUBE)
        ...
        if constexpr (A_TYPE::format == CubeFormat::VECTOR) {
            ...
            if constexpr (enSequentialWrite) {
                ...
                int32_t dstOffset = curRow * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM() * dimN
                                    + curCol * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseN();
                TransNZ2NDForDstUB(co2Local[dstOffset], trans,
                    CeilAlign(dimN, MATMUL_MODULE(CopyCubeOutUtils)->GetBlockCount()), baseHeight, baseBlockWidth,
                    baseBlockHeight);
            }
        }
    }
    ...
    __aicore__ inline void CopyCo12Co2WithQuant(const LocalTensor<DstT>& dst, const LocalTensor<L0cT>& src,
        int32_t curCol, int32_t baseBlockHeight, int32_t baseBlockWidth, DataCopyParams& dataCopyInfo,
        DataCopyEnhancedParams& enhancedParams)
    {
    ...
    __aicore__ inline void CopyCo12Co2WithoutQuant(const LocalTensor<DstT>& dst, const LocalTensor<L0cT>& src,
        int32_t curCol, int32_t baseBlockHeight, int32_t baseBlockWidth)
    {
        DataCopyParams dataCopyInfo;
        dataCopyInfo.blockCount = baseBlockWidth;
        dataCopyInfo.blockLen = baseBlockHeight;
    ...
    __aicore__ inline void CopyCo12Local(const LocalTensor<DstT>& localBuf, const LocalTensor<L0cT>& co1Local,
        int32_t curCol, int32_t baseBlockHeight, int32_t baseBlockWidth)
    {
        DataCopyParams dataCopyInfo;
        dataCopyInfo.blockCount = 1;
        ...
        int32_t baseBlockWidth)
    {
        ...
        if (dstStride >= UINT16_MAX) {
            ...
            DataCopy(gm[dstOffset], localBuf,
    ...
    {
        DataCopyParams dataCopyInfo{static_cast<uint16_t>(baseBlockWidth),
        ...
        int32_t baseBlockWidth)
    {
        ...
        MATMUL_MODULE(CopyCubeOutUtils)
            ->template CopyTrans2GM<enSequentialWrite>(gm, trans, curRow, curCol, baseHeight, baseWidth,
                baseBlockHeight, baseBlockWidth, dstOffset, offset, dstStride, isComputeLineByLine, isTargetAligned);
    }
        ...
        int32_t baseBlockWidth)
    {
        ...
        for (int32_t i = 0; i < baseHeight; i++) {
            if (calcWidth > 0) {
                DataCopy(gm[dstOffset], localBuf[srcOffset],
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_DATACOPY_H