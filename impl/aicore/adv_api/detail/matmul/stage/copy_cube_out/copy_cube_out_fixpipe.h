...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_FIXPIPE_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_FIXPIPE_H
...
private:
        ...
        int32_t baseHeight, int32_t baseWidth, int32_t baseBlockHeight, int32_t baseBlockWidth)
    {
        ...
        if constexpr (C_TYPE::format == CubeFormat::ND || C_TYPE::format == CubeFormat::ND_ALIGN) {
            CopyOutNZ2ND<enSequentialWrite, T, IS_INTRA_BLOCK>(
            ...
            static_assert(!(FIXPIPE_MODE == McgShfMode::DUAL_DST_SPLIT_N && (A_TYPE::ibShare && B_TYPE::ibShare)
                              && PhyPosIsUB(C_TYPE::pos) && MatmulFeatureTrait<MM_CFG>::IsSupportL0CToUB()),
                "if split N when copy cube out, NZ is not supported");
            CopyOutNZ2NZ<enSequentialWrite, T, IS_INTRA_BLOCK>(
                dst, co1Local, curRow, curCol, baseHeight, baseWidth, baseBlockHeight, baseBlockWidth);
#if defined(__DAV_C310__) || defined(__DAV_310R6__)
        } else if constexpr (C_TYPE::format == CubeFormat::COLUMN_MAJOR) {
            CopyOutNZ2DN<enSequentialWrite, T, IS_INTRA_BLOCK>(
                dst, co1Local, curRow, curCol, baseHeight, baseWidth, baseBlockHeight, baseBlockWidth);
#endif
        } else {
    ...
    __aicore__ inline void SplitMNParamsCheck(int32_t baseWidth)
    {
        if constexpr (PhyPosIsGM(C_TYPE::pos)) {
            static_assert(
                ...
                "if split M or N when copy cube out, GM is not supported on C310.");
        ...
        int32_t baseHeight, int32_t baseWidth, int32_t baseBlockHeight, int32_t baseBlockWidth)
    {
        ...
        if constexpr (!enSequentialWrite) {
            if constexpr (!ToMatmulConfig(MM_CFG).isEnableChannelSplit) {
                stride = static_cast<uint32_t>(GetOrgM<IS_INTRA_BLOCK>() * DST_C0SIZE);
                if constexpr (!IsBasic(MM_CFG)) {
                    dstOffset =
                        static_cast<int64_t>(
                            static_cast<int64_t>(curCol * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseN())
                            * GetOrgM<IS_INTRA_BLOCK>())
                        + static_cast<int64_t>(curRow * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM())
                              * DST_C0SIZE;
                }
                ...
                stride = static_cast<uint32_t>(CeilAlign(GetOrgM<IS_INTRA_BLOCK>(), BLOCK_CUBE) * DST_C0SIZE);
                if constexpr (!IsBasic(MM_CFG)) {
                    dstOffset =
                        static_cast<int64_t>(
                            static_cast<int64_t>(curCol * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseN())
                            * CeilAlign(GetOrgM<IS_INTRA_BLOCK>(), BLOCK_CUBE))
                        + static_cast<int64_t>(curRow * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM())
                              * DST_C0SIZE;
                }
            }
    ...
    __aicore__ inline void CopyOutNZ2DN(const T& dst, const LocalTensor<SrcT>& co1Local, int32_t curRow, int32_t curCol,
        int32_t baseHeight, int32_t baseWidth, int32_t baseBlockHeight, int32_t baseBlockWidth)
    {
        static_assert(PhyPosIsGM(C_TYPE::pos), "Unsupported TPosition for output matrix.");
        auto stride = baseHeight;
        int64_t dstOffset = 0;
        if constexpr (!enSequentialWrite) {
            stride = GetOrgHeight<IS_INTRA_BLOCK>();
            if constexpr (!IsBasic(MM_CFG)) {
                dstOffset = static_cast<int64_t>(
                                static_cast<int64_t>(curCol * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseN())
                                * stride)
        ...
        if constexpr (!enSequentialWrite) {
            if constexpr (!ToMatmulConfig(MM_CFG).isEnableChannelSplit) {
                dstOffset = static_cast<int64_t>(
                            ...
                            + static_cast<int64_t>(curRow * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM())
                                  * DST_C0SIZE;
                stride = static_cast<uint32_t>(GetOrgM<IS_INTRA_BLOCK>() * DST_C0SIZE * sizeof(DstT) / ONE_BLK_SIZE);
            } else {
                dstOffset = static_cast<int64_t>(
                                static_cast<int64_t>(curCol * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseN())
                                * Ceil(GetOrgM<IS_INTRA_BLOCK>(), BLOCK_CUBE))
                                * BLOCK_CUBE
                            + static_cast<int64_t>(curRow * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM())
                                  * DST_C0SIZE;
                stride = static_cast<uint32_t>(Ceil(GetOrgM<IS_INTRA_BLOCK>(), BLOCK_CUBE) * BLOCK_CUBE * DST_C0SIZE
        ...
        int32_t baseBlockWidth)
    {
        ...
        DataCopyOutParams param(baseBlockWidth,
            static_cast<uint16_t>(baseHeight * BLOCK_CUBE * sizeof(SrcT) / ONE_BLK_SIZE), 0, dstStrideIn, nSize,
            EnUnitFlag(MM_CFG), curRow, curCol);
        ...
        (IMPL::CallBack::DataCopyOutPtr)(reinterpret_cast<__gm__ void*>(dst[dstOffset].address_), co1LocalInt8,
            reinterpret_cast<void*>(&param), MATMUL_MODULE(MatmulUserDefineInfo)->GetUserDefineInfo(),
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_COPY_CUBE_OUT_FIXPIPE_H
