...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_QUANT_QUANT_PROCESSOR_FIXPIPE_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_QUANT_QUANT_PROCESSOR_FIXPIPE_H
...
public:
    ...
    __aicore__ inline void SetQuantVector(const GlobalTensor<uint64_t>& quantTensor)
    {
        ...
        } else if constexpr (IsTypeOneOfV<SrcT, fp8_e4m3fn_t, fp8_e5m2_t,
                                 hifloat8_t> && IsSameTypeV<DstT, bfloat16_t>) {
        ...
        LocalTensor<uint64_t>& quantTensor, const int32_t curN, const int32_t baseUseN)
    {
        if (isPerChannel_) {
            ...
            if constexpr (C_TYPE::format == CubeFormat::ND || C_TYPE::format == CubeFormat::ND_ALIGN) {
                ...
                CopyDeqTensorToL1(
                    quantTensor, quantTensor_[quantTensorGMOffset], DivCeil(baseUseN, BLOCK_CUBE) * BLOCK_CUBE);
            }
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_QUANT_QUANT_PROCESSOR_FIXPIPE_H
