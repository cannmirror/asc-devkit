...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_QUANT_QUANT_PROCESSOR_DATACOPY_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_QUANT_QUANT_PROCESSOR_DATACOPY_H
...
class MatmulQuantProcessor<IMPL, A_TYPE, C_TYPE, MM_CFG,
    enable_if_t<(IsQuantSenario<typename C_TYPE::T, typename A_TYPE::T>() && MatmulFeatureTrait<MM_CFG>::IsNeedUB())>> {
...
public:
    ...
    __aicore__ inline void UpdateDataCopyParamForQuant(DataCopyEnhancedParams& enhancedParams, int curCol)
    {
        if constexpr (IsSameType<DstT, half>::value) {
            ...
            if (quantMode_ == QuantMode_t::QF322B8_PRE || quantMode_ == QuantMode_t::REQ8) {
                enhancedParams.deqScale = DeqScale::DEQ8;
                enhancedParams.deqValue = quantScalar_;
            } else if (quantMode_ == QuantMode_t::VQF322B8_PRE || quantMode_ == QuantMode_t::VREQ8) {
...
private:
    __aicore__ inline void CopyQuantTensorImpl(
        LocalTensor<uint64_t>& quantTensor, const int32_t curN, const int32_t baseUseN)
    {
        quantTensor = MATMUL_MODULE(LocalWorkspace)
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_OUT_QUANT_QUANT_PROCESSOR_DATACOPY_H
