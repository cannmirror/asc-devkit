...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_MDL_SPARSE_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_MDL_SPARSE_H
...
class CopyCubeIn<IMPL, INPUT_TYPE, MM_CFG,
    enable_if_t<!MatmulFeatureTrait<MM_CFG>::IsNeedUB()
                && GetCopyCubeInType<INPUT_TYPE, MM_CFG>() == CopyCubeInType::SPARSE_MDL>>
...
public:
    ...
    {
        ASCENDC_ASSERT((IsSameTypeV<TransT, int8_t>),
            { KERNEL_LOG(KERNEL_ERROR, "Sparse matmul only support int8 inputDtype."); });
        ASCENDC_ASSERT((tileHeight % 2 == 0),
            { KERNEL_LOG(KERNEL_ERROR, "Sparse matmul only support tileHeight is an integer mutiople of 2."); });
        ...
        if (MATMUL_MODULE(CubeInBuffer)->Hit(posL1, bufferPos)) {
            ...
            MATMUL_MODULE(DataCopyUtils)
                ->CopySparseIdxToCubeFromGM(l1Idx, gmIdx, curRow, curCol, tileHeight >> 1, tileWidth);
