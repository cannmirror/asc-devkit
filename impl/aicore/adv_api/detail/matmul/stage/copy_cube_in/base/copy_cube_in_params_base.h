...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_PARAMS_BASE_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_PARAMS_BASE_H
...
private:
    ...
    __aicore__ inline int32_t GetBaseSize()
    {
        if constexpr (INPUT_TYPE::format == CubeFormat::VECTOR
                      && !(IsTypeOneOfV<SrcT, float> || IsNeedC0Align<SrcT>())) {
            return GetBaseWidth();
        } else {
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_PARAMS_BASE_H
