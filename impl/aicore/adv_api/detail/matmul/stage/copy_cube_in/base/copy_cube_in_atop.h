...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_ATOP_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_ATOP_H
...
class CopyCubeIn<IMPL, INPUT_TYPE, MM_CFG, Void, CopyIn> : public CopyCubeInBase<IMPL, MM_CFG, INPUT_TYPE> {
...
public:
    ...
    __aicore__ inline void ClearLoadData(
        const LocalTensor<TransT>& tensor = NULL_TENSOR<TransT>, int32_t curRow = 0, int32_t curCol = 0)
    {
...
private:
    __aicore__ inline void CopyTensor(
        const LocalTensor<TransT>& l1, int32_t curRow, int32_t curCol, int32_t tileHeight, int32_t tileWidth)
    {
        CopyIn<INPUT_TYPE, MM_CFG> copyIn;
        if constexpr (!INPUT_TYPE::isTrans) {
            copyIn(l1, MATMUL_MODULE(MatmulTensorInfo)->GetGlobalTensor(), curRow, curCol, tileHeight, tileWidth,
            ...
            copyIn(l1, MATMUL_MODULE(MatmulTensorInfo)->GetGlobalTensor(), curCol, curRow, tileWidth, tileHeight,
