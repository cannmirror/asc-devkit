...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_USING_UB_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_USING_UB_H
...
public:
        ...
        if constexpr (DoMatmulMDL(MM_CFG) && ToMatmulConfig(MM_CFG).enableL1CacheUB) {
            if (MATMUL_MODULE(CopyCubeInParams)->GetDepthL1CacheUB() > 0) {
                GetTPipePtr()->InitBuffer(MATMUL_MODULE(DataCopyUtils)->GetQidUbCache(), 1,
                    MATMUL_MODULE(CopyCubeInParams)->GetDepthL1CacheUB() * MATMUL_MODULE(CopyCubeInParams)->GetStepCol()
    ...
    __aicore__ inline void ClearLoadData(
        const LocalTensor<TRANS_T>& aMatrix = NULL_TENSOR<TRANS_T>, int32_t curRow = 0, int32_t curCol = 0)
    {
        if constexpr (DoMatmulMDL(MM_CFG)) {
            auto bufferPos = MATMUL_MODULE(CopyCubeInParams)->GetBufferPos();
