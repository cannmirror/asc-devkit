...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_UBTOL1_SINGLESHAPE_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_UBTOL1_SINGLESHAPE_H
...
public:
        ...
        int32_t curRow, int32_t curCol, int32_t tileHeight, int32_t tileWidth, const ScheduleContext& context = {})
    {
        ...
        if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
            tbuffTmp.dataLen = MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreM()
                               * MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreK() * sizeof(TransT);
        } else {
            tbuffTmp.dataLen = MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreK()
                               * MATMUL_MODULE(MatmulShapeInfo)->GetSingleCoreN() * sizeof(TransT);
        }
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BASE_COPY_CUBE_IN_UBTOL1_SINGLESHAPE_H
