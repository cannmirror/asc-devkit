...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BATCH_BATCH_COPY_CUBE_IN_USING_UB_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BATCH_BATCH_COPY_CUBE_IN_USING_UB_H
...
class BatchCopyCubeIn<IMPL, MM_CFG, INPUT_TYPE,
    enable_if_t<(MatmulFeatureTrait<MM_CFG>::IsNeedUB())
                ...
                && (INPUT_TYPE::format == CubeFormat::ND) && (INPUT_TYPE::layout == LayoutMode::NORMAL)
                && PhyPosIsGM(INPUT_TYPE::pos)>> : public BatchCopyCubeInBase<IMPL, MM_CFG, INPUT_TYPE> {
private:
        ...
        MATMUL_MODULE(CubeInBuffer)
            ->Init(
                MATMUL_MODULE(BatchCopyCubeInParams)->GetBatchNum()
                    * CeilAlign(
                        MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleWidth<INPUT_TYPE::isTrans>(), c0Size_)
                    * CeilAlign(MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleHeight<INPUT_TYPE::isTrans>(),
                        BLOCK_CUBE),
                1);
    }
...
private:
        ...
        const int32_t outerIdx, const int32_t splitIdx, const int32_t splitSize)
    {
        ...
        auto alignHeight =
            CeilAlign(MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleHeight<isTrans>(), BLOCK_CUBE);
        ...
        if constexpr (ToMatmulConfig(MM_CFG).enVecND2NZ) {
            ...
            if constexpr (isKRow) {
                MATMUL_MODULE(DataCopyWrapper)
                    ->CopyND2NZOnTheFly(dstTensor[dstOffset], srcGlobal, 0, 0,
                        MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleHeight<isTrans>(),
                        batchBlock * alignWidth, batchBlock * alignWidth);
            } else {
                MATMUL_MODULE(DataCopyWrapper)
                    ->CopyND2NZOnTheFly(dstTensor[dstOffset], srcGlobal, 0, 0, batchBlock * alignHeight,
    ...
    __aicore__ inline enable_if_t<tag == InputTypeTag::A, void> CopyND2NZThroughVec(
        const LocalTensor<TransT>& dstTensor, const GlobalTensor<SrcT>& srcTensor, int32_t batchNum,
        int32_t batchOuterIdx, int32_t splitOuterIdx, int32_t alignHeight, int32_t alignWidth)
    {
        ...
        if (ubEnough) {
            ...
            int colNum = Ceil(MATMUL_MODULE(CopyCubeInParams)->template GetTotalCol<isTrans>()
                                  * MATMUL_MODULE(CopyCubeInParams)->template GetBaseWidth<isTrans>(),
                c0Size_);
            for (auto iterBatch = 0; iterBatch < batchNum; ++iterBatch) {
                for (auto i = 0; i < colNum; ++i) {
                    MATMUL_MODULE(DataCopyWrapper)
                        ->CopyND2NZWithVecOp(dstTensor[dstOffset], srcTensor[srcOffset], 0, i * c0Size_,
            ...
            if constexpr (isTrans) {
                MATMUL_MODULE(DataCopyWrapper)
                    ->CopyND2NZOnTheFly(dstTensor, srcTensor, 0, 0,
                        MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleHeight<isTrans>(),
                ...
                if (tail == 0) {
                    for (int iterBatch = 0; iterBatch < batchNum; ++iterBatch) {
                        MATMUL_MODULE(DataCopyWrapper)
                            ->CopyND2NZOnTheFly(dstTensor[dstOffset], srcTensor[srcOffset], 0, 0,
                                MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleHeight<isTrans>(),
                                MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleWidth<isTrans>(),
                                MATMUL_MODULE(CopyCubeInParams)->template GetOrgWidth<isTrans>());
                        dstOffset +=
                            MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleHeight<isTrans>() * alignWidth;
                        srcOffset += srcStride;
                    }
                } else {
                    for (int iterBatch = 0; iterBatch < batchNum; ++iterBatch) {
                        ...
                        for (auto i = 0; i < MATMUL_MODULE(CopyCubeInParams)->template GetTotalRow<isTrans>(); ++i) {
                            MATMUL_MODULE(DataCopyWrapper)
                                ->CopyND2NZOnTheFly(dstTensor[dstOffset], srcTensor[innerSrcOffset], 0, 0,
                                    MATMUL_MODULE(MatmulTensorInfo)->template GetBaseUseHeight<isTrans>(),
                                    MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleWidth<isTrans>(),
                                    MATMUL_MODULE(CopyCubeInParams)->template GetOrgWidth<isTrans>());
                            dstOffset +=
                                MATMUL_MODULE(MatmulTensorInfo)->template GetBaseUseHeight<isTrans>() * alignWidth;
                            innerSrcOffset +=
                                MATMUL_MODULE(MatmulTensorInfo)->template GetBaseUseHeight<isTrans>()
                                * MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleWidth<isTrans>();
                        }
    ...
    template <bool isTrans = false, InputTypeTag tag = INPUT_TYPE::TAG>
    __aicore__ inline enable_if_t<tag == InputTypeTag::B, void> CopyND2NZThroughVec(
        const LocalTensor<TransT>& dstTensor, const GlobalTensor<SrcT>& srcTensor, int32_t batchNum,
        int32_t batchOuterIdx, int32_t splitOuterIdx, int32_t alignHeight, int32_t alignWidth)
    {
        auto srcStride = GetSingleSize();
        ...
        if (ubEnough) {
            ...
            for (auto iterBatch = 0; iterBatch < batchNum; ++iterBatch) {
                ...
                if constexpr (isTrans) {
                    ...
                    if (tail != 0) {
                        MATMUL_MODULE(DataCopyWrapper)
                            ->CopyND2NZWithVecOp(dstTensor[dstOffset], srcTensor[srcOffset], 0, colNum * c0Size_,
            ...
            if constexpr (isTrans) {
                MATMUL_MODULE(DataCopyWrapper)
                    ->CopyND2NZOnTheFly(dstTensor, srcTensor, 0, 0, batchNum * alignHeight,
                        MATMUL_MODULE(BatchCopyCubeInParams)->template GetSingleWidth<isTrans>(),
                        MATMUL_MODULE(CopyCubeInParams)->template GetOrgWidth<isTrans>());
            } else {
                MATMUL_MODULE(DataCopyWrapper)
...
class BatchCopyCubeIn<IMPL, MM_CFG, INPUT_TYPE,
    enable_if_t<(MatmulFeatureTrait<MM_CFG>::IsNeedUB())
                && GetCopyCubeInType<INPUT_TYPE, MM_CFG>() == CopyCubeInType::BMM
                && (INPUT_TYPE::format == CubeFormat::NZ) && (INPUT_TYPE::layout == LayoutMode::NORMAL)
...
public:
    ...
    {
        if (MATMUL_MODULE(BatchCopyCubeInParams)->IsTranspose()) {
            CopyBatchToCube<true, INPUT_TYPE::TAG == InputTypeTag::A>(dstTensor, outerIdx, splitIdx, splitSize);
        } else {
            CopyBatchToCube<false, INPUT_TYPE::TAG == InputTypeTag::B>(dstTensor, outerIdx, splitIdx, splitSize);
        }
    }
...
private:
    constexpr static int32_t c0Size_ = AuxGetC0Size<TransT>();
    template <bool isTrans = false, bool isKRow = false>
    __aicore__ inline void CopyBatchToCube(
        LocalTensor<TransT>& dstTensor, const int32_t outerIdx, const int32_t splitIdx, const int32_t splitSize)
    {
        ...
        for (auto i = 0; i < batchBlock; ++i) {
            MATMUL_MODULE(DataCopyWrapper)
                ->CopyNZ2NZ(dstTensor[dstOffset], srcTensor[srcOffset], 0, 0, alignHeight, alignWidth, alignHeight,
                    iskRowDirec);
            dstOffset += dstStride;
            srcOffset += srcStride;
        }
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BATCH_BATCH_COPY_CUBE_IN_USING_UB_H