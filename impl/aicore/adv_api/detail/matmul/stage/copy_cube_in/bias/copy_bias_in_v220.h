...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BIAS_COPY_BIAS_IN_V220_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BIAS_COPY_BIAS_IN_V220_H
...
class CopyBiasIn<IMPL, A_TYPE, BIAS_TYPE, MM_CFG,
    enable_if_t<!MatmulFeatureTrait<MM_CFG>::IsNeedUB() && ToMatmulConfig(MM_CFG).enableSetBias
    ...
    using TensorT =
        typename Conditional<(PhyPosIsGM(BIAS_TYPE::pos)
                                 || (PhyPosIsUB(BIAS_TYPE::pos) && !MatmulFeatureTrait<MM_CFG>::IsSupportUBToL1()
