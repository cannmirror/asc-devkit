...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BIAS_LOAD_BIAS_TO_C2_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_BIAS_LOAD_BIAS_TO_C2_H
...
class LoadBias2C2<IMPL, A_TYPE, BIAS_TYPE, MM_CFG,
    enable_if_t<ToMatmulConfig(MM_CFG).enableSetBias && !MatmulFeatureTrait<MM_CFG>::IsNeedUB()
                && (DoMatmulMDL(MM_CFG)
                    || isNormEnableScheduler<A_TYPE,
                        MM_CFG> || IsBmmEnableScheduler<A_TYPE, MM_CFG> || DoMatmulSpecialMDL(MM_CFG)
...
public:
    ...
    __aicore__ inline void Load(const LocalTensor<L0cT>& biasC2, const LocalTensor<BiasT>& bias, int32_t dataLen)
    {
        constexpr auto biasType = IsSameType<L0cT, BiasT>::value ? 2 : 1; // 2:f32, 1:f16/bf16
        if (MatmulFeatureTrait<MM_CFG>::IsSupportLoad2dV2()) {
            uint16_t lenBurst = CeilDiv(dataLen * biasType * DOUBLE_NUM, LEN_CEIL_DIV_NUM);
            if constexpr (IsSameType<L0cT, BiasT>::value) {
