...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_USING_UB_ND_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_USING_UB_ND_H
        ...
        const int row, const int col, const int height, const int width, const int gCol)
    {
        ...
        if constexpr (IsSameTypeV<TransT, half> && IsSameTypeV<SrcT, int8_t>) {
            ...
            MATMUL_MODULE(MatmulAntiQuantProcessor)
                ->AntiQuantCompute(trans, transTensor, isBankConflict, MATMUL_MODULE(CopyCubeInParams)->IsTranspose(),
        ...
        const DstTensor& dst, const SrcTensor& src, int row, int col, int height, int width, int gCol)
    {
        ...
        if (tail) {
            ...
            mask[0] = mask_tail_64bit + (mask_tail_64bit << FIRST_16BIT_OFFSET_MM_API)
                      + (mask_tail_64bit << SECOND_16BIT_OFFSET_MM_API)
                      + (mask_tail_64bit << THIRD_16BIT_OFFSET_MM_API);
            mask[1] = mask[0];
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_USING_UB_ND_H
