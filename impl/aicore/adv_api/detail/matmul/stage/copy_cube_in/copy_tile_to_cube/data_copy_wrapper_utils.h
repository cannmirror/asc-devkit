...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_UTILS_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_UTILS_H
...
template <typename SrcT, typename TransT>
__aicore__ inline void CopyNZ2NZImpl(const LocalTensor<TransT>& dst, const GlobalTensor<SrcT>& src, const int32_t row,
    const int32_t col, const int32_t height, const int32_t width, const int32_t gRow, const bool kAlignToC0Size = false)
{
...
template <typename SrcT, typename TransT>
__aicore__ inline void CopyNZ2NZImpl(const LocalTensor<TransT>& dst, const LocalTensor<SrcT>& src, const int32_t row,
    ...
    if (gCol % oriC0Size) {
        ...
        if (srcStride >= UINT16_MAX) {
            ...
            for (int i = 0; i < loopNum; ++i) {
                ...
                DataCopy(transTensor[i * MAX_BLOCK_COUNT_SIZE_MM_API * blocklen * ONE_BLK_SIZE / sizeof(SrcT)],
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_UTILS_H
