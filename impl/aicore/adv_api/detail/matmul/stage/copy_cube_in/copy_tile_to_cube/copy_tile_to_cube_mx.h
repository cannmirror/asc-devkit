...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_COPY_TILE_TO_CUBE_MX_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_COPY_TILE_TO_CUBE_MX_H
...
private:
    ...
    __aicore__ inline void CopyTileToCubeFromGM(const LocalTensor<TransT>& dst, const GlobalTensor<TransT>& src,
        int32_t curRow, int32_t curCol, int32_t tileHeight, int32_t tileWidth, int32_t widthFactor)
    {
        ...
        if constexpr (INPUT_TYPE::scaleFormat == CubeFormat::ND) {
            ...
            MATMUL_MODULE(DataCopyWrapper)
                ->CopyScaleNZ2NZ(dst, src, curRow * baseHeight, curCol * baseWidth, tileHeight, tileWidth, orgWidth);
        }
    }
};
} // namespace Detail
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_COPY_TILE_TO_CUBE_MX_H