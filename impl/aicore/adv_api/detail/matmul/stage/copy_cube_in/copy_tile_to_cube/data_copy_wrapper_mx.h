...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_MX_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_DATA_COPY_WRAPPER_MX_H
...
public:
    ...
    __aicore__ inline void CopyScaleNZ2NZ(const LocalTensor<fp8_e8m0_t>& dst, const GlobalTensor<fp8_e8m0_t>& src,
        ...
        const bool kAlignToC0Size = false)
    {
        ...
        // scale: 16*2 fractal
        int blockLen = BLOCK_CUBE * width * sizeof(fp8_e8m0_t) / ONE_BLK_SIZE;
        int srcStride = BLOCK_CUBE * (gRow - width) * sizeof(fp8_e8m0_t) / ONE_BLK_SIZE;
        if (srcStride >= UINT16_MAX) {
            for (int i = 0; i < Ceil(height, BLOCK_CUBE); ++i) {
                DataCopy(dst[i * BLOCK_CUBE * width], src[srcOffset + i * gRow * BLOCK_CUBE],
                    {1, static_cast<uint16_t>(blockLen), 0, 0});
            }
        } else {
            uint16_t nburst = Ceil(height, BLOCK_CUBE);
            int dstStride = 0;
    ...
    __aicore__ inline void CopyScaleDN2NZ(const LocalTensor<TransT>& dst, const GlobalTensor<TransT>& src,
        const int row, const int col, const int height, const int width, const int gScaleCol)
    {
        ...
        const int32_t row, const int32_t col, const int32_t height, const int32_t width, const int32_t gRow)
    {
        ASCENDC_ASSERT((gRow >= width), {
            KERNEL_LOG(KERNEL_ERROR,
                "MXND2NZ width larger than origin matrix width, origin matrix width is %d, which should be no less than width %d.",
                gRow, width);
        });
