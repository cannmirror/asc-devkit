...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_COPY_TILE_TO_CUBE_USING_UB_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_COPY_TILE_TO_CUBE_USING_UB_H
...
private:
        ...
        const LocalTensor<TransT>& aMatrix, int curRow, int curCol, int tileHeight, int tileWidth)
    {
        ...
        if constexpr (INPUT_TYPE::TAG == InputTypeTag::B
                      && IsSameTypeV<TransT, int8_t> && IsSameTypeV<SrcT, int8_t> && !IS_TRANS) {
            ...
            if constexpr (ToMatmulConfig(MM_CFG).enVecND2NZ) {
                ...
                MATMUL_MODULE(DataCopyWrapper)
                    ->CopyND2NZOnTheFly(aMatrix, aGlobal,
                        curRow * MATMUL_MODULE(CopyCubeInParams)->template GetBaseHeight<IS_TRANS>(),
                        curCol * MATMUL_MODULE(CopyCubeInParams)->template GetBaseWidth<IS_TRANS>(), tileHeight,
                        tileWidth, MATMUL_MODULE(CopyCubeInParams)->template GetOrgWidth<IS_TRANS>());
            }
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_COPY_TILE_TO_CUBE_COPY_TILE_TO_CUBE_USING_UB_H
