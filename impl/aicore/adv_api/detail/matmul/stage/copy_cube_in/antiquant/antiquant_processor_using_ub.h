...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_ANTIQUANT_ANTIQUANT_PROCESSOR_USING_UB_H
#define AICORE_ADV_API_DETAIL_MATMUL_STAGE_COPY_CUBE_IN_ANTIQUANT_ANTIQUANT_PROCESSOR_USING_UB_H
...
class MatmulAntiQuantProcessor<IMPL, A_TYPE, B_TYPE, MM_CFG,
...
public:
        ...
        bool isBankConflict, bool isTranspose, int32_t baseWidth)
    {
        ...
        if constexpr (ToMatmulConfig(MM_CFG).isPerTensor) {
            ...
            if (isTranspose) {
                uint32_t quantN =
                    CeilAlignT<int32_t>(MATMUL_MODULE(NLoop)->GetBaseShape(), ANTI_QUANT_ALIGN_SIZE_MM_API);
            ...
            if constexpr (ToMatmulConfig(MM_CFG).hasAntiQuantOffset) {
                if (isTranspose) {
                    AscendAntiQuant<SrcT, TransT, true>(quantOut, quantIn, antiQuantOffsetTensor_,
                        antiQuantScaleTensor_, sharedLocal,
        ...
        const LocalTensor<SrcT>& quantIn, const LocalTensor<uint8_t>& sharedLocal, bool isTranspose)
    {
        ...
        if constexpr (ToMatmulConfig(MM_CFG).hasAntiQuantOffset) {
            if (isTranspose) {
                AscendAntiQuant<SrcT, TransT, true>(quantOut, quantIn, antiQuantOffsetScalar_, antiQuantScaleScalar_,
                    sharedLocal,
                    CeilAlignT<int32_t>(MATMUL_MODULE(KLoop)->GetTileShapeB(), ANTI_QUANT_ALIGN_SIZE_MM_API));
            } else {
                AscendAntiQuant<SrcT, TransT, false>(quantOut, quantIn, antiQuantOffsetScalar_, antiQuantScaleScalar_,
                    sharedLocal,
                    CeilAlignT<int32_t>(MATMUL_MODULE(KLoop)->GetTileShapeB(), ANTI_QUANT_ALIGN_SIZE_MM_API));
            }
        } else {
            if (isTranspose) {
                AscendAntiQuant<SrcT, TransT, true>(quantOut, quantIn, antiQuantScaleScalar_, sharedLocal,
                    CeilAlignT<int32_t>(MATMUL_MODULE(KLoop)->GetTileShapeB(), ANTI_QUANT_ALIGN_SIZE_MM_API));
            } else {
                AscendAntiQuant<SrcT, TransT, false>(quantOut, quantIn, antiQuantScaleScalar_, sharedLocal,
                    CeilAlignT<int32_t>(MATMUL_MODULE(KLoop)->GetTileShapeB(), ANTI_QUANT_ALIGN_SIZE_MM_API));
            }
        }
#endif
    ...
    __aicore__ inline LocalTensor<uint8_t> GetSharedLocal(bool isTranspose, int32_t baseWidth)
    {
        ...
        if (isTranspose) {
            ...
            int32_t tmpBuffOffset =
                MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetTransLength() - tmpBuffSize - scaleUbSize;
            sharedLocal = MATMUL_MODULE(LocalWorkspace)
                              ->GetWorkspaceWithOffset(tmpBuffOffset)
