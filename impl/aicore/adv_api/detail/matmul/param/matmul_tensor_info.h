...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_TENSOR_INFO_H
#define AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_TENSOR_INFO_H
...
class MatmulTensorInfo<IMPL, MM_CFG, INPUT_TYPE,
    enable_if_t<IsIntrablock<MM_CFG> && !HasScalePosition<INPUT_TYPE>::value>> {
    using SrcT = typename INPUT_TYPE::T;
...
template <typename IMPL, const auto& MM_CFG, class INPUT_TYPE>
class MatmulTensorInfo<IMPL, MM_CFG, INPUT_TYPE,
    enable_if_t<IsSparseMatmul<MM_CFG, INPUT_TYPE> && !HasScalePosition<INPUT_TYPE>::value>> {
    using SrcT = typename INPUT_TYPE::T;
...
template <typename IMPL, const auto& MM_CFG, class INPUT_TYPE>
class MatmulTensorInfo<IMPL, MM_CFG, INPUT_TYPE,
    enable_if_t<HasScalePosition<INPUT_TYPE>::value
...
private:
    ...
    {
        ASCENDC_ASSERT((isTransposeScaleA <= INPUT_TYPE::isTrans), {
    ...
    {
        ASCENDC_ASSERT((isTransposeScaleB <= INPUT_TYPE::isTrans), {
            KERNEL_LOG(KERNEL_ERROR,
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_TENSOR_INFO_H
