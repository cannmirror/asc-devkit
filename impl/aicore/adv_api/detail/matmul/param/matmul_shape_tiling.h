...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_SHAPE_TILING_H
#define AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_SHAPE_TILING_H
...
public:
    ...
    __aicore__ inline void CheckTiling()
    {
        ...
        if constexpr (!HasScalePosition<typename IMPL::AType>::value
                      && !HasScalePosition<typename IMPL::BType>::value) {
            ShapeVaildCheck<SrcT, L0cT>();
            DepthCheck();
            ConfigCommonCheck();
            ConfigSpecificCheck();
        } else {
            MxShapeVaildCheck<SrcT, L0cT>();
            DepthCheck();
        }
#else
...
private:
    ...
    __aicore__ inline void ShapeVaildCheck()
    {
        ...
        ASCENDC_ASSERT((tiling_.GetBaseM() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                               * L0ABUseSizeFactor
                           <= L0ASize_),
            {
        ...
        ASCENDC_ASSERT((tiling_.GetBaseN() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                               * L0ABUseSizeFactor
                           <= L0BSize_),
            {
        ...
        if constexpr ((DoMatmulNorm(MM_CFG) || DoMatmulMDL(MM_CFG)) && ToMatmulConfig(MM_CFG).isA2B2Shared) {
            ASCENDC_ASSERT((tiling_.GetBaseM() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                ...
                {
                    KERNEL_LOG(KERNEL_ERROR,
                        "baseM * baseK is %d , which should be not larger than A2 Size / 2 when isA2B2Shared is enable %d",
                        tiling_.GetBaseM() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE,
                        L0ASize_ / Impl::DB_FACTOR);
                });
            ASCENDC_ASSERT((tiling_.GetBaseN() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                ...
                {
                    KERNEL_LOG(KERNEL_ERROR,
                        "baseN * baseK is %d , which should be not larger than B2 Size / 2 when isA2B2Shared is enable %d",
                        tiling_.GetBaseN() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE,
                        L0BSize_ / Impl::DB_FACTOR);
                });
        }
    ...
    template <typename SrcT, typename L0cT>
    __aicore__ inline void MxShapeVaildCheck()
    {
        ...
        ASCENDC_ASSERT(((tiling_.GetBaseM() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                                * L0ABUseSizeFactor
                            + tiling_.GetBaseM() * tiling_.GetBaseK() / 32 * sizeof(fp8_e8m0_t) * L0ABUseSizeFactor)
                           <= L0ASize_),
            {
                KERNEL_LOG(KERNEL_ERROR,
                    "baseM * baseK + baseM * baseScaleK is %d , which should be not larger than L0ASize_ %d",
                    tiling_.GetBaseM() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                            * L0ABUseSizeFactor
                        + tiling_.GetBaseM() * tiling_.GetBaseK() / 32 * sizeof(fp8_e8m0_t) * L0ABUseSizeFactor,
                    L0ASize_);
        ...
        ASCENDC_ASSERT((tiling_.GetBaseN() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                                   * L0ABUseSizeFactor
                               + tiling_.GetBaseN() * tiling_.GetBaseK() / 32 * sizeof(fp8_e8m0_t) * L0ABUseSizeFactor
                           <= L0BSize_),
            {
                KERNEL_LOG(KERNEL_ERROR,
                    "baseN * baseK + baseN * baseScaleK is %d , which should be not larger than L0BSize_ %d",
                    tiling_.GetBaseN() * tiling_.GetBaseK() * AscendC::GetBitSize<SrcT>() / ONE_BYTE_BIT_SIZE
                            * L0ABUseSizeFactor
                        + tiling_.GetBaseN() * tiling_.GetBaseK() / 32 * sizeof(fp8_e8m0_t) * L0ABUseSizeFactor,
                    L0BSize_);
    ...
    __aicore__ inline void ConfigCommonCheck()
    {
        ...
        if (IMPL::CType::format == CubeFormat::ND
            ...
            ASCENDC_ASSERT((false),
                { KERNEL_LOG(KERNEL_ERROR, "N dims need to be aligined to 32B when ND format output in v200."); });
        }
#endif
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_SHAPE_TILING_H
