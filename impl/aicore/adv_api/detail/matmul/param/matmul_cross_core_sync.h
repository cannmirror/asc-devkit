...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_CROSS_CORE_SYNC_H
#define AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_CROSS_CORE_SYNC_H
...
template <typename IMPL, typename A_TYPE, typename B_TYPE, typename C_TYPE, typename BIAS_TYPE, const auto& MM_CFG>
class MatmulCrossCoreSync<IMPL, A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, MM_CFG,
...
public:
    ...
    __aicore__ inline void WaitL1ReadyForInputUB()
    {
        if constexpr (GetPhyType(A_TYPE::pos) == Hardware::UB || GetPhyType(B_TYPE::pos) == Hardware::UB
                      || IsScalePosUB<A_TYPE>() || IsScalePosUB<B_TYPE>()
...
#endif // AICORE_ADV_API_DETAIL_MATMUL_PARAM_MATMUL_CROSS_CORE_SYNC_H
