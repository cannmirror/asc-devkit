 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_UTILS_MATMUL_TYPE_DEF_H
#define AICORE_ADV_API_DETAIL_MATMUL_UTILS_MATMUL_TYPE_DEF_H
...
struct ConstantType {
    ...
    constexpr __aicore__ inline operator value_type() const noexcept
    {
        return value;
    }
};
...
template <TPosition POSITION, TPosition SCALE_POSITION, CubeFormat FORMAT, typename TYPE, bool ISTRANS = false,
    TPosition SRCPOS = TPosition::GM, CubeFormat SCALE_FORMT = FORMAT, bool SCALE_ISTRANS = ISTRANS,
    TPosition SCALE_SRCPOS = SRCPOS>
