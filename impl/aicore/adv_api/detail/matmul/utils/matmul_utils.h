...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_UTILS_MATMUL_UTILS_H
#define AICORE_ADV_API_DETAIL_MATMUL_UTILS_MATMUL_UTILS_H
...
constexpr TQueConfig gCO1Config = {.nd2nz = false,
    .nz2nd = false,
    .scmBlockGroup = false,
...
struct DataCopyOutParams {
    ...
    __aicore__ DataCopyOutParams(const uint16_t count, const uint16_t len, const uint16_t srcStrideIn,
        const uint32_t dstStrideIn, const uint16_t nSize, const bool unitFlag, const int curMPos, const int curNPos)
    {
        cBurstNum = count;
        burstLen = len;
...
__aicore__ inline constexpr bool IsL0Cache()
{
    ...
    if constexpr (ToMatmulConfig(MM_CFG).doMultiDataLoad
...
template <typename A_TYPE, const auto& MM_CFG>
constexpr bool isNormDisableScheduler = DoMatmulNorm(MM_CFG)
