...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_DFX_DFX_PROXY_H
#define AICORE_ADV_API_DETAIL_MATMUL_DFX_DFX_PROXY_H
...
struct DfxProxy : MODULE {
    ...
    __aicore__ inline operator MODULE*()
    {
        return this;
    }
};
...
#define MATMUL_DEF_DFX_PROXY_FUNC(M_, MODULE, FUNC)                                                                    \
    ...
    __aicore__ inline auto FUNC(Args&&... args)                                                                        \
        ->enable_if_t<!is_void_v<decltype(MODULE().MODULE::FUNC(std::forward<Args>(args)...))>,                        \
...
#define MATMUL_COUNT_ARGS(...) MATMUL_COUNT_ARGS_IMPL(__VA_ARGS__, 9, 8, 7, 6, 5, 4, 3, 2, 1)

#define MATMUL_DEF_PROXY_FUNC_1(M_, MODULE, FUNC1) MATMUL_DEF_DFX_PROXY_FUNC(M_, MODULE, FUNC1)
...
#define MATMUL_DEF_DFX_FUNCS_IMPL2(N, M_, MODULE, ...) MATMUL_DEF_PROXY_FUNC_##N(M_, MODULE, __VA_ARGS__)

#define MATMUL_DEF_DFX_FUNCS_IMPL(N, M_, MODULE, ...) MATMUL_DEF_DFX_FUNCS_IMPL2(N, M_, MODULE, __VA_ARGS__)
...
#define MATMUL_DFX_PROXY_REGISTER(MODULE, ...)                                                                         \
    ...
    struct DfxProxy<const IMPL, typename IMPL::MODULE> {                                                               \
        ...
        struct FuncProxy {                                                                                             \
            ...
            __aicore__ inline const auto& operator*()                                                                  \
            {                                                                                                          \
                return m_;                                                                                             \
            }                                                                                                          \
            MATMUL_DEF_DFX_FUNCS(m_, MODULE, __VA_ARGS__)                                                              \
