 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_OUT_BUFFER_CUBE_OUT_BUFFER_CO1_SHARED_H
#define AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_OUT_BUFFER_CUBE_OUT_BUFFER_CO1_SHARED_H
...
public:
    ...
    __aicore__ inline void FreeTensor(LocalTensor<L0cT>& co1Local)
    {
        ...
        if constexpr (ToMatmulConfig(MM_CFG).iterateMode != IterateMode::ITERATE_MODE_ALL) {
            co1UsedNumber--;
            for (int i = 0; i < co1UsedNumber; i++) { co1UsedList[i] = co1UsedList[i + 1]; }
        }
    }
