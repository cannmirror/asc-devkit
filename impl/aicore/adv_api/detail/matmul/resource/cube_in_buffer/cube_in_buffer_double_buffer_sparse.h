...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_IN_BUFFER_CUBE_IN_BUFFER_DOUBLE_BUFFER_SPARSE_H
#define AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_IN_BUFFER_CUBE_IN_BUFFER_DOUBLE_BUFFER_SPARSE_H
...
class CubeInBuffer<IMPL, INPUT_TYPE, MM_CFG, POLICY_TYPE,
    enable_if_t<GetCubeInBufferType<INPUT_TYPE, MM_CFG>() == CubeInBufferType::DOUBLE_BUFFER_SPARSE>> {
...
public:
    ...
    __aicore__ inline void Init(int32_t baseBlockSize, int32_t cacheNum)
    {
        ...
        ASCENDC_ASSERT(stepSize > 0, { KERNEL_LOG(KERNEL_ERROR, "stepSize should not be less than 0."); });
...
private:
    ...
    __aicore__ inline void SetCache(
        bool isPong, const LocalTensor<TransT>& cacheTensor, const LocalTensor<uint8_t>& idxCacheTensor)
    {
        if (isPong) {
