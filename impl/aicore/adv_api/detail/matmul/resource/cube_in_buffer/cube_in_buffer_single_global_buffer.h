 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_IN_BUFFER_CUBE_IN_BUFFER_SINGLE_GLOBAL_BUFFER_H
#define AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_IN_BUFFER_CUBE_IN_BUFFER_SINGLE_GLOBAL_BUFFER_H
    ...
    __aicore__ inline bool IsTailBlock()
    {
        if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
            return (MATMUL_MODULE(MLoop)->GetBaseShape() != MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseM())
                   || (MATMUL_MODULE(KLoop)->GetBaseShape()
