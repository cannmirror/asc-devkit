...
#ifndef AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_IN_BUFFER_CUBE_IN_BUFFER_DOUBLE_BUFFER_H
#define AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_CUBE_IN_BUFFER_CUBE_IN_BUFFER_DOUBLE_BUFFER_H
...
class CubeInBuffer<IMPL, INPUT_TYPE, MM_CFG, POLICY_TYPE,
    enable_if_t<(GetCubeInBufferType<INPUT_TYPE, MM_CFG>() == CubeInBufferType::DOUBLE_BUFFER
        ...
        if constexpr (isScaleTag) {
            if constexpr (INPUT_TYPE::TAG == InputTypeTag::scaleA) {
                return MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetStepKa()
                       ...
                       * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetScaleFactorA();
            } else {
                return MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetStepKb()
                       * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetStepN()
                       * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetScaleFactorB();
            }
        } else {
        ...
        if constexpr (MatmulFeatureTrait<MM_CFG>::IsNeedUB() && ToMatmulConfig(MM_CFG).enVecND2NZ) {
            if constexpr (INPUT_TYPE::format == CubeFormat::ND) {
                ...
                if constexpr (INPUT_TYPE::isTrans) {
                    if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
                        bool isBankConflict = Ceil(MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetStepM()
                        ...
                        bool isBankConflict = Ceil(MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetStepKb()
                    ...
                    if constexpr (INPUT_TYPE::TAG == InputTypeTag::A) {
                        bool isBankConflict = Ceil(MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetStepKa()
                                                       * MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseK(),
                                                  c0Size)
                                                  * ONE_BLK_SIZE % BANK_CONFLICT_SIZE
                                              == 0;
                        ...
                        bool isBankConflict = Ceil(MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetStepN()
