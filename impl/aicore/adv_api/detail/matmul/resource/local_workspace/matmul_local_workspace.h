 ...
 */
#ifndef AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_LOCAL_WORKSPACE_MATMUL_LOCAL_WORKSPACE_H
#define AICORE_ADV_API_DETAIL_MATMUL_RESOURCE_LOCAL_WORKSPACE_MATMUL_LOCAL_WORKSPACE_H
...
#else
        ASCENDC_ASSERT(
            (false), { KERNEL_LOG(KERNEL_ERROR, "current vecrsion do not support SetLocalWorkspace interface!"); });
#endif
    }
...
private:
    __aicore__ inline int32_t GetAUsedSpace()
    {
        ...
        if constexpr (A_TYPE::pos != TPosition::TSCM) {
            ...
            } else if (MATMUL_MODULE(MatmulShapeInfo)->IsTransposeA()
        ...
        if constexpr (B_TYPE::pos != TPosition::TSCM) {
            if (IsSameType<typename A_TYPE::T, int8_t>::value && IsSameType<typename B_TYPE::T, int8_t>::value
                ...
                if (!MATMUL_MODULE(MatmulShapeInfo)->IsTransposeB()
                    ...
                    return MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetBaseK() * ONE_BLK_SIZE;
                } else if (MATMUL_MODULE(MatmulShapeInfo)->IsTransposeB()
                           && (MATMUL_MODULE(MatmulShapeTiling)->GetTiling().GetSingleCoreK() % c0Size != 0)) {
