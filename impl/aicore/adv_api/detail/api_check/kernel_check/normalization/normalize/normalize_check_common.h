 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_NORMALIZE_NORMALIZE_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_NORMALIZE_NORMALIZE_CHECK_COMMON_H
...
private:
        ...
        const float epsilon, const NormalizePara& para)
    {
        ...
        ASCENDC_ASSERT((para.rLengthWithPadding * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[Normalize] The para.rLengthWithPadding is %u, should be 32B aligned.",
        ...
        const float epsilon, const NormalizePara& para)
    {
        KERNEL_LOG(KERNEL_INFO,
            "[Normalize] The config.reducePattern is %d, config.aLength is %u, config.isOnlyOutput is %d.",
            config.reducePattern, config.aLength, config.isOnlyOutput);
        KERNEL_LOG(KERNEL_INFO,
            "[Normalize] The para.aLength is %u, para.rLength is %u, para.rLengthWithPadding is %u.", para.aLength,
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_NORMALIZE_NORMALIZE_CHECK_COMMON_H
