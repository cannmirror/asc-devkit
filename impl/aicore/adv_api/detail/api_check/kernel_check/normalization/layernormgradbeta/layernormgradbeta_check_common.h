 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_LAYERNORMGRADBETA_LAYERNORMGRADBETA_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_LAYERNORMGRADBETA_LAYERNORMGRADBETA_CHECK_COMMON_H
...
public:
    ...
    __aicore__ inline void CheckLayerNormGradBetaParams(const LocalTensor<T>& outputPdGamma,
        const LocalTensor<T>& outputPdBeta, const LocalTensor<T>& resForGamma, const LocalTensor<T>& inputDy,
...
private:
        ...
        const LayerNormGradBetaTiling& tiling)
    {
        ASCENDC_ASSERT(((outputPdGamma.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[LayerNormGradBeta] The outputPdGamma length is %u, should be 32B aligned.",
                outputPdGamma.GetSize() * sizeof(T));
        });
        ASCENDC_ASSERT(((outputPdBeta.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[LayerNormGradBeta] The outputPdBeta length is %u, should be 32B aligned.",
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_LAYERNORMGRADBETA_LAYERNORMGRADBETA_CHECK_COMMON_H
