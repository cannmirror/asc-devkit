 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_LAYERNORMGRAD_LAYERNORMGRAD_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_LAYERNORMGRAD_LAYERNORMGRAD_CHECK_COMMON_H
...
public:
    ...
    __aicore__ inline void CheckLayerNormGradParams(const LocalTensor<T>& outputPdX, const LocalTensor<T>& resForGamma,
        ...
        const LocalTensor<T>& inputMean, const LocalTensor<T>& inputGamma, LocalTensor<uint8_t>& sharedTmpBuffer,
        T epsilon, LayerNormGradTiling& tiling, const LayerNormGradShapeInfo& shapeInfo = {})
    {
...
private:
        ...
        T epsilon, LayerNormGradTiling& tiling, const LayerNormGradShapeInfo& shapeInfo = {})
    {
        ASCENDC_ASSERT(((inputGamma.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[LayerNormGrad] The inputGamma length is %u, should be 32B aligned.",
...
public:
        ...
        T epsilon, LayerNormGradTiling& tiling, const LayerNormGradShapeInfo& shapeInfo = {})
    {
        ...
        CheckLayerNormGradParamsClass::CheckLayerNormGradParams(outputPdX, resForGamma, inputDy, inputX, inputVariance,
            inputMean, inputGamma, sharedTmpBuffer, epsilon, tiling, shapeInfo);
    };
};

} // namespace HighLevelApiCheck
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_LAYERNORMGRAD_LAYERNORMGRAD_CHECK_COMMON_H
