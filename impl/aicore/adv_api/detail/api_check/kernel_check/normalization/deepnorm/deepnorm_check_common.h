 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_DEEPNORM_DEEPNORM_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_DEEPNORM_DEEPNORM_CHECK_COMMON_H
...
private:
        ...
        const T alpha, const T epsilon, DeepNormTiling& tiling)
    {
        ASCENDC_ASSERT(((betaLocal.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[DeepNorm] The betaLocal length is %u, should be 32B aligned.",
                betaLocal.GetSize() * sizeof(T));
        });
        ASCENDC_ASSERT(((gammaLocal.GetSize() * sizeof(T)) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[DeepNorm] The gammaLocal length is %u, should be 32B aligned.",
...
public:
        ...
        const T alpha, const T epsilon, DeepNormTiling& tiling)
    {
        ...
        CheckDeepNormParamsClass::CheckDeepNormParams<T, isReuseSrc, isBasicBlock>(dstLocal, meanLocal, rstdLocal,
            srcLocal, gxLocal, betaLocal, gammaLocal, sharedTmpBuffer, alpha, epsilon, tiling);
    };
};

} // namespace HighLevelApiCheck
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_NORMALIZATION_DEEPNORM_DEEPNORM_CHECK_COMMON_H
