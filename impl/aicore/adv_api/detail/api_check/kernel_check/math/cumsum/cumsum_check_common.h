 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_MATH_CUMSUM_CUMSUM_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_MATH_CUMSUM_CUMSUM_CHECK_COMMON_H
...
private:
        ...
        const LocalTensor<T>& srcTensor, const CumSumInfo& cumSumInfo)
    {
        ...
        ASCENDC_ASSERT(((cumSumInfo.inner * sizeof(T) % ONE_BLK_SIZE == 0) || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[CumSum] The result of cumSumInfo.inner * sizeof(T) is %u, "
    ...
    __aicore__ inline void PrintParameters(const LocalTensor<T>& dstTensor, const LocalTensor<T>& lastRowTensor,
        const LocalTensor<T>& srcTensor, const CumSumInfo& cumSumInfo)
    {
        KERNEL_LOG(KERNEL_INFO, "[CumSum] cumSumInfo.outter is %u, cumSumInfo.inner is %u.", cumSumInfo.outter,
            cumSumInfo.inner);
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_MATH_CUMSUM_CUMSUM_CHECK_COMMON_H
