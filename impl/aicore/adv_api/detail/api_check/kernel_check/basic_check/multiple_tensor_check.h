 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_BASIC_CHECK_MULTIPLE_TENSOR_CHECK_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_BASIC_CHECK_MULTIPLE_TENSOR_CHECK_H
...
private:
    ...
    {
        (SingleTensorReuseLoopCheck(Std::get<Is>(tensorTuple), Std::get<Is>(tensorStringTuple), tensorTuple,
             tensorStringTuple, Is, tuple_sequence<T>{}),
            ...);
    }

    template <typename T, typename U, typename W, size_t... Is>
    __aicore__ inline void SingleTensorReuseLoopCheck(const LocalTensor<W>& checkTensor,
        __gm__ const char* tensorString, T tensorTuple, U tensorStringTuple, const uint32_t index,
    ...
    template <typename T, typename U>
    __aicore__ inline void TensorReuseLoopCheck(const LocalTensor<T>& checkTensor, __gm__ const char* checkTensorString,
        const LocalTensor<U>& tensor, __gm__ const char* tensorString, const uint32_t indexCheck,
        const uint32_t indexTensor)
    {
        ...
        ASCENDC_ASSERT((!ans) || HighLevelAPIParametersPrint, {
            KERNEL_LOG(KERNEL_ERROR,
                "[%s] The input Tensor (%s) and (%s) cannot be reused, their start address is %u, tensor size is %u.",
    ...
    __aicore__ inline void PrintTensorInfo(const LocalTensor<T>& checkTensor, __gm__ const char* checkTensorString)
    {
        ...
        uint64_t checkPhyAddr = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(checkTensor.GetPhyAddr()));
        KERNEL_LOG(KERNEL_INFO, "[%s] CheckTensor (%s) physical address is (%u, %u)!", apiName, checkTensorString,
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_BASIC_CHECK_MULTIPLE_TENSOR_CHECK_H
