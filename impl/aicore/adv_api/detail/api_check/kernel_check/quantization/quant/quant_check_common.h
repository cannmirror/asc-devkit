 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_QUANTIZATION_QUANT_QUANT_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_QUANTIZATION_QUANT_QUANT_CHECK_COMMON_H
...
private:
        ...
        const uint32_t scaleCount, const uint32_t calCount)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[AscendQuant] The calCount is %u, should be an integer multiple of scaleCount %u.", calCount,
                scaleCount);
        });
        ...
        const uint32_t scaleCount, const uint32_t calCount)
    {
        ...
        KERNEL_LOG(KERNEL_INFO,
            "[AscendQuant] The config.calcCount is %u, config.scaleCount is %u, config.offsetCount is %u.",
...
private:
        ...
        const uint32_t calCount)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[AscendQuant] The scaleCount is %u, should be equal to offsetCount %u.",
    ...
    __aicore__ inline void PrintParameters(const LocalTensor<int8_t>& dstTensor, const LocalTensor<T>& srcTensor,
        ...
        const uint32_t calCount)
    {
        KERNEL_LOG(KERNEL_INFO, "[AscendQuant] The calCount is %u, scaleCount is %u, offsetCount is %u.", calCount,
            scaleCount, offsetCount);
        KERNEL_LOG(KERNEL_INFO,
            "[AscendQuant] The config.calcCount is %u, config.scaleCount is %u, config.offsetCount is %u.",
            config.calcCount, config.scaleCount, config.offsetCount);
...
public:
        ...
        const uint32_t calCount)
    {
        ...
        CheckAscendQuantChannelOffsetTensorParamsClass::CheckAscendQuantChannelOffsetTensorParams<T, isReuseSource,
            config>(
            dstTensor, srcTensor, sharedTmpBuffer, scaleTensor, offsetTensor, scaleCount, offsetCount, calCount);
    };
};
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_QUANTIZATION_QUANT_QUANT_CHECK_COMMON_H
