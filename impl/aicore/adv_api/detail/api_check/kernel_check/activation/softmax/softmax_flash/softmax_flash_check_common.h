 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_FLASH_SOFTMAX_FLASH_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_FLASH_SOFTMAX_FLASH_CHECK_COMMON_H
...
public:
    ...
    __aicore__ inline void CheckSoftMaxFlashSameTParams(const LocalTensor<T>& dstTensor,
    ...
    {
        VerifyingParameters<T, isReuseSource, isBasicBlock>(dstTensor, sumTensor, maxTensor, srcTensor, expMaxTensor,
            inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
        if constexpr (HighLevelAPIParametersPrint) {
            PrintParameters<T, isReuseSource, isBasicBlock>(dstTensor, sumTensor, maxTensor, srcTensor, expMaxTensor,
                inSumTensor, inMaxTensor, tiling, isUpdate, softmaxShapeInfo);
        }
    }
...
private:
        ...
        bool isUpdate, const SoftMaxShapeInfo& softmaxShapeInfo)
    {
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[SoftMaxFlash] The softmaxShapeInfo.srcK is %u, should be 32B aligned.",
        ...
        if (isBasicBlock == true && (softmaxShapeInfo.srcK != 0 || softmaxShapeInfo.srcM != 0)) {
            ...
            ASCENDC_ASSERT((softmaxShapeInfo.srcM % CHECK_SOFTMAXFLASH_SRCM_SIZE == 0 || HighLevelAPIParametersPrint), {
                KERNEL_LOG(KERNEL_ERROR,
                    "[SoftMaxFlash] The softmaxShapeInfo.srcM is %u, should be an integer multiple of 8.",
...
public:
        ...
        const SoftMaxShapeInfo& softmaxShapeInfo)
    {
        ...
        CheckSoftMaxFlashSameTParamsClass::CheckSoftMaxFlashSameTParams<T, isReuseSource, isBasicBlock>(dstTensor,
            sumTensor, maxTensor, srcTensor, expMaxTensor, inSumTensor, inMaxTensor, tiling, isUpdate,
            softmaxShapeInfo);
    };
...
public:
    template <typename T, bool isReuseSource = false, bool isBasicBlock = false>
    __aicore__ inline void CheckSoftMaxFlashParams(const LocalTensor<half>& dstTensor,
        const LocalTensor<float>& sumTensor, const LocalTensor<float>& maxTensor, const LocalTensor<half>& srcTensor,
        const LocalTensor<half>& expMaxTensor, const LocalTensor<float>& inSumTensor,
...
public:
    ...
    __aicore__ inline CheckFuncClassSoftmaxFlash(__gm__ const char* apiName) :
        ReuseSourceCheckFuncBasicClass(apiName), DataTypeCheckFuncBasicClass(apiName),
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_FLASH_SOFTMAX_FLASH_CHECK_COMMON_H
