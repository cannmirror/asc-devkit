 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_FLASHV3_SOFTMAX_FLASHV3_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_FLASHV3_SOFTMAX_FLASHV3_CHECK_COMMON_H
...
public:
    ...
    {
        VerifyingParameters<T, U, isUpdate, isReuseSource, isBasicBlock, isDataFormatNZ, config>(dstTensor, meanTensor,
            expSumTensor, maxTensor, srcTensor, expMaxTensor, inMeanTensor, inExpSumTensor, inMaxTensor,
            sharedTmpBuffer, tiling, params);
        if constexpr (HighLevelAPIParametersPrint) {
            PrintParameters<T, U, isUpdate, isReuseSource, isBasicBlock, isDataFormatNZ, config>(dstTensor, meanTensor,
                expSumTensor, maxTensor, srcTensor, expMaxTensor, inMeanTensor, inExpSumTensor, inMaxTensor,
                sharedTmpBuffer, tiling, params);
        }
    }
...
private:
        ...
        const SoftMaxParams& params)
    {
        ...
        ASCENDC_ASSERT((params.loopCnt >= 1 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(
                KERNEL_ERROR, "[SoftMaxFlashV3] The params.loopCnt is %u, should be larger than 0.", params.loopCnt);
        });
        ...
        ASCENDC_ASSERT((params.srcM == params.oriSrcM || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[SoftMaxFlashV3] The params.srcM is %u, params.oriSrcM is %u, they should be equal.", params.srcM,
                params.oriSrcM);
        });
        ASCENDC_ASSERT((params.srcK == params.oriSrcK || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[SoftMaxFlashV3] The params.srcK is %u, params.oriSrcK is %u, they should be equal.", params.srcK,
                params.oriSrcK);
        });
        ASCENDC_ASSERT(
...
public:
        ...
        const SoftMaxParams& params)
    {
        ...
        CheckSoftMaxFlashV3ParamsClass::CheckSoftMaxFlashV3Params<T, U, isUpdate, isReuseSource, isBasicBlock,
            isDataFormatNZ, config>(dstTensor, meanTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SOFTMAX_FLASHV3_SOFTMAX_FLASHV3_CHECK_COMMON_H
