 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SIMPLE_SOFTMAX_SIMPLE_SOFTMAX_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SIMPLE_SOFTMAX_SIMPLE_SOFTMAX_CHECK_COMMON_H
...
public:
    ...
    __aicore__ inline void CheckSimpleSoftMaxParams(const LocalTensor<T1>& dstTensor,
        const LocalTensor<T2>& inSumTensor, const LocalTensor<T2>& inMaxTensor, const LocalTensor<T1>& srcTensor,
...
private:
        ...
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo)
    {
        ASCENDC_ASSERT((softmaxShapeInfo.srcK * sizeof(T1) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[SimpleSoftMax] The softmaxShapeInfo.srcK is %u, should be 32B aligned.",
...
public:
        ...
        const SoftMaxTiling& tiling, const SoftMaxShapeInfo& softmaxShapeInfo)
    {
        ...
        CheckSimpleSoftMaxParamsClass::CheckSimpleSoftMaxParams<T1, T2, isReuseSource, isBasicBlock, isDataFormatNZ,
            config>(dstTensor, inSumTensor, inMaxTensor, srcTensor, sharedTmpBuffer, tiling, softmaxShapeInfo);
    };
};

} // namespace HighLevelApiCheck
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_SOFTMAX_SIMPLE_SOFTMAX_SIMPLE_SOFTMAX_CHECK_COMMON_H
