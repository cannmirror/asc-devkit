 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_GELU_FASTER_GELU_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_GELU_FASTER_GELU_CHECK_COMMON_H
...
private:
    ...
    __aicore__ inline void PrintParameters(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal,
        const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t dataSize)
    {
        KERNEL_LOG(KERNEL_INFO, "[FasterGelu] The highPrecision is %d, highPerformance is %d.", highPrecision,
            highPerformance);
    }
};
...
public:
    ...
    __aicore__ inline CheckFuncClassFasterGelu(__gm__ const char* apiName) :
        MultipleTensorCheckFuncBasicClass(apiName), SingleTensorCheckFuncBasicClass(apiName),
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_GELU_FASTER_GELU_CHECK_COMMON_H
