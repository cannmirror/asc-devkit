 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_GELU_FASTER_GELUV2_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_GELU_FASTER_GELUV2_CHECK_COMMON_H
...
private:
    ...
    __aicore__ inline void PrintParameters(const LocalTensor<T>& dstLocal, const LocalTensor<T>& srcLocal,
        const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t dataSize)
    {
        KERNEL_LOG(KERNEL_INFO, "[FasterGeluV2] The highPrecision is %d, highPerformance is %d.", highPrecision,
            highPerformance);
    }
};
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_ACTIVATION_GELU_FASTER_GELUV2_CHECK_COMMON_H
