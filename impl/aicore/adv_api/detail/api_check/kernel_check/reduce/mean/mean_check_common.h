 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_REDUCE_MEAN_MEAN_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_REDUCE_MEAN_MEAN_CHECK_COMMON_H
...
public:
    ...
    __aicore__ inline void CheckMeanParamsMeanParams(const MeanParams& meanParams)
    {
        ...
        ASCENDC_ASSERT(((meanParams.inner * sizeof(T) % ONE_BLK_SIZE == 0) || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[%s] MeanParams.inner must be 32B aligned, but the actual value is %u!",
                this->apiName, meanParams.inner);
        });
        bool ans = ((meanParams.n >= 1) && (meanParams.n <= meanParams.inner));
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[%s] MeanParams.n must be greater than or equal to 1 and less than or equal to MeanParams.inner, but actual only %u!",
    ...
    __aicore__ inline void CheckMeanParamsSrcTensorSize(const LocalTensor<T>& srcTensor, const MeanParams& meanParams)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[%s] srcTensor size should be greater than or equal to %u, but actual only %u!",
        ...
        const LocalTensor<uint8_t>& sharedTmpBuffer, uint32_t tmpBufferSize)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[%s] sharedTmpBuffer size should be greater than or equal to %u, but actual only %u!", this->apiName,
                tmpBufferSize, sharedTmpBuffer.GetSize());
        });
        if constexpr (HighLevelAPIParametersPrint) {
            KERNEL_LOG(
...
public:
    ...
    {
        DataTypeCheckFuncBasicClass::DataTypeVerifyingParameters<Std::tuple<T, accType>, Std::tuple<half, half>,
            Std::tuple<half, float>, Std::tuple<float, float>>(
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_REDUCE_MEAN_MEAN_CHECK_COMMON_H
