 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_REDUCE_SUM_SUM_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_REDUCE_SUM_SUM_CHECK_COMMON_H
...
public:
    ...
    __aicore__ inline void CheckSumParamsSumParams(const LocalTensor<T>& srcTensor, const SumParams& sumParams)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[%s] SumParams.n must be greater than or equal to 1 and less than or equal to SumParams.inner, but actual only %u",
    ...
    __aicore__ inline void CheckSumParamsDstTensorSize(const LocalTensor<T>& dstTensor, const SumParams& sumParams)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[%s] dstTensor size must be greater than 0, but actual only %u", this->apiName,
                dstTensor.GetSize());
        });
    ...
    __aicore__ inline void CheckSumParamsSrcTensorSize(const LocalTensor<T>& srcTensor)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[%s] srcTensor size must be greater than 0, but actual only %u", this->apiName,
                srcTensor.GetSize());
        });
        if constexpr (HighLevelAPIParametersPrint) {
        ...
        const LocalTensor<uint8_t>& sharedTmpBuffer, const SumParams& sumParams)
    {
        ...
        ASCENDC_ASSERT((ans || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[%s] sharedTmpBuffer size must be greater than 0, but actual only %u",
                this->apiName, sharedTmpBuffer.GetSize());
        });
        if constexpr (HighLevelAPIParametersPrint) {
            KERNEL_LOG(
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_REDUCE_SUM_SUM_CHECK_COMMON_H
