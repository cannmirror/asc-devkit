 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_INDEX_ARITHPROGRESSION_ARITHPROGRESSION_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_INDEX_ARITHPROGRESSION_ARITHPROGRESSION_CHECK_COMMON_H
...
private:
        ...
        const LocalTensor<T>& dstLocal, const T firstValue, const T diffValue, const int32_t count)
    {
        ASCENDC_ASSERT((count > 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(
                KERNEL_ERROR, "[ArithProgression] The count parameter cannot be %d, should be greater than 0.", count);
        });
        ASCENDC_ASSERT((static_cast<float>(diffValue) >= static_cast<float>(0) || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[ArithProgression] The diffValue parameter cannot be %f, should be greater than or equal to 0.",
...
public:
        ...
        const LocalTensor<T>& dstLocal, const T firstValue, const T diffValue, const int32_t count)
    {
        ...
        SingleTensorCheckFuncBasicClass::TensorVerifyingParameters(VA_ARGS_TO_MAKE_TUPLE(dstLocal),
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_INDEX_ARITHPROGRESSION_ARITHPROGRESSION_CHECK_COMMON_H
