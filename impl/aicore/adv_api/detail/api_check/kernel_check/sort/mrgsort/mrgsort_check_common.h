...
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_SORT_MRGSORT_MRGSORT_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_SORT_MRGSORT_MRGSORT_CHECK_COMMON_H
...
private:
    ...
    template <typename T, bool isExhaustedSuspension = false>
    __aicore__ inline void VerifyingParametersCommonVersion(const LocalTensor<T>& dst,
        const MrgSortSrcList<T>& sortList, const uint16_t elementCountList[4], uint32_t sortedNum[4], uint16_t validBit,
        const int32_t repeatTime)
    {
        ...
        for (uint8_t i = 0; i < MRG_SORT_ELEMENT_LEN; i++) {
            ASCENDC_ASSERT(
                ...
                {
                    KERNEL_LOG(KERNEL_ERROR, "[%s] The elementCountList[%u] is %u, should in range [1, %u].",
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_SORT_MRGSORT_MRGSORT_CHECK_COMMON_H