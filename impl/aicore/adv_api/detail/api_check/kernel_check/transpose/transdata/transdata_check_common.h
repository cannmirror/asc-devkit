 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_TRANSPOSE_TRANSDATA_TRANSDATA_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_TRANSPOSE_TRANSDATA_TRANSDATA_CHECK_COMMON_H
...
private:
    ...
    __aicore__ inline void CheckTransDataConfig()
    {
        ...
        if constexpr (HighLevelAPIParametersPrint) {
            KERNEL_LOG(KERNEL_INFO, "[%s] The parameter config srcFormat is %s!", apiName,
                DataFormatToString(config.srcFormat));
            KERNEL_LOG(KERNEL_INFO, "[%s] The parameter config dstFormat is %s!", apiName,
        ...
        const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor, const TransDataParams<U, S>& params)
    {
        ...
        if constexpr (HighLevelAPIParametersPrint) {
            KERNEL_LOG(
                KERNEL_INFO, "[%s] The NCDHW Layout Shape is (%d, %d, %d, %d, %d)!", this->apiName, n, c, d, h, w);
            KERNEL_LOG(KERNEL_INFO, "[%s] The NDC1HWC0 Layout Shape is (%d, %d, %d, %d, %d, %d)!", this->apiName,
                ndc1hwc0N, ndc1hwc0D, ndc1hwc0C1, ndc1hwc0H, ndc1hwc0W, ndc1hwc0C0);
        }
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_TRANSPOSE_TRANSDATA_TRANSDATA_CHECK_COMMON_H
