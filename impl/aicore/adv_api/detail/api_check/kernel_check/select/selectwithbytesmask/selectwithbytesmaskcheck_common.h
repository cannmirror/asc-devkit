 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_SELECT_SELECTWITHBYTESMASK_SELECTWITHBYTESMASKCHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_SELECT_SELECTWITHBYTESMASK_SELECTWITHBYTESMASKCHECK_COMMON_H
...
private:
    template <typename T, typename U, bool isReuseMask, bool reverse = false>
    __aicore__ inline void VerifyingParameters(const LocalTensor<T>& dst, const LocalTensor<T>& srcTensor, T srcScalar,
        ...
        const SelectWithBytesMaskShapeInfo& info)
    {
        ...
        ASCENDC_ASSERT((info.maskLastAxis % CHECK_SELECT_WITH_BYTES_MASK == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[SelectWithBytesMask] The info.maskLastAxis %u must be an integer multiple of 16.", info.maskLastAxis);
        });
    ...
    template <typename T, typename U, bool isReuseMask, bool reverse = false>
    __aicore__ inline void PrintParameters(const LocalTensor<T>& dst, const LocalTensor<T>& srcTensor, T srcScalar,
        const LocalTensor<U>& mask, const LocalTensor<uint8_t>& sharedTmpBuffer,
        const SelectWithBytesMaskShapeInfo& info)
    {
...
public:
        ...
        const SelectWithBytesMaskShapeInfo& info)
    {
        ...
        DataTypeCheckFuncBasicClass::DataTypeVerifyingParameters<U, bool, uint8_t, int8_t, uint16_t, int16_t, uint32_t,
            int32_t>("second template parameter (U) is not bool/uint8_t/int8_t/uint16_t/int16_t/uint32_t/int32_t");
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_SELECT_SELECTWITHBYTESMASK_SELECTWITHBYTESMASKCHECK_COMMON_H
