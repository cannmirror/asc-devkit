 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_FILTER_DROPOUT_DROPOUT_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_FILTER_DROPOUT_DROPOUT_CHECK_COMMON_H
...
private:
        ...
        const DropOutShapeInfo& info)
    {
        ...
        ASCENDC_ASSERT((info.firstAxis > 0 || HighLevelAPIParametersPrint),
            { KERNEL_LOG(KERNEL_ERROR, "[DropOut] The info.firstAxis parameter must > 0.", info.firstAxis); });
        ASCENDC_ASSERT((info.srcLastAxis > 0 || HighLevelAPIParametersPrint),
            { KERNEL_LOG(KERNEL_ERROR, "[DropOut] The info.srcLastAxis parameter must > 0.", info.srcLastAxis); });
        ASCENDC_ASSERT((info.maskLastAxis > 0 || HighLevelAPIParametersPrint),
            { KERNEL_LOG(KERNEL_ERROR, "[DropOut] The info.maskLastAxis parameter must > 0.", info.maskLastAxis); });
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_FILTER_DROPOUT_DROPOUT_CHECK_COMMON_H
