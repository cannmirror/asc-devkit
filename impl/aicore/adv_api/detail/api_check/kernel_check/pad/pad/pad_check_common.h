 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_PAD_PAD_PAD_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_PAD_PAD_PAD_CHECK_COMMON_H
...
private:
        ...
        PadParams& padParams, const LocalTensor<uint8_t>& sharedTmpBuffer, PadTiling& tiling)
    {
        ASCENDC_ASSERT((padParams.leftPad * sizeof(T) < ONE_BLK_SIZE || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[Pad] The result of padParams.leftPad * sizeof(T) cannot be %u, should be less than 32.",
                padParams.leftPad * sizeof(T));
        });
        ASCENDC_ASSERT((padParams.rightPad * sizeof(T) < ONE_BLK_SIZE || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[Pad] The result of padParams.rightPad * sizeof(T) cannot be %u, should be less than 32.",
        ...
        if (tiling.srcWidth * sizeof(T) % ONE_BLK_SIZE == 0) {
            ASCENDC_ASSERT((tiling.srcWidth != tiling.srcOriWidth || HighLevelAPIParametersPrint), {
                KERNEL_LOG(KERNEL_ERROR,
                    "[Pad] The tiling.srcWidth is %u, tiling.srcOriWidth is %u, which can not be equal.",
    ...
    __aicore__ inline void PrintParameters(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor,
        PadParams& padParams, const LocalTensor<uint8_t>& sharedTmpBuffer, PadTiling& tiling)
    {
        KERNEL_LOG(KERNEL_INFO, "[Pad] The tiling.srcWidth is %u, tiling.srcOriWidth is %u.", tiling.srcWidth,
            tiling.srcOriWidth);
        KERNEL_LOG(KERNEL_INFO, "[Pad] The padParams.leftPad is %u, padParams.rightPad is %u.", padParams.leftPad,
            padParams.rightPad);
    }
};
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_PAD_PAD_PAD_CHECK_COMMON_H
