 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_PAD_PAD_UNPAD_CHECK_COMMON_H
#define AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_PAD_PAD_UNPAD_CHECK_COMMON_H
        ...
        UnPadParams& unPadParams, LocalTensor<uint8_t>& sharedTmpBuffer, UnPadTiling& tiling)
    {
        ASCENDC_ASSERT(((tiling.srcWidth) * sizeof(T) % ONE_BLK_SIZE == 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[UnPad] The result of tiling.srcWidth * sizeof(T) cannot be %u, "
        ...
        ASCENDC_ASSERT((unPadParams.rightPad > 0 || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR, "[UnPad] The unPadParams.rightPad is %u, should be greater than 0.",
                unPadParams.rightPad);
        });
        ASCENDC_ASSERT((unPadParams.rightPad * sizeof(T) < ONE_BLK_SIZE || HighLevelAPIParametersPrint), {
            KERNEL_LOG(KERNEL_ERROR,
                "[UnPad] The result of unPadParams.rightPad * sizeof(T) cannot be %u, "
...
#endif // AICORE_ADV_API_DETAIL_API_CHECK_KERNEL_CHECK_PAD_PAD_UNPAD_CHECK_COMMON_H
