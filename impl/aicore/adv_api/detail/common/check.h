 ...
 */
#ifndef AICORE_ADV_API_DETAIL_COMMON_CHECK_H
#define AICORE_ADV_API_DETAIL_COMMON_CHECK_H
    ...
    const LocalTensor<T>& checkTensor, __gm__ const char* tensorInfo, __gm__ const char* apiInfo)
{
    ...
    ASCENDC_ASSERT((calCount <= checkTensor.GetSize()), {
        KERNEL_LOG(KERNEL_ERROR, "The %s parameter cannot be %u, should not be larger than %s size %u in %s.",
            calCountInfo, calCount, tensorInfo, checkTensor.GetSize(), apiInfo);
    });
#endif
}
...
#endif // AICORE_ADV_API_DETAIL_COMMON_CHECK_H