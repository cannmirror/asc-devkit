...
#ifndef AICORE_ADV_API_DETAIL_HOST_LOG_H
#define AICORE_ADV_API_DETAIL_HOST_LOG_H
#include <cassert>
#include <cstdint>
        ...
        if (!(cond)) {                                                                                                 \
            if (AlogRecord == nullptr) {                                                                               \
                ...
                if (AlogCheckDebugLevel(ASCENDC_MODULE_NAME, DLOG_ERROR) == 1) {                                       \
                    AlogRecord(ASCENDC_MODULE_NAME, DLOG_TYPE_DEBUG, DLOG_ERROR, "[%s:%d][%s] " format "\n", __FILE__, \
    ...
    do {                                                                                                               \
        if (AlogRecord == nullptr) {                                                                                   \
            ...
            if (AlogCheckDebugLevel(ASCENDC_MODULE_NAME, DLOG_INFO) == 1) {                                            \
                AlogRecord(ASCENDC_MODULE_NAME, DLOG_TYPE_DEBUG, DLOG_INFO, "[%s:%d][%s] " format "\n", __FILE__,      \
                    __LINE__, __FUNCTION__, ##__VA_ARGS__);                                                            \
            }                                                                                                          \
        }                                                                                                              \
    } while (0)
    ...
    do {                                                                                                               \
        if (AlogRecord == nullptr) {                                                                                   \
            ...
            if (AlogCheckDebugLevel(ASCENDC_MODULE_NAME, DLOG_WARN) == 1) {                                            \
                AlogRecord(ASCENDC_MODULE_NAME, DLOG_TYPE_DEBUG, DLOG_WARN, "[%s:%d][%s] " format "\n", __FILE__,      \
                    __LINE__, __FUNCTION__, ##__VA_ARGS__);                                                            \
            }                                                                                                          \
        }                                                                                                              \
    } while (0)
    ...
    do {                                                                                                               \
        if (AlogRecord == nullptr) {                                                                                   \
            ...
            if (AlogCheckDebugLevel(ASCENDC_MODULE_NAME, DLOG_DEBUG) == 1) {                                           \
                AlogRecord(ASCENDC_MODULE_NAME, DLOG_TYPE_DEBUG, DLOG_DEBUG, "[%s:%d][%s] " format "\n", __FILE__,     \
                    __LINE__, __FUNCTION__, ##__VA_ARGS__);                                                            \
            }                                                                                                          \
        }                                                                                                              \
    } while (0)
#endif // AICORE_ADV_API_DETAIL_HOST_LOG_H
