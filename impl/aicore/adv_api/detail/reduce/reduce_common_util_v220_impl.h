 ...
 */
#ifndef AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_UTIL_V220_IMPL_H
#define AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_UTIL_V220_IMPL_H
    ...
    if (tailBlkNum == 0 || firstAxis < DEFAULT_BLK_NUM) {
        ...
        for (int32_t i = 0; i < blkMaxRepeat; i++) {
            ...
            BlockReduceCompute<T, apiMode>(dstTensor[i * MAX_REPEAT_TIMES * DEFAULT_BLK_NUM],
                srcTensor[i * MAX_REPEAT_TIMES * elePerRep], blkMainRepeat, mainMask, 1, DEFAULT_REPEAT_STRIDE);
            PipeBarrier<PIPE_V>();
        }
    } else {
        ...
        for (int32_t i = 0; i < blkMaxRepeat; i++) {
            if (i == blkMaxRepeat - 1) {
                BlockReduceCompute<T, apiMode>(dstTensor[i * MAX_REPEAT_TIMES * DEFAULT_BLK_NUM],
                    srcTensor[i * MAX_REPEAT_TIMES * elePerRep], blkRepeatTail - 1, mainMask, 1, DEFAULT_REPEAT_STRIDE);
                PipeBarrier<PIPE_V>();
                ...
                BlockReduceCompute<T, apiMode>(dstTensor[i * MAX_REPEAT_TIMES * DEFAULT_BLK_NUM],
                    srcTensor[i * MAX_REPEAT_TIMES * elePerRep], MAX_REPEAT_TIMES, mainMask, 1, DEFAULT_REPEAT_STRIDE);
                PipeBarrier<PIPE_V>();
            }
        }
    }
    ...
    const LocalTensor<T>& tmpTensor, uint32_t firstAxis, uint32_t lastAxis, uint32_t padLast)
{
    ...
    if constexpr (!isReuseSource) {
        ...
        BinaryRepeatParams blockMainParams{
            blkStridePerRow, blkStridePerRow, blkStridePerRow, blkStridePerRep, blkStridePerRep, blkStridePerRep};
    ...
    const LocalTensor<T>& tmpTensor, uint32_t firstAxis, uint32_t lastAxis, uint32_t padLast)
{
    ...
    if constexpr (!isReuseSource) {
        ...
        for (int32_t i = 0; i < firstAxis; i++) {
            Adds<T, false>(tmpTensor[i * elePerRep], srcTensor[i * padLast], static_cast<T>(0), MASK_PLACEHOLDER, 1,
                defaultUnaryParams);
            PipeBarrier<PIPE_V>();
        }
        ...
        for (int32_t i = 1; i < repCount; i++) {
            ...
            for (int32_t j = 0; j < firstAxis; j++) {
                func(tmpTensor[j * elePerRep], tmpTensor[j * elePerRep], srcTensor[j * padLast + i * elePerRep],
    ...
    const LocalTensor<T>& tmpTensor, uint32_t firstAxis, uint32_t lastAxis, uint32_t padLast)
{
    ASCENDC_ASSERT((dstTensor.GetSize() >= firstAxis), {
        KERNEL_LOG(KERNEL_ERROR, "dstTensor must be greater than or equal to %u, current size if %u", firstAxis,
            dstTensor.GetSize());
    });
    constexpr uint32_t elePerBlk = ONE_BLK_SIZE / sizeof(T);
    constexpr uint32_t elePerRep = ONE_REPEAT_BYTE_SIZE / sizeof(T);
    ...
    const LocalTensor<T>& dst, const LocalTensor<T>& src, const LocalTensor<T>& tmp, const ReduceParams& params)
{
    ...
    for (int i = 0; i < params.first; i++) {
        ...
        Cast<half, uint8_t, false>(tmpBuf, src[i * params.padLast], RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
        ...
        Adds<half, false>(tmpBuf[params.padLast + i * halfBlkElements], tmpBuf, halfZero, MASK_PLACEHOLDER, 1,
    ...
    Cast<uint8_t, half, false>(dst, tmpBuf, RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
    ...
    const LocalTensor<T>& tmpTensor, uint32_t firstAxis, uint32_t lastAxis, uint32_t padLast)
{
    ASCENDC_ASSERT((dstTensor.GetSize() >= lastAxis), {
        KERNEL_LOG(KERNEL_ERROR, "dstTensor must be greater than or equal to %u, current size if %u", lastAxis,
            dstTensor.GetSize());
    });
...
#endif // AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_UTIL_V220_IMPL_H