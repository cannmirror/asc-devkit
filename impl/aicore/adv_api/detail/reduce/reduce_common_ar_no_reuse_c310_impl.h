 ...
 */
#ifndef AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_AR_NO_REUSE_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_AR_NO_REUSE_C310_IMPL_H
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t j = 0; j < static_cast<uint16_t>(aLength); j++) {
                ...
                for (uint16_t i = 0; i < repeatTimes2; i++) {
                    ...
                    ReduceSum(dst0Reg, dst0Reg, pregFull);
                    DataCopy<T, MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(
                        (tmpBuf + repeatTimes1 + i), dst0Reg, pregOne);
                }
                ...
                for (uint16_t i = 0; i < repeatTimes3; i++) {
                    ...
                    ReduceSum(dst0Reg, dst0Reg, pregFull);
                    DataCopy<T, MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(
                        (tmpBuf + repeatTimes1 + repeatTimes2 + i), dst0Reg, pregOne);
                }
...
template <class T>
__aicore__ inline void ReduceAR(__ubuf__ T* dst, __ubuf__ T* src, uint32_t dimA, uint32_t dimR)
{
...
#endif // AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_AR_NO_REUSE_C310_IMPL_H