 ...
 */
#ifndef AICORE_ADV_API_DETAIL_REDUCE_REDUCE_MAX_REDUCE_MAX_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_REDUCE_REDUCE_MAX_REDUCE_MAX_C310_IMPL_H
    ...
    __ubuf__ T* dstAddr, __ubuf__ T* srcAddr, const uint32_t srcShape[])
{
    if ((srcShape[1] * sizeof(T)) % 32 == 0) {
        ReduceARReuseSource<T, MicroAPI::RegTraitNumTwo,
            MicroAPI::Max<T, MicroAPI::MaskMergeMode::ZEROING, MicroAPI::RegTensor<T, MicroAPI::RegTraitNumTwo>>,
            MicroAPI::ReduceMax<T, MicroAPI::MaskMergeMode::ZEROING, MicroAPI::RegTensor<T, MicroAPI::RegTraitNumTwo>>>(
...
__aicore__ inline void ReduceMaxCompute(
    const LocalTensor<T>& dst, const LocalTensor<T>& src, const uint32_t srcShape[], bool srcInnerPad)
{
    ...
    static_assert(isReuseSource, "ReduceMax only support isReuseSource true on current device!");
    ASCENDC_ASSERT(
        (!srcInnerPad), { KERNEL_LOG(KERNEL_ERROR, "ReduceMax only support srcInnerPad false on current device!!"); });

    __ubuf__ T* dstAddr = (__ubuf__ T*)dst.GetPhyAddr();
    __ubuf__ T* srcAddr = (__ubuf__ T*)src.GetPhyAddr();
    if constexpr (std::is_same_v<pattern, Pattern::Reduce::AR>) {
        if constexpr (SupportBytes<T, 8>()) {
            ReduceMaxARB64ReuseSourceCompute<T>(dstAddr, srcAddr, srcShape);
        } else {
            ReduceMaxARReuseSourceCompute<T>(dstAddr, srcAddr, srcShape);
        }
    } else {
        if constexpr (SupportBytes<T, 8>()) {
            ReduceMaxRAB64ReuseSourceCompute<T>(dstAddr, srcAddr, srcShape);
        } else {
            ReduceMaxRAReuseSourceCompute<T>(dstAddr, srcAddr, srcShape);
        }
    }
}
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_REDUCE_REDUCE_MAX_REDUCE_MAX_C310_IMPL_H