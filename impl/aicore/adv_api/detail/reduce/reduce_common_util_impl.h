 ...
 */
#ifndef AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_UTIL_IMPL_H
#define AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_UTIL_IMPL_H
    ...
    uint32_t oneBlkMask, uint32_t oneBlkElems, uint32_t blkNum, uint64_t& maskLow, uint64_t& maskHigh)
{
    ...
    if constexpr (sizeof(T) == sizeof(half)) {
        ...
        for (int32_t k = 0; k < maskLBlkNum; k++) { maskLow += (((1ULL << oneBlkMask) - 1ULL) << (k * oneBlkElems)); }
        for (int32_t k = 0; k < maskHBlkNum; k++) { maskHigh += (((1ULL << oneBlkMask) - 1ULL) << (k * oneBlkElems)); }
    } else if constexpr (sizeof(T) == sizeof(float)) {
        for (int32_t k = 0; k < blkNum; k++) { maskLow += (((1ULL << oneBlkMask) - 1ULL) << (k * oneBlkElems)); }
    }
}
...
#endif // AICORE_ADV_API_DETAIL_REDUCE_REDUCE_COMMON_UTIL_IMPL_H