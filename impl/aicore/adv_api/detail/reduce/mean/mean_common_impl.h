 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_REDUCE_MEAN_MEAN_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_REDUCE_MEAN_MEAN_COMMON_IMPL_H
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const MeanParams& meanParams)
{
    ...
    for (uint32_t row = 0; row < meanParams.outter; ++row) {
        ...
        PipeBarrier<PIPE_V>();
        RepeatReduceSum<float, false>(TmpTensor[meanParams.inner], TmpTensor, 1, MASK_PLACEHOLDER, DEFAULT_BLK_STRIDE,
        ...
        while (reduceNums > 1) {
            ...
            RepeatReduceSum<float, false>(TmpTensor[meanParams.inner], TmpTensor[meanParams.inner], 1, MASK_PLACEHOLDER,
        ...
        PipeBarrier<PIPE_V>();
        Cast<half, float, false>(castTensor, TmpTensor[meanParams.inner], RoundMode::CAST_NONE, MASK_PLACEHOLDER, 1,
            {1, 1, HALF_DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE});
        PipeBarrier<PIPE_V>();
        RepeatReduceSum<half, false>(dstTensor[row], castTensor, 1, MASK_PLACEHOLDER, DEFAULT_BLK_STRIDE,
            DEFAULT_BLK_STRIDE, DEFAULT_BLK_STRIDE, DEFAULT_REPEAT_STRIDE);
    }
    SetMaskNorm();
    ResetMask();
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const MeanParams& meanParams)
{
    ...
    for (uint32_t row = 0; row < meanParams.outter; ++row) {
        uint32_t reduceNums = repeateTimes;
        SetVectorMask<T>(0, meanParams.n);
        RepeatReduceSum<T, false>(TmpTensor, srcTensor[row * meanParams.inner], 1, MASK_PLACEHOLDER, DEFAULT_BLK_STRIDE,
            DEFAULT_BLK_STRIDE, DEFAULT_BLK_STRIDE, DEFAULT_REPEAT_STRIDE);
        PipeBarrier<PIPE_V>();
        while (reduceNums > 1) {
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const MeanParams& meanParams)
{
    ...
    if constexpr (sizeof(T) == sizeof(half) && sizeof(accType) == sizeof(float)) {
        ...
        CHECK_FUNC_HIGHLEVEL_API(Mean, (T, accType, isReuseSource, isBasicBlock, reduceDim),
