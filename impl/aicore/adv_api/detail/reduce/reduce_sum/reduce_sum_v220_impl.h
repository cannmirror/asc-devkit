 ...
 */
#ifndef AICORE_ADV_API_DETAIL_REDUCE_REDUCE_SUM_REDUCE_SUM_V220_IMPL_H
#define AICORE_ADV_API_DETAIL_REDUCE_REDUCE_SUM_REDUCE_SUM_V220_IMPL_H
...
__aicore__ inline void ReduceSumComputeSliceAAxis(const LocalTensor<T>& dst, const LocalTensor<T>& src,
    const LocalTensor<T>& tmpBuf, const uint32_t repeat, const uint32_t perRowReduceSize, const uint32_t tailLen,
    const ReduceParams& params)
{
    ...
    if constexpr (isReuseSource) {
        ...
        for (uint32_t i = 0; i < repeat; i++) {
            ...
            Add<T, false>(tmpBuf[curOffset], tmpBuf[curOffset], src[curOffset + perRowReduceSize], MASK_PLACEHOLDER, 1,
                params.defaultParam);
        }
        PipeBarrier<PIPE_V>();
        ...
        for (uint32_t i = 0; i < repeat; i++) {
            curOffset = i * params.padLast;
            SetVectorMask<T, MaskMode::COUNTER>(tailLen);
            Add<T, false>(tmpBuf[i * perRowReduceSize], tmpBuf[i * perRowReduceSize], src[curOffset + perRowReduceSize],
...
__aicore__ inline void ReduceSumComputeSliceRAxis(const LocalTensor<T>& dst, const LocalTensor<T>& src,
    const LocalTensor<T>& tmpBuf, const uint32_t repeat, const uint32_t perRowReduceSize, const uint32_t tailLenN,
    const uint32_t tailLenRemain, const ReduceParams& params)
{
    SetMaskNorm();
    uint32_t srcOffset;
    uint32_t tmpOffset;
...
__aicore__ inline void ReduceSumComputeTail(const LocalTensor<T>& dst, const LocalTensor<T>& src,
    const LocalTensor<T>& tmpBuf, const uint32_t repeat, const uint32_t perRowReduceSize, const ReduceParams& params)
{
    ...
    const LocalTensor<T>& tmpBuf, const uint32_t perRowReduceSize, const ReduceParams& params)
{
    ...
    if (params.last <= B32_DATA_NUM_PER_REPEAT) {
        ...
        for (uint32_t i = 0; i < maxRepOffsetN; i++) {
            ...
            WholeReduceSum<T, false>(dst[dstMaxRepNOffset], src[srcMaxRepNOffset], MASK_PLACEHOLDER, maxRepeatTimes, 1,
                1, params.padLast / params.elePerBlk);
        }
        if (maxRepeatOffsetTail > 0) {
            ...
            SetVectorMask<T, MaskMode::NORMAL>(params.last);
            WholeReduceSum<T, false>(dst[dstMaxRepTailOffset], src[srcMaxRepTailOffset], MASK_PLACEHOLDER,
                maxRepeatOffsetTail, 1, 1, params.padLast / params.elePerBlk);
        }
    } else {
        SetMaskCount();
        ...
        if (maxRepeatOffsetTail > 0) {
            ...
            ReduceSumInLargeLast<T, isReuseSource>(dst[dstMaxRepTailOffset], src[srcMaxRepTailOffset],
                tmpBuf[tmpBufMaxRepNOffset], maxRepeatOffsetTail, perRowReduceSize, params);
        }
        SetMaskNorm();
    }
    PipeBarrier<PIPE_V>();
...
__aicore__ inline void ReduceSumCommon(const LocalTensor<T>& dstTensor, const LocalTensor<T>& srcTensor,
    ...
    const ReduceParams& reduceParams)
{
    ...
    if constexpr (IsSameType<pattern, Pattern::Reduce::AR>::value) {
        ...
        ASCENDC_ASSERT((dstTensor.GetSize() >= last), {
            KERNEL_LOG(KERNEL_ERROR, "dstTensor must be greater than or equal to %u, current size if %u", last,
                dstTensor.GetSize());
        });
...
#endif // AICORE_ADV_API_DETAIL_REDUCE_REDUCE_SUM_REDUCE_SUM_V220_IMPL_H