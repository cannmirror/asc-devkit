 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_PAD_BROADCAST_BROADCAST_COMMON_UTILS_H
#define AICORE_ADV_API_DETAIL_PAD_BROADCAST_BROADCAST_COMMON_UTILS_H
    ...
    const LocalTensor<T>& zeroTemp, const uint32_t firstDim, const uint32_t blockDim)
{
    ...
    for (uint32_t i = 0; i < orCounts; i++) {
        Or<uint16_t, false>(dstLocal[transTmpBufferOffset].template ReinterpretCast<uint16_t>(),
            ...
            MASK_PLACEHOLDER, repeateTimes, binaryParams);
...
#endif // AICORE_ADV_API_DETAIL_PAD_BROADCAST_BROADCAST_COMMON_UTILS_H