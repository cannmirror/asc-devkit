 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_PAD_BROADCAST_BROADCAST_V200_IMPL_H
#define AICORE_ADV_API_DETAIL_PAD_BROADCAST_BROADCAST_V200_IMPL_H
    ...
    const uint32_t dstShape[dim], const uint32_t srcShape[dim], LocalTensor<T>& tmpBuffer)
{
    ...
    if (firstDim * sizeof(T) % ONE_BLK_SIZE == 0) {
        ...
        for (uint32_t i = 0; i < rangeM; i++) {
            TwoDimBroadCastLastDimAlign200<T, isReuseSource>(dstLocal[dstLocalOffset], srcLocal[srcLocalOffset],
                zeroTemp, tmpBuffer[blockSize], oneRepeateSize, blockDim);
