 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_PAD_BROADCAST_BROADCAST_V220_IMPL_H
#define AICORE_ADV_API_DETAIL_PAD_BROADCAST_BROADCAST_V220_IMPL_H
    ...
    uint32_t oneBlockElementNum, LocalTensor<T>& brcbOneBlockTempBuffer)
{
    ...
    for (uint32_t i = 0; i < brcbCount; i++) {
        Brcb(brcbOneBlockTempBuffer[brcbOneBlockTempBufferOffset], srcLocal[brcbSrcOffset], brcbMaxRepeateTimes,
    ...
    if (tailBrcbRepeateTime != 0) {
        Brcb(brcbOneBlockTempBuffer[brcbOneBlockTempBufferOffset], srcLocal[brcbSrcOffset], tailBrcbRepeateTime,
            {1, DEFAULT_REPEAT_STRIDE});
    }
    PipeBarrier<PIPE_V>();
}
    ...
    LocalTensor<T>& tmpBuffer, const uint32_t firstDim, const uint32_t blockDim)
{
    ...
    for (uint32_t i = 0; i < CopyCounts; i++) {
        Copy<T, false>(dstLocal[dstOffset], tmpBuffer[brcbOneBlockTempBufferOffset], MASK_PLACEHOLDER, MAX_REPEAT_TIMES,
            copyRepeatParams);
    ...
    if (tailsCopyRepeateTimes != 0) {
        Copy<T, false>(dstLocal[dstOffset], tmpBuffer[brcbOneBlockTempBufferOffset], MASK_PLACEHOLDER,
            tailsCopyRepeateTimes, copyRepeatParams);
    }
    PipeBarrier<PIPE_V>();
}
    ...
    LocalTensor<T>& tmpBuffer, const uint32_t firstDim, const uint32_t blockDim)
{
    ...
    for (uint32_t i = 0; i < CopyCounts; i++) {
        Copy<T, false>(copyTempBuffer[dstOffset], tmpBuffer[brcbOneBlockTempBufferOffset], MASK_PLACEHOLDER,
            MAX_REPEAT_TIMES, copyRepeatParams);
