 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_UTILS_INIT_GLOBAL_MEMORY_INIT_GLOBAL_MEMORY_V200_IMPL_H
#define AICORE_ADV_API_DETAIL_UTILS_INIT_GLOBAL_MEMORY_INIT_GLOBAL_MEMORY_V200_IMPL_H
...
__aicore__ inline void InitGlobalMemoryImpl(GlobalTensor<T>& gmWorkspaceAddr, const uint64_t size, const T value)
{
    ...
    if (tail != 0) {
        if ((tail * sizeof(T)) % ONE_BLK_SIZE == 0) {
            ...
            comOffset = round * roundSize;
            DataCopy(gmWorkspaceAddr[comOffset], popBuffer, repeatParams);
            for (uint64_t i = comOffset + tailAlign / sizeof(T); i < size; ++i) { gmWorkspaceAddr.SetValue(i, value); }
        }
    }
    PipeBarrier<PIPE_MTE3>();
}
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_UTILS_INIT_GLOBAL_MEMORY_INIT_GLOBAL_MEMORY_V200_IMPL_H
