 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANTIZE_ANTIQUANTIZE_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANTIZE_ANTIQUANTIZE_C310_IMPL_H
...
__aicore__ inline void CastAntiQuantizeSrcToFp32(MicroAPI::RegTensor<float>& f32SrcVreg,
    MicroAPI::RegTensor<SrcT>& srcVreg, MicroAPI::RegTensor<half>& f16Vreg, MicroAPI::MaskReg& preg)
{
...
__aicore__ inline void AntiQuantizePerGroupForColCommon(const LocalTensor<dstT>& dstTensor,
...
template <typename dstT, typename SrcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantizePerGroupForRowCommonTailBlock(__local_mem__ dstT* dstUb, __local_mem__ SrcT* srcUb,
...
template <typename dstT, typename SrcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantizePerGroupForRowCommon(const LocalTensor<dstT>& dstTensor,
    const LocalTensor<SrcT>& srcTensor, const LocalTensor<scaleT>& scaleTensor, const LocalTensor<scaleT>& offsetTensor,
    const AscendAntiQuantParam& para)
{
    __local_mem__ dstT* dstUb = (__local_mem__ dstT*)dstTensor.GetPhyAddr();
...
__aicore__ inline void PerTensorProcessCommon(
    __local_mem__ DstT* dst, __local_mem__ SrcT* src, const DstT offset, const DstT scale, const uint32_t srcCalCount)
{
    MicroAPI::MaskReg preg;
...
__aicore__ inline void AntiQuantizePerTensor(const LocalTensor<DstT>& dst, const LocalTensor<SrcT>& src,
    const DstT offset, const DstT scale, const LocalTensor<uint8_t>& sharedTmpBuffer, const AntiQuantizeParams& params)
{
    ...
    __local_mem__ DstT* offset, __local_mem__ DstT* scale, const uint32_t M, const uint32_t N)
{
    ...
    for (uint16_t i = 0; i < repeat; ++i) {
        ...
        for (uint16_t j = 0; j < (uint16_t)M; ++j) {
            ...
            MicroAPI::Cast<DstT, float, LayoutZMrgZRndRSatS>(outReg, f32Vreg, preg);
            MicroAPI::DataCopy<DstT, MicroAPI::StoreDist::DIST_PACK_B32>(dst + j * N + i * sregLower, outReg, preg);
        }
    }
}

template <bool hasOffset, typename SrcT, typename DstT>
__aicore__ inline void AntiQuantPerChannelNoTranspose(const LocalTensor<DstT>& dst, const LocalTensor<SrcT>& src,
    const LocalTensor<DstT>& offset, const LocalTensor<DstT>& scale, const uint32_t M, const uint32_t N)
{
...
{
    static_assert(SupportType<DstT, half, bfloat16_t>(), "This AntiQuantize only support half/bfloat16_6 output dtype");

    AntiQuantPerChannelNoTranspose<config.hasOffset, SrcT, DstT>(
        dstTensor, srcTensor, offset, scale, params.m, params.n);
}
} // namespace AscendC
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANTIZE_ANTIQUANTIZE_C310_IMPL_H
