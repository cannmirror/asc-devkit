 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANT_ASCEND_ANTIQUANT_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANT_ASCEND_ANTIQUANT_C310_IMPL_H
...
__aicore__ inline void SelectZeroNan(MicroAPI::RegTensor<bfloat16_t>& b16vreg, MicroAPI::RegTensor<uint16_t>& bf16Zero,
    ...
    MicroAPI::RegTensor<uint16_t>& e8m0Nan, MicroAPI::MaskReg& selPreg, MicroAPI::MaskReg& preg)
{
    ...
    Select<uint16_t>(
        (MicroAPI::RegTensor<uint16_t>&)b16vreg, (MicroAPI::RegTensor<uint16_t>&)b16vreg, bf16Zero, selPreg);
    Compare<uint16_t, CMPMODE::NE>(selPreg, (MicroAPI::RegTensor<uint16_t>&)b16vreg, e8m0Nan, preg);
    Select<uint16_t>(
        (MicroAPI::RegTensor<uint16_t>&)b16vreg, (MicroAPI::RegTensor<uint16_t>&)b16vreg, bf16Nan, selPreg);
}

template <typename OutputDataType>
__aicore__ inline void CastScale(
    __local_mem__ OutputDataType* dst, __local_mem__ fp8_e8m0_t* scale, const uint32_t srcCalCount)
{
    ...
    MicroAPI::RegTensor<uint16_t>& e8m0Nan)
{
    ...
    for (uint16_t i = 0; i < (uint16_t)repeatTimes; ++i) {
        ...
        MicroAPI::DataCopy<uint8_t, MicroAPI::LoadDist::DIST_UNPACK4_B8>(
            (MicroAPI::RegTensor<uint8_t>&)fp4vreg0, (__local_mem__ uint8_t*)srcUb + (i * sregLower / HALF_FACTOR));
    ...
    __local_mem__ OutputDataType* scale, const uint32_t srcCalCount, const uint16_t newRepeatTime)
{
    ...
    MicroAPI::RegTensor<SrcType> fp4vreg0;
    MicroAPI::RegTensor<half> halfvreg2;
    uint32_t sreg1 = (uint32_t)srcCalCount;
    for (uint16_t i = 0; i < (uint16_t)newRepeatTime; i++) {
        ...
        if constexpr (SupportType<OutputDataType, half>()) {
            ...
            MicroAPI::DataCopy<bfloat16_t, MicroAPI::StoreDist::DIST_NORM_B16>(
                dst + (2 * i) * sregLower, b16vreg2, preg1);
        }
        ...
        if constexpr (SupportType<OutputDataType, half>()) {
            ...
            MicroAPI::DataCopy<half, MicroAPI::StoreDist::DIST_NORM_B16>(
                dst + (2 * i + 1) * sregLower, halfvreg2, preg1);
        } else {
            MicroAPI::Mul<bfloat16_t>(b16vreg2, scaleReg2, b16vreg2, preg1);
            MicroAPI::DataCopy<bfloat16_t, MicroAPI::StoreDist::DIST_NORM_B16>(
                dst + (2 * i + 1) * sregLower, b16vreg2, preg1);
        }
    }
}
    ...
    for (uint16_t i = 0; i < repeatTimes; i++) {
        for (uint16_t j = 0; j < ANTIQUANT_FP4_PERGROUP_SIZE; j++) {
            ...
            AntiQuantProcessByLine<SrcType, OutputDataType>(
                dstUb, srcUb, scaleUb, N, bf16Zero, bf16Nan, e8m0Zero, e8m0Nan);
        }
    }
}
    ...
    const uint32_t N)
{
    ...
    VF_CALL<AntiQuantProcessByNum<SrcType, OutputDataType>>(dstUb, srcUb, tmpbufferUb, srcCalCount, newRepeatTime);
}

template <typename SrcType, typename OutputDataType, bool isTranspose>
__aicore__ inline void AscendAntiQuantImpl(const LocalTensor<OutputDataType>& dst, const LocalTensor<SrcType>& src,
    const LocalTensor<fp8_e8m0_t>& scale, const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t K,
    ...
    const OutputDataType offset, const OutputDataType scale, const uint32_t srcCalCount)
{
    ...
    for (uint16_t i = 0; i < (uint16_t)repeat; ++i) {
        ...
        if constexpr (SupportType<OutputDataType, bfloat16_t>()) {
            ...
            MicroAPI::Adds<float, float, MicroAPI::MaskMergeMode::ZEROING>(f32vreg, f32vreg, float(offset), preg);
            MicroAPI::Muls<float, float, MicroAPI::MaskMergeMode::ZEROING>(f32vreg, f32vreg, float(scale), preg);
        }
...
__aicore__ inline void PerTensorProcessForB8(__local_mem__ half* dst, __local_mem__ SrcType* src, const half offset,
    const half scale, const uint32_t srcCalCount)
{
    __VEC_SCOPE__
    {
        MicroAPI::MaskReg preg;
...
template <typename SrcType>
__aicore__ inline void PerTensorProcessForB8(__local_mem__ bfloat16_t* dst, __local_mem__ SrcType* src,
    const bfloat16_t offset, const bfloat16_t scale, const uint32_t srcCalCount)
{
    __VEC_SCOPE__
    {
        ...
        uint32_t f16sreg = (uint32_t)ANTIQUANT_B16_VF_LEN;
    ...
    __local_mem__ OutputDataType* offset, __local_mem__ OutputDataType* scale, const uint32_t K, const uint32_t N)
{
    ...
    for (uint16_t i = 0; i < repeat; ++i) {
        ...
        for (uint16_t j = 0; j < (uint16_t)K; ++j) {
            ...
            MicroAPI::Cast<OutputDataType, float, LayoutZMrgZRndRSatS>(outReg, f32vreg, preg);
            MicroAPI::DataCopy<OutputDataType, MicroAPI::StoreDist::DIST_PACK_B32>(
                dst + j * N + i * sregLower, outReg, preg);
        }
    }
}
    ...
    __local_mem__ OutputDataType* offset, __local_mem__ OutputDataType* scale, const uint32_t srcCalCount)
{
    ...
    uint32_t scaleLen = (uint32_t)ANTIQUANT_B32_VF_LEN;
...
template <typename SrcType>
__aicore__ inline void PerchannelTransposeForB8(__local_mem__ half* dst, __local_mem__ SrcType* src,
...
template <typename SrcType>
__aicore__ inline void PerchannelTransposeForB8(__local_mem__ bfloat16_t* dst, __local_mem__ SrcType* src,
        ...
        for (uint16_t i = 0; i < (uint16_t)N; i++) {
            ...
            for (uint16_t j = 0; j < repeat; ++j) { // process single line
                ...
                MicroAPI::DataCopy<bfloat16_t, MicroAPI::StoreDist::DIST_PACK_B32>(
                    dst + i * K + j * sregLower, b16vreg, preg);
            }
        }
    }
}
...
template <typename SrcType, typename OutputDataType>
__aicore__ inline void PerchannelTransposeForFp8(__local_mem__ OutputDataType* dst, __local_mem__ SrcType* src,
    __local_mem__ OutputDataType* offset, __local_mem__ OutputDataType* scale, const uint32_t K, const uint32_t N)
{
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < (uint16_t)N; i++) {
            ...
            for (uint16_t j = 0; j < (uint16_t)repeat; ++j) {
                preg = MicroAPI::UpdateMask<uint32_t>(sreg);
                MicroAPI::DataCopy<SrcType, MicroAPI::LoadDist::DIST_UNPACK4_B8>(vreg, src + i * K + j * sregLower);
                MicroAPI::Cast<float, SrcType, layoutZMrgZ>(f32vreg, vreg, preg);
                ...
                MicroAPI::Cast<OutputDataType, float, LayoutZMrgZRndRSatS>(outReg, f32vreg, preg);
                MicroAPI::DataCopy<OutputDataType, MicroAPI::StoreDist::DIST_PACK_B32>(
                    dst + i * K + j * sregLower, outReg, preg);
            }
        }
    }
}
...
__aicore__ inline void LoadPerTokenScaleAndOffset(__local_mem__ scaleT* scaleUb, __local_mem__ scaleT* offsetUb,
...
template <typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void LoadPerTokenTransposeScaleAndOffset(__local_mem__ scaleT* scaleUb,
    __local_mem__ scaleT* offsetUb, MicroAPI::RegTensor<scaleT>& scaleVreg, MicroAPI::RegTensor<scaleT>& offsetVreg)
{
    if constexpr (SupportType<scaleT, half, bfloat16_t>()) {
...
__aicore__ inline void GetPerGroupScaleAndOffset(__local_mem__ T* scaleUb, __local_mem__ T* offsetUb,
    const int32_t start, const AscendAntiQuantParam& para, MicroAPI::RegTensor<T>& scaleReg,
...
__aicore__ inline void StoreF32Res(
    __local_mem__ dstT* dstAddr, MicroAPI::RegTensor<float>& vreg, MicroAPI::MaskReg& preg)
{
...
__aicore__ inline void LoadSrc(__local_mem__ srcT* srcAddr, MicroAPI::RegTensor<srcT>& srcVreg)
{
    if constexpr (SupportType<scaleT, half, bfloat16_t>()) {
...
template <typename scaleT, typename srcT>
__aicore__ inline void ConvertSrc(
...
template <typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AddOffsetIfExist(
...
__aicore__ inline void GenZeroVreg(MicroAPI::RegTensor<scaleT>& vreg)
{
    if constexpr (SupportType<scaleT, half, bfloat16_t>()) {
...
template <typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void ConvertToF32ScaleAndOffset(MicroAPI::RegTensor<scaleT>& scaleVreg,
...
template <typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void LoadNormScaleAndOffset(__local_mem__ scaleT* scaleAddr, __local_mem__ scaleT* offsetAddr,
    MicroAPI::RegTensor<scaleT>& scaleVreg, MicroAPI::RegTensor<scaleT>& offsetVreg)
{
...
template <typename dstT, typename srcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantPerTokenForFp8(const LocalTensor<dstT>& dstTensor, const LocalTensor<srcT>& srcTensor,
    const LocalTensor<scaleT>& scaleTensor, const LocalTensor<scaleT>& offsetTensor, const AscendAntiQuantParam& para)
{
    __local_mem__ dstT* dstUb = (__local_mem__ dstT*)dstTensor.GetPhyAddr();
    __local_mem__ srcT* srcUb = (__local_mem__ srcT*)srcTensor.GetPhyAddr();
...
__aicore__ inline void ReplaceBf16VmulsWithVmul(
    MicroAPI::RegTensor<bfloat16_t>& vreg, const bfloat16_t scale, MicroAPI::MaskReg& preg)
{
...
template <typename dstT, typename srcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantPerGroupForColFp4(const LocalTensor<dstT>& dstTensor,
    const LocalTensor<srcT>& srcTensor, const LocalTensor<scaleT>& scaleTensor, const AscendAntiQuantParam& para)
{
    __local_mem__ dstT* dstUb = (__local_mem__ dstT*)dstTensor.GetPhyAddr();
    __local_mem__ srcT* srcUb = (__local_mem__ srcT*)srcTensor.GetPhyAddr();
    __local_mem__ scaleT* scaleUb = (__local_mem__ scaleT*)scaleTensor.GetPhyAddr();
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < rowNum; ++i) {
            ...
            for (uint16_t j = 0; j < repeat; ++j) {
                ...
                MicroAPI::DataCopy<uint8_t, MicroAPI::LoadDist::DIST_UNPACK4_B8>((MicroAPI::RegTensor<uint8_t>&)srcVreg,
...
__aicore__ inline void AntiQuantPerGroupForRowFp4OneRow(__local_mem__ dstT* dstAddr, __local_mem__ srcT* srcAddr,
...
template <typename dstT, typename srcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantPerGroupForRowFp4(const LocalTensor<dstT>& dstTensor,
    const LocalTensor<srcT>& srcTensor, const LocalTensor<scaleT>& scaleTensor, const AscendAntiQuantParam& para)
{
    __local_mem__ dstT* dstUb = (__local_mem__ dstT*)dstTensor.GetPhyAddr();
    __local_mem__ srcT* srcUb = (__local_mem__ srcT*)srcTensor.GetPhyAddr();
        ...
        for (uint16_t i0 = 0; i0 < mainRowGroup; ++i0) {
            for (uint16_t i1 = 0; i1 < static_cast<uint16_t>(para.groupSize); ++i1) {
                AntiQuantPerGroupForRowFp4OneRow<dstT, srcT, scaleT>(dstUb + (i0 * para.groupSize + i1) * para.n,
                    srcUb + ((i0 * para.groupSize + i1) * para.n) / 2, scaleUb + i0 * para.n, dstVreg, srcVreg,
                    scaleVreg, dstSrcVreg, dstScaleVreg, preg, repeat, para.n, vecLen);
            }
        }
...
template <typename dstT, typename srcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantPerGroupForColFp8(const LocalTensor<dstT>& dstTensor,
    const LocalTensor<srcT>& srcTensor, const LocalTensor<scaleT>& scaleTensor, const LocalTensor<scaleT>& offsetTensor,
    const AscendAntiQuantParam& para)
{
    __local_mem__ dstT* dstUb = (__local_mem__ dstT*)dstTensor.GetPhyAddr();
                ...
                if constexpr (config.hasOffset) {
                    if constexpr (SupportType<scaleT, float>()) {
                        MicroAPI::Add<float, MicroAPI::MaskMergeMode::ZEROING>(
                            f32SrcVreg, f32SrcVreg, offsetVreg, preg);
                    } else {
                        MicroAPI::Add<float, MicroAPI::MaskMergeMode::ZEROING>(
                            f32SrcVreg, f32SrcVreg, f32OffsetVreg, preg);
                    }
                }
                if constexpr (SupportType<scaleT, float>()) {
...
template <typename dstT, typename srcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantPerGroupForRowB8TailBlock(__local_mem__ dstT* dstUb, __local_mem__ srcT* srcUb,
...
template <typename dstT, typename srcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantPerGroupForRowFp8TailBlock(__local_mem__ dstT* dstUb, __local_mem__ srcT* srcUb,
    __local_mem__ scaleT* scaleUb, __local_mem__ scaleT* offsetUb, uint16_t repeat, uint16_t tailRow, uint32_t n,
    uint32_t vecLen)
{
    MicroAPI::MaskReg preg;
    ...
    for (uint16_t i = 0; i < tailRow; ++i) {
        ...
        for (uint16_t j = 0; j < repeat; ++j) {
            LoadNormScaleAndOffset<scaleT, config>(
                (scaleUb + j * vecLen), (offsetUb + j * vecLen), scaleVreg, offsetVreg);
            preg = MicroAPI::UpdateMask<uint32_t>(sreg);
            MicroAPI::DataCopy<srcT, MicroAPI::LoadDist::DIST_UNPACK4_B8>(srcVreg, srcUb + i * n + j * vecLen);
            MicroAPI::Cast<float, srcT, layoutZMrgZ>(f32SrcVreg, srcVreg, preg);
            if constexpr (SupportType<scaleT, float>()) {
...
template <typename dstT, typename srcT, typename scaleT, const AscendAntiQuantConfig& config>
__aicore__ inline void AntiQuantPerGroupForRowFp8(const LocalTensor<dstT>& dstTensor,
    const LocalTensor<srcT>& srcTensor, const LocalTensor<scaleT>& scaleTensor, const LocalTensor<scaleT>& offsetTensor,
    const AscendAntiQuantParam& para)
{
    __local_mem__ dstT* dstUb = (__local_mem__ dstT*)dstTensor.GetPhyAddr();
    ...
    __VEC_SCOPE__
    {
        ...
        AntiQuantPerGroupForRowFp8TailBlock<dstT, srcT, scaleT, config>(dstUb + mainRowGroup * para.groupSize * para.n,
            srcUb + mainRowGroup * para.groupSize * para.n, scaleUb + mainRowGroup * para.n,
            offsetUb + mainRowGroup * para.n, repeat, tailRow, para.n, vecLen);
    }
}
    ...
    const LocalTensor<scaleT>& offsetTensor, const AscendAntiQuantParam& para)
{
    ...
    if constexpr (policy == AscendAntiQuantPolicy::PER_TOKEN) {
        ...
        static_assert(SupportType<srcT, int8_t, fp8_e4m3fn_t, fp8_e5m2_t, hifloat8_t, fp4x2_e1m2_t, fp4x2_e2m1_t>(),
...
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANT_ASCEND_ANTIQUANT_C310_IMPL_H
