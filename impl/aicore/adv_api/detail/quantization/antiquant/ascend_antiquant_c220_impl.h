 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANT_ASCEND_ANTIQUANT_C220_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANT_ASCEND_ANTIQUANT_C220_IMPL_H
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const uint32_t K, const uint32_t N)
{
    ...
    for (uint32_t i = 0; i < N / ANTIQUANT_SINGLE_N_SIZE_BF16; i++) {
        ...
        PipeBarrier<PIPE_V>();
        // 4.cast back to bf16
        Cast<bfloat16_t, float, false>(dst[ANTIQUANT_SINGLE_N_SIZE_BF16 * i], resultBuffer, RoundMode::CAST_RINT,
            MASK_PLACEHOLDER, K, f322f16Params);
        PipeBarrier<PIPE_V>();
    }
    SetMaskNorm();
    ResetMask();
...
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_ANTIQUANT_ASCEND_ANTIQUANT_C220_IMPL_H