 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_QUANT_ASCEND_QUANT_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_QUANT_ASCEND_QUANT_COMMON_IMPL_H
...
__aicore__ inline void AscendQuantImpl(const LocalTensor<int8_t>& dstTensor, const LocalTensor<T>& srcTensor,
    const float scale, const float offset, uint32_t calCount)
{
    ...
    const uint32_t offsetCount, const uint32_t calCount)
{
    ...
    bool ans = PopStackBuffer<uint8_t, TPosition::LCM>(stackTensor);
    ASCENDC_ASSERT((ans), { KERNEL_LOG(KERNEL_ERROR, "PopStackBuffer Error!"); });
    AscendQuantImpl<T, isReuseSource>(
    ...
    const uint32_t scaleCount, const uint32_t offsetCount, const uint32_t calCount)
{
    ...
    ASCENDC_ASSERT((scaleCount <= scaleTensor.GetSize()), {
        KERNEL_LOG(KERNEL_ERROR, "scaleCount is %u, which should not larger than scaleTensor size %u.", scaleCount,
            scaleTensor.GetSize());
    });
    ASCENDC_ASSERT((offsetCount <= offsetTensor.GetSize()), {
        KERNEL_LOG(KERNEL_ERROR, "offsetCount is %u, which should not larger than offsetTensor size %u.", offsetCount,
            offsetTensor.GetSize());
    });
    ASCENDC_ASSERT((scaleCount == offsetCount && scaleCount > 0), {
        KERNEL_LOG(KERNEL_ERROR, "scaleCount is %u, which should be equal to offsetCount %u and not zero.", calCount,
            srcTensor.GetSize());
    });
...
#endif
template <typename dstT, typename srcT, typename scaleT, bool isReuseSource, const AscendQuantConfig& config,
    ...
    const uint32_t scaleCount, const uint32_t offsetCount, const uint32_t calCount)
{
    ...
    ASCENDC_ASSERT((scaleCount == offsetCount && scaleCount > 0), {
        KERNEL_LOG(KERNEL_ERROR, "scaleCount is %u, which should be equal to offsetCount %u and not zero.", scaleCount,
            offsetCount);
    });
    ASCENDC_ASSERT((calCount % scaleCount == 0 && calCount > 0), {
        KERNEL_LOG(KERNEL_ERROR, "calCount is %u, which should be integral multiple of scaleCount %u and not zero.",
...
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_QUANT_ASCEND_QUANT_COMMON_IMPL_H
