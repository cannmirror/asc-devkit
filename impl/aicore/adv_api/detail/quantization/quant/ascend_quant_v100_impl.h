 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_QUANT_ASCEND_QUANT_V100_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_QUANT_ASCEND_QUANT_V100_IMPL_H
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, const float scale, const float offset, const uint32_t calCount)
{
    ...
    for (uint32_t i = 0; i < loopCount; ++i) {
        NormCompute(dstTensor[splitSize * i], srcTensor[splitSize * i], sharedTmpBuffer.ReinterpretCast<half>(),
...
{
    ASCENDC_ASSERT(false, { KERNEL_LOG(KERNEL_ERROR, "This device do not support per channel quant!"); });
}
template <typename T, bool isReuseSource = false, const AscendQuantConfig& config = ASCEND_QUANT_DEFAULT_CFG>
__aicore__ inline void AscendQuantImpl(const LocalTensor<int8_t>& dstTensor, const LocalTensor<T>& srcTensor,
...
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_QUANT_ASCEND_QUANT_V100_IMPL_H
