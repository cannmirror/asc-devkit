 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_DEQUANT_ASCEND_DEQUANT_COMMON_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_DEQUANT_ASCEND_DEQUANT_COMMON_IMPL_H
...
__aicore__ inline bool IsCalCountValid(const LocalTensor<int32_t>& srcTensor, uint32_t calCount)
{
    ...
    ASCENDC_ASSERT((calCount > 0 && calCount <= srcTensor.GetSize()), {
        KERNEL_LOG(KERNEL_ERROR, "calCount is %u, which should be in range (0, srcTensor.GetSize() %u]", calCount,
            srcTensor.GetSize());
    });
    return true;
}
    ...
    const LocalTensor<int32_t>& srcTensor, const LocalTensor<dstT>& dstTensor, DequantParams& params)
{
    ...
    ASCENDC_ASSERT(params.m * params.n <= srcTensor.GetSize(), {
        KERNEL_LOG(KERNEL_ERROR, "params.m %u * params.n %u \
    ...
    ASCENDC_ASSERT((params.calCount > 0 && params.calCount <= params.n), {
        KERNEL_LOG(KERNEL_ERROR, "params.calCount is %u, which should be in range (0, params.n %u]", params.calCount,
            params.n);
    });
...
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_DEQUANT_ASCEND_DEQUANT_COMMON_IMPL_H