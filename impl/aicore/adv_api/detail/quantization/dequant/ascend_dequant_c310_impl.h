 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_DEQUANT_ASCEND_DEQUANT_C310_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_DEQUANT_ASCEND_DEQUANT_C310_IMPL_H
...
__aicore__ inline void LoadPerTokenScale(__local_mem__ scaleT* addr, MicroAPI::RegTensor<scaleT>& vreg)
{
...
__aicore__ inline void StoreRes(__local_mem__ dstT* dstAddr, MicroAPI::RegTensor<float>& vreg, MicroAPI::MaskReg& preg)
{
...
template <typename T>
__aicore__ inline void GetPerGroupScale(__local_mem__ T* scaleUb, const int32_t start, const AscendDeQuantParam& para,
...
template <typename dstT, typename srcT, typename scaleT, const AscendDeQuantConfig& config>
__aicore__ inline void DeQuantPerGroupForRowTailBlock(__local_mem__ dstT* dstUb, __local_mem__ srcT* srcUb,
    __local_mem__ scaleT* scaleUb, uint16_t repeat, uint16_t tailRow, uint32_t n, uint32_t vecLen)
{
    MicroAPI::MaskReg preg;
    MicroAPI::RegTensor<scaleT> scaleVreg;
    ...
    const LocalTensor<scaleT>& scaleTensor, const LocalTensor<scaleT>& offsetTensor, const AscendDeQuantParam& para)
{
    ...
    __VEC_SCOPE__
    {
        ...
        DeQuantPerGroupForRowTailBlock<dstT, srcT, scaleT, config>(dstUb + mainRowGroup * para.groupSize * para.n,
            srcUb + mainRowGroup * para.groupSize * para.n, scaleUb + mainRowGroup * para.n, repeat, tailRow, para.n,
            vecLen);
    }
}
...
template <typename dstT, typename srcT, typename scaleT, const AscendDeQuantConfig& config>
__aicore__ inline void AscendDeQuantPerGroupForCol(const LocalTensor<dstT>& dstTensor,
...
template <typename dstT, typename srcT, typename scaleT, const AscendDeQuantConfig& config>
__aicore__ inline void AscendDeQuantPerGroupForRow(const LocalTensor<dstT>& dstTensor,
    const LocalTensor<srcT>& srcTensor, const LocalTensor<uint8_t>& sharedTmpBuffer,
    const LocalTensor<scaleT>& scaleTensor, const LocalTensor<scaleT>& offsetTensor, const AscendDeQuantParam& para)
{
...
template <typename dstT, typename srcT, typename scaleT, const AscendDeQuantConfig& config,
    const AscendDeQuantPolicy& policy>
...
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_DEQUANT_ASCEND_DEQUANT_C310_IMPL_H
