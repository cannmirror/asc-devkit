 ...
 * \brief
 */
#ifndef AICORE_ADV_API_DETAIL_QUANTIZATION_QUANTIZE_QUANTIZE_IMPL_H
#define AICORE_ADV_API_DETAIL_QUANTIZATION_QUANTIZE_QUANTIZE_IMPL_H
#include "kernel_tensor.h"
#include "kernel_tiling/kernel_tiling.h"
#include "quantization/quantize_utils.h"
...
{
    static_assert(
        SupportType<Tuple<SrcT, ScaleT, DstT>, Tuple<half, half, fp8_e4m3fn_t>, Tuple<half, half, fp8_e5m2_t>,
...
__aicore__ inline void StoreRes(__local_mem__ DstT* dstAddr, MicroAPI::RegTensor<DstT>& vreg, MicroAPI::MaskReg& mask)
{
    ...
    if constexpr (SupportType<ScaleT, bfloat16_t>()) {
        ...
        if constexpr (castTrait.roundMode == RoundMode::CAST_RINT || castTrait.roundMode == RoundMode::CAST_ROUND
            ...
            MicroAPI::Cast<int16_t, float, LayoutZMrgZRndRSatS>(
                (MicroAPI::RegTensor<int16_t>&)f32Vreg2, f32Vreg2, mask2);
        }
...
template <typename T>
__aicore__ inline void GetPerGroupScale(
    __local_mem__ T* scaleUb, const int32_t start, const uint32_t groupSize, MicroAPI::RegTensor<T>& scaleVreg)
{
...
template <typename T>
__aicore__ inline void GetPerGroupScaleToFloat(__local_mem__ T* scaleAddr, const int32_t start,
    const uint32_t groupSize, MicroAPI::RegTensor<float>& floatVreg, MicroAPI::RegTensor<T>& tempVreg,
...
template <typename T>
__aicore__ inline void DuplicateScalarToFloatVector(
    MicroAPI::RegTensor<float>& floatVreg, const T& scalar, MicroAPI::RegTensor<T>& tempVreg, MicroAPI::MaskReg& mask)
{
    if constexpr (SupportType<T, half, bfloat16_t>()) {
...
__aicore__ inline void CastFp32DstToExpect(
    MicroAPI::RegTensor<float>& srcVreg, MicroAPI::RegTensor<DstT>& dstVreg, MicroAPI::MaskReg& mask)
{
...
template <typename DstT, typename SrcT, typename ActualScaleT, bool hasOffset, bool isScalarOffset,
    const MicroAPI::CastTrait& castTrait>
__aicore__ inline void QuantizePerGroupForKRowCommonOneRow(__local_mem__ DstT* dstAddr, __local_mem__ SrcT* srcAddr,
    __local_mem__ ActualScaleT* scaleAddr, __local_mem__ ActualScaleT* offsetAddr, MicroAPI::RegTensor<DstT>& dstVreg,
    MicroAPI::RegTensor<float>& srcVreg, MicroAPI::RegTensor<float>& scaleVreg, MicroAPI::RegTensor<float>& offsetVreg,
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i0 = 0; i0 < mainRowGroup; ++i0) {
            for (uint16_t i1 = 0; i1 < static_cast<uint16_t>(params.groupSize); ++i1) {
                if constexpr (isScalarOffset) {
                    QuantizePerGroupForKRowCommonOneRow<DstT, SrcT, ActualScaleT, config.hasOffset, isScalarOffset,
                        castTrait>(dstUb + (i0 * params.groupSize + i1) * params.n,
                        srcUb + (i0 * params.groupSize + i1) * params.n, scaleUb + i0 * params.n, nullptr, dstVreg,
                        srcVreg, scaleVreg, offsetVreg, repeat, params.n, vecLen);
                } else {
                    QuantizePerGroupForKRowCommonOneRow<DstT, SrcT, ActualScaleT, config.hasOffset, isScalarOffset,
                        castTrait>(dstUb + (i0 * params.groupSize + i1) * params.n,
                        srcUb + (i0 * params.groupSize + i1) * params.n, scaleUb + i0 * params.n,
                        offsetUb + i0 * params.n, dstVreg, srcVreg, scaleVreg, offsetVreg, repeat, params.n, vecLen);
                }
            }
        }
        for (uint16_t i = 0; i < tailRow; ++i) {
            if constexpr (isScalarOffset) {
                QuantizePerGroupForKRowCommonOneRow<DstT, SrcT, ActualScaleT, config.hasOffset, isScalarOffset,
                    castTrait>(dstUb + (mainRowGroup * params.groupSize + i) * params.n,
                    srcUb + (mainRowGroup * params.groupSize + i) * params.n, scaleUb + mainRowGroup * params.n,
                    nullptr, dstVreg, srcVreg, scaleVreg, offsetVreg, repeat, params.n, vecLen);
            } else {
                QuantizePerGroupForKRowCommonOneRow<DstT, SrcT, ActualScaleT, config.hasOffset, isScalarOffset,
                    castTrait>(dstUb + (mainRowGroup * params.groupSize + i) * params.n,
                    srcUb + (mainRowGroup * params.groupSize + i) * params.n, scaleUb + mainRowGroup * params.n,
                    offsetUb + mainRowGroup * params.n, dstVreg, srcVreg, scaleVreg, offsetVreg, repeat, params.n,
                    vecLen);
            }
        }
    }
...
__aicore__ inline void QuantizePerGroupForKRowFp4OneRow(__local_mem__ DstT* dstAddr, __local_mem__ SrcT* srcAddr,
    __local_mem__ ActualScaleT* scaleAddr, MicroAPI::RegTensor<DstT>& dstVreg, MicroAPI::RegTensor<float>& srcVreg,
    MicroAPI::RegTensor<float>& scaleVreg, uint16_t repeat, uint32_t n, uint32_t vecLen)
{
    MicroAPI::MaskReg mask;
    MicroAPI::RegTensor<SrcT> tempVreg;
    MicroAPI::RegTensor<ActualScaleT> tempScaleVreg;
        ...
        for (uint16_t i0 = 0; i0 < mainRowGroup; ++i0) {
            for (uint16_t i1 = 0; i1 < static_cast<uint16_t>(params.groupSize); ++i1) {
                QuantizePerGroupForKRowFp4OneRow<DstT, SrcT, ActualScaleT, castTrait>(
                    ...
                    srcUb + (i0 * params.groupSize + i1) * params.n, scaleUb + i0 * params.n, dstVreg, srcVreg,
                    scaleVreg, repeat, params.n, vecLen);
            }
        }
        for (uint16_t i = 0; i < tailRow; ++i) {
            QuantizePerGroupForKRowFp4OneRow<DstT, SrcT, ActualScaleT, castTrait>(
        ...
        for (uint16_t i = 0; i < static_cast<uint16_t>(params.m); ++i) {
            ...
            for (uint16_t j = 0; j < repeat; ++j) {
                ...
                if constexpr (SupportType<ActualScaleT, half, bfloat16_t>()) {
                    ...
                    if constexpr (config.hasOffset && !isScalarOffset) {
                        MicroAPI::DataCopy<ActualScaleT, MicroAPI::LoadDist::DIST_NORM>(
                            offsetVreg, offsetUb + j * vecLen);
                    }
                }
...
#endif // AICORE_ADV_API_DETAIL_QUANTIZATION_QUANTIZE_QUANTIZE_IMPL_H
