...
bool Conv3dBpFilterTilingBase::CheckLoad3DLimits()
{
    if (static_cast<uint32_t>(this->attrInfo.strideH) > LOAD3D_MAX_STRIDE_H_W
        ...
        TILING_LOG_ERROR("Attrs not satisfying load3d's limits: strideH=%ld, strideW=%ld, which must <= %u.",
    ...
    if (static_cast<uint32_t>(this->attrInfo.dilationH) > LOAD3D_MAX_DILATION_H_W
        ...
        TILING_LOG_ERROR("Attrs not satisfying load3d's limits: dilationH=%ld, dilationW=%ld, which must <= %u.",
    ...
    if (padLoad3dInvalid) {
        TILING_LOG_ERROR("Attrs not satisfying load3d's limits: padUp=%ld, padDown=%ld, padLeft=%ld, padRight=%ld,"
                         "which must <= %u.",
            this->attrInfo.padUp, this->attrInfo.padDown, this->attrInfo.padLeft, this->attrInfo.padRight,
            LOAD3D_MAX_PAD);
        return false;
