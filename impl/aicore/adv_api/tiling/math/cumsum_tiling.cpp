    ...
    const bool isReuseSource, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    ASCENDC_HOST_ASSERT(((srcShape.GetDim(1) > 0) && ((srcShape.GetDim(1) * typeSize) % oneBlockSize == 0)), return,
    ...
    if (!isLastAxis) {
        ...
        minValue = minCastTempBuffer
                   + transDataTo5HDAddrListSize * srcShape.GetDim(1) * typeSize
