...
struct PowTiling {
    ...
    constexpr PowTiling(uint32_t halfTTIn, uint32_t intTTIn, uint32_t floatTTIn, uint32_t halfTSIn, uint32_t intTSIn,
        uint32_t floatTSIn) :
        halfTT(halfTTIn),
        intTT(intTTIn), floatTT(floatTTIn), halfTS(halfTSIn), intTS(intTSIn), floatTS(floatTSIn)
    {}
};
...
constexpr PowTiling POW_TILING_PARAM_V200(POW_TENSOR_TENSOR_HALF_CALC_PROC_V200, POW_TENSOR_TENSOR_INT_CALC_PROC_V200,
    POW_TENSOR_TENSOR_FLOAT_CALC_PROC_V200, POW_TENSOR_SCALAR_HALF_CALC_PROC_V200, POW_TENSOR_SCALAR_INT_CALC_PROC_V200,
...
inline uint32_t GetPowerMaxTmpSizeC310(
    const ge::Shape& srcShape1, const ge::Shape& srcShape2, const bool typeIsInt, const bool isReuseSource)
{
    ...
    if (!typeIsInt) {
        ...
        if (src1IsTensor) {
            for (const auto dim : shapeDims1) { inputSize *= dim; }
        } else if (src2IsTensor) {
            for (const auto dim : shapeDims2) { inputSize *= dim; }
        }
...
inline uint32_t GetPowerMinTmpSize(const platform_ascendc::SocVersion& soc, const ge::Shape& srcShape1,
    const ge::Shape& srcShape2, const bool typeIsInt, const uint32_t typeSize, const bool isReuseSource)
{
    (void)(isReuseSource);
...
*/
void GetPowerTmpBufferFactorSize(const platform_ascendc::SocVersion& soc, const bool baseIsTensor,
    const bool expIsTensor, const bool typeIsInt, const uint32_t typeSize, uint32_t& maxLiveNodeCount,
...
void GetPowerTmpBufferFactorSizeC310(
...
{
    ASCENDC_HOST_ASSERT(platform_ascendc::PlatformAscendCManager::GetInstance() != nullptr, return,
    ...
    const uint32_t typeSize, uint32_t& maxLiveNodeCount, uint32_t& extraBuffer)
{
    ...
    if (socVersion == platform_ascendc::SocVersion::ASCEND910_95
        ...
        GetPowerTmpBufferFactorSize(
            socVersion, baseIsTensor, expIsTensor, typeIsInt, typeSize, maxLiveNodeCount, extraBuffer);
    }
}

void GetPowerMaxMinTmpSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const PowerConfig& config,
    ...
    const bool isReuseSource, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    if (socVersion == platform_ascendc::SocVersion::ASCEND910_95
        || socVersion == platform_ascendc::SocVersion::ASCEND910_55) {
        if (config.algo == PowerAlgo::INTRINSIC) {
            maxValue = 0u;
            minValue = 0u;
        } else if (config.algo == PowerAlgo::DOUBLE_FLOAT_TECH) {
            maxValue = GetPowerMaxTmpSizeC310(srcShape1, srcShape2, typeIsInt, isReuseSource);
            minValue = maxValue;
        }
    } else {
...
void GetPowerTmpBufferFactorSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const PowerConfig& config,
