...
#include "activation/softmax_tilingdata.h"
...
inline std::vector<uint32_t> GetLastAxisShapeND(const ge::Shape& srcShape)
{
    ...
    for (uint32_t i = 0; i < shapeDims.size(); i++) { calculateSize *= shapeDims[i]; }
...
inline void AdjustToBasicBlockBaseM(uint32_t& baseM, const uint32_t srcM, const uint32_t srcK)
{
    if (baseM > BASIC_TILE_NUM && srcM % BASIC_TILE_NUM == 0 && srcK % SOFTMAX_BASICBLOCK_UNIT == 0) { // basicblock
        ...
        while (srcM % baseM != 0) { baseM -= BASIC_TILE_NUM; }
...
uint32_t GetSoftMaxMaxTmpSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const ge::Shape& srcShape,
    const uint32_t dataTypeSize, UNUSED const bool isReuseSource)
{
    const uint32_t inputSize = srcShape.GetShapeSize();
    ASCENDC_HOST_ASSERT(inputSize > 0, return 0, "input srcShape size must be greater than 0.");
    ASCENDC_HOST_ASSERT(
    ...
    if (soc == platform_ascendc::SocVersion::ASCEND910_95 || soc == platform_ascendc::SocVersion::ASCEND910_55) {
        ...
        uint32_t needSize = std::max(needSize1, needSize2);
        return needSize * SOFTMAX_FLOAT_SIZE;
    } else {
        return GetSoftMaxMaxTmpSize(srcShape, dataTypeSize, isReuseSource);
    }
}
...
uint32_t GetSoftMaxMinTmpSize(const platform_ascendc::PlatformAscendC& ascendcPlatform, const ge::Shape& srcShape,
    const uint32_t dataTypeSize, UNUSED const bool isReuseSource)
{
    const uint32_t inputSize = srcShape.GetShapeSize();
    ASCENDC_HOST_ASSERT(inputSize > 0, return 0, "input srcShape size must be greater than 0.");
    ...
    if (soc == platform_ascendc::SocVersion::ASCEND910_95 || soc == platform_ascendc::SocVersion::ASCEND910_55) {
        ...
        return GetSoftMaxMinTmpSize(srcShape, dataTypeSize, isReuseSource);
    }
}
    ...
    const ge::Shape& srcShape, const uint32_t dataTypeSize, const bool isUpdate, UNUSED const bool isReuseSource)
{
    ...
    uint32_t needSize = !isUpdate ? elementNumPerBlk + srcK + SOFTMAX_BASICBLOCK_UNIT :
    ...
    const ge::Shape& srcShape, const uint32_t dataTypeSize, const bool isFront, UNUSED const bool isReuseSource)
{
    ...
    if (dataTypeSize == SOFTMAX_HALF_SIZE) {
        needSize = srcM
                   * (elementNumPerBlk * SOFTMAX_TMPBUFFER_COUNT + srcK * SOFTMAXGRAD_TMPBUFFER_COUNT
...
uint32_t GetSoftMaxFlashV2MaxTmpSize(const platform_ascendc::PlatformAscendC& ascendcPlatform,
...
uint32_t GetSoftMaxFlashV2MinTmpSize(const platform_ascendc::PlatformAscendC& ascendcPlatform,
    const ge::Shape& srcShape, const uint32_t dataTypeSize1, const uint32_t dataTypeSize2, const bool isUpdate,
    const bool isBasicBlock, UNUSED const bool isFlashOutputBrc)
{
    const uint32_t inputSize = srcShape.GetShapeSize();
    ...
    if (soc == platform_ascendc::SocVersion::ASCEND910_95 || soc == platform_ascendc::SocVersion::ASCEND910_55) {
        if (isUpdate) {
            uint32_t size1 = (srcM * SOFTMAX_TMPBUFFER_COUNT + SOFTMAX_BASICBLOCK_UNIT - 1) / SOFTMAX_BASICBLOCK_UNIT
                             * SOFTMAX_BASICBLOCK_UNIT;
            if (dataTypeSize1 == SOFTMAX_HALF_SIZE) {
                ...
                needMinSize = size1 + srcM * (elementNumPerBlk * SOFTMAX_TMPBUFFER_COUNT);
            }
        } else {
    ...
    const bool isBasicBlock, const bool isFlashOutputBrc)
{
    ...
    if (isFlashOutputBrc && (softmaxFlashTiling.get_rangeM() > 1 || softmaxFlashTiling.get_tailM() != 0)) {
        ASCENDC_HOST_ASSERT((softmaxFlashTiling.get_reduceM() % (SOFTMAX_DEFAULT_BLK_SIZE / dataTypeSize1) == 0),
            return,
