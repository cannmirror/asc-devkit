 ...
 * \brief
 */
#ifndef AICORE_ADV_API_TILING_MATMUL_MATMUL_CONSTANT_TILING_IMPL_H
#define AICORE_ADV_API_TILING_MATMUL_MATMUL_CONSTANT_TILING_IMPL_H
...
__aicore__ constexpr L1Status GetL1StatusAL1FullLoad(const MatmulConfig& mmCFG, int32_t l1Size)
{
    ...
    l1Status.kBL1 = MinValue<int32_t>(CalcL1MaxLen<A_TYPE, B_TYPE, BIAS_TYPE>((bL1Size - biasSize - dequantSize),
                                          l1Status, mmCFG, kbAlignValue, L1TilingType::KBL1_16),
        k);
...
__aicore__ constexpr L1Status GetL1StatusBL1FullLoad(const MatmulConfig& mmCFG, int32_t l1Size)
{
    ...
    l1Status.kAL1 = MinValue<int32_t>(CalcL1MaxLen<A_TYPE, B_TYPE, BIAS_TYPE>((aL1Size - biasSize - dequantSize),
                                          l1Status, mmCFG, kaAlignValue, L1TilingType::KAL1_16),
        k);
    ...
    if (l1Status.kAL1 == k) {
        l1Status.mAL1 = MinValue<int32_t>(CalcL1MaxLen<A_TYPE, B_TYPE, BIAS_TYPE>(
...
__aicore__ constexpr L1Status GetL1StatusMFirst(const L1Status& l1Status, const MatmulConfig& mmCFG, int32_t l1Size)
{
    ...
    l1MFirst.mAL1 =
        MaxValue<int32_t>(MinValue<int32_t>(CalcL1MaxLen<A_TYPE, B_TYPE, BIAS_TYPE>(aL1Size - biasSize - dequantSize,
                                                l1MFirst, mmCFG, kaAlignValue, L1TilingType::M_AL1),
    ...
    l1MFirst.nBL1 =
        MaxValue<int32_t>(MinValue<int32_t>(CalcL1MaxLen<A_TYPE, B_TYPE, BIAS_TYPE>(bL1Size - biasSize - dequantSize,
                                                l1MFirst, mmCFG, kbAlignValue, L1TilingType::N_BL1),
...
__aicore__ constexpr L1Status GetL1StatusNFirst(const L1Status& l1Status, const MatmulConfig& mmCFG, int32_t l1Size)
{
    ...
    l1NFirst.nBL1 =
        MaxValue<int32_t>(MinValue<int32_t>(CalcL1MaxLen<A_TYPE, B_TYPE, BIAS_TYPE>(bL1Size - biasSize - dequantSize,
                                                l1Status, mmCFG, kbAlignValue, L1TilingType::N_BL1),
                              GetMaxNBL1(mmCFG), nRepeat),
            1);
    ...
    l1NFirst.mAL1 =
        MaxValue<int32_t>(MinValue<int32_t>(CalcL1MaxLen<A_TYPE, B_TYPE, BIAS_TYPE>(aL1Size - biasSize - dequantSize,
                                                l1NFirst, mmCFG, kaAlignValue, L1TilingType::M_AL1),
                              GetMaxMAL1(mmCFG), mRepeat),
            1);
    ...
    const L1Status& l1Db, const MatmulConfig& mmCFG, int32_t l1Size, int32_t biasSize, int32_t dequantSize)
{
    ...
    if (kMaxAxis == 0) {
        ...
        bL1Size = l1Size - aL1Size;
        l1Status.kBL1 = MinValue<int32_t>((bL1Size - dequantSize - biasSize)
                                              / (l1Status.nBL1 * mmCFG.basicN * l1Status.dbBL1 * mmCFG.basicK * kL0
        ...
        aL1Size = l1Size - bL1Size;
        l1Status.kAL1 = MinValue<int32_t>((aL1Size - dequantSize - biasSize)
                                              / (l1Status.mAL1 * mmCFG.basicM * l1Status.dbAL1 * mmCFG.basicK * kL0
...
__aicore__ constexpr bool CalcAL1FullLoadTiling(int32_t l1Size, MatmulApiStaticTiling& tiling)
{
    ...
    while (tiling.stepKa % stepKb != 0 && stepKb % tiling.stepKa != 0 && stepKb > 1) { stepKb--; }
...
__aicore__ constexpr MxScaleStatus GetMxScaleFactor(const MatmulApiStaticTiling& tiling, int32_t l1Size)
{
    ...
    int maxScaleFactorA = CeilNoLog<int32_t>(kStep, tiling.stepKa);
    int maxScaleFactorB = CeilNoLog<int32_t>(kStep, tiling.stepKb);

    mxScaleFactor.scaleFactorA =
        remainedL1Size / Impl::MX_L1_BUFFER_NUM / (tiling.stepKa * tiling.baseM * tiling.baseK / Impl::SCALE_K_SIZE);
    mxScaleFactor.scaleFactorB =
        remainedL1Size / Impl::MX_L1_BUFFER_NUM / (tiling.stepKb * tiling.baseN * tiling.baseK / Impl::SCALE_K_SIZE);
    mxScaleFactor.scaleFactorA =
        mxScaleFactor.scaleFactorA > maxScaleFactorA ? maxScaleFactorA : mxScaleFactor.scaleFactorA;
    mxScaleFactor.scaleFactorB =
        mxScaleFactor.scaleFactorB > maxScaleFactorB ? maxScaleFactorB : mxScaleFactor.scaleFactorB;

    // scaleFactor is in range of [1, 127]
    mxScaleFactor.scaleFactorA = mxScaleFactor.scaleFactorA >= 1 ? mxScaleFactor.scaleFactorA : 1;
    mxScaleFactor.scaleFactorB = mxScaleFactor.scaleFactorB >= 1 ? mxScaleFactor.scaleFactorB : 1;
    mxScaleFactor.scaleFactorA = mxScaleFactor.scaleFactorA <= Impl::SCALE_FACTOR_MAX_VALUE ?
                                     mxScaleFactor.scaleFactorA :
                                     Impl::SCALE_FACTOR_MAX_VALUE;
    mxScaleFactor.scaleFactorB = mxScaleFactor.scaleFactorB <= Impl::SCALE_FACTOR_MAX_VALUE ?
                                     mxScaleFactor.scaleFactorB :
                                     Impl::SCALE_FACTOR_MAX_VALUE;

    // 8bit: 0~6bit:scaleFactor, 7bit(reserved):double buffer flag
    mxScaleFactor.scaleFactorA = mxScaleFactor.scaleFactorA & 0x7f;
    mxScaleFactor.scaleFactorB = mxScaleFactor.scaleFactorB & 0x7f;
    mxScaleFactor.mxTypePara =
        static_cast<int32_t>(static_cast<uint32_t>(mxScaleFactor.mxTypePara) | mxScaleFactor.scaleFactorA);
    mxScaleFactor.mxTypePara =
        static_cast<int32_t>(static_cast<uint32_t>(mxScaleFactor.mxTypePara) | (mxScaleFactor.scaleFactorB << 8U));
...
__aicore__ constexpr void GetMxMatmulApiTiling(MatmulApiStaticTiling& tiling, int32_t l1Size)
{
    ...
    if constexpr (HasScalePosition<A_TYPE>::value || HasScalePosition<B_TYPE>::value) {
        ...
        int32_t needUsedL1Size =
            tiling.baseM * tiling.baseK * tiling.depthA1 * GetBitSize<typename A_TYPE::T>() / ONE_BYTE_BIT_SIZE
            + tiling.baseM * tiling.baseK / Impl::SCALE_K_SIZE * tiling.depthA1 * mxScaleFactor.scaleFactorA * 1
            + tiling.baseN * tiling.baseK * tiling.depthB1 * GetBitSize<typename B_TYPE::T>() / ONE_BYTE_BIT_SIZE
            + tiling.baseN * tiling.baseK / Impl::SCALE_K_SIZE * tiling.depthB1 * mxScaleFactor.scaleFactorB * 1
        ...
        if (needUsedL1Size > l1Size) {
            ...
            tiling.dbL0A = 1;
            tiling.dbL0B = 1;
            tiling.mxTypePara = Impl::MIN_MX_PARAM; // scaleFactorA = 1, scaleFactorB = 1
        }
    }
#endif
}
