...
#ifndef AICORE_ADV_API_TILING_MATMUL_MATMUL_TILING_ALGORITHM_H
#define AICORE_ADV_API_TILING_MATMUL_MATMUL_TILING_ALGORITHM_H
...
constexpr float MATMUL_EPSILION = 1.0E-4F;
constexpr float MEMORY_EPSILION = 0.05F;
...
struct ComputeIntensitySmallShape {
    ...
    {
        if (std::fabs(rhs.memoryTraffic - memoryTraffic) > MEMORY_EPSILION * memoryTraffic) {
...
struct ComputeIntensity {
    ...
    {
        if (std::fabs(avgIntensity - rhs.avgIntensity) > MATMUL_EPSILION) {
            return avgIntensity > rhs.avgIntensity;
        }
        if (std::fabs(computeCycle - rhs.computeCycle) < 0.01f * rhs.computeCycle) {
            return bandRatio > rhs.bandRatio;
...
private:
    ...
    int32_t GetIteratorOrder(const SingleCoreStatus& singleCoreStatus, const int32_t singleCoreM,
    ...
    void GetMxUsedL1Size(const SingleCoreStatus& singleCoreStatus, const int32_t k0Size, int32_t& dataUsedL1Size,
        int32_t& scaleUsedL1Size, int32_t& biasUsedL1Size) const;
    ...
    void CalcBaseShape(const SingleCoreStatus& singleCoreStatus, int32_t& baseM, int32_t& baseN, int32_t& baseK,
    ...
    void CalcMultiCoreBlockDims(const MatmulRunParas& params, const ComputeBaseBlock& baseBlock,
        CoreStatusPack& coreStatus, BlockDimCalculator& blockDimRes);
    void CalcMultiCoreBlockDimsPost(
