    ...
    const int32_t& k0, const int32_t& majorDimFactor, const int32_t& minorDimFactor) const
{
    ...
    l0Status.kL0 = k0;
    const float tmpL0cUse = static_cast<float>(l0Status.mL0 * l0Status.nL0 * l0Status.dbL0C * MIN_FRACTAL_SIZE
                                               * FP32_BYTES * 1.0 / tilingIns_->bufferPool_.l0CSize);
...
void MatmulTilingAlgorithm::GetL0bAlign(std::vector<int32_t>& factors) const
{
    ...
    if (DTYPE_BIT_TAB.at(tilingIns_->bType_.dataType) == DTYPE_BIT_TAB.at(DataType::DT_INT8)
        ...
        for (auto& num : factors) { num = MathUtil::Align(num, alignSize); }
    }
    return;
}
            ...
            for (auto& k0 : k0Factors) {
                ...
                if (((tilingIns_->aType_.dataType == DataType::DT_FLOAT && tilingIns_->aType_.type == CubeFormat::NZ
                        ...
                        || (tilingIns_->bType_.dataType == DataType::DT_FLOAT
                            && tilingIns_->bType_.type == CubeFormat::NZ && !tilingIns_->bType_.isTrans))
                    && k0 == 1) {
                    k0 = NUM_TWO;
                }
    ...
    const int32_t alignValue, const L1TilingType axisName) const
{
    ...
    if (axisName == L1TilingType::M_AL1) {
        axisMaxLen =
            resL1Size
            / (MathUtil::Align(l1Status.kAL1, alignValue) * l0Status.mL0 * l1Status.dbAL1 * C0_SIZE * C0_BYTE_SIZE);
    }
    if (axisName == L1TilingType::N_BL1) {
        axisMaxLen =
            resL1Size
            / (MathUtil::Align(l1Status.kBL1, alignValue) * l0Status.nL0 * l1Status.dbBL1 * C0_SIZE * C0_BYTE_SIZE
...
void MatmulTilingAlgorithm::GetNearestFactor(const int32_t& base, int32_t& factor, int32_t capValue) const
{
    ...
    while ((factor > capValue) || (factor > 0 && base % factor != 0)) { factor--; }
}
...
{
    // if b martix in L1, then b matrix must full load, goto b matrix full load patch
    if (tilingIns_->bType_.pos == TPosition::TSCM) {
        return;
    }
    ...
    if (curL1Size > 0 && curL1Size <= tilingIns_->bufferPool_.l1Size && a1Length < tilingIns_->bufferPool_.ubSize) {
        ...
        if (IsUbNd2Nz()) {
            ...
            l1Status.kBL1 = min(CalL1MaxLen(min(l1Status.bL1Size - biasSize - dequantSize, b1Length), l1Status,
                                    l0Status, kbAlignValue, L1TilingType::KBL1_16),
                coreStatus.k);
        }
...
void MatmulTilingAlgorithm::L1StatusBl1FullLoad(const CoreStatusPack& coreStatus, const L0StatusPack& l0Status,
    L1StatusPack& l1Status, int32_t res[][IDX_SEVEN]) const
{
    // if a martix in L1, then a matrix must full load, goto a matrix full load patch
    if (tilingIns_->aType_.pos == TPosition::TSCM) {
        return;
    }
    const int32_t mRepeat = MathUtil::CeilDivision(coreStatus.m, l0Status.mL0);
    ...
    if (curL1Size > 0 && curL1Size <= tilingIns_->bufferPool_.l1Size && b1Length < tilingIns_->bufferPool_.ubSize) {
        ...
        if (IsUbNd2Nz()) {
            ...
            l1Status.kAL1 = min(CalL1MaxLen(min(l1Status.aL1Size - biasSize - dequantSize, a1Length), l1Status,
                                    l0Status, kaAlignValue, L1TilingType::KAL1_16),
                coreStatus.k);
        }
    ...
    const CoreStatusPack& coreStatus, const L0StatusPack& l0Status, L1StatusPack& l1Status, const int32_t& kbl1Db) const
{
    ...
    if (bothDoubleBuffer) {
        ...
        if (GetL1Size(l1Status, l0Status) > tilingIns_->bufferPool_.l1Size
            ...
            l1Status.dbBL1 = DB_OFF;
            if (GetL1Size(l1Status, l0Status) > tilingIns_->bufferPool_.l1Size
                || GetAL1UbSize(l1Status, l0Status) + GetBL1UbSize(l1Status, l0Status)
    ...
    const CoreStatusPack& coreStatus, const L0StatusPack& l0Status, L1StatusPack& l1Status) const
{
    ...
    l1Mfirst.mAL1 = max(min(min(CalL1MaxLen(l1Mfirst.aL1Size - biasSize - dequantSize, l1Mfirst, l0Status, kaAlignValue,
                                    L1TilingType::M_AL1),
                                l1Mfirst.maxMAL1),
                            mRepeat),
        1);
    if (IsUbNd2Nz()) {
        l1Mfirst.mAL1 = max(min(min(CalL1MaxLen(min(l1Mfirst.aL1Size - biasSize - dequantSize, a1Length), l1Mfirst,
                                        l0Status, kaAlignValue, L1TilingType::M_AL1),
                                    l1Mfirst.maxMAL1),
                                mRepeat),
            1);
    }
    ...
    l1Mfirst.nBL1 = max(min(min(CalL1MaxLen(l1Mfirst.bL1Size - biasSize - dequantSize, l1Mfirst, l0Status, kbAlignValue,
                                    L1TilingType::N_BL1),
                                l1Mfirst.maxNBL1),
                            nRepeat),
        1);
    if (IsUbNd2Nz()) {
        l1Mfirst.nBL1 = max(min(min(CalL1MaxLen(min(l1Mfirst.bL1Size - biasSize - dequantSize, b1Length), l1Mfirst,
                                        l0Status, kbAlignValue, L1TilingType::N_BL1),
                                    l1Mfirst.maxNBL1),
                                nRepeat),
            1);
    }
    ...
    l1Nfirst.nBL1 = max(min(min(CalL1MaxLen(l1Nfirst.bL1Size - biasSize - dequantSize, l1Nfirst, l0Status, kbAlignValue,
                                    L1TilingType::N_BL1),
                                l1Nfirst.maxNBL1),
                            nRepeat),
        1);
    if (IsUbNd2Nz()) {
        l1Nfirst.nBL1 = max(min(min(CalL1MaxLen(min(l1Nfirst.bL1Size - biasSize - dequantSize, b1Length), l1Nfirst,
                                        l0Status, kbAlignValue, L1TilingType::N_BL1),
                                    l1Nfirst.maxNBL1),
                                nRepeat),
            1);
    ...
    l1Nfirst.mAL1 = max(min(min(CalL1MaxLen(l1Nfirst.aL1Size - biasSize - dequantSize, l1Nfirst, l0Status, kaAlignValue,
                                    L1TilingType::M_AL1),
                                l1Nfirst.maxMAL1),
                            mRepeat),
        1);
    if (IsUbNd2Nz()) {
        l1Nfirst.mAL1 = max(min(min(CalL1MaxLen(min(l1Nfirst.aL1Size - biasSize - dequantSize, a1Length), l1Nfirst,
                                        l0Status, kaAlignValue, L1TilingType::M_AL1),
                                    l1Nfirst.maxMAL1),
                                mRepeat),
            1);
    ...
    const CoreStatusPack& coreStatus, const L0StatusPack& l0Status, L1StatusPack& l1Status) const
{
    ...
    if (GetL1Size(l1Status, l0Status) > 0 && GetL1Size(l1Status, l0Status) <= tilingIns_->bufferPool_.l1Size) {
        ...
        int32_t perK = min((tilingIns_->bufferPool_.l1Size - biasSize - dequantSize)
            ...
            coreStatus.k);
        if (IsUbNd2Nz()) {
            perK = min(min(tilingIns_->bufferPool_.l1Size - biasSize - dequantSize, tilingIns_->bufferPool_.ubSize)
                           / (l0Status.mL0 * C0_SIZE * C0_BYTE_SIZE * l1Status.dbAL1
                               + C0_SIZE * l0Status.nL0 * C0_BYTE_SIZE * l1Status.dbBL1)
                           / l0Status.kL0 * l0Status.kL0,
                coreStatus.k);
...
void MatmulTilingAlgorithm::L1StatusNeitherFullLoad(const CoreStatusPack& coreStatus, const L0StatusPack& l0Status,
    L1StatusPack& l1Status, int32_t res[][IDX_SEVEN]) const
{
    // if b martix in L1, then b matrix must full load, skip non-full process
    if (tilingIns_->aType_.pos == TPosition::TSCM || tilingIns_->bType_.pos == TPosition::TSCM) {
        return;
    }
    if (g_tempCfg.l1DB == DB_ON) {
    ...
    const CoreStatusPack& coreStatus, const L0StatusPack& l0Status, L1StatusPack& l1Status) const
{
    ...
    int32_t bothFullLoadFactors[L1_FACTORS_LEN] = {
        coreStatus.k, coreStatus.k, l1Status.maxMAL1, l1Status.maxNBL1, DB_OFF, DB_OFF};
    ...
    const int32_t kAl1FactorOne = res[IDX_ONE][IDX_ZERO] > 0 ?
                                      MathUtil::CeilDivision(MathUtil::CeilDivision(GetSingleK(), reduceSize),
                                          (coreStatus.kDim * res[IDX_ONE][IDX_ZERO])) :
                                      1;
    const int32_t kBl1FactorTwo = res[IDX_TWO][IDX_THREE] > 0 ?
                                      MathUtil::CeilDivision(MathUtil::CeilDivision(GetSingleK(), reduceSize),
                                          (coreStatus.kDim * res[IDX_TWO][IDX_THREE])) :
                                      1;
    const int32_t kAl1FactorZero = res[IDX_ZERO][IDX_ZERO] > 0 ?
                                       MathUtil::CeilDivision(MathUtil::CeilDivision(GetSingleK(), reduceSize),
                                           (coreStatus.kDim * res[IDX_ZERO][IDX_ZERO])) :
                                       1;
    const int32_t kBl1FactorZero = res[IDX_ZERO][IDX_THREE] > 0 ?
                                       MathUtil::CeilDivision(MathUtil::CeilDivision(GetSingleK(), reduceSize),
    ...
    int32_t& l1Size, int32_t& l0cSize, int32_t& ubSize, int32_t a1LengthCache, int32_t b1LengthCache) const
{
    ...
    if (tilingIns_->socVersion == platform_ascendc::SocVersion::ASCEND910
        ...
        // case3: output GM/VECCAL, format is ND, for now not re-use input and ouput non-aligned, is related with db
        // open (1) output GM, format is NZ/ND_ALIGN/ND, need restore in ub, ND and D is non-aligned , then add more
        // 32B, ub->gm NZ->ND format and data type trnasform (2) output VECCALC,format is ND_ALIGN/ND(D alined), need
        ...
        if (tilingIns_->aType_.isTrans) {
            isBankConflict =
                MathUtil::CeilDivision(l1Status.mAL1 * l0Status.mL0 * C0_SIZE, C0_SIZE) * blockSize % bankLen == 0 ?
                    true :
                    false;
            bankConflictSize = l0Status.kL0 * reduceSize * C0_SIZE * MathUtil::CeilDivision(l1Status.kAL1, l0Status.kL0)
                               * DTYPE_BIT_TAB.at(tilingIns_->aType_.dataType) / BITS_PER_BYTE;
        } else {
            isBankConflict =
        ...
        if (tilingIns_->bType_.isTrans) {
            ...
            isBankConflict =
                MathUtil::CeilDivision(l1Status.nBL1 * l0Status.nL0 * C0_SIZE, C0_SIZE) * blockSize % bankLen == 0 ?
                    true :
                    false;
            bankConflictSize = l0Status.kL0 * reduceSize * C0_SIZE * MathUtil::CeilDivision(l1Status.kBL1, l0Status.kL0)
                               * DTYPE_BIT_TAB.at(tilingIns_->bType_.dataType) / BITS_PER_BYTE;
        }
    }
    if (isBankConflict) {
    ...
    BlockDimCalculator& blockDimRes, const MatmulRunParas& params) const
{
    ...
    const bool aKNotfulloadPlsbKNotFullLoad =
        (n0 * blockDimRes.kBytes + m0 * k0 * C0_SIZE * C0_BYTE_SIZE) * dbBuffer > tilingIns_->bufferPool_.l1Size
        && (m0 * blockDimRes.kBytes + n0 * k0 * C0_SIZE * C0_BYTE_SIZE) * dbBuffer > tilingIns_->bufferPool_.l1Size;
    if (aKNotfulloadPlsbKNotFullLoad) {
    ...
    const bool aKfulloadPlsbKFullLoad = (m0 + n0) * blockDimRes.kBytes * dbBuffer <= tilingIns_->bufferPool_.l1Size;
    if (aKfulloadPlsbKFullLoad) {
        const int32_t m1 = MathUtil::CeilDivision((tilingIns_->bufferPool_.l1Size - n0 * blockDimRes.kBytes * dbBuffer),
                               (blockDimRes.kBytes * dbBuffer * m0))
                           * m0;
        const int32_t n1 = MathUtil::CeilDivision((tilingIns_->bufferPool_.l1Size - m0 * blockDimRes.kBytes * dbBuffer),
                               (blockDimRes.kBytes * dbBuffer * n0))
                           * n0;
    ...
    const bool afulloadPlsbKNotFullLoad =
        (blockDimRes.amatSize * blockDimRes.kBytes + n0 * k0 * C0_SIZE * C0_BYTE_SIZE * dbBuffer)
        <= tilingIns_->bufferPool_.l1Size;
    const bool aKfulloadPlsbKNotFullLoad =
        (m0 * blockDimRes.kBytes * dbBuffer + n0 * k0 * C0_SIZE * C0_BYTE_SIZE * dbBuffer)
        <= tilingIns_->bufferPool_.l1Size;
    if (afulloadPlsbKNotFullLoad || aKfulloadPlsbKNotFullLoad) {
        blockDimRes.tmpAmatSize = blockDimRes.oriAmatSize * blockDims.n;
        blockDimRes.tmpBmatSize = blockDimRes.oriBmatSize * MathUtil::CeilDivision(params.m32, m0);
        blockDimRes.tmpLoadSize = blockDimRes.tmpAmatSize + blockDimRes.tmpBmatSize;
        UpdateBlockDimCalculator(blockDimRes);
    ...
    const bool aKNotfulloadPlsbFullLoad =
        (blockDimRes.bmatSize * blockDimRes.kBytes + m0 * k0 * C0_SIZE * C0_BYTE_SIZE * dbBuffer)
        <= tilingIns_->bufferPool_.l1Size;
    const bool aKNotfulloadPlsbKFullLoad =
        (n0 * blockDimRes.kBytes * dbBuffer + m0 * k0 * C0_SIZE * C0_BYTE_SIZE * dbBuffer)
        <= tilingIns_->bufferPool_.l1Size;
    if (aKNotfulloadPlsbFullLoad || aKNotfulloadPlsbKFullLoad) {
...
int32_t MatmulTilingAlgorithm::LoopNumFromSingleCoreToL0(
    const CoreStatusPack& coreStatus, const DimFactor& blockDimsFactor) const
{
...
int32_t MatmulTilingAlgorithm::GetBigPackageCondition(
    const CoreStatusPack& coreStatus, const BlockDimCalculator& blockDimRes, const MatmulRunParas& params) const
{
...
{
    blockDimRes.kNum =
        (blockDim.k == 0) ? 0 : (params.k32 / blockDim.k * C0_SIZE * REDUCE_BLOCK_SIZE); // contain k * 16
    ...
    const bool updateConditionCoreUsed =
        (!updateConditionBp)
        && ((loopNum < blockDimRes.loopNumToL0)
    ...
    if ((updateConditionBp2 || updateConditionCoreUsed || updateConditionLoadsize || updateConditionBatchNDim)
        ...
        blockDimRes.bigPackage = (!tilingIns_->bType_.isTrans ? coreStatus.n >= minSize : true)
                                 ...
                                 && (blockDim.n * blockDim.m * blockDim.k > 1);
        splitCoreFlag_ = true;
    }
}

bool MatmulTilingAlgorithm::UserPolicy(
    const TilingPolicy policy, const CoreStatusPack& coreStatus, const BlockDimCalculator& blockDimRes) const
{
    constexpr int32_t minMNSize = 16;
    constexpr int32_t minKSize = 64;
    constexpr int32_t minTotalSize = 128;
    ...
    MatmulRunParas& params, const int32_t& coreNum, bool splitKFlag) const
{
    ...
    if (params.n32 > MIN_MTE1_LOAD) {
        specialScenario =
            specialScenario
            || (splitKFlag
                && ((static_cast<uint32_t>(params.nMapped) & static_cast<uint32_t>(MIN_MTE1_LOAD - 1)) != 0));
    }
    if (params.m32 > MIN_MTE1_LOAD) {
        specialScenario =
            specialScenario
            || (splitKFlag
                && ((static_cast<uint32_t>(params.mMapped) & static_cast<uint32_t>(MIN_MTE1_LOAD - 1)) != 0));
    }
    ...
    const MatmulRunParas& params, const int32_t sm, const int32_t sn, ComputeBaseBlock& baseBlock) const
{
    ...
    baseBlock.baseK = min(baseBlock.baseK, MathUtil::Align(static_cast<int32_t>(params.oriShapeKa),
                                               BASIC_SIZE_32)); // 32 is more efficient than 16
    if (sm >= basicSize128) {
    ...
    const MatmulRunParas& params, const std::pair<int32_t, int32_t>& factor, ComputeBaseBlock& baseBlock) const
{
    ...
    return {{mFactor, nFactor}, totalCycles, (totalMemory > 0) ? totalCycles / totalMemory : 0, bandRatio, totalMemory,
        baseBlock};
}
...
{
    int32_t coreUse = singelBlockDim_ ? tilingIns_->blockDim :
                                        coreStatus.batchDim * coreStatus.mDim * coreStatus.kDim * coreStatus.nDim;
    coreUse = coreUse > numOfBlock_ ? numOfBlock_ : coreUse;
    int32_t singleCoreM;
    int32_t singleCoreN;
    int32_t singleCoreK;
...
bool MatmulTilingAlgorithm::DoMultiCoreSplitMNTiling(
    const MatmulRunParas& params, CoreStatusPack& coreStatus, BlockDimCalculator& blockDimRes)
{
...
bool MatmulTilingAlgorithm::CalcNBuffer33BlockDims(
    const MatmulRunParas& params, const ComputeBaseBlock& baseBlock, CoreStatusPack& coreStatus) const
{
    ...
    if (coreStatus.mDim * coreStatus.kDim > numOfBlock_) {
        TILING_LOG_WARNING("M %d (baseM %d) or K %d (baseK %d) is too large to find a valid NBuffer33 single core "
                           "shape within %d cores. Remnind to slice M or K in test code.",
    ...
    for (const auto& factor : dimPairs) { results.push_back(CalcComputeIntensity(params, baseBlock, factor)); }
    std::sort(results.begin(), results.end());
    for (const auto& res : results) {
        TILING_LOG_DEBUG("intent:%f, cycle: %f, band: %f, mDim: %d, nDim: %d\n", res.avgIntensity, res.computeCycle,
            res.bandRatio, res.dimFactor.first, res.dimFactor.second);
    }
    ...
    CoreStatusPack& coreStatus, BlockDimCalculator& blockDimRes)
{
    ...
    std::vector<ComputeIntensity> results;
    for (const auto& factor : factors) { results.push_back(CalcComputeIntensity(params, baseBlock, factor)); }
    // 排序结果
    std::sort(results.begin(), results.end());
    for (auto v : results) {
        TILING_LOG_DEBUG("intent:%f, cycle: %f, band: %f, mDim: %d, nDim: %d\n", v.avgIntensity, v.computeCycle,
            v.bandRatio, v.dimFactor.first, v.dimFactor.second);
    }
    coreStatus.batchDim = 1;
...
void MatmulTilingAlgorithm::CalcMultiCoreBlockDimsPost(
    const MatmulRunParas& params, CoreStatusPack& coreStatus, BlockDimCalculator& blockDimRes)
{
...
void MatmulTilingAlgorithm::AddOptimalFactors(
    const std::string& opType, const MatmulRunParas& params, BlockDimCalculator& blockDimRes) const
{
    (void)(opType);
...
void MatmulTilingAlgorithm::GenBlockDimsMapFactors(
    const std::string& opType, MatmulRunParas& params, BlockDimCalculator& blockDimRes) const
{
    const int32_t coreNum = numOfBlock_;
    ...
    MathUtil::GetBlockFactors(
        blockDimRes.batchDimFactors, params.batch32, params.batchMapped, coreNum, min(coreNum, params.batch32));
...
void MatmulTilingAlgorithm::GetBlockDim(
    const std::string& opType, MatmulRunParas& params, CoreStatusPack& coreStatus, BlockDimCalculator& blockDimRes)
{
...
void MatmulTilingAlgorithm::NonFactorMap(
    const std::string& opType, MatmulRunParas& param, BlockDimCalculator& blockDimRes) const
{
    (void)(opType);
...
void MatmulTilingAlgorithm::FillParam(MatmulRunParas& param)
{
    ...
    if (tilingIns_->singleCoreM != -1 || tilingIns_->singleCoreK != -1 || tilingIns_->singleCoreN != -1) {
        ...
        singelBlockDim_ = true;
        numOfBlock_ = 1;
    } else {
        realM = GetSingleM();
        realK = GetSingleK();
        realN = GetSingleN();
        singelBlockDim_ = false;
...
bool MatmulTilingAlgorithm::CheckFinaleParams(const CoreStatusPack& coreStatus) const
{
    ...
    int32_t biasL1Size = tilingIns_->isBias ? tilingIns_->tiling_.get_singleCoreN() * tilingIns_->tiling_.get_BatchNum()
                                                  * dateDtypeSize / BITS_PER_BYTE :
                                              0;
...
void MatmulTilingAlgorithm::CheckL0DB(SingleCoreStatus& singleCoreStatus, const int32_t baseK) const
{
    ...
    if (baseM * baseK * DTYPE_BIT_TAB.at(tilingIns_->aType_.dataType) / BITS_PER_BYTE
        > tilingIns_->bufferPool_.l0ASize / DB_ON) {
        singleCoreStatus.l0Status.dbL0A = DB_OFF;
    }
    if (baseN * baseK * DTYPE_BIT_TAB.at(tilingIns_->bType_.dataType) / BITS_PER_BYTE
        > tilingIns_->bufferPool_.l0BSize / DB_ON) {
    ...
    int32_t& dataUsedL1Size, int32_t& scaleUsedL1Size, int32_t& biasUsedL1Size) const
{
    ...
    // scale is fp8e8m0
    scaleUsedL1Size = depthA1 * baseM * baseK / SCALE_K_SIZE + depthB1 * baseN * baseK / SCALE_K_SIZE;
    // bias is fp32
...
void MatmulTilingAlgorithm::GetMxScaleFactor(
    const SingleCoreStatus& singleCoreStatus, const int32_t k0Size, int32_t& mxTypePara) const
{
    // determine whether the scenario is MX
    if (tilingIns_->madType_ != MatrixMadType::MXMODE) {
        return;
    ...
    uint8_t scaleFactorA = 1;
    uint8_t scaleFactorB = 1;
    ...
    scaleFactorA = static_cast<uint8_t>(remainedL1Size / MX_L1_BUFFER_NUM / (stepKa * baseM * baseK / SCALE_K_SIZE));
    scaleFactorB = static_cast<uint8_t>(remainedL1Size / MX_L1_BUFFER_NUM / (stepKb * baseN * baseK / SCALE_K_SIZE));
    scaleFactorA = scaleFactorA > maxScaleFactorA ? maxScaleFactorA : scaleFactorA;
    scaleFactorB = scaleFactorB > maxScaleFactorB ? maxScaleFactorB : scaleFactorB;

    // scaleFactor is in range of [1, 127]
    scaleFactorA = scaleFactorA >= static_cast<uint8_t>(1) ? scaleFactorA : static_cast<uint8_t>(1);
    scaleFactorB = scaleFactorB >= static_cast<uint8_t>(1) ? scaleFactorB : static_cast<uint8_t>(1);
    scaleFactorA = scaleFactorA <= SCALE_FACTOR_MAX_VALUE ? scaleFactorA : SCALE_FACTOR_MAX_VALUE;
    scaleFactorB = scaleFactorB <= SCALE_FACTOR_MAX_VALUE ? scaleFactorB : SCALE_FACTOR_MAX_VALUE;
    ...
    if ((tilingIns_->aType_.type == CubeFormat::ND && tilingIns_->aType_.isTrans == true
        ...
        && (tilingIns_->bType_.type == CubeFormat::ND && tilingIns_->bType_.isTrans == false
            && tilingIns_->bType_.scalePos == TPosition::TSCM)) {
        scaleFactorA = static_cast<uint8_t>(1);
        scaleFactorB = static_cast<uint8_t>(1);
    }
    ...
    scaleFactorA = scaleFactorA & static_cast<uint8_t>(0x7f);
    scaleFactorB = scaleFactorB & static_cast<uint8_t>(0x7F);
    mxTypePara = static_cast<int32_t>(static_cast<uint32_t>(mxTypePara) | scaleFactorA);
    mxTypePara = static_cast<int32_t>(static_cast<uint32_t>(mxTypePara) | static_cast<uint32_t>(scaleFactorB << 8U));
}
    ...
    int32_t& singleCoreM, int32_t& singleCoreN, int32_t& singleCoreK) const
{
    ...
    if (numOfBlock_ > 1) {
        ...
        bool needAlign =
            multiCoreScenario == MultiCoreScenario::SPLIT_MN || multiCoreScenario == MultiCoreScenario::SPLIT_SMALL_MN;
        bool needOutputAlign = NeedOutputAlign(singleCoreM, singleCoreN, singleCoreK);
        (void)AlignSingleShape(needAlign && (!tilingIns_->bType_.isTrans || needOutputAlign), param.n32 * C0_SIZE,
            coreStatus.nDim, bAlignSize, singleCoreN);
        (void)AlignSingleShape(
            needAlign && tilingIns_->aType_.isTrans, param.m32 * C0_SIZE, coreStatus.mDim, aAlignSize, singleCoreM);
        if (tilingIns_->enableSplitK_) {
            if (tilingIns_->aType_.dataType == DataType::DT_FLOAT
                || tilingIns_->bType_.dataType == DataType::DT_FLOAT) {
    ...
    const CoreStatusPack& coreStatus, SingleCoreStatus& singleCoreStatus) const
{
    ...
    if (coreStatus.batchDim * coreStatus.mDim * coreStatus.kDim * coreStatus.nDim > numOfBlock_) {
    ...
    int32_t blockM = MathUtil::CeilDivision(tilingIns_->tiling_.get_singleCoreM(), tilingIns_->tiling_.get_baseM());
    if (singleCoreStatus.l1Status.mAL1 != blockM) {
        singleCoreStatus.l1Status.mAL1 = blockM;
    }
...
int64_t MatmulTilingAlgorithm::Process()
{
    ...
    if (numOfBlock_ != 1 && tilingIns_->bType_.pos == TPosition::TSCM) {
        if (!splitCoreFlag_) {
            TILING_LOG_WARNING("Multi core split B TSCM full loaded is not sucess.");
            return 1;
        }
    }
    // single-core logic
