 ...
 * \brief
 */
#ifndef AICORE_ADV_API_TILING_MATMUL_MATMUL_CONSTANT_TILING_UTILS_H
#define AICORE_ADV_API_TILING_MATMUL_MATMUL_CONSTANT_TILING_UTILS_H
...
struct MxScaleStatus {
    uint8_t scaleFactorA;
    uint8_t scaleFactorB;
    int32_t mxTypePara;
};
...
__aicore__ constexpr int32_t GetBL1Size(const L1Status& l1Status, const MatmulConfig& mmCFG)
{
    ...
    if constexpr (PhyPosIsL1(B_TYPE::pos)) {
        ...
        curB1Size = l1Status.dbBL1 * l1Status.nBL1 * mmCFG.basicN * CeilNoLog<int32_t>(l1Status.kBL1, kL0)
                    * Align(mmCFG.basicK, static_cast<uint32_t>(GetKBAlignValue<A_TYPE, B_TYPE>()
                                                                * GetReduceC0Size<typename B_TYPE::T>()))
...
{
    int res = factor;
    while ((res > base) || (res > 0 && base % res != 0)) { res--; }
    return res;
}
...
__aicore__ constexpr int32_t GetTransLength(const MatmulConfig& mmCFG, const L1Status& l1Status)
{
    ...
    if constexpr (A_TYPE::format == CubeFormat::ND
                  && (A_TYPE::pos == TPosition::VECIN || A_TYPE::pos == TPosition::VECCALC
                      || A_TYPE::pos == TPosition::VECOUT)) {
        a1Length = mmCFG.singleCoreM * mmCFG.singleCoreK * GetBitSize<typename A_TYPE::T>() / ONE_BYTE_BIT_SIZE;
    }
    // B matrix ND2NZ
    if constexpr (B_TYPE::format == CubeFormat::ND
                  && (B_TYPE::pos == TPosition::VECIN || B_TYPE::pos == TPosition::VECCALC
                      || B_TYPE::pos == TPosition::VECOUT)) {
