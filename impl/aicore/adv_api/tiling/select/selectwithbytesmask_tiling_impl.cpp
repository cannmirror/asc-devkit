...
inline uint32_t PaddingTo(uint32_t a, uint32_t b)
{
    return (a + b - 1) / b * b;
}

inline bool IsScalar(const ge::Shape& shape)
{
    return shape.GetDimNum() == 0 || shape.GetShapeSize() == 1;
}

inline uint32_t ComputeBufferSize(const ge::Shape& src0Shape, const ge::Shape& src1Shape, const ge::Shape& maskShape,
    ...
    const ge::Shape& maskShape, const uint32_t maskTypeSize, const bool isReuseMask, const char* funcName)
{
    ...
    ASCENDC_HOST_ASSERT(
        ...
        continue, "[SelectWithBytesMask][%s] The value of maskTypeSize is %u, should be 1 or 2 or 4.", funcName,
        maskTypeSize);

    bool ans = (src0Shape.GetDimNum() == SELECTWITHMASK_DIM_TWO) || (src1Shape.GetDimNum() == SELECTWITHMASK_DIM_TWO);
    ASCENDC_HOST_ASSERT(ans, continue,
        "[SelectWithBytesMask][%s] The src0Shape dimension number is %zu, "
...
{
    CheckSelectWithBytesMaskParams(
        src0Shape, src1Shape, srcTypeSize, maskShape, maskTypeSize, isReuseMask, "GetSelectWithBytesMaskMinTmpSize");
...
{
    CheckSelectWithBytesMaskParams(
        src0Shape, src1Shape, srcTypeSize, maskShape, maskTypeSize, isReuseMask, "GetSelectWithBytesMaskMaxTmpSize");
...
{
    CheckSelectWithBytesMaskParams(
        src0Shape, src1Shape, srcTypeSize, maskShape, maskTypeSize, isReuseMask, "GetSelectWithBytesMaskMaxMinTmpSize");
    maxValue =
