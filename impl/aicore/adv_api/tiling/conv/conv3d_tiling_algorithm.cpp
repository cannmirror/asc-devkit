...
void Conv3dTilingAlgorithm::GetL1TilingRange()
{
    ...
    uint64_t multiMAL1Max =
        CeilDiv(CeilDiv(tilingIns_->shapeInfo.singleM, tilingIns_->cubeInfo.m0) * tilingIns_->cubeInfo.m0,
...
void Conv3dTilingAlgorithm::InitL1TiLing()
{
    ...
    this->l1TilingInitMap[L1TilingMode::ALL_FULL_LOAD].SetIdx(this->l1TilingRange.kAL1Range.size() - 1,
        this->l1TilingRange.kBL1Range.size() - 1, this->l1TilingRange.mAL1ValueRange.size() - 1,
...
void Conv3dTilingAlgorithm::GetL0TilingRange()
{
    ...
    uint64_t nL0Max = std::min(
        ...
        tilingIns_->platformInfo.l0CSize
            / (tilingIns_->cubeInfo.m0 * this->doubleBufferValue.pbCL0
    ...
    uint64_t mL0Max = std::min(
        ...
        tilingIns_->platformInfo.l0CSize
            / (tilingIns_->cubeInfo.n0 * this->doubleBufferValue.pbCL0
                * g_dtypeSizeTab.at(tilingIns_->cubeInfo.madType)));
