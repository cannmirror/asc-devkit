...
#include "conv/conv3d/conv3d_tiling_base.h"
...
bool Conv3dTilingBase::CheckInputAttr()
{
    ...
    bool padInvalidFlag = (this->attrInfo.padLeft < 0 || this->attrInfo.padRight < 0 || this->attrInfo.padUp < 0
...
bool Conv3dTilingBase::CheckSingleInputInfo()
{
    ...
    if (this->shapeInfo.cinOpt <= 0 || this->shapeInfo.coutOpt <= 0) {
        TILING_LOG_ERROR("Illegal cinOpt and coutOpt have set: cinOpt=%ld, coutOpt=%ld.", this->shapeInfo.cinOpt,
...
{
    if (this->hasBias) {
        std::vector<ConvCommonApi::ConvDtype> paramsType = {this->descInfo.fMapType.dtype,
            this->descInfo.weightType.dtype, this->descInfo.biasType.dtype, this->descInfo.outputType.dtype};
...
bool Conv3dTilingBase::CheckLoad3DLimits()
{
    if (static_cast<uint32_t>(this->attrInfo.strideH) > LOAD3D_MAX_STRIDE_H_W
        ...
        TILING_LOG_ERROR("Attrs not satisfy Load3D's limits: strideH=%ld, strideW=%ld, which must <= %u.",
    ...
    if (static_cast<uint32_t>(this->attrInfo.dilationH) > LOAD3D_MAX_DILATION_H_W
        ...
        TILING_LOG_ERROR("Attrs not satisfy Load3D's limits: dilationH=%ld, dilationW=%ld, which must <= %u.",
    ...
    if (padLoad3dInvalid) {
        TILING_LOG_ERROR("Attrs not satisfy Load3D's limits: padUp=%ld, padDown=%ld, padLeft=%ld, padRight=%ld, "
                         "which must <= %u.",
            this->attrInfo.padUp, this->attrInfo.padDown, this->attrInfo.padLeft, this->attrInfo.padRight,
            LOAD3D_MAX_PAD);
        return false;
    ...
    if (tmpkHWSize > LOAD3D_MAX_DDR2L1_SIZE) {
        TILING_LOG_ERROR("Weight shape not satisfy Load3D's limits: kH*kW*k0=%lu, which must <= %u.", tmpkHWSize,
...
bool Conv3dTilingBase::CheckParamsOverflow()
{
    uint64_t prod;
    bool isOverflow =
        MulWithOverflowCheck(prod, static_cast<uint64_t>(this->shapeInfo.orgDi),
        ...
        || MulWithOverflowCheck(prod, static_cast<uint64_t>(this->attrInfo.groupOpt),
            static_cast<uint64_t>(this->shapeInfo.singlekD), CeilDiv(this->shapeInfo.cinOpt, this->cubeInfo.k0),
            static_cast<uint64_t>(this->shapeInfo.singlekH), static_cast<uint64_t>(this->shapeInfo.singlekW),
            CeilDiv(this->shapeInfo.coutOpt, this->cubeInfo.n0),
        ...
        || MulWithOverflowCheck(prod, this->shapeCalc.orgDo, static_cast<uint64_t>(this->attrInfo.groupOpt),
