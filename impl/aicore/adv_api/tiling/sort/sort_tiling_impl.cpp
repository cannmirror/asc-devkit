    ...
    const platform_ascendc::PlatformAscendC& ascendcPlatform, const uint32_t elemCount, const uint32_t dataTypeSize)
{
    ...
    ASCENDC_HOST_ASSERT(uint64_t(elemCount * dataTypeSize) < ubSize, continue,
        "[%s][%s] The size of srcShape is %luB, should be less than UB size.", apiName, hostFuncName,
        uint64_t(elemCount * dataTypeSize));
    return;
}
} // namespace
    ...
    const SortConfig& config, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    ASCENDC_HOST_ASSERT(socVersion == platform_ascendc::SocVersion::ASCEND910_95
        ...
        return, "Unsupported SocVersion for Sort API.");
    std::set<ge::DataType> supportValueType = {ge::DT_INT8, ge::DT_UINT8, ge::DT_FLOAT, ge::DT_FLOAT16, ge::DT_BF16,
        ge::DT_INT16, ge::DT_INT32, ge::DT_UINT16, ge::DT_UINT32, ge::DT_UINT64, ge::DT_INT64};
    std::set<ge::DataType> supportIndexType = {ge::DT_INT32, ge::DT_UINT32, ge::DT_INT64, ge::DT_UINT64};
    ASCENDC_HOST_ASSERT(
        (!config.hasSrcIndex || config.hasDstIndex), return, "Sort API couldn't only have source input index.");
    ASCENDC_HOST_ASSERT(
