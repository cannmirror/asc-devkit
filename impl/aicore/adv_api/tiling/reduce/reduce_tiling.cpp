...
inline void CheckParams(std::vector<int64_t> shapeDims, bool isSrcInnerPad, ReducePattern pattern, uint32_t first,
    uint32_t last, std::string apiName, std::string funcName)
{
    ASCENDC_HOST_ASSERT(shapeDims.size() == ALLOWED_SHAPE_DIM, return, "[%s][%s] srcShape dims must be 2.",
        apiName.c_str(), funcName.c_str());
    ASCENDC_HOST_ASSERT(isSrcInnerPad, return, "[%s][%s] isSrcInnerPad must be true on this platform.", apiName.c_str(),
        funcName.c_str());
    ASCENDC_HOST_ASSERT(pattern == ReducePattern::AR || pattern == ReducePattern::RA, return,
        "[%s][%s] Currently only support AR and RA pattern.", apiName.c_str(), funcName.c_str());
    ASCENDC_HOST_ASSERT(first > 0 && last > 0, return, "[%s][%s] both first and last axis must be greater than 0.",
        apiName.c_str(), funcName.c_str());
}
} // namespace
...
void GetReduceMaxMaxMinTmpSize(const ge::Shape& srcShape, const ge::DataType dataType, ReducePattern pattern,
    bool isSrcInnerPad, bool isReuseSource, uint32_t& maxValue, uint32_t& minValue)
{
    ASCENDC_HOST_ASSERT(dataType == ge::DT_FLOAT || dataType == ge::DT_FLOAT16, return,
    ...
    bool isSrcInnerPad, bool isReuseSource, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    if (dataType == ge::DT_UINT8) {
        GetReduceAnyAllCommonTmpSize(srcShape, pattern, isSrcInnerPad, isReuseSource, maxValue, minValue, "ReduceAny",
    ...
    bool isSrcInnerPad, bool isReuseSource, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    if (dataType == ge::DT_UINT8) {
        GetReduceAnyAllCommonTmpSize(srcShape, pattern, isSrcInnerPad, isReuseSource, maxValue, minValue, "ReduceAll",
    ...
    bool isSrcInnerPad, bool isReuseSource, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    GetReduceSumMeanCommonTmpSize(srcShape, pattern, isSrcInnerPad, isReuseSource, maxValue, minValue, "ReduceMean",
