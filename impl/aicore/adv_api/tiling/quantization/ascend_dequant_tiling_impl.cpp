    ...
    const char* apiName, const char* hostFuncName, const ge::Shape& srcShape, const uint32_t typeSize)
{
    ...
    ASCENDC_HOST_ASSERT(typeSize == DEQUANT_INT32_SIZE, return, "[%s][%s] Type size %u is unsupported!", apiName,
        hostFuncName, typeSize);
    return;
}
} // namespace
    ...
    const ge::Shape& srcShape, const uint32_t typeSize, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    // gurantee deqScaleSize >= deqScale.GetSize()   Ex: uint32_t inner = 8, bf16 deqScale = 16
