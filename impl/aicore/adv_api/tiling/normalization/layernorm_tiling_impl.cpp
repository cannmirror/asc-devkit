    ...
    const bool isComputeRstd, const bool isOnlyOutput, uint32_t& maxValue, uint32_t& minValue)
{
    ...
    GetNormalizeMaxMinTmpSize(
        srcShape, typeSize, typeSize, isReuseSource, isComputeRstd, isOnlyOutput, maxNormalizeValue, minNormalizeValue);
    ...
    optiling::LayerNormSeparateTiling& tiling)
{
    ...
    if (soc == platform_ascendc::SocVersion::ASCEND910_95 || soc == platform_ascendc::SocVersion::ASCEND910_55) {
        ...
        float k2Rec = float(1) / float(rLengthOverflow);
        float k2RRec = float(rLengthOverflow) / float(rLength);
