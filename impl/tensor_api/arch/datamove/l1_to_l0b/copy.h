...
constexpr LoadDataTrait DEFAULT_COPY_L1_TO_L0B_TRAIT;
...
private:
    template<const LoadDataTrait& trait = DEFAULT_COPY_L1_TO_L0B_TRAIT, typename T, typename U>
    __aicore__ inline static void LoadData(const T& dst, const U& src)
    {
        ...
        using Tensor2Tensor = typename CopyL12L0BTensor2TensorNoCoord<dstPos, srcPos, CURRENT_ARCH_VERSION>::type;
        Tensor2Tensor::template Run<trait, T, U>(dst, src);
    }

    template<const LoadDataTrait& trait = DEFAULT_COPY_L1_TO_L0B_TRAIT, typename T, typename U, class Coord>
    __aicore__ inline static void LoadData(const T& dst, const U& src, const Coord& coord)
    {
        constexpr Hardware dstPos = GetHardPos<T>();
        constexpr Hardware srcPos = GetHardPos<U>();
        using Tensor2Tensor = typename CopyL12L0BTensor2Tensor<dstPos, srcPos, CURRENT_ARCH_VERSION>::type;
