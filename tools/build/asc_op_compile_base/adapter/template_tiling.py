...
def safe_parse_value(s):
    ...
    try:
        if "DT_" in s:
            assert s[:5] == "C_DT_", f"datatype only support C_DT_*, but now is [{s}]"
            datatype = s.split("::")[-1]
            if datatype[:2] == "C_":
                datatype = datatype[2:]
        ...
        CommonUtility.print_compile_log("", f"Invalid C_DT_ value: {e}", AscendCLogLevel.LOG_ERROR)
    except Exception:
        pass
    try:
        if "FORMAT_" in s:
            assert s[:9] == "C_FORMAT_", f"format only support C_FORMAT_*, but now is [{s}]"
        ...
        CommonUtility.print_compile_log("", f"Invalid C_FORMAT_ value: {e}", AscendCLogLevel.LOG_ERROR)
    except Exception:
        pass
    CommonUtility.print_compile_log(
        "", "can't convert str : [{}], it is invalid value.".format(s), AscendCLogLevel.LOG_ERROR
    )
    return -1
...
def extract_decl_param_options(op_info: OpInfo, option_name="dtype"):
    if op_info is None:
        return [],[]
    assert isinstance(op_info, OpInfo), f"op_info is't OpInfo object, Please check it."
    support_option_dict = {"dtype": TilingParamType.TPL_DATATYPE, "format": TilingParamType.TPL_DATAFORMAT}
    assert (
        option_name in support_option_dict.keys()
    ), f"now the function option_name only support : {support_option_dict}"
    decl_input_indexes = []
    decl_output_indexes = []
    deck_select_indexes = []
    ...
    for declare_param in TILING_DECLARE_MAP:
        ...
        for val in declare_param.values:
            if val >= ASCENDC_TPL_INPUT_BIAS and val < ASCENDC_TPL_OUTPUT_BIAS:
                ...
                pass
    assert len(decl_input_indexes) == 0 or (
        max(decl_input_indexes) < len(op_info.inputs) and min(decl_input_indexes) >= 0
    ), f"exist invalid input index : {decl_input_indexes}, please check it."
    assert len(decl_output_indexes) == 0 or (
        max(decl_output_indexes) < len(op_info.outputs) and min(decl_output_indexes) >= 0
    ), f"exist invalid output index : {decl_output_indexes}, please check it."
    assert len(decl_input_indexes) == len(
        set(decl_input_indexes)
    ), f"exist duplicate input index : {decl_input_indexes}"
    assert len(decl_output_indexes) == len(
        set(decl_output_indexes)
    ), f"exist duplicate output index : {decl_output_indexes}"
    decl_input_options = [op_info.inputs[idx][option_name] for idx in decl_input_indexes]
    decl_output_options = [op_info.outputs[idx][option_name] for idx in decl_output_indexes]
