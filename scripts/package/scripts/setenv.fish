#!/usr/bin/env fish
# Perform setenv for asc-devkit package
# ----------------------------------------------------------------------------------------------------------
...
function get_install_param
    ...
    set -l install_info_key_array Asc-Devkit_Install_Type Asc-Devkit_Feature_Type Asc-Devkit_UserName Asc-Devkit_UserGroup Asc-Devkit_Install_Path_Param Asc-Devkit_Arch_Linux_Path Asc-Devkit_Hetero_Arch_Flag
...
function get_install_dir
    ...
    set -l hetero_arch (get_install_param "Asc-Devkit_Hetero_Arch_Flag" "$install_info")
    if test "$param_mult_ver" = "multi_version"
        if test "$hetero_arch" = "y"
            echo (realpath $curpath/../../../../../latest)/asc-devkit
        else
            echo (realpath $curpath/../../../latest)/asc-devkit
        end
    else
