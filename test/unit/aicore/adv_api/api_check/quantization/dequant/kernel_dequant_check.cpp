...
TEST_F(DequantAPICheck, DequantAPICheckTestSuccess)
{
    ...
    AscendC::CHECK_FUNC_HIGHLEVEL_API(AscendDequant,
        (float, float, true, AscendC::DeQuantMode::DEQUANT_WITH_SINGLE_ROW),
...
TEST_F(DequantAPICheck, DequantAPICheckTestSubFailure)
{
    ...
    uint64_t startCounts = AscendC::KernelRaise::GetInstance().GetRaiseCount();
    AscendC::CHECK_FUNC_HIGHLEVEL_API(AscendDequant,
        (uint8_t, uint8_t, true, AscendC::DeQuantMode::DEQUANT_WITH_SINGLE_ROW),
