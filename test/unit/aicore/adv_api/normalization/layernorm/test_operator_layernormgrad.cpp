    ...
    uint32_t sLength, uint32_t hLength)
{
    ...
    LayerNormGrad<dataType, isReuseSource>(outputPdXLocal, resForGammaLocal, inputDyLocal, inputXLocal,
        inputVarianceLocal, inputMeanLocal, inputGammaLocal, (dataType)epsilon, layerNormGradTiling, {DataFormat::ND});
    PipeBarrier<PIPE_ALL>();
...
TEST_P(layernormgradTestSuite, layernormgradTestCase)
{
    ...
    param.cal_func(outputPdXGm, resForGammaGm, inputDyGm, inputXGm, inputVarianceGm, inputMeanGm, inputGammaGm, bLength,
        sLength, hLength);
