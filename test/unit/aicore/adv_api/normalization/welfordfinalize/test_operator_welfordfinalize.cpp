...
public:
    ...
    __aicore__ inline void Init(GM_ADDR inputMean_gm, GM_ADDR inputVariance_gm, GM_ADDR counts_gm,
        GM_ADDR outputMean_gm, GM_ADDR outputVariance_gm, uint32_t rnLength, uint32_t abLength, uint32_t head,
        uint32_t headLength, uint32_t tail, uint32_t tailLength)
    {
    ...
    __aicore__ inline void Compute()
    {
        ...
        if constexpr (isCounts) {
            if constexpr (sharedTmp) {
                WelfordFinalize<false>(meanLocal, varianceLocal, inputMeanLocal, inputVarianceLocal, inputCountsLocal,
                    tmpLocalTensor, para);
            } else {
...
__aicore__ void kernel_WelfordFinalize_test(GM_ADDR inMeanGm, GM_ADDR inVarGm, GM_ADDR countsGm, GM_ADDR outMeanGm,
    GM_ADDR outVarGm, uint32_t rnLength, uint32_t abLength, uint32_t head, uint32_t headLength, uint32_t tail,
    uint32_t tailLength)
{
...
TEST_P(WelfordFinalizeTestSuite, WelfordFinalizeTestCase)
{
    ...
    param.calFunc(inMeanGm, inVarGm, countsGm, outMeanGm, outVarGm, param.rnLength, param.abLength, param.head,
        param.headLength, param.tail, param.tailLength);
