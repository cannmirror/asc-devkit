...
INSTANTIATE_TEST_CASE_P(TEST_PACKAGE_batchnorm, batchnormTestSuite,
    ::testing::Values(batchnormTestParams{16, 16, 16, 16, sizeof(half), main_batchnorm_test<half, false, true>},
...
TEST_P(batchnormTestSuite, batchnormTestCase)
{
    ...
    for (int32_t i = 0; i < bshLength * typeSize; i++) { EXPECT_EQ(output_gm[i], 0x00); }
}