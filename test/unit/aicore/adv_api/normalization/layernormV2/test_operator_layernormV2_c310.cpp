    ...
    GM_ADDR outputMeanGm, GM_ADDR outputRstdGm, uint32_t aLength, uint32_t rLength, uint32_t rLengthWithPadding)
{
    ...
    for (int32_t i = 0; i < aLength; i++) {
        DataCopyPad(inputXLocal[i * rLengthWithPadding], inputXGlobal[i * rLengthWithPadding],
    ...
    for (int32_t i = 0; i < aLength; i++) {
        DataCopyPad(outputGlobal[i * rLengthWithPadding], outputLocal[i * rLengthWithPadding],
...
TEST_P(layernormrstdTestSuite, LayernormrstdTestCase)
{
    ...
    for (int32_t i = 0; i < aLength * rLengthWithPadding * typeSizeT; i++) { EXPECT_EQ(outputGm[i], 0x00); }
}