    ...
    TilingParams& tilingParam, int32_t isTransposeAIn, int32_t isTransposeBIn, bool enSequentialWrite)
{
    ...
    if constexpr (C_TYPE::pos == TPosition::VECCALC) {
        ...
        while (mm.Iterate()) { mm.GetTensorC(gmC, 0, enSequentialWrite); }
    }
    mm.DisableBias();
    PipeBarrier<PIPE_ALL>();
    if constexpr (A_TYPE::pos == TPosition::VECCALC) {
    ...
    {                                                                                                                                                                                                                                                                                                           \
        ...
        {                                                                                                                                                                                                                                                                                                       \
            ...
            if (cType::pos == AscendC::TPosition::CO1) {                                                                                                                                                                                                                                                        \
                main_kernel_matmul_for_L0cBufferExtend<aType, bType, cType, biasType, mmCFG>(left_global,                                                                                                                                                                                                       \
                    right_global, output_global, bias_global, tilingParams, isTransposeA, isTransposeB,                                                                                                                                                                                                         \
                    enSequentialWrite);                                                                                                                                                                                                                                                                         \
            } else if (enTiling) {                                                                                                                                                                                                                                                                              \
                main_kernel_matmul<aType, bType, cType, biasType, mmTiling>(left_global, right_global, output_global,                                                                                                                                                                                           \
                    bias_global, tilingParams, isTransposeA, isTransposeB, enSequentialWrite);                                                                                                                                                                                                                  \
            } else if (enOuter) {                                                                                                                                                                                                                                                                               \
                if (enOrderM) {                                                                                                                                                                                                                                                                                 \
                    main_kernel_matmul<aType, bType, cType, biasType, normOuterM>(left_global, right_global,                                                                                                                                                                                                    \
                        output_global, bias_global, tilingParams, isTransposeA, isTransposeB, enSequentialWrite);                                                                                                                                                                                               \
                } else {                                                                                                                                                                                                                                                                                        \
                    main_kernel_matmul<aType, bType, cType, biasType, normOuterN>(left_global, right_global,                                                                                                                                                                                                    \
                        output_global, bias_global, tilingParams, isTransposeA, isTransposeB, enSequentialWrite);                                                                                                                                                                                               \
                }                                                                                                                                                                                                                                                                                               \
            } else {                                                                                                                                                                                                                                                                                            \
                main_kernel_matmul<aType, bType, cType, biasType, mmCFG>(left_global, right_global, output_global,                                                                                                                                                                                              \
                    bias_global, tilingParams, isTransposeA, isTransposeB, enSequentialWrite);                                                                                                                                                                                                                  \
            }                                                                                                                                                                                                                                                                                                   \
...
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case1_910B1, GM, GM, GM, GM, ND, ND, ND, ND, float, float,
    float, float, 0, 0, CFG_NORM, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case1_910B1, GM, GM, GM, GM, ND, ND, ND, ND, float, float,
    float, float, 0, 0, CFG_NORM, true, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case1_910B1, GM, GM, GM, GM, ND, ND, NZ, ND, float, float,
    float, float, 0, 0, CFG_NORM, true, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case2_910B1, GM, GM, GM, GM, SCALAR, ND, ND, ND, half, half,
    float, float, 0, 0, CFG_NORM, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case3_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 0, 0, CFG_MDL, false, false, false, false); // MDL
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case3_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 1, 0, CFG_MDL, false, false, false, false); // MDL + A trans
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case3_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 0, 1, CFG_MDL, false, false, false, false); // MDL + B trans
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case3_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 0, 0, CFG_NORM, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case4_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 0, 0, CFG_NORM, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case5_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 1, 1, CFG_MDL, false, false, false, false); // tail K + B trans
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case5_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 1, 0, CFG_MDL, false, false, false, false); // tail K + B not trans
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case6_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 1, 1, CFG_MDL, false, false, false, false); // tail K + B trans
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case7_910B1, GM, GM, GM, GM, ND, ND, ND, ND, int8_t, int8_t,
    int8_t, int32_t, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case7_910B1, GM, GM, GM, GM, ND, ND, ND, ND, int8_t, int8_t,
    int8_t, int32_t, 0, 0, CFG_MDL, true, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case8_910B1, GM, GM, GM, GM, ND, ND, ND, ND, int8_t, int8_t,
    int8_t, int32_t, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case9_910B1, GM, GM, GM, GM, ND, ND, ND, ND, int8_t, int8_t,
    int8_t, int32_t, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case10_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half,
    float, float, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case11_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half,
    float, float, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case12_910B1, GM, GM, GM, GM, ND, ND, ND, ND, int8_t, int8_t,
    int32_t, int32_t, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case12_910B1, VECCALC, VECCALC, GM, GM, ND, ND, ND, ND, half,
    half, float, float, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case12_910B1, VECCALC, VECCALC, GM, GM, NZ, NZ, ND, ND, half,
    half, float, float, 0, 0, CFG_NORM, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case12_910B1, VECCALC, VECCALC, GM, GM, ND, ND, ND, ND, half,
    half, int8_t, float, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case12_910B1, VECCALC, VECCALC, GM, GM, ND, ND, ND, ND, half,
    half, int8_t, float, 0, 0, CFG_NORM, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case12_910B1, VECCALC, VECCALC, GM, GM, NZ, NZ, ND, ND, half,
    half, int8_t, float, 0, 0, CFG_MDL, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case12_910B1, VECCALC, VECCALC, GM, GM, NZ, NZ, ND, ND, half,
    half, int8_t, float, 0, 0, CFG_NORM, false, false, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case3_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 0, 0, CFG_MDL, false, true, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case4_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half, float,
    float, 0, 0, CFG_NORM, false, true, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case7_910B1, GM, GM, GM, GM, ND, ND, ND, ND, int8_t, int8_t,
    int8_t, int32_t, 0, 0, CFG_MDL, false, true, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case9_910B1, GM, GM, GM, GM, ND, ND, ND, ND, int8_t, int8_t,
    int8_t, int32_t, 0, 0, CFG_NORM, false, true, false, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case13_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half,
    float, float, 0, 0, CFG_NORM, false, false, true, true);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case14_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half,
    float, float, 1, 1, CFG_NORM, false, false, true, true);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case15_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half,
    float, float, 0, 0, CFG_NORM, false, false, true, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case16_910B1, GM, GM, GM, GM, ND, ND, ND, ND, half, half,
    float, float, 1, 1, CFG_NORM, false, false, true, false);
KERNEL_MATMUL_TESTCASE(TEST_KERNEL_MATMUL, tiling_params_case17_910B1, GM, GM, CO1, GM, ND, ND, ND, ND, half, half,
    float, float, 1, 1, CFG_NORM, false, false, false, false);
