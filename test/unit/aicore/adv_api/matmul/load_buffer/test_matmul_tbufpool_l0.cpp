...
#include "detail/matmul/stage/split/load_to_l0b/load_to_l0b.h"
#include "detail/matmul/stage/compute/mmad_compute.h"
#include "detail/matmul/resource/l0_buffer/tbuf_pool_l0.h"
...
public:
    ...
    void LoadA(LocalTensor<A_T>& l0A, const LocalTensor<A_T>& l1A, uint16_t aL1M, uint16_t aL1K, uint16_t madM,
    ...
    void LoadB(LocalTensor<A_T>& l0B, const LocalTensor<A_T>& l1B, uint16_t bL1N, uint16_t bL1K, uint16_t madN,
...
TEST_F(test_matmul_tbufpool_l0, case0)
{
    ...
    mm.LoadB(l0b, l1b, 0, 0, 16, 16, 0, 0, false);
    bufferPool.EnQue();
    bufferPool.DeQue();
