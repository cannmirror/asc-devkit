...
INSTANTIATE_TEST_CASE_P(TEST_PACKAGE_arithprogression, arithprogressionTestSuite,
    ::testing::Values(arithprogressionTestParams{1, 2, 15, sizeof(half), mainArithprogressionTest<half>},
...
TEST_P(arithprogressionTestSuite, arithprogressionTestCase)
{
    ...
    for (int32_t i = 0; i < param.count * param.typeSize; i++) { EXPECT_EQ(dstGm[i], 0x00); }
}
