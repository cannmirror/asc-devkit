...
INSTANTIATE_TEST_CASE_P(TEST_OPEARATION_ANTIQUANTWEIGHT, AntiquantWeightTestsuite,
    ::testing::Values(antiquantWeightParams{2048, 32, 64, 1024, testAntiQuantWeight<int8_t, half>},
...
TEST_P(AntiquantWeightTestsuite, testAntiquantWeight)
{
    ...
    param.cal_func(dstGm, srcGm, offsetGm, scaleGm, param.elementCountOfInput, param.elementCountOfOffset, param.K,
        param.tmpBufferSize);

    for (int32_t i = 0; i < param.elementCountOfInput; i++) { EXPECT_EQ(dstGm[i], 0x00); }
}