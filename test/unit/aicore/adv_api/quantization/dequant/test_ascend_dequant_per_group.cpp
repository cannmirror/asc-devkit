...
public:
    ...
    __aicore__ inline void Init(GM_ADDR src_gm, GM_ADDR dst_gm, GM_ADDR scale_gm, GM_ADDR offset_gm, uint32_t m_num,
        uint32_t n_num, uint32_t calCount_num, uint32_t gs_num)
    {
        m = m_num;
        n = n_num;
...
INSTANTIATE_TEST_CASE_P(TEST_OPEARATION_ASCEND_QUANT_PER_GROUP, AscendDeQuantPerGroupTestsuite,
    ::testing::Values(AscendDeQuantPerGroupTestParams{10, 256, 1280, 32, Entry<int32_t, half, half, false, 1, true>},
