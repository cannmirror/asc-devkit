...
private:
    ...
    __aicore__ inline void CopyOut()
    {
        ...
        for (int32_t i = bufferSize - remainVal; i < bufferSize; i++) { dstGlobal.SetValue(i, dstLocal.GetValue(i)); }
...
INSTANTIATE_TEST_CASE_P(TEST_OPEARATION_ADVANCE_API_POWER, AdvancePowerTestSuite,
    ::testing::Values(PowerTestParams{4, 32, 0, testPower<float, 1>}, PowerTestParams{4, 4096, 1, testPower<float, 1>},
...
TEST_P(AdvancePowerTestSuite, testPower)
{
    ...
    for (int32_t i = 0; i < inDataSize; i++) { EXPECT_EQ(outputGm[i], 0x00); }
}
