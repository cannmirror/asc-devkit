...
template <typename T>
void main_vec_hypot_level2_demo(__gm__ uint8_t* __restrict__ dstGm, __gm__ uint8_t* __restrict__ src0Gm,
    __gm__ uint8_t* __restrict__ src1Gm, uint32_t dataSize, TestMode testMode)
{
    ...
    for (int32_t i = 0; i < dataSize; ++i) { input1_local.SetValue(i, 1); }
