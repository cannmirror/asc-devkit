...
TEST_F(TestTiling, TestSoftMaxFlashV2TilingMaxMinTmpSize)
{
    ...
    softmaxflashV2NeedMinLength = GetSoftMaxFlashV2MinTmpSize(
        ascendcPlatform, softmaxShape, dataTypeSize1, dataTypeSize2, isUpdate, isBasicBlock, isFlashOutputBrc);
    ...
    softmaxflashV2NeedMinLength = GetSoftMaxFlashV2MinTmpSize(
        ascendcPlatform, softmaxShape, dataTypeSize1, dataTypeSize2, isUpdate, isBasicBlock, isFlashOutputBrc);
    EXPECT_EQ(softmaxflashV2NeedMinLength, 29568);
    ...
    softmaxflashV2NeedMinLength = GetSoftMaxFlashV2MinTmpSize(
        ascendcPlatform, softmaxShape, dataTypeSize1, dataTypeSize2, isUpdate, isBasicBlock, isFlashOutputBrc);
    EXPECT_EQ(softmaxflashV2NeedMinLength, 4352);
    ...
    softmaxflashV2NeedMinLength = GetSoftMaxFlashV2MinTmpSize(
        ascendcPlatform, softmaxShape, dataTypeSize1, 1, isUpdate, isBasicBlock, isFlashOutputBrc);
    EXPECT_EQ(softmaxflashV2NeedMinLength, 0);

    softmaxflashV2NeedMaxLength = GetSoftMaxFlashV2MaxTmpSize(
        ascendcPlatform, softmaxShape, 1, dataTypeSize2, isUpdate, isBasicBlock, isFlashOutputBrc);
...
TEST_F(TestTiling, TestSoftMaxFlashV2Tiling)
{
    ...
    softmaxflashV2NeedMinLength =
        GetSoftMaxFlashV2MinTmpSize(ascendcPlatform, softmaxShape, inputTypeSize, maxSumTypeSize, true, false);
    EXPECT_EQ(softmaxflashV2NeedMinLength, (128 * 2 + 128 * (128 + 16 * 2)) * 4);
    softmaxflashV2NeedMinLength =
        GetSoftMaxFlashV2MinTmpSize(ascendcPlatform, softmaxShape, inputTypeSize, maxSumTypeSize, false, true);
    ...
    softmaxflashV2NeedMaxLength =
        GetSoftMaxFlashV2MaxTmpSize(ascendcPlatform, softmaxShape, inputTypeSize, maxSumTypeSize, true, false);
    EXPECT_EQ(softmaxflashV2NeedMaxLength, 128 * (128 + 64 + 16 * 2) * 4);
    softmaxflashV2NeedMaxLength =
        GetSoftMaxFlashV2MaxTmpSize(ascendcPlatform, softmaxShape, inputTypeSize, maxSumTypeSize, false, true);
    EXPECT_EQ(softmaxflashV2NeedMaxLength, 128 * (128 + 64 + 16) * 4);
...
TEST_F(TestTiling, TestWelfordUpdateTiling)
{
    ...
    gert::TilingContext* context = fe::GetFakeTilingContext();
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(context->GetPlatformInfo());
    GetWelfordUpdateMaxMinTmpSize(
        shape1d, dtypeTSize, dtypeUSize, isReuseSource, false, ascendcPlatform, maxSize, minSize);
    EXPECT_EQ(minSize, 0);
    EXPECT_EQ(maxSize, 0);
}
...
TEST_F(TestTiling, TestLayerNormRstdTiling)
{
    ...
    GetLayerNormMaxMinTmpSize(
        layernormShape, typeSize, isReuseSource, isComputeRstd, isOnlyOutput, ascendcPlatform, maxValue, minValue);
    EXPECT_EQ(maxValue, 128 * typeSize + 128 * typeSize);
    EXPECT_EQ(minValue, 128 * typeSize + 128 * typeSize);
    GetNormalizeMaxMinTmpSize(layernormShape, typeSize, typeSize, isReuseSource, isComputeRstd, isOnlyOutput,
        ascendcPlatform, maxValue, minValue);
    EXPECT_EQ(maxValue, 0);
    EXPECT_EQ(minValue, 0);
    GetLayerNormNDTilingInfo(
        layernormShape, stackBufferSize, typeSize, isReuseSource, isComputeRstd, ascendcPlatform, tiling);
...
{
    matmul_tiling::PlatformInfo plat{.socVersion = platform_ascendc::SocVersion::ASCEND910B,
        .l1Size = 524288,
        .l0CSize = 131072,
        .ubSize = 196608,
        .l0ASize = 65536,
...
{
    matmul_tiling::PlatformInfo plat{.socVersion = platform_ascendc::SocVersion::ASCEND310P,
        .l1Size = 1048576,
        .l0CSize = 262144,
        .ubSize = 262144,
        .l0ASize = 65536,
...
TEST_F(TestTiling, TestPowerTilingWithConfig)
{
    ...
    EXPECT_EQ(maxVal, 512 * 4 * 6);
    EXPECT_EQ(minVal, 256 * 6);
    GetPowerMaxMinTmpSize(plat, intrinsicConfig, powerShape, powerShape, false, 2, false, maxVal, minVal);
    EXPECT_EQ(maxVal, 512 * 2 * 14 + 256);
    EXPECT_EQ(minVal, 256 * 7 + 256);
    std::vector<int64_t> scalar_shape = {1};
    auto scalarShape = ge::Shape(scalar_shape);
    ...
    EXPECT_EQ(maxVal, 512 * 4 * 7);
    EXPECT_EQ(minVal, 256 * 7);
    GetPowerMaxMinTmpSize(plat, doubleFloatTechConfig, powerShape, scalarShape, false, 4, false, maxVal, minVal);
    EXPECT_EQ(maxVal, 512 * 4 * 5 + 256);
    EXPECT_EQ(minVal, 256 * 5 + 256);
}
...
TEST_F(TestTiling, TestPowerTilingFactorSizeWithConfig)
{
    ...
    EXPECT_EQ(maxLiveNodeCnt, 5);
    EXPECT_EQ(extraBuf, 256);
    GetPowerTmpBufferFactorSize(plat, intrinsicConfig, false, true, true, 4, maxLiveNodeCnt, extraBuf);
    EXPECT_EQ(maxLiveNodeCnt, 7);
    EXPECT_EQ(extraBuf, 0);
    GetPowerTmpBufferFactorSize(plat, intrinsicConfig, false, true, false, 2, maxLiveNodeCnt, extraBuf);
    EXPECT_EQ(maxLiveNodeCnt, 14);
    EXPECT_EQ(extraBuf, 256);
    GetPowerTmpBufferFactorSize(plat, doubleFloatTechConfig, true, true, false, 4, maxLiveNodeCnt, extraBuf);
    EXPECT_EQ(maxLiveNodeCnt, 4);
    EXPECT_EQ(extraBuf, 256);
    GetPowerTmpBufferFactorSize(plat, doubleFloatTechConfig, true, true, true, 4, maxLiveNodeCnt, extraBuf);
    EXPECT_EQ(maxLiveNodeCnt, 6);
    EXPECT_EQ(extraBuf, 0);
    GetPowerTmpBufferFactorSize(plat, doubleFloatTechConfig, true, true, false, 2, maxLiveNodeCnt, extraBuf);
    EXPECT_EQ(maxLiveNodeCnt, 14);
    EXPECT_EQ(extraBuf, 256);
}
