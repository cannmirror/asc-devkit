...
INSTANTIATE_TEST_CASE_P(TEST_OPEARATION_SUM, MeanTestSuite,
    ::testing::Values(MeanTestParams{{1, 32, 2}, sizeof(half), main_Mean_test<half, float>},
...
TEST_P(MeanTestSuite, MeanTestCase)
{
    ...
    for (int32_t i = 0; i < param.meanParams.outter; i++) { EXPECT_EQ(dstGm[i], 0x00); }
}
