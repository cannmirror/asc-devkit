...
asc_reduce_config是reduce类操作中控制操作数地址步长的数据结构。具体使用方法可参考[如何使用高维切分计算API](../general_instruction.md#如何使用高维切分计算api)。
...
|字段名|字段含义|
|----------|----------|
| dst_repeat_stride | 目的操作数相邻迭代间的地址步长。<br>输入类型位宽为16bit时，单位为16Byte，输入类型位宽为32bit时，单位为32Byte。默认值：1。 |
