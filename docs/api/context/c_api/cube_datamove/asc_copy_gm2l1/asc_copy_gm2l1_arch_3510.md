...
- 填充模式
    ...
    ```cpp
    __aicore__ inline void asc_copy_gm2l1(__cbuf__ void* dst, __gm__ void* src, uint32_t burst_num, uint32_t burst_len, uint8_t pad_func_mode, uint64_t src_stride, uint32_t dst_stride)
    ```

- 填充模式（同步）

    ```cpp
    __aicore__ inline void asc_copy_gm2l1_sync(__cbuf__ void* dst, __gm__ void* src, uint32_t burst_num, uint32_t burst_len, uint8_t pad_func_mode, uint64_t src_stride, uint32_t dst_stride)
    ```
...
| burst_len | 输入 | 待搬运的每个连续传输数据块的长度，单位为32个字节。 |
...
```cpp
//待搬运的连续传输数据块个数为1
constexpr uint32_t burst_num = 1;
//待搬运的每个连续传输数据块的长度为128个字节
constexpr uint32_t burst_len = 4;
//不对数据做padding
...
asc_copy_gm2l1(dst, src, burst_num, burst_len, pad_func_mode, src_stride, dst_stride);
```
