...
根据输入的行数和列数，生成[层次化表述法](../../../Layout和层次化表述法.md)的NZ Layout结构，该结构为连续的NZ Layout结构，即数据紧密排列。
...
|参数名| 输入/输出 |描述|
|--|--|--|
|T|输入|基础数据类型，例如half或者float。|
|row|输入|把数据看做连续的二维数据时，对应的行数。|
|column|输入|把数据看做连续的二维数据时，对应的列数。|
