...
LocalTensor用于存放AI Core中Local Memory（内部存储）的数据，它包含[ViewEngine]()（数据指针）和[Layout]()（布局信息），支持多维数据的存储和访问。
...
## 调用示例
  ...
  // 示例 4: GM 内存张量
  auto gmTensor = MakeTensor(         // 创建 GM 内存张量
      MakeGMmemPtr<half>(gmAddr),     // 省略gmAddr初始化
      MakeDNLayout<half>(128, 128));
  auto gmValue = gmTensor(10, 20);    // 访问 GM 数据
