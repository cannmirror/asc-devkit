...
| 参数名 | 描述 |
| -- | -- |
| T | 目的操作数的数据类型，通过MakeTensor构造的[LocalTensor类型](../struct/tensor/Tensor.md)，存储位置支持L1，数据格式支持ND、NZ、ZN、ZZ和NN。 |
| U | 源操作数的数据类型，通过MakeTensor构造的[LocalTensor类型](../struct/tensor/Tensor.md)，存储位置支持GM，数据格式仅支持ND、DN、NZ、ZN、ZZ、NN、ScaleAND、ScaleADN、ScaleBND和ScaleBDN。 |
...
VerifyingDataCopyTemplate表达式为模板参数T和U的校验表达式，VerifyingDataCopyTemplateWithCoord表达式为模板参数T、U和Coord的校验表达式，执行效果如下：

- 校验表达式成立，表明输入参数符合DataCopy接口入参数据类型约束，DataCopy函数表达式返回值为void。
- 校验表达式不成立，表明输入参数不符合DataCopy接口入参数据类型约束时，DataCopy函数声明失效，继而会导致编译错误。
...
更多样例请参考[TensorAPI样例代码](../../../../../examples/02_features/05_tensor_api)。
