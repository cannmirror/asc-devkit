...
Tensor API通过`Copy`接口统一执行不同通路数据搬运。该接口用于将L1 Buffer中的矩阵数据搬运到L0B Buffer。

该通路以512 Byte数据分形为基本搬运单位。不同数据类型对应的单分形矩阵形态如下：
...
接口支持非转置搬运和转置搬运。源张量和目的张量的Layout需要匹配当前通路支持的布局组合。
...
| `dst` | 输出 | 目的张量，存储位置必须为`Location::L0B`，即L0B Buffer。目的地址需要满足L0B Buffer的分形和对齐约束。 |
| `src` | 输入 | 源张量，存储位置必须为`Location::L1`，即L1 Buffer。源地址需要满足L1 Buffer的分形和对齐约束。 |
