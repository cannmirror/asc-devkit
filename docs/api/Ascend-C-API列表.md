...
Ascend C 提供了一套层次化的API 体系，涵盖了从底层C 扩展到高阶C++ 类库的完整能力。它支持开发者以标准C/C++ 语法为基础，在AI Core\(SIMD/SIMT\) 及AI CPU 等多种编程模型下，灵活实现精细化的内存管理与高效的矢量/矩阵运算。

## API分类总览

下表展示了Ascend C API 的总体分类，帮助开发者根据编程模型和功能需求快速定位所需API。
