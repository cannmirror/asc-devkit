...
- 当repeat=1 时，表示进行一次迭代，可自由设置element_length_..., if_exhausted_suspension, valid_bit参数。
- 当repeat>1 时，表示进行多次迭代，必须满足以下条件：包含四条源队列，四个源队列的长度一致且在内存中连续存储，valid_bit的值为15，if_exhausted_suspension的值为false。
