...
| [asc_atomic_add](标量计算/asc_atomic_add.md) | 对Global Memory中的数据与指定数据执行原子加操作。 |
| [asc_atomic_sub](标量计算/asc_atomic_sub.md) | 对Global Memory中的数据与指定数据执行原子减操作。 |
| [asc_atomic_or](标量计算/asc_atomic_or.md) | 对Global Memory中的数据与指定数据执行原子或操作。 |
| [asc_atomic_xor](标量计算/asc_atomic_xor.md) | 对Global Memory中的数据与指定数据执行原子异或操作。 |
| [asc_atomic_and](标量计算/asc_atomic_and.md) | 对Global Memory中的数据与指定数据执行原子与操作。 |
| [asc_atomic_max](标量计算/asc_atomic_max.md) | 对Global Memory中的数据与指定数据执行原子求最大值操作。 |
| [asc_atomic_min](标量计算/asc_atomic_min.md) | 对Global Memory中的数据与指定数据执行原子求最小值操作。 |
| [asc_atomic_inc](标量计算/asc_atomic_inc.md) | 对Global Memory中address指向的计数器执行原子递增操作，如果address上的数值大于等于指定数值val，则对address赋值为0，否则将address上数值加1。 |
| [asc_atomic_dec](标量计算/asc_atomic_dec.md) | 对Global Memory中address指向的计数器执行原子递减操作，如果address上的数值等于0或大于指定数值val，则对address赋值为val，否则将address上数值减1。 |
| [asc_atomic_exch](标量计算/asc_atomic_exch.md) | 对Global Memory中address指向的元素执行原子赋值操作。 |
| [asc_atomic_cas](标量计算/asc_atomic_cas.md) | 对Global Memory中address指向的元素执行原子比较赋值操作，如果address上的数值等于指定数值compare，则对address赋值为指定数值val，否则address的数值不变。 |
...
| <cann-filter npu_type = "950" > [asc_set_store_atomic_config_v2（废弃）](原子操作/asc_set_store_atomic_config_v2_deprecated.md)| 设置原子操作启用位与原子操作类型的值，适用于Ascend 950PR/Ascend 950DT。</cann-filter>|
