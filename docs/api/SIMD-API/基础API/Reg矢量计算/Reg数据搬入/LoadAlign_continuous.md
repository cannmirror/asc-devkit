...
**图1**  数据搬入分布模式图示<a id="fig-loadalign-dist"></a>  
...
| postUpdateStride | 输入 | 地址更新步长，int32_t类型，单位：元素个数。postUpdateStride * sizeof(T)的对齐约束请参考[表3 LoadDist参数说明（单搬入模式）](#表3-LoadDist参数说明（单搬入模式）)。根据postMode的取值有两种情况：<br>&bull; POST_MODE_NORMAL：不支持。<br>&bull; POST_MODE_UPDATE：实际搬运UB起始地址为srcAddr，搬运后执行地址更新srcAddr += postUpdateStride。 |
...
- 当RegTensor模板参数RegTrait为RegTraitNumOne和RegTraitNumTwo时，支持情况如下：
    ...
    | RegTraitNumTwo | &bull; 单搬入 普通搬运接口<br>&bull; 单搬入PostUpdate扩展搬运接口 | complex32/b64 |
...
**图2**  regTrait为RegTraitNumOne时的三类搬运接口调用方式  
