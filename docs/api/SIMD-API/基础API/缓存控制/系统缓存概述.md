...
## Cache大小与Cache Line大小<a name="zh-cn_topic_0000002583420201_section2339145717205"></a>

Cache Line（缓存行）是缓存的最小操作单元。访问内存时，会将相邻数据作为一个Cache Line加载到缓存中，利用空间局部性提升访问效率。Cache Line大小决定了Cache中地址的对齐要求，若数据跨两个Cache Line，会触发多次缓存访问。

<cann-filter npu-type="910b,A3">

以[NPU架构版本2201](https://gitcode.com/cann/asc-devkit/blob/master/docs/guide/编程指南/语言扩展层/SIMD-BuiltIn关键字.md)为例，表2展示了该硬件架构下三类Cache的Cache Line大小和Cache大小。

**表 2**  Cache Line大小与Cache大小

| Cache类型 | Cache Line大小 | Cache大小 |
|------------|---------------|-----------|
| L2 Cache | 256B | 192MB |
| DCache | 64B | 32KB |
| ICache | 2KB | 32KB |
...
| Invalid | &bull; 将指定地址范围的Cache Line标记为“无效”，使其从缓存中移除。<br>&bull; 保证下一次访问这些内存地址时，会从GM重新加载数据，而不是使用可能过期的缓存数据。<br>&bull; 注意：Invalid操作不检查缓存行是否为“脏”（dirty，表示该数据已被修改但尚未写回到GM），若存在脏数据将被直接丢弃。 |
...
表4按照Cache类型（L2 Cache、DCache、ICache）汇总了与缓存控制相关的接口及其功能说明。
