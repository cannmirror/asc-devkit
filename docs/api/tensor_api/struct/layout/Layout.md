...
| 参数名 | 描述 |
| -------- | ------ |
| T | [Std::tuple](../../../Utils-API/CPP标准库/容器函数/容器函数.md)结构类型，用于定义数据的逻辑形状，例如二维矩阵的行数和列数或多维张量的各维度大小。 |
| U | [Std::tuple](../../../Utils-API/CPP标准库/容器函数/容器函数.md)结构类型，用于定义各维度在内存中的步长，即同维度相邻元素在内存中的间隔，间隔的单位为元素，与Shape的维度信息一一对应。 |
...
- 功能说明
  ...
  使用给定shape和stride构造Layout对象。构造Layout对象时传入的Shape和Stride结构，需是[`Std::tuple`](../../../Utils-API/CPP标准库/容器函数/容器函数.md)结构类型，且满足Std::tuple结构类型的使用约束。
