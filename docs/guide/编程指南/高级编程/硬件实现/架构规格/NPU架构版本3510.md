...
-   核间同步
    ...
    -   **一致性要求**
        ...
        CrossCoreSetFlag 的模板参数和flagId必须与CrossCoreWaitFlag完全一致，否则视为不同的flagId。例如，CrossCoreSetFlag<0x0, PIPE\_MTE3\>\(0x8\) 和CrossCoreSetFlag<0x2, PIPE\_FIX\>\(0x8\) 设置的不是同一个flagId。
