...
-   **通过CMake方式仿真编译算子**
    -   方法一：灵活控制不同的target是否开启仿真编译。修改CMakeList，使用target\_link\_libraries与target\_link\_directories手动配置链接库与路径：
    ...
    -   方法二：通过向CMake传入变量CMAKE\_ASC\_RUN\_MODE和CMAKE\_ASC\_ARCHITECTURES来统一开启仿真编译。命令示例如下：
