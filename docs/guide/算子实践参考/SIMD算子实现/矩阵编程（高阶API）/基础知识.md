...
>[!NOTE]说明 
>本节内容为使用高阶API进行矩阵乘法的编程指导。使用高阶API进行实际的矩阵编程时，需要通过[API参考](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/context/矩阵计算-28.md)确认接口支持的产品型号。
...
在完成Matmul矩阵乘法时，主要涉及到两种分形格式ND和[NZ](../../../编程指南/概念原理和术语/神经网络和算子/数据排布格式.md#li19960204116136)。其它的数据格式请参考[数据排布格式](../../../编程指南/概念原理和术语/神经网络和算子/数据排布格式.md)。
...
-   NZ：为满足AI Core中Cube计算单元高性能计算的需要，引入该特殊格式。
    ...
    关于矩阵ND到NZ格式转换的样例请参考[Matmul输入矩阵ND到NZ格式转换的算子样例](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/03_libraries/00_matrix/matmul_nz)。
