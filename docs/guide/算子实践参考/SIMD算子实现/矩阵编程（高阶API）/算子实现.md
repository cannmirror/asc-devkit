...
1.  **创建Matmul对象**
    ...
    -   默认为MIX模式（包含矩阵计算和矢量计算），该场景下通常不定义ASCENDC\_CUBE\_ONLY宏，如果在程序中使用了ASCENDC\_CUBE\_ONLY宏，则必须使用ASCEND\_IS\_AIC宏和ASCEND\_IS\_AIV宏将Cube计算和Vector计算隔离开，更多内容请参考[融合算子编程](../融合算子编程/)。
...
2.  **初始化操作。**
    ...
    >    ```
    >-   若算子工程不是[自定义算子工程](../../../编程指南/附录/工程化算子开发/概述.md)，也不是带有[HAVE\_WORKSPACE](../../../编程指南/附录/基于样例工程完成Kernel直调.md#table481718169817)编译宏的[Kernel直调算子工程](../../../编程指南/附录/基于样例工程完成Kernel直调.md)，框架不会自动设置workspace，需要在kernel侧的Matmul初始化前，通过SetSysWorkSpace设置系统workspace。
    >    ```
