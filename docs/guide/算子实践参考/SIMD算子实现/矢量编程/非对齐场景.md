...
-   **通过Duplicate逐行清零**。计算前，针对每一行数据，调用基础API [Duplicate](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/基础API/Memory矢量计算/数据填充/Duplicate.md)对冗余数据位置填充0值。
...
-   **使用UnPad接口去除冗余数据后搬出**。待搬出的有效数据总长度满足32字节对齐时，可使用高阶API [UnPad](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/高阶API/张量变换/UnPad.md)接口去除冗余数据并完整搬出。
-   **使用GatherMask收集有效数据后搬出**。待搬出的有效数据总长度大于等于32字节时，可使用[GatherMask](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/基础API/Memory矢量计算/比较与选择/GatherMask.md)重新收集有效数据，保证搬出的有效数据起始地址和数据长度32字节对齐。
-   **带冗余数据搬出**。注意多核处理时开启原子加（使用[SetAtomicAdd](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/基础API/原子操作/SetAtomicAdd.md)接口），防止数据踩踏。
...
-   <a name="zh-cn_topic_0000001945665465_li1891184254913"></a>样例二：[通过Duplicate逐行清零+带冗余数据搬出](https://gitcode.com/cann/asc-devkit/tree/master/examples/01_simd_cpp_api/02_features/03_basic_api/02_memory_vector_compute/abs_duplicate)。
    ...
    所以在初始化时，需要对GM数据进行清零，清零代码如下，示例中多个核都调用[Fill](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/SIMD-API/高阶API/张量变换/Fill-97.md)接口进行清零，需要调用SyncAll进行核间同步。
