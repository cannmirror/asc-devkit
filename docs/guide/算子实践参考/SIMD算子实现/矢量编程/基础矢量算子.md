...
1.  明确算子的数学表达式及计算逻辑。
    ...
    计算逻辑是：Ascend C提供的[矢量计算接口](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0035.html)的操作元素都为[LocalTensor](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0006.html)，输入数据需要先从外部存储（Global Memory）搬运进片上存储（Unified Buffer），然后使用计算接口完成两个输入参数相加，得到最终结果，再搬出到外部存储上。Ascend C  Add算子的计算逻辑如下图所示。
...
4.  确定算子实现所需接口。
    -   实现涉及外部存储和内部存储间的数据搬运，查看Ascend C  API参考中的数据搬运接口，需要使用[DataCopy](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0101.html)来实现数据搬运。
    -   本样例只涉及矢量计算的加法操作，查看Ascend C  API参考中的[矢量计算](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0023.html)接口，初步分析可使用基础算术Add接口[Add](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0035.html)实现x+y。
    -   使用[Queue队列](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0137.html)管理计算中使用的Tensor数据结构，具体使用[EnQue](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0140.html)、[DeQue](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0141.html)等接口。
...
-   任务间交互使用到的内存、临时变量的内存统一使用[Pipe内存管理对象](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0108.html)进行管理。
...
-   设置输入输出Global Tensor的Global Memory内存地址。
    ...
    本样例中的分配方案是：数据整体长度TOTAL\_LENGTH为1 \* 2048，使用GlobalTensor类的[SetGlobalBuffer](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_00024.html)接口设定该核上Global Memory的起始地址以及长度。

    ```
    xGm.SetGlobalBuffer((__gm__ half *)x, TOTAL_LENGTH);
    ```

-   通过[Pipe内存管理对象](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0108.html)为输入输出Queue分配内存。
...
2.  Stage2：Compute函数实现。
    ...
    4.  使用[FreeTensor](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/910beta1/API/ascendcopapi/atlasascendc_api_07_0139.html)释放不再使用的LocalTensor。
