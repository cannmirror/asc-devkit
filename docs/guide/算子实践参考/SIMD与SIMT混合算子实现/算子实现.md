...
4.  明确分核策略、SIMT线程配置和SIMD Reg矢量计算API循环调用次数。
    ...
    对于SIMD Reg矢量计算实现，单核处理数据量为1024，Reg矢量计算API单次处理的数据长度one\_repeat\_size为[GetVecLen](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/context/GetVecLen.md)/sizeof\(float\)，API的循环调用次数repeat\_times为1024/one\_repeat\_size。
...
3.  启动SIMD VF函数simd\_adds，对Unified Buffer上的数据做加1计算。
    ...
    2.  使用[GetVecLen](https://gitcode.com/cann/asc-devkit/blob/master/docs/api/context/GetVecLen.md)接口除以单个数据长度，计算Reg矢量计算API单次处理的数据量one\_repeat\_size。使用单核应处理的数据量index\_total\_length\_per\_block除以单次处理数据量one\_repeat\_size，计算Reg矢量计算API循环调用次数。
