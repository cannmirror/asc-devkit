...
2.  启动SIMT VF函数simt\_gather，从input中获取指定索引的数据。
    ...
    2.  使用[Alloc](../../../api/SIMD-API/基础API/资源管理/内存管理/LocalMemAllocator/Alloc.md)接口申请Unified Buffer内存空间，并将该Tensor作为simt\_gather函数的输出。
...
3.  启动SIMD VF函数simd\_adds，对Unified Buffer上的数据做加1计算。
    ...
    1.  使用[Alloc](../../../api/SIMD-API/基础API/资源管理/内存管理/LocalMemAllocator/Alloc.md)接口申请Unified Buffer内存空间，并将该Tensor作为simt\_adds函数的输出。
