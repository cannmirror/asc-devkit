# SIMT编程模式实现Scatter算子样例

## 概述

样例基于Ascend C SIMT编程方式实现一维Scatter算子，将输入数组中的元素按照`index`指定的位置离散写入输出数组。样例支持配置不同的一维输入长度和输出长度，默认构造`input_length = 10`、`output_length = 50`的稀疏写入场景。该样例重点展示SIMT编程模式适合处理离散访存、不规则访存这类场景：每个线程可以按照独立的`index`计算自己的写回地址，直接表达“按元素分散写入”的并行逻辑。

对于Scatter这类算子，输出地址通常不是连续递增的，而是由输入索引动态决定。SIMT编程方式可以让一个线程负责一个或多个元素，线程内部完成索引读取、边界判断和离散写回，避免把不规则地址映射强行整理成连续访问流程。
