 ...
 * \brief
 */
#ifndef ACT_INCLUDE_MATMUL_BLOCK_SPARSE_BLOCK_MMAD_MULTI_BLOCK_ON_KAXIS_WITH_LAYOUT_H
#define ACT_INCLUDE_MATMUL_BLOCK_SPARSE_BLOCK_MMAD_MULTI_BLOCK_ON_KAXIS_WITH_LAYOUT_H
...
namespace Block {
/*
    Data types: float16 / bfloat16_t
    Fromat：CubeFormat::ND
*/
        ...
        for (auto mIndex = 0; mIndex < mIter; ++mIndex) {
            for (auto nIndex = 0; nIndex < nIter; ++nIndex) {
                ...
                for (auto kOuterIndex = 0; kOuterIndex < kOuterIter; ++kOuterIndex) {
                    ...
                    auto l1BIdx = qidL1BIdx_.template AllocTensor<L1IdxTensorTrait>();
                    l1BIdx.SetTensorTrait(L1IdxTensorTrait(l1BIndexLayout));
                    ...
                    copyGmToB1Idx(l1BIdx, const_cast<AscendC::GlobalTensor<SparseIdxTrait>&>(sparseIdx),
                                  AscendC::MakeCoord(bL1Row * L0_N, bL1Col * L1_K / INDEX_MATRIX_OFFSET));

                    qidL1B_.EnQue(l1B);
                    qidL1BIdx_.EnQue(l1BIdx);
                    qidL1A_.DeQue();
                    qidL1B_.DeQue();
                    qidL1BIdx_.DeQue();
                    ...
                    for (auto kInnerIndex = 0; kInnerIndex < kInnerIter; ++kInnerIndex) {
                        ...
                        copyB1ToB2_(l0B, l1B, AscendC::MakeCoord(0, bL0Col * L0_K / DENSE_MATRIX_B_OFFSET), l1BIdx);
                        ...
                        l0ALocal.SetAddr(l0A.address_);
                        l0BLocal.SetAddr(l0B.address_);
