...
#ifndef ACT_INCLUDE_MATMUL_BLOCK_BLOCK_MMAD_MULTI_BLOCK_BIAS_WITH_LAYOUT_H
#define ACT_INCLUDE_MATMUL_BLOCK_BLOCK_MMAD_MULTI_BLOCK_BIAS_WITH_LAYOUT_H
...
template <class L1TileShape_, class L0TileShape_, class AType_, class BType_, class CType_, class BiasType_,
          class TileCopy_>
class BlockMmad<
    MatmulMultiBlockBiasWithLayout<>, L1TileShape_, L0TileShape_, AType_, BType_, CType_, BiasType_, TileCopy_,
    AscendC::Std::enable_if_t<
        !AscendC::Std::is_same_v<TileCopy_, Tile::TileCopy<Arch::Ascend910_95, Tile::CopyOutSplitMWithParams>> &&
        !AscendC::Std::is_same_v<TileCopy_, Tile::TileCopy<Arch::Ascend910_95, Tile::CopyOutSplitNWithParams>>>> {
public:
...
public:
    static_assert(AscendC::PhyPosIsUB(CType::pos), "Only output to UB support splitM or splitN");
...
public:
    ...
    __aicore__ inline void IterateAll(DstTensor& c, const SrcATensor& a, const SrcBTensor& b, const Shape& actualShape)
    {
        static_assert(AscendC::is_global_tensor_v<SrcATensor> && AscendC::is_global_tensor_v<SrcBTensor>,
                      "Only support GM input");
        static_assert(AscendC::is_local_tensor_v<DstTensor>, "Output only support local tensor");
        ...
        AscendC::GlobalTensor<typename BType::T> bGlobal;
        bGlobal.address_ = b.address_;
        AscendC::LocalTensor<typename CType::T> cLocal;
        cLocal.SetAddr(c.address_);
        ...
        matmul_.SetSingleShape(Get<MNK_M>(actualShape), Get<MNK_N>(actualShape), Get<MNK_K>(actualShape));
        matmul_.IterateAll(cLocal);

        c.SetAddr(cLocal.address_);
    }
