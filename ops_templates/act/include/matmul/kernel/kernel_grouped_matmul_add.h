...
constexpr int64_t MATRIX_INNER_DIM_LIMIT_SIZE_ARCH35 = 2097151L;
...
public:
    ...
    __aicore__ inline uint64_t GetSplitValueFromGroupList(uint64_t groupIdx, uint64_t groupListType)
    {
    ...
    __aicore__ inline bool Init(const Params& params, uint64_t groupIdx)
    {
        ...
        uint64_t splitValue = GetSplitValueFromGroupList(groupIdx, params.gmmParams.groupListType);
    ...
    __host_aicore__ static Status CheckShape(const ProblemShape& shape)
    {
        ...
        if (!transA && k > MATRIX_INNER_DIM_LIMIT_SIZE_ARCH35) { // mk matrix k limit
            return Status::mkErrorMatrixExceedsLimit;
        }

        if (transA && m > MATRIX_INNER_DIM_LIMIT_SIZE_ARCH35) { // km matrix m limit
            return Status::kmErrorMatrixExceedsLimit;
        }
        if (!transB && n > MATRIX_INNER_DIM_LIMIT_SIZE_ARCH35) { // kn matrix n limit
            return Status::knErrorMatrixExceedsLimit;
        }

        if (transB && k > MATRIX_INNER_DIM_LIMIT_SIZE_ARCH35) { // nk matrix k limit
