...
本样例介绍了使用了带L2切分的错位分核调度器的Matmul算子。
- 算子功能：基于Layout实现的带L2切分的使用错位分核策略的Matmul算子。
...
在样例目录`examples/04_l2_misplace_core_matmul`下执行run.sh脚本命令，可参考[quickstart](../../docs/01_quickstart.md#编译运行)了解执行脚本参数说明。
```
# cd examples/04_l2_misplace_core_matmul
