...
本样例介绍了对带L2切分的错位分核的Matmul结果进行量化计算的构建。
- 算子功能：基于Layout实现的带L2切分的使用错位分核策略的Matmul算子，并将int8_t的结果量化为float16类型。
...
在样例目录`examples/06_l2_misplace_core_quant_matmul`下执行run.sh脚本命令，可参考[quickstart](../../docs/01_quickstart.md#编译运行)了解执行脚本参数说明。本样例中的量化操作在Vector Core上进行epilogue操作，run.sh脚本会将[`device_matmul.h`](../../include/matmul/device/device_matmul.h)里的kernel type由KERNEL_TYPE_AIC_ONLY改成KERNEL_TYPE_MIX_AIC_1_2，在样例执行结束后，run.sh脚本将kernel type改回原值。
```
