...
本样例介绍了使用了错位分核调度器的BatchMatmul算子。
- 算子功能：基于Layout实现的使用错位分核策略的BatchMatmul算子。
- 使用的BlockMmad模板：MatmulMultiBlockWithLayout。
- 使用的scheduler模板：MisplaceCoreScheduler。
- 调用方式：<<<>>>直调。
...
在样例目录`examples/02_batch_matmul`下执行run.sh脚本命令，可参考[quickstart](../../docs/01_quickstart.md#编译运行)了解执行脚本参数说明。
```
# cd examples/02_batch_matmul
