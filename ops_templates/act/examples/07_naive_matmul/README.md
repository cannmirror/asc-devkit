...
本样例介绍了使用了基础K轴遍历调度器的Matmul算子。
- 算子功能：简单for循环的基础K轴遍历Matmul算子。
- 使用的BlockMmad模板：MatmulNaivePipelineWithLayout。
...
在样例目录`examples/07_naive_matmul`下执行run.sh脚本命令，可参考[quickstart](../../docs/01_quickstart.md#编译运行)了解执行脚本参数说明。
```
# cd examples/07_naive_matmul
