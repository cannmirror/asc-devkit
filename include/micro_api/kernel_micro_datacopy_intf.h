...
__simd_callee__ inline void DataCopy(
    U &dstReg, __ubuf__ T *&srcUbAddr, uint32_t dataBlockStride, uint32_t repeatStride, MaskReg &mask);
...
template <typename T = DefaultType, DataCopyMode dataMode, PostLiteral postMode, typename U>
__simd_callee__ inline void DataCopy(
    __ubuf__ T *&dstUbAddr, U &srcReg, uint32_t dataBlockStride, uint32_t repeatStride, MaskReg &mask);

// vldas/vldus
template <typename T> __simd_callee__ inline void DataCopyUnAlignPre(UnalignReg &ureg, __ubuf__ T *srcUbAddr);
