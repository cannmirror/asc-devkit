 ...
 */
#ifndef AICORE_ADV_API_QUANTIZATION_ASCEND_QUANT_H
#define AICORE_ADV_API_QUANTIZATION_ASCEND_QUANT_H
#include "quantization/ascend_quant_utils.h"
#if __CCE_AICORE__ == 100 || __CCE_AICORE__ == 220 || __CCE_AICORE__ == 200 || defined(__DAV_C310__)                   \
    || defined(__DAV_310R6__)
#include "kernel_tensor.h"
#include "detail/quantization/quant/ascend_quant_common_impl.h"
namespace AscendC {
#pragma begin_pipe(V)
...
{
    AscendQuantImpl<T, isReuseSource>(dstTensor, srcTensor, sharedTmpBuffer, scaleTensor, offset, scaleCount, calCount);
}
...
{
    AscendQuant<T, isReuseSource>(dstTensor, srcTensor, scaleTensor, offsetTensor, scaleTensor.GetSize(),
...
__aicore__ inline void AscendQuant(const LocalTensor<int8_t>& dstTensor, const LocalTensor<T>& srcTensor,
    const LocalTensor<T>& scaleTensor, const LocalTensor<T>& offsetTensor)
{
    AscendQuant<T, isReuseSource, config>(dstTensor, srcTensor, scaleTensor, offsetTensor, scaleTensor.GetSize(),
        offsetTensor.GetSize(), srcTensor.GetSize());
}
#endif
#pragma end_pipe
...
#endif
#endif // AICORE_ADV_API_QUANTIZATION_ASCEND_QUANT_H
