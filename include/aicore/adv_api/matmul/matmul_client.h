 ...
 * \brief
 */
#ifndef AICORE_ADV_API_MATMUL_MATMUL_CLIENT_H
#define AICORE_ADV_API_MATMUL_MATMUL_CLIENT_H

#include "kernel_operator.h"
#include "matmul/constant_tiling.h"
...
#include "detail/matmul/kfc/matmul_server_aux.h"
#endif

namespace AscendC {

constexpr int32_t VECTOR_QUANT_MODE = 2;
constexpr int32_t NUM_EIGHT = int32_t(8);
constexpr uint16_t NUM_SIXTEEN = uint16_t(16);
constexpr uint16_t NUM_THIRTYTWO = uint16_t(32);
constexpr uint16_t NUM_FORTYEIGHT = uint16_t(48);
...
public:
    ...
    __aicore__ inline void SetTensorAWithCopy(
        const GlobalTensor<SrcAT>& gm, const LocalTensor<SrcAT>& leftMatrix, bool isTransposeA = false)
    {
        ASSERT(!ToMatmulConfig(MM_CFG).enableMixDualMaster
    ...
    __aicore__ inline void SetSelfDefineData(T dataPtr)
    {
        ...
        if constexpr (sizeof(T) == 4) {
            ...
            for (int i = 0; i < sizeof(T) / sizeof(uint32_t); i++) { *(ptrMsg + i) = *(ptr + i); }
                ...
                if (sync || waitIterateAll) {
                    ...
                    if constexpr (A_TYPE::ibShare && B_TYPE::ibShare) {
                        CrossCoreSetFlag<INTRA_MODE, PIPE_FIX>(GetIntraFlagId(cubeObj.cubeObj[0].instID,
                            static_cast<uint8_t>(VEC_WAIT_INTRA_Enum::WAIT_FIXP),
    ...
    __aicore__ inline void IterateAll(const LocalTensor<DstT>& ubCmatrix, uint8_t enAtomic = 0)
    {
        ...
        ASSERT(!(A_TYPE::ibShare && B_TYPE::ibShare)
               && "IterateAll LocalTensor not support when sameab"
                  " is enabled");
        if (ubCmatrix.GetPosition() == static_cast<int32_t>(TPosition::TSCM)) {
            kfcMsg_.body.cAddr = GetTscmAddr(ubCmatrix);
            kfcMsg_.body.cIsTscm = 1;
        ...
        const uint32_t matrixStrideC = 0, const bool enPartialSum = false, const uint8_t enAtomic = 0)
    {
        ...
        ASSERT(kfcMsg_.body.isFirstIter == 1);
        ASSERT(!(A_TYPE::ibShare && B_TYPE::ibShare)
               && "IterateBatch not support when when sameab"
                  " is enabled");
        kfcMsg_.body.cAddr = reinterpret_cast<uint64_t>(gm.GetPhyAddr());
        kfcMsg_.body.enSequentialWrite = enSequentialWrite;
        kfcMsg_.body.sync = sync;
        ...
        const uint32_t matrixStrideC = 0, const bool enPartialSum = false, const uint8_t enAtomic = 0)
    {
        ...
        static_assert(A_TYPE::layout != LayoutMode::NONE && B_TYPE::layout != LayoutMode::NONE,
            "BMM does not support the layout being NONE");
    ...
    __aicore__ inline void GetTensorC(const LocalTensor<DstT>& c, uint8_t enAtomic = 0, bool enSequentialWrite = false,
...
private:
    ...
    __aicore__ inline void PostMessage()
    {
        ...
        for (int i = 0; i < sizeof(kfcMsg_) / sizeof(uint64_t); i++, tmp1++, tmp2++) { *tmp1 = *tmp2; }
            ...
            if (sync || waitIterateAll) {
                ...
                if constexpr (A_TYPE::ibShare && B_TYPE::ibShare) {
                    CrossCoreSetFlag<INTRA_MODE, PIPE_FIX>(
                        GetIntraFlagId(cubeObj.cubeObj[0].instID, static_cast<uint8_t>(VEC_WAIT_INTRA_Enum::WAIT_FIXP),
                            1U)); // 1 means sub_block 1
                }
            }
            cubeObj.cubeObj[0].mul.End();
    ...
    __aicore__ inline void PrepareABFromGM()
    {
        ...
        if constexpr (GetPhyType(A_TYPE::pos) == Hardware::GM || GetPhyType(B_TYPE::pos) == Hardware::GM
            ...
            if constexpr (!A_TYPE::ibShare && !B_TYPE::ibShare) {
                // Op sometimes excute ub ->gm in MTE3 before using iterate, can find matched wait flag in matmul server
    ...
    template <class T>
    __aicore__ inline void NDPadZeros(LocalTensor<T>& dst, const int height, const int width, const int gCol)
    {
    ...
    __aicore__ inline void NDTrans2NZ(
        LocalTensor<T>& dst, const LocalTensor<T>& src, const int calcHigh, const int calcWidth)
    {
    ...
    template <class T>
    __aicore__ inline void CopyNDBlock(const LocalTensor<T>& transTensor, const LocalTensor<T>& src,
        const int64_t srcOffset, const int height, const int width)
    {
        int srcStride = 0;
    ...
    __aicore__ inline void CopyUB2L1ND2NZ(LocalTensor<T>& dst, LocalTensor<T>& src, const uint32_t row,
        const uint32_t col, const uint32_t gCol, bool isTrans)
    {
        if constexpr (ToMatmulConfig(MM_CFG).enVecND2NZ) {
            if (row % BLOCK_CUBE == 0 && col % BLOCK_CUBE == 0) {
                LocalTensor<T> tmpBuffer = localWorkspace_.ReinterpretCast<T>();
                CopyUBND2UBNZ(tmpBuffer, src, row, col, gCol);
                DataCopy(dst, tmpBuffer, row * col);
            } else {
                ...
                int64_t size = Ceil(row, BLOCK_CUBE) * BLOCK_CUBE * Ceil(col, BLOCK_CUBE) * BLOCK_CUBE;
    ...
    __aicore__ inline void CopyND2NZOnTheFly(
        const LocalTensor<T>& dst, LocalTensor<T>& src, const int height, const int width, const int gCol, bool isTrans)
    {
        int tail = width % c0Size_;
    ...
    __aicore__ inline void TailPadZero(
        const LocalTensor<T>& trans, const LocalTensor<T>& src, const int height, const int width, const int gCol)
    {
        int calcWidth = width / c0Size_; // cube block numbers that do not need to be pad zero
        int tail = width % c0Size_;
        ...
        if constexpr (IsSupportB8<T>()) {
            ...
            for (int i = 0; i < height; i++) {
                Adds(tmpTrans[i * c0Size_], src[tailSrcoffset], static_cast<int8_t>(0), tail);
    ...
    __aicore__ inline void CopyND2NZOnTheFlyWithoutTail(
        const LocalTensor<T>& dst, LocalTensor<T>& src, const int height, const int width, const int gCol)
    {
        int calcWidth = width / c0Size_; // cube block numbers that do not need to be pad zero
        int dstOffset = 0;
        int srcOffset = 0;
    ...
    __aicore__ inline void CopyUBND2UBNZ(
        LocalTensor<T>& dst, LocalTensor<T>& src, const uint32_t row, const uint32_t col, const uint32_t gCol)
    {
        uint32_t calcHigh = Ceil(row, BLOCK_CUBE);
        uint32_t calcWidth = Ceil(col, c0Size_);
    ...
    __aicore__ inline void CopyUB2L1NZ2NZ(
        const LocalTensor<T>& dst, const LocalTensor<T>& src, const uint32_t row, const uint32_t col)
    {
        if constexpr (A_TYPE::format == CubeFormat::NZ && (A_TYPE::ibShare && B_TYPE::ibShare)) {
            uint16_t blockCount = static_cast<uint16_t>(Ceil(col, c0Size_));
            // Concat V0 and V1 A matrix, then row need devide MIX_NUM;
            uint16_t blockLen = static_cast<uint16_t>(Ceil(row / MIX_NUM * sizeof(T) * c0Size_, ONE_BLK_SIZE));
            uint16_t dstStride = blockLen;
            DataCopy(dst, src, {blockCount, blockLen, 0, dstStride});
        } else {
            if constexpr (HasScalePosition<A_TYPE>::value || HasScalePosition<B_TYPE>::value) {
                ...
                uint32_t rowAlign = Ceil(row, BLOCK_CUBE) * BLOCK_CUBE;
                uint32_t colAlign = Ceil(col, BLOCK_CUBE) * BLOCK_CUBE;
    ...
    __aicore__ inline void CopyUbAToL1(bool isTrans)
    {
        ...
        if constexpr (B_TYPE::ibShare && !A_TYPE::ibShare) {
            tbufOutTmp.dataLen = Ceil(singleCoreM_, BLOCK_CUBE) * BLOCK_CUBE * Ceil(singleCoreK_, BLOCK_CUBE)
                                 * BLOCK_CUBE * sizeof(SrcAT) / splitNum;
        } else {
            tbufOutTmp.dataLen = Ceil(singleCoreM_, BLOCK_CUBE) * BLOCK_CUBE * Ceil(singleCoreK_, BLOCK_CUBE)
                                 * BLOCK_CUBE * sizeof(SrcAT);
        }
        if ((B_TYPE::ibShare && !A_TYPE::ibShare) && GetSubBlockIdxImpl() == 1 && isTrans) {
            tbufOutTmp.bufferAddr = Ceil(singleCoreK_, BLOCK_CUBE) * BLOCK_CUBE / splitNum * ONE_BLK_SIZE;
        } else {
        ...
        LocalTensor<SrcAT> a = GetVecTensor<SrcAT>(aAddr_, sizeAmatrix_ / sizeof(SrcAT));
        if constexpr (PhyPosIsUB(A_TYPE::pos) && A_TYPE::format == CubeFormat::ND) {
            if (isTrans) {
                CopyUB2L1ND2NZ<SrcAT, InputTypeTag::A>(
                    leftMatrix, a, singleCoreK_, singleCoreM_, singleCoreM_, isTrans);
            } else {
                CopyUB2L1ND2NZ<SrcAT, InputTypeTag::A>(
                    leftMatrix, a, singleCoreM_, singleCoreK_, singleCoreK_, isTrans);
            }
        } else if constexpr (PhyPosIsUB(A_TYPE::pos) && A_TYPE::format == CubeFormat::NZ) {
            if (isTrans) {
                CopyUB2L1NZ2NZ(leftMatrix, a, singleCoreK_, singleCoreM_);
            } else {
                CopyUB2L1NZ2NZ(leftMatrix, a, singleCoreM_, singleCoreK_);
            }
        } else if constexpr (PhyPosIsUB(A_TYPE::pos) && A_TYPE::format == CubeFormat::VECTOR) {
            if (isTrans) {
                ASCENDC_ASSERT((!isTrans), { KERNEL_LOG(KERNEL_ERROR, "A vector does not support transpose."); });
                return;
            }
            DataCopy(leftMatrix, a[0], {1, static_cast<uint16_t>(Ceil(singleCoreK_, c0Size_)), 0, 0});
        }
    }
    ...
    __aicore__ inline void CopyUbAToL1StaticTiling(bool isTrans)
    {
        ...
        constexpr uint32_t splitNum = 2;
        if constexpr (B_TYPE::ibShare && !A_TYPE::ibShare) {
            tbufOutTmp.dataLen = Ceil(ToMatmulConfig(MM_CFG).singleCoreK, BLOCK_CUBE) * BLOCK_CUBE
                                 * Ceil(ToMatmulConfig(MM_CFG).singleCoreK, BLOCK_CUBE) * BLOCK_CUBE * sizeof(SrcAT)
                                 / splitNum;
        } else {
            tbufOutTmp.dataLen = Ceil(ToMatmulConfig(MM_CFG).singleCoreK, BLOCK_CUBE) * BLOCK_CUBE
                                 * Ceil(ToMatmulConfig(MM_CFG).singleCoreK, BLOCK_CUBE) * BLOCK_CUBE * sizeof(SrcAT);
        ...
        if ((B_TYPE::ibShare && !A_TYPE::ibShare) && GetSubBlockIdxImpl() == 1 && isTrans) {
            tbufOutTmp.bufferAddr =
                Ceil(ToMatmulConfig(MM_CFG).singleCoreK, BLOCK_CUBE) * BLOCK_CUBE / splitNum * ONE_BLK_SIZE;
        } else {
            tbufOutTmp.bufferAddr = kfcMsg_.body.aAddr;
        }
#if ASCENDC_CPU_DEBUG
        ...
        if constexpr (PhyPosIsUB(A_TYPE::pos) && A_TYPE::format == CubeFormat::ND) {
            if (isTrans) {
                CopyUB2L1ND2NZ<SrcAT, InputTypeTag::A>(leftMatrix, a, ToMatmulConfig(MM_CFG).singleCoreK,
                    ToMatmulConfig(MM_CFG).singleCoreM, singleCoreM_, isTrans);
            } else {
                CopyUB2L1ND2NZ<SrcAT, InputTypeTag::A>(leftMatrix, a, ToMatmulConfig(MM_CFG).singleCoreM,
                    ToMatmulConfig(MM_CFG).singleCoreK, singleCoreK_, isTrans);
            }
        } else if constexpr (PhyPosIsUB(A_TYPE::pos) && A_TYPE::format == CubeFormat::NZ) {
            if (isTrans) {
                CopyUB2L1NZ2NZ(leftMatrix, a, ToMatmulConfig(MM_CFG).singleCoreK, ToMatmulConfig(MM_CFG).singleCoreM);
            } else {
                CopyUB2L1NZ2NZ(leftMatrix, a, ToMatmulConfig(MM_CFG).singleCoreM, ToMatmulConfig(MM_CFG).singleCoreK);
            }
        } else if constexpr (PhyPosIsUB(A_TYPE::pos) && A_TYPE::format == CubeFormat::VECTOR) {
            if (isTrans) {
                ASCENDC_ASSERT((!isTrans), { KERNEL_LOG(KERNEL_ERROR, "A vector does not support transpose."); });
    ...
    __aicore__ inline void CopyUbBToL1(bool isTrans)
    {
        ...
        tbufOutTmp.dataLen =
            Ceil(singleCoreK_, BLOCK_CUBE) * BLOCK_CUBE * Ceil(singleCoreN_, BLOCK_CUBE) * BLOCK_CUBE * sizeof(SrcBT);
        ...
        LocalTensor<SrcBT> b = GetVecTensor<SrcBT>(bAddr_, sizeBmatrix_ / sizeof(SrcBT));
        if constexpr (PhyPosIsUB(B_TYPE::pos) && B_TYPE::format == CubeFormat::ND) {
            if (isTrans) {
                CopyUB2L1ND2NZ<SrcBT, InputTypeTag::B>(
                    rightMatrix, b, singleCoreN_, singleCoreK_, singleCoreK_, isTrans);
            } else {
                CopyUB2L1ND2NZ<SrcBT, InputTypeTag::B>(
                    rightMatrix, b, singleCoreK_, singleCoreN_, singleCoreN_, isTrans);
            }
        } else if constexpr (PhyPosIsUB(B_TYPE::pos) && B_TYPE::format == CubeFormat::NZ) {
            if (isTrans) {
                CopyUB2L1NZ2NZ(rightMatrix, b, singleCoreN_, singleCoreK_);
            } else {
                CopyUB2L1NZ2NZ(rightMatrix, b, singleCoreK_, singleCoreN_);
            }
        }
    }
...
#endif
        leftMatrix.SetAddr(tbufOutTmp);
        LocalTensor<ScaleT> a = GetVecTensor<ScaleT>(aScaleAddr_, sizeScaleAmatrix_ / sizeof(ScaleT));
        if constexpr (PhyMxScalePosIsUB<A_TYPE>()) {
            if constexpr (A_TYPE::scaleFormat == CubeFormat::NZ) {
                if (isTrans) {
                    CopyUB2L1NZ2NZ(leftMatrix, a, singleCoreK_ / NUM_THIRTYTWO, singleCoreM_);
                } else {
                    CopyUB2L1NZ2NZ(leftMatrix, a, singleCoreM_, singleCoreK_ / NUM_THIRTYTWO);
                }
            }
        }
    }
...
#if ASCENDC_CPU_DEBUG
        tbufOutTmp.absAddr = GetTPipePtr()->GetBaseAddr(static_cast<uint8_t>(TPosition::B1));
#endif
        rightMatrix.SetAddr(tbufOutTmp);
        LocalTensor<ScaleT> b = GetVecTensor<ScaleT>(bScaleAddr_, sizeScaleBmatrix_ / sizeof(ScaleT));
        if constexpr (PhyMxScalePosIsUB<B_TYPE>()) {
            if constexpr (B_TYPE::scaleFormat == CubeFormat::NZ) {
                if (isTrans) {
                    CopyUB2L1NZ2NZ(rightMatrix, b, singleCoreN_, singleCoreK_ / NUM_THIRTYTWO);
                } else {
                    CopyUB2L1NZ2NZ(rightMatrix, b, singleCoreK_ / NUM_THIRTYTWO, singleCoreN_);
                }
            }
        }
    }
