 ...
 * \brief
 */
#ifndef AICORE_ADV_API_MATMUL_TILING_H
#define AICORE_ADV_API_MATMUL_TILING_H
#include "detail/matmul/utils/matmul_config_impl.h"
...
__aicore__ constexpr MatmulConfig GetNormalConfig(const bool intrinsicsLimit = false, const bool batchLoop = false,
    ...
    const bool enableMixDualMaster = false, const bool isNBatchOut = false)
{
    return {.doNorm = true,
        .doBasicBlock = false,
...
__aicore__ constexpr MatmulConfig GetBasicConfig(const uint32_t basicM, const uint32_t basicN, const uint32_t basicK,
...
#endif // AICORE_ADV_API_MATMUL_TILING_H
