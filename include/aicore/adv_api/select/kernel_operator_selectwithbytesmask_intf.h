/**
 * Copyright (c) 2024 Huawei Technologies Co., Ltd.
 * This file is a part of the CANN Open Software.
 * Licensed under CANN Open Software License Agreement Version 1.0 (the "License").
 * Please refer to the License for details. You may not use this file except in compliance with the License.
 * THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT, MERCHANTABILITY, OR FITNESS FOR A PARTICULAR PURPOSE.
 * See LICENSE in the root of the software repository for the full text of the License.
 */

/* !
 * \file kernel_operator_selectwithbytesmask_intf.h
 * \brief
 */
#ifndef AICORE_ADV_API_SELECT_KERNEL_OPERATOR_SELECTWITHBYTESMASK_INTF_H
#define AICORE_ADV_API_SELECT_KERNEL_OPERATOR_SELECTWITHBYTESMASK_INTF_H
#if __CCE_AICORE__ == 220 || __CCE_AICORE__ == 200 || defined(__DAV_C310__) || defined(__DAV_310R6__)
#include "selectwithbytesmask.h"

namespace AscendC {
[[deprecated(__FILE__
    " is deprecated, please use sekectwithbytesmask.h instead!")]] typedef void SelectWithBytesMaskDeprecatedHeader;
using AscendCModuleSelectWithBytesMaskInterface = SelectWithBytesMaskDeprecatedHeader;
} // namespace AscendC
#endif
#endif // AICORE_ADV_API_SELECT_KERNEL_OPERATOR_SELECTWITHBYTESMASK_INTF_H
