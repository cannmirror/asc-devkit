 ...
 * \brief
 */
#ifndef AICORE_ADV_API_NORMALIZATION_LAYERNORM_TILING_H
#define AICORE_ADV_API_NORMALIZATION_LAYERNORM_TILING_H
...
[[deprecated("GetLayerNormNDTillingInfo is deprecated, please use GetLayerNormNDTilingInfo instead!")]] void
GetLayerNormNDTillingInfo(const ge::Shape& srcShape, const uint32_t stackBufferSize, const uint32_t typeSize,
 ...
 * \param [in] isReuseSource: indicate whether to reuse source tensor. Reserved paramater.
 * \param [in] isInplace: indicate whether outputs that are not calculated are multiplexed inputs.
 * \param [in] ascendcPlatform: platform info
 * \param [out] maxValue: max size required for tmp buffer
 * \param [out] minValue: min size required for tmp buffer
 */
void GetWelfordUpdateMaxMinTmpSize(const ge::Shape& srcShape, const uint32_t typeSizeT, const uint32_t typeSizeU,
    const bool isReuseSource, const bool isInplace, const platform_ascendc::PlatformAscendC& ascendcPlatform,
    uint32_t& maxValue, uint32_t& minValue);
/*!
 ...
 */
void GetLayerNormMaxMinTmpSize(const ge::Shape& srcShape, const uint32_t typeSize, const bool isReuseSource,
    const bool isComputeRstd, const bool isOnlyOutput, const platform_ascendc::PlatformAscendC& ascendcPlatform,
    uint32_t& maxValue, uint32_t& minValue);
/*!
 ...
 */
void GetLayerNormNDTilingInfo(const ge::Shape& srcShape, const uint32_t stackBufferSize, const uint32_t typeSize,
    const bool isReuseSource, const bool isComputeRstd, const platform_ascendc::PlatformAscendC& ascendcPlatform,
    optiling::LayerNormSeparateTiling& tiling);

} // namespace AscendC
#endif // AICORE_ADV_API_NORMALIZATION_LAYERNORM_TILING_H
