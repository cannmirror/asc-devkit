...
from adapter import template_tiling
...
class TestCompileOp(unittest.TestCase):
    ...
    def test_template_tiling_err_tpl(self):
        ...
        self.assertEqual(
            e.exception.args,
            (
                "Cannot find flag in ASCENDC_TPL_UINT_SEL Z! Value should be in [ASCENDC_TPL_UI_RANGE, ASCENDC_TPL_UI_LIST, ASCENDC_TPL_UI_MIX].",
            ),
        )
    ...
    def test_template_tiling_ui_range(self):
        ...
        self.assertEqual(
            e.exception.args,
            (
                "UI_RANGE declare parse failed, because the announced length of ASCENDC_TPL_UINT_DECL Z is greater than actual values' length.",
            ),
        )
    ...
    def test_template_tiling_duplicated(self):
        ...
        self.assertEqual(
            e.exception.args,
            (
                "There is duplicated number in ASCENDC_TPL_DECL_UINT Z! Duplicated List: [0, 1, 2, 3, 4, 3, 6].",
            ),
        )
        ...
        with self.assertRaises(RuntimeError) as e:
            ...
            result = decode_tiling()
        self.assertEqual(
            e.exception.args, ("ASCENDC_TPL_SELECT has duplicated definitions!",)
        )
    ...
    def test_template_tiling_name(self):
        ...
        self.assertEqual(
            e.exception.args,
            ("There is missing marco define: S in ASCENDC_TPL_BOOL_SEL.",),
        )
    ...
    def test_template_invalid_params(self):
        ...
        with self.assertRaises(RuntimeError) as e:
            extract_template_tiling_info(declare_param_str, select_param_str)
        self.assertEqual(
            e.exception.args, ("Length of ASCENDC_TPL_UINT_SEL Z is too short.",)
        )
        ...
        with self.assertRaises(RuntimeError) as e:
            extract_template_tiling_info(declare_param_str, select_param_str)
        self.assertEqual(
            e.exception.args, ("values of ASCENDC_TPL_FORMAT_SEL Y is empty!",)
        )
        select_param_str = (
            "@@ASCENDC_TPL_LISTS@@ = {@@{@@ASCENDC_TPL_DTYPE_SEL_X@@ = { 10, 30},@@ASCENDC_TPL_FORMAT_SEL_Y@@ = { 15},@@ASCENDC_TPL_UINT_SEL_Z@@ = {1, 4, 6},@@ASCENDC_TPL_BOOL_SEL_S@@ = {0, 1}},"
        ...
        self.assertEqual(
            e.exception.args,
            (
                "There is invalid number in ASCENDC_TPL_BOOL_SEL S! Value should only be in [0, 1].",
            ),
        )
        ...
        self.assertEqual(
            e.exception.args,
            (
                "Bit width:1 in ASCENDC_TPL_UINT_DECL Z is not enough to represent all values: [0, 1, 2, 3, 4, 5, 6]! Please make sure 2^bitWidth is greater than or equal to the [number of values].",
            ),
        )
        ...
        self.assertEqual(
            e.exception.args,
            ("Bit width in ASCENDC_TPL_UINT_DECL Z cannot be less than zero!",),
        )
        ...
        self.assertEqual(
            e.exception.args,
            (
                "S has different type in ASCENDC_TPL_FORMAT_SEL and ASCENDC_TPL_BOOL_DECL.",
            ),
        )
        select_param_str = (
        ...
        self.assertEqual(
            e.exception.args,
            ("Cannot find ASCENDC_TPL_BOOL_SEL name: XXX in ASCENDC_TPL_BOOL_DECL.",),
        )
        ...
        self.assertEqual(
            e.exception.args,
            ("name:Z, type:UINT, Total bit width cannot be greater than 64!",),
        )
    ...
    def test_template_tiling_datatype(self):
        ...
        with tbe.common.context.op_context.OpContext():
            with mock.patch.object(
                tbe.common.context.get_context(), "get_addition", return_value=""
            ):
                with mock.patch.object(
                    template_tiling,
                    "safe_parse_value",
                    wraps=template_tiling.safe_parse_value,
                ) as mock_safe_parse_value:
                    extract_template_tiling_info(declare_param_str, select_param_str)
                    dtype_options, dtype_select_indexes = extract_decl_param_options(
                        op_info, "dtype"
                    )
                    result = decode_tiling()
        ...
        self.assertEqual(
            decoded_info.get("paramArgs"),
            ["TypeFromId<27>::type", "TypeFromId<3>::type", "1"],
        )
    ...
    def test_template_tiling_datatype_parse_values(self):
        ...
        self.assertEqual(
            extract_expr("{C_DT_FLOAT16, (13 * 2 + 1), C_DT_UNKNOWN, C_FORMAT_ND}"),
            [1, 27, 2],
        )
    ...
    def test_template_tiling_datatype_invalid_parse_value(self):
        declare_param_str = "@@ASCENDC_TPL_ARGS_DECL_AddTemplateCustom@@ = {@@ASCENDC_TPL_DATATYPE_DECL_D_T_X@@ = {C_DT_UNKNOWN},};"

        with self.assertRaises(RuntimeError) as e:
            extract_template_tiling_info(declare_param_str, "")
        self.assertEqual(
            e.exception.args, ("values of ASCENDC_TPL_DATATYPE_DECL D_T_X is empty!",)
        )
