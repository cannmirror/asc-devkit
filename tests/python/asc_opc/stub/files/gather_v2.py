...
class GatherV2:
    ...
    def __init__(
        ...
        para_check.check_dtype(
            self.indices_dtype, indices_support_dtype_list, param_name="indices"
        )
        para_check.check_dtype(
            self.axis_dtype,
            (constant.DATA_TYPE_INT32, constant.DATA_TYPE_INT64),
            param_name="axis",
        )
        if self.y_dtype != self.params_dtype:
            error_manager_vector.raise_err_inputs_dtype_not_equal(
                kernel_name, "y", "x", self.y_dtype, self.params_dtype
            )
    ...
    def get_tiling_args(self, tiling_ub):
        ...
        None
        """
        self.params_pre = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_pre"
        )
        self.params_pre.set_as(tiling_ub[1])
        self.params_axis = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_axis"
        )
        self.params_axis.set_as(tiling_ub[2])
        self.params_row = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_row"
        )
        self.params_row.set_as(tiling_ub[3])
        self.indices_num = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="indices_num"
        )
        self.indices_num.set_as(tiling_ub[4])

        self.cache_params = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="cache_params"
        )
        self.cache_params.set_as(tiling_ub[5])
        self.need_core_num = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="need_core_num"
        )
        self.need_core_num.set_as(tiling_ub[6])
        self.tail_process_core = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="tail_process_core"
        )
        self.tail_process_core.set_as(tiling_ub[7])
        self.indices_num_each_core = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="indices_num_each_core"
        )
        self.indices_num_each_core.set_as(tiling_ub[8])
        self.indices_num_remaining = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="indices_num_remaining"
        )
        self.indices_num_remaining.set_as(tiling_ub[9])
        self.indices_loop_num = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="indices_loop_num"
        )
        self.indices_loop_num.set_as(tiling_ub[10])
        self.indices_row_num_once = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="indices_row_num_once"
        )
        self.indices_row_num_once.set_as(tiling_ub[11])
        self.indices_row_num_last = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="indices_row_num_last"
        )
        self.indices_row_num_last.set_as(tiling_ub[12])

        self.row_num_once_ub = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="row_num_once_ub"
        )
        self.row_num_once_ub.set_as(tiling_ub[13])
        self.row_num_once_tail_ub = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="row_num_once_tail_ub"
        )
        self.row_num_once_tail_ub.set_as(tiling_ub[14])
        self.inner_loop_num = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="inner_loop_num"
        )
        self.inner_loop_num.set_as(tiling_ub[15])
        self.row_num_last_ub = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="row_num_last_ub"
        )
        self.row_num_last_ub.set_as(tiling_ub[16])
        self.row_num_last_tail_ub = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="row_num_last_tail_ub"
        )
        self.row_num_last_tail_ub.set_as(tiling_ub[17])
        self.inner_loop_num_last = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="inner_loop_num_last"
        )
        self.inner_loop_num_last.set_as(tiling_ub[18])

        self.params_total = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_total"
        )
        self.params_total.set_as(tiling_ub[19])
        self.one_row_loop = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="one_row_loop"
        )
        self.one_row_loop.set_as(tiling_ub[20])
        self.one_row_tail = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="one_row_tail"
        )
        self.one_row_tail.set_as(tiling_ub[21])
        self.params_pre_each_core = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_pre_each_core"
        )
        self.params_pre_each_core.set_as(tiling_ub[22])
        self.params_pre_remaining = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_pre_remaining"
        )
        self.params_pre_remaining.set_as(tiling_ub[23])

        self.indices_row = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="indices_row"
        )
        self.indices_row.set_as(tiling_ub[24])
        self.params_batch_each_core = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_batch_each_core"
        )
        self.params_batch_each_core.set_as(tiling_ub[25])
        self.params_batch_remaining = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_batch_remaining"
        )
        self.params_batch_remaining.set_as(tiling_ub[26])
        self.params_batch = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="params_batch"
        )
    ...
    def gather_v2_compute_tiling(self):
        ...
        remain_half_ub_size = (
            self.ub_size - Constant.CACHE_UB_SIZE - Constant.RESERVED_UB_SIZE
        ) // 2
        tik_instance = self.tik_instance

        # get tiling data
        tiling_ub = tik_instance.Tensor(
            self.tiling_dtype,
        ...
        tik_instance.data_move(
            ...
            1,
            ceil_value(
                Constant.TILING_ARG_NUM * Constant.TYPE_LEN_DICT.get(self.tiling_dtype),
                constant.BLOCK_SIZE,
            ),
            0,
            0,
        ...
        tiling_mode = self.tik_instance.Scalar(
            dtype=self.tiling_dtype, name="tiling_mode"
        )
        ...
        with tik_instance.for_range(
            0, self.core_num, block_num=self.core_num
        ) as block_id:
            with tik_instance.if_scope(block_id < self.need_core_num):
                ...
                if self.batch_dims != 0:
                    ...
                    with tik_instance.if_scope(tiling_mode == Constant.TILING_MODE_20):
                        with tik_instance.new_stmt_scope():
                            self._set_mode_paras(
                                Constant.MODE_LESS_THAN_32B,
                                Constant.PARAMS_CACHED_UB,
                    ...
                    with tik_instance.if_scope(tiling_mode == Constant.TILING_MODE_35):
                        with tik_instance.new_stmt_scope():
                            self._set_mode_paras(
                                Constant.MODE_LARGE_PARAMS_ROW,
                                Constant.PARAMS_NOT_CACHED,
                                Constant.INDICES_CACHED_ALL,
                            )
                            self.compute_with_batch_dims(half_ub_size, block_id)
                    ...
                    with tik_instance.if_scope(tiling_mode == Constant.TILING_MODE_40):
                        with tik_instance.new_stmt_scope():
                            self._set_mode_paras(
                                Constant.MODE_SMALL_PARAMS_ROW,
                                Constant.PARAMS_CACHED_UB,
                                Constant.INDICES_CACHED_ONE_ROW,
                            )
                            self.compute_with_batch_dims(remain_half_ub_size, block_id)
    ...
    def compute_mode_1(self, half_ub_size, quarter_ub_size, block_id):
        ...
        """
        tik_instance = self.tik_instance
        indices_ub = tik_instance.Tensor(
            self.indices_dtype,
            ((half_ub_size + 256) // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        res_ub = tik_instance.Tensor(
            self.params_dtype,
            ((quarter_ub_size + constant.BLOCK_SIZE) // self.params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
        block_ub = tik_instance.Tensor(
            self.params_dtype,
            ((quarter_ub_size + constant.BLOCK_SIZE) // self.params_dsize,),
            name="block_ub",
            scope=tik.scope_ubuf,
        ...
        with tik_instance.for_range(0, self.params_pre) as pre_i:
            ...
            with tik_instance.for_range(0, self.indices_loop_num) as indices_loop_i:
                ...
                tik_instance.data_move(
                    indices_ub,
                    self.indices[indices_num_offset],
                    0,
                    1,
                    ceil_value(
                ...
                with tik_instance.if_scope(self.inner_loop_num > 0):
                    with tik_instance.for_range(0, self.inner_loop_num) as inner_loop_i:
                        ...
                        self.process_mode_1(
                            ub_tensor_list, offset_list, quarter_ub_size, self.x
                        )
            ...
            with tik_instance.if_scope(self.indices_row_num_last > 0):
                ...
                with tik_instance.if_scope(self.row_num_last_tail_ub > 0):
                    inner_indices_offset = (
                        self.inner_loop_num_last * self.row_num_last_ub
                    )
                    ub_tensor_list = [
                        self.row_num_last_tail_ub,
                        indices_ub,
                        res_ub,
                        block_ub,
                    ]
            ...
            with tik_instance.if_scope(
                tik.all(
                    self.indices_num_remaining > 0, block_id == self.tail_process_core
                )
            ):
    ...
    def process_mode_1(self, ub_tensor_list, offset_list, quarter_ub_size, x_src):
        ...
        res_ub_to_gm_loop_num = (
            row_num_last * self.params_row + res_ub_to_gm_per_loop_num - 1
        ) // res_ub_to_gm_per_loop_num
        # elements num in block_ub
        block_per_loop_num = quarter_ub_size / constant.BLOCK_SIZE

        res_to_gm_loop_num = tik_instance.Scalar(
            dtype=self.tiling_dtype, name="res_to_gm_loop_num"
        )
        row_num_last_remaining = tik_instance.Scalar(
            dtype=self.tiling_dtype, name="row_num_last_remaining"
        )
        ...
        with tik_instance.for_range(0, res_to_gm_loop_num) as row_g:
            with tik_instance.if_scope(res_to_gm_loop_num > 1):
                ...
                with tik_instance.else_scope():
                    row_num_last_remaining.set_as(
                        row_num_last - (row_num_last // res_to_gm_loop_num) * row_g
                    )
            ...
            burst_len_res = ceil_value(
                row_num_last_remaining * self.params_row * self.params_dsize,
                constant.BLOCK_SIZE,
            )
            output_offset = (
                pre_i * self.indices_num + indices_num_offset + inner_indices_offset
            ) * self.params_row + row_g * (
                row_num_last // res_to_gm_loop_num
            ) * self.params_row
            ...
            with tik_instance.for_range(0, x_to_block_loop_num) as row_h:
                with tik_instance.for_range(0, block_per_loop_num) as row_i:
                    ...
                    indices_value = tik_instance.Scalar(
                        dtype=self.indices_dtype, name="indices_value", init_value=0
                    )
                    indices_value.set_as(
                        indices_ub[
                            inner_indices_offset
                            + row_g * (row_num_last // res_to_gm_loop_num)
                            + (row_h * block_per_loop_num + row_i)
                        ]
                    )
                    gm_offset = (
                        pre_i * self.params_axis + indices_value
                    ) * self.params_row

                    # copy params row from gm to block_ub
                    tik_instance.data_move(
                        block_ub[self.block_elem * row_i],
                        x_src[gm_offset],
                        0,
                        1,
                        1,
            ...
            with tik_instance.new_stmt_scope(disable_sync=True):
                ...
                with tik_instance.for_range(
                    tail_indices, row_num_last_remaining
                ) as row_i:
                    # compute gm offset of x
                    indices_value = tik_instance.Scalar(
                        dtype=self.indices_dtype, name="indices_value", init_value=0
                    ...
                    # copy params row from gm to block_ub
                    tik_instance.data_move(
                        block_ub[self.block_elem * (row_i - tail_indices)],
                        x_src[gm_offset],
                        0,
                        1,
                        1,
            ...
            with tik_instance.if_scope(tik.all(tail_elem != 0, burst_len_res > 1)):
                ...
                with tik_instance.for_range(0, self.block_elem) as num_i:
                    block_ub[num_i].set_as(
                        res_ub[
                            row_num_last_remaining * self.params_row
                            - self.block_elem
                            + num_i
                        ]
                    )
                ...
                tik_instance.data_move(
                    self.y[output_offset], res_ub, 0, 1, burst_len_res - 1, 0, 0
                )
                tik_instance.data_move(
                    self.y[
                        output_offset
                        + (row_num_last_remaining * self.params_row - self.block_elem)
                    ],
                    block_ub,
                    0,
                    1,
            ...
            with tik_instance.else_scope():
                tik_instance.data_move(
                    self.y[output_offset], res_ub, 0, 1, burst_len_res, 0, 0
                )
    ...
    def process_remaining_tail_mode_1(self, pre_i, indices_ub, res_ub, x_src):
        ...
        tik_instance.data_move(
            ...
            1,
            ceil_value(
                self.indices_num_remaining * self.indices_dsize, constant.BLOCK_SIZE
            ),
            0,
            0,
        )
        ...
        output_offset = (
            pre_i * self.indices_num + indices_num_offset
        ) * self.params_row

        with tik_instance.for_range(
            0, self.indices_num_remaining, thread_num=1
        ) as row_i:
            indices_value = tik_instance.Scalar(
                dtype=self.indices_dtype, name="indices_value", init_value=0
            )
    ...
    def compute_mode_13(self, half_ub_size, block_id):
        ...
        res_ub = tik_instance.Tensor(
            self.params_dtype,
            ((half_ub_size + constant.BLOCK_SIZE) // self.params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
        ...
        if tbe_platform_adapter.api_check_support("tik.vcopy") or self.is_vector_core:
            ...
            x_cbuf = tik_instance.Tensor(
                self.params_dtype,
                (self.l1_size // self.params_dsize,),
                name="x_l1",
            ...
            tik_instance.data_move(
                ...
                0,
                1,
                ceil_value(self.params_total, self.block_elem),
                0,
                0,
            )
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def process_loop_mode_13(
        self, loop_num, indices_num_offset, pre_i, indices_ub, res_ub, x_src
    ):
        """
    ...
    def process_last_mode_13(self, ub_tensor_list, offset_list, x_src):
        ...
        indices_num_offset, inner_indices_offset, pre_i = offset_list
        burst_len_res = ceil_value(
            row_num_last * self.params_row * self.params_dsize, constant.BLOCK_SIZE
        )
        output_offset = (
            pre_i * self.indices_num + indices_num_offset + inner_indices_offset
        ) * self.params_row
        ...
        with tik_instance.if_scope(tik.all(tail_elem != 0, burst_len_res > 1)):
            ...
            with tik_instance.for_range(0, self.block_elem) as num_i:
                block_ub[num_i].set_as(
                    res_ub[row_num_last * self.params_row - self.block_elem + num_i]
                )
            ...
            tik_instance.data_move(
                self.y[
                    output_offset + (row_num_last * self.params_row - self.block_elem)
                ],
                block_ub,
                0,
                1,
    ...
    def compute_mode_4(self, half_ub_size, block_id):
        ...
        tik_instance.data_move(
            x_ub, self.x, 0, 1, ceil_value(self.params_total, self.block_elem), 0, 0
        )
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def process_loop_mode_4(
        self, loop_num, indices_num_offset, pre_i, indices_ub, res_ub, x_ub
    ):
        """
    ...
    def compute_mode_2(self, half_ub_size, block_id):
        ...
        indices_ub = tik_instance.Tensor(
            self.indices_dtype,
            (half_ub_size // indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        res_ub = tik_instance.Tensor(
            self.params_dtype,
            (half_ub_size // params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
        with tik_instance.for_range(0, self.params_pre) as pre_i:
            ...
            with tik_instance.for_range(0, self.indices_loop_num) as indices_loop_i:
                ...
                tik_instance.data_move(
                    indices_ub,
                    self.indices[indices_offset],
                    0,
                    1,
                    ceil_value(
                        self.indices_row_num_once * indices_dsize, constant.BLOCK_SIZE
                    ),
                    0,
                    0,
                )

                self.indices_inner_gather_2(
                    self.indices_row_num_once, indices_offset, pre_i, indices_ub, res_ub
                )
                # process last one loop in indices_loop_num
                self.indices_inner_gather_tail_2(
                    self.indices_row_num_once, indices_offset, pre_i, indices_ub, res_ub
                )
            ...
            with tik_instance.if_scope(self.indices_row_num_last > 0):
                ...
                tik_instance.data_move(
                    ...
                    1,
                    ceil_value(
                        self.indices_row_num_last * indices_dsize, constant.BLOCK_SIZE
                    ),
                    0,
                    0,
                )
                ...
                self.indices_inner_gather_2(
                    self.indices_row_num_last, indices_offset, pre_i, indices_ub, res_ub
                )
            ...
            with tik_instance.if_scope(
                tik.all(
                    self.indices_num_remaining > 0,
                    block_id < self.indices_num_remaining,
                )
            ):
                indices_offset = (
                    self.indices_num_each_core * self.need_core_num + block_id
                )
                self.indices_inner_gather_remain_2(
                    indices_offset, pre_i, indices_ub, res_ub
                )

    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def indices_inner_gather_2(
        self, loop_num, indices_offset, pre_i, indices_ub, res_ub
    ):
        """
        process row_num_once_ub indices for tiling mode 2
        """
        tik_instance = self.tik_instance
        burst_len_row = ceil_value(
        ...
        with tik_instance.for_range(0, loop_num - 1) as row_i:
            ...
            gm_offset = (pre_i * self.params_axis + indices_value) * self.params_row
            output_offset = (
                pre_i * self.indices_num + indices_offset + row_i
            ) * self.params_row

            tik_instance.data_move(res_ub, self.x[gm_offset], 0, 1, burst_len_row, 0, 0)
            tik_instance.data_move(
                self.y[output_offset], res_ub, 0, 1, burst_len_row, 0, 0
            )

    def indices_inner_gather_tail_2(
        self, loop_num, indices_offset, pre_i, indices_ub, res_ub
    ):
        """
        ...
        gm_offset = (pre_i * self.params_axis + indices_value) * self.params_row
        output_offset = (
            pre_i * self.indices_num + indices_offset + loop_num - 1
        ) * self.params_row
        ...
        block_ub = tik_instance.Tensor(
            self.params_dtype, (self.block_elem,), name="block_ub", scope=tik.scope_ubuf
        )
        with tik_instance.for_range(0, self.block_elem) as num_i:
            block_ub[num_i].set_as(res_ub[self.params_row - self.block_elem + num_i])

        tik_instance.data_move(
            self.y[output_offset], res_ub, 0, 1, burst_len_row - 1, 0, 0
        )
        tik_instance.data_move(
            self.y[output_offset + (self.params_row - self.block_elem)],
            block_ub,
            0,
            1,
            1,
    ...
    def compute_mode_5(self, half_ub_size, block_id):
        ...
        indices_ub = tik_instance.Tensor(
            self.indices_dtype,
            (half_ub_size // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        res_ub = tik_instance.Tensor(
        ...
        with tik_instance.for_range(0, self.params_pre) as pre_i:
            ...
            with tik_instance.if_scope(
                ...
                self.process_remaining_tail_mode_5(
                    indices_num_offset, pre_i, indices_ub, res_ub, half_ub_params_elem
                )

    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def process_loop_mode_5(
        self,
        loop_num,
        indices_num_offset,
        pre_i,
        ...
        with tik_instance.for_range(0, loop_num) as row_i:
            output_offset = (
                pre_i * self.indices_num + indices_num_offset + row_i
            ) * self.params_row
            indices_value = tik_instance.Scalar(
                dtype=self.indices_dtype, name="indices_value", init_value=0
            )
            ...
            with tik_instance.for_range(0, self.one_row_loop) as row_inner_i:
                ...
                tik_instance.data_move(
                    res_ub,
                    self.x[gm_offset + row_inner_i * half_ub_params_elem],
                    0,
                    1,
                    burst_len_sub_row,
                    0,
                ...
                tik_instance.data_move(
                    self.y[output_offset + row_inner_i * half_ub_params_elem],
                    res_ub,
                    0,
                    1,
                    burst_len_sub_row,
            ...
            with tik_instance.if_scope(self.one_row_tail > 0):
                ...
                tik_instance.data_move(
                    res_ub,
                    self.x[gm_offset + (self.params_row - self.one_row_tail)],
                    0,
                    1,
                    burst_len_sub_row_last,
                    0,
                    0,
                )
                ...
                with tik_instance.if_scope(
                    tik.all(
                        self.one_row_tail % self.block_elem != 0, loop_num - 1 == row_i
                    )
                ):
                    with tik_instance.for_range(0, self.block_elem) as num_i:
                        block_ub[num_i].set_as(
                            res_ub[self.one_row_tail - self.block_elem + num_i]
                        )

                    tik_instance.data_move(
                        self.y[output_offset + (self.params_row - self.one_row_tail)],
                        res_ub,
                        0,
                        1,
                        burst_len_sub_row_last - 1,
                        0,
                        0,
                    )
                    tik_instance.data_move(
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def process_remaining_tail_mode_5(
        self, indices_num_offset, pre_i, indices_ub, res_ub, half_ub_params_elem
    ):
        """
        ...
        # move one indices data to ub from gm
        tik_instance.data_move(
            indices_ub, self.indices[indices_num_offset], 0, 1, 1, 0, 0
        )
    ...
    def compute_mode_32b_aligned(self, half_ub_size, block_id, x_src):
        ...
        res_ub = tik_instance.Tensor(
            self.params_dtype,
            (half_ub_size // self.params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
        ...
        with tik_instance.for_range(0, self.params_pre) as pre_i:
            ...
            with tik_instance.if_scope(
                ...
                self.process_remaining_tail_32b_aligned(
                    indices_num_offset, pre_i, indices_ub, res_ub, x_src
                )

    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def process_loop_32b_aligned(
        self, loop_num, indices_num_offset, pre_i, indices_ub, res_ub, x_src
    ):
        """
        ...
        with tik_instance.for_range(0, loop_num) as inner_loop_i:
            ...
            with tik_instance.new_stmt_scope(disable_sync=True):
                with tik_instance.for_range(
                    0, self.row_num_once_ub, thread_num=2
                ) as row_i:
                    indices_value = tik_instance.Scalar(
                        dtype=self.indices_dtype, name="indices_value", init_value=0
                    )
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def process_remaining_tail_32b_aligned(
        self, indices_num_offset, pre_i, indices_ub, res_ub, x_src
    ):
        """
    ...
    def compute_mode_8(self, remain_half_ub_size, block_id):
        ...
        indices_ub = tik_instance.Tensor(
            self.indices_dtype,
            ((remain_half_ub_size + 256) // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        res_ub = tik_instance.Tensor(
        ...
        with tik_instance.for_range(range_left, range_right) as pre_i:
            self.compute_mode_less_than_32b_tiling_params_ub(
                pre_i, indices_ub, res_ub, x_ub
            )
        with tik_instance.if_scope(
            tik.all(self.params_pre_remaining > 0, block_id < self.params_pre_remaining)
        ):
    ...
    def compute_mode_less_than_32b_tiling_params_ub(
        self, pre_i, indices_ub, res_ub, x_src
    ):
        """
        ...
        with tik_instance.for_range(0, self.indices_loop_num) as indices_loop_i:
            ...
            tik_instance.data_move(
                ...
                1,
                ceil_value(
                    self.indices_row_num_once * self.indices_dsize, constant.BLOCK_SIZE
                ),
                0,
                0,
            )
        ...
        with tik_instance.if_scope(self.indices_row_num_last > 0):
            ...
            tik_instance.data_move(
                ...
                1,
                ceil_value(
                    self.indices_row_num_last * self.indices_dsize, constant.BLOCK_SIZE
                ),
                0,
                0,
            )
    ...
    def compute_mode_9(self, half_ub_size, quarter_ub_size, block_id):
        ...
        res_ub = tik_instance.Tensor(
            self.params_dtype,
            ((quarter_ub_size + 256) // self.params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
        block_ub = tik_instance.Tensor(
        ...
        range_right = (block_id + 1) * self.params_pre_each_core
        with tik_instance.for_range(range_left, range_right) as pre_i:
            self.compute_mode_less_than_32b_tiling_params(
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def compute_mode_less_than_32b_tiling_params(
        self, pre_i, quarter_ub_size, indices_ub, res_ub, block_ub, x_src
    ):
        """
        compute for tiling mode of less than 32B when tiling params_pre
    ...
    def compute_mode_10(self, remain_half_ub_size, block_id):
        ...
        indices_ub = tik_instance.Tensor(
            self.indices_dtype,
            (remain_half_ub_size // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        res_ub = tik_instance.Tensor(
            self.params_dtype,
            (remain_half_ub_size // self.params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
    ...
    def compute_mode_11(self, half_ub_size, block_id):
        ...
        range_right = (block_id + 1) * self.params_pre_each_core
        with tik_instance.for_range(range_left, range_right) as pre_i:
            self.compute_mode_32b_aligned_tiling_params(
    ...
    def compute_mode_14(self, block_id):
        ...
        idx_start = tik_inst.Scalar(
            dtype=self.tiling_dtype,
            name="idx_start",
            init_value=block_id * self.indices_num_each_core,
        )
        idx_num_cur_core = tik_inst.Scalar(
            dtype=self.tiling_dtype,
        ...
        with tik_inst.if_scope(block_id >= self.tail_process_core):
            idx_start.set_as(
                block_id * self.indices_num_remaining + self.tail_process_core
            )
            idx_num_cur_core.set_as(self.indices_num_remaining)
        indices_value = tik_inst.Scalar(
            dtype=self.indices_dtype, name="indices_value", init_value=0
        )
        CurIdxTuple = collections.namedtuple(
            "CurIdxTuple", ["idx_start", "idx_num_cur_core", "indices_value"]
        )
        ...
        RowOffsetTuple = collections.namedtuple(
            "RowOffsetTuple", ["x_pre_offset", "y_pre_offset", "y_offset"]
        )
        row_offset_tuple = RowOffsetTuple(x_pre_offset, y_pre_offset, y_offset)

        with tik_inst.if_scope(
            tik.all(
                self.params_row <= row_inner_loop_elem,
    ...
    def compute_mode_14_x32b_le64k(self, row_offset_tuple, cur_idx_tuple):
        ...
        x0_ub = tik_inst.Tensor(
            self.params_dtype,
            (self.impl_mode_x0_ub_size // self.params_dsize,),
            name="x0_ub",
            scope=tik.scope_ubuf,
        )
        res_ub = tik_inst.Tensor(
            self.params_dtype,
            (self.impl_mode_res_ub_size // self.params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
        indices_ub = tik_inst.Tensor(
            self.indices_dtype,
            (self.impl_mode_indices_ub_size // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )

        row_size = tik_inst.Scalar(
            dtype=self.tiling_dtype,
            name="row_size",
            init_value=self.params_row * self.params_dsize,
        )
        row_burst = tik_inst.Scalar(
            dtype=self.tiling_dtype,
            name="row_burst",
        ...
        idx_num_cur_loop = tik_inst.Scalar(
            dtype=self.tiling_dtype, name="idx_num_cur_loop"
        )
        idx_num_cur_batch = tik_inst.Scalar(
            dtype=self.tiling_dtype, name="idx_num_cur_batch"
        )

        # unroll num is 16
        idx_values = tik_inst.ScalarArray(
            dtype=self.indices_dtype, length=16, name="idx_values", init_value=0
        )
        ...
        def compute_for_once_ub(idx_loop_i, idx_batch_i):
            ...
            with tik_inst.for_range(0, idx_num_cur_batch // 16) as idx_i:
                for roll_i in range(16):
                    idx_values[roll_i].set_as(
                        indices_ub[
                            idx_batch_i * self.row_num_once_ub + idx_i * 16 + roll_i
                        ]
                    )
                with tik_inst.if_scope(
                    tik.all(
                    ...
                    for roll_i in range(16):
                        tik_inst.data_move(
                            res_ub[(idx_i * 16 + roll_i) * self.params_row],
                            x0_ub,
                            0,
                            1,
                            row_burst,
                    ...
                    for roll_i in range(16):
                        ...
                        with tik_inst.else_scope():
                            tik_inst.data_move(
                                res_ub[(idx_i * 16 + roll_i) * self.params_row],
                                self.x[
                                    x_pre_offset + idx_values[roll_i] * self.params_row
                                ],
                                0,
                                1,
                                row_burst,
            ...
            with tik_inst.for_range(
                floor_align(idx_num_cur_batch, 16), idx_num_cur_batch
            ) as idx_i:
                indices_value.set_as(
                    indices_ub[idx_batch_i * self.row_num_once_ub + idx_i]
                )
                with tik_inst.if_scope(indices_value == 0):
                    tik_inst.data_move(
                        res_ub[idx_i * self.params_row], x0_ub, 0, 1, row_burst, 0, 0
                    )
                with tik_inst.else_scope():
                    tik_inst.data_move(
                        res_ub[idx_i * self.params_row],
            ...
            y_offset.set_as(
                y_pre_offset
                + (idx_loop_i * idx_num_per_loop + idx_batch_i * self.row_num_once_ub)
                * self.params_row
            )
            tik_inst.data_move(
                self.y[y_offset], res_ub, 0, 1, idx_num_cur_batch * row_burst, 0, 0
            )
        ...
        def compute_for_one_idx_loop(idx_loop_i):
            ...
            tik_inst.data_move(
                indices_ub,
                self.indices[idx_start + idx_loop_i * idx_num_per_loop],
                0,
                1,
                ceil_value(idx_num_cur_loop * self.indices_dsize, constant.BLOCK_SIZE),
                0,
                0,
            )
            ...
            with tik_inst.for_range(
                0, idx_num_cur_loop // self.row_num_once_ub
            ) as idx_batch_i:
        ...
        with tik_inst.for_range(0, self.params_pre) as pre_i:
            ...
            with tik_inst.for_range(
                0, idx_num_cur_core // idx_num_per_loop
            ) as idx_loop_i:
    ...
    def compute_mode_14_gt32b(self, row_offset_tuple, cur_idx_tuple):
        ...
        indices_ub = tik_inst.Tensor(
            self.indices_dtype,
            (self.impl_mode_x0_ub_size // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )

        row_inner_loop_elem = self.impl_mode_x0_ub_size // self.params_dsize
        row_tail_size = tik_inst.Scalar(
            dtype=self.tiling_dtype,
            name="row_tail_size",
        ...
        def compute_for_one_idx_loop(row_inner_offset, row_inner_burst, idx_loop_i):
            ...
            with tik_inst.for_range(0, idx_num_cur_loop) as idx_i:
                ...
                y_offset.set_as(
                    y_pre_offset
                    + (idx_loop_i * idx_num_per_loop + idx_i) * self.params_row
                    + row_inner_offset
                )
                with self.tik_instance.if_scope(indices_value == 0):
                    self.tik_instance.data_move(
                        self.y[y_offset], x0_ub, 0, 1, row_inner_burst, 0, 0
                    )
                with self.tik_instance.else_scope():
                    x_offset = (
                        x_pre_offset
                        + indices_value * self.params_row
                        + row_inner_offset
                    )
                    self.tik_instance.data_move(
                        res_ub, self.x[x_offset], 0, 1, row_inner_burst, 0, 0
                    )
                    self.tik_instance.data_move(
                        self.y[y_offset], res_ub, 0, 1, row_inner_burst, 0, 0
                    )
        ...
        def compute_for_incomplete_row(row_inner_offset, row_inner_burst):
            ...
            self.tik_instance.data_move(
                x0_ub,
                self.x[x_pre_offset + row_inner_offset],
                0,
                1,
                row_inner_burst,
                0,
            ...
            with tik_inst.if_scope(idx_num_cur_core % idx_num_per_loop != 0):
                idx_num_cur_loop.set_as(idx_num_cur_core % idx_num_per_loop)
                compute_for_one_idx_loop(
                    row_inner_offset,
                    row_inner_burst,
                    idx_num_cur_core // idx_num_per_loop,
                )
        ...
        with tik_inst.for_range(0, self.params_pre) as pre_i:
            ...
            with tik_inst.for_range(
                0, self.params_row // row_inner_loop_elem
            ) as row_inner_loop_i:
                compute_for_incomplete_row(
                    row_inner_loop_i * row_inner_loop_elem,
                    self.impl_mode_x0_ub_size // constant.BLOCK_SIZE,
                )
            with tik_inst.if_scope(row_tail_size >= constant.BLOCK_SIZE):
                compute_for_incomplete_row(
                    floor_align(self.params_row, row_inner_loop_elem),
    ...
    def compute_mode_14_lt32b(self, row_offset_tuple, cur_idx_tuple):
        ...
        x0_ub = tik_inst.Tensor(
            self.params_dtype, (self.block_elem,), name="x0_ub", scope=tik.scope_ubuf
        )
        xi_ub = tik_inst.Tensor(
            self.params_dtype, (self.block_elem,), name="xi_ub", scope=tik.scope_ubuf
        )
        res_ub = tik_inst.Tensor(
            self.params_dtype,
            (self.impl_mode_res_ub_size // self.params_dsize,),
        ...
        self.row_num_once_ub.set_as(
            floor_align(self.impl_mode_res_ub_size, row_size * constant.BLOCK_SIZE)
            // row_size
        )

        res_tail_offset = tik_inst.Scalar(
            dtype=self.tiling_dtype, name="res_tail_offset"
        )
        ...
        def compute_for_once_ub(idx_loop_i, idx_batch_i):
            ...
            self.tik_instance.data_move(
                ...
                0,
                1,
                idx_num_cur_batch * row_size // constant.BLOCK_SIZE,
                0,
                0,
            )
        ...
        def compute_for_tail():
            ...
            """
            with self.tik_instance.for_range(0, idx_num_cur_batch) as idx_i:
                res_tail_offset.set_as(
                    self.block_elem * 2 - (idx_i + 1) * self.params_row
                )
            ...
            y_offset.set_as(
                y_pre_offset + idx_num_cur_core * self.params_row - self.block_elem
            )
            self.tik_instance.data_move(
                self.y[y_offset], res_ub[self.block_elem], 0, 1, 1, 0, 0
            )
        ...
        def compute_for_one_idx_loop(idx_loop_i):
            ...
            with tik_inst.for_range(
                ...
                compute_for_once_ub(idx_loop_i, idx_batch_i)
            with tik_inst.if_scope(
                idx_num_cur_loop % self.row_num_once_ub * row_size
                >= constant.BLOCK_SIZE
            ):
                idx_num_cur_batch.set_as(idx_num_cur_loop % self.row_num_once_ub)
                compute_for_once_ub(
            ...
            with tik_inst.if_scope(
                idx_num_cur_loop % self.row_num_once_ub * row_size % constant.BLOCK_SIZE
                != 0
            ):
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def params_row_less_than_32b(
        self, indices_loop_offset, batch_i, block_id, pre_i, is_last
    ):
        """
        ...
        params_offset_base = (
            params_batch_diff * self.params_pre * self.params_axis
            + pre_i * self.params_axis
        )

        with tik_instance.for_range(0, loop_num) as inner_loop_i:
            inner_indices_offset = (
                indices_offset_base + inner_loop_i * self.row_num_once_ub
            )
            output_offset = (
                output_offset_base
                + pre_i * self.indices_row
                + (indices_loop_offset + inner_loop_i * self.row_num_once_ub)
            ) * self.params_row
            ...
            with tik_instance.for_range(0, self.row_num_once_ub, thread_num=2) as row_i:
                ...
                res_ub_offset = row_i * self.params_row
                if (
                    self.cached_types.get("cached_types_params")
                    == Constant.PARAMS_CACHED_UB
                ):
                    with tik_instance.for_range(0, self.params_row) as i:
                        res_ub[res_ub_offset + i].set_as(params_ub[params_offset + i])
                else:
                    temp_ub_1 = tik_instance.Tensor(
                        self.params_dtype,
                        (self.block_elem,),
                        name="temp_ub_1",
                        scope=tik.scope_ubuf,
                    ...
                    tik_instance.data_move(
                        temp_ub_1[0], params_ub[params_offset], 0, 1, 1, 0, 0
                    )
                    with tik_instance.for_range(0, self.params_row) as i:
        ...
        with tik_instance.if_scope(tail_num > 0):
            burst_len_res = ceil_value(
                tail_num * self.params_row * self.params_dsize, constant.BLOCK_SIZE
            )
            inner_indices_offset = indices_offset_base + loop_num * self.row_num_once_ub
            output_offset = (
                output_offset_base
                + pre_i * self.indices_row
                + (indices_loop_offset + loop_num * self.row_num_once_ub)
            ) * self.params_row
            ...
            with tik_instance.for_range(0, tail_num, thread_num=2) as row_i:
                ...
                if (
                    ...
                    tik_instance.data_move(
                        temp_ub_2[0], params_ub[params_offset], 0, 1, 1, 0, 0
                    )
                    with tik_instance.for_range(0, self.params_row) as i:
            ...
            with tik_instance.if_scope(tik.all(tail_elem != 0, burst_len_res > 1)):
                ...
                with tik_instance.for_range(0, self.block_elem) as num_i:
                    block_ub[num_i].set_as(
                        res_ub[tail_num * self.params_row - self.block_elem + num_i]
                    )
                ...
                tik_instance.data_move(
                    self.y[
                        output_offset + (tail_num * self.params_row - self.block_elem)
                    ],
                    block_ub,
                    0,
                    1,
    ...
    def params_row_more_than_32b(
        ...
        if (
            ...
            indices_offset_base = 0
        params_offset_base = (
            batch_i * self.params_pre * self.params_axis + pre_i * self.params_axis
        )
        output_offset_base = batch_i * self.params_pre * self.indices_row
        ...
        with tik_instance.for_range(0, loop_num - 1) as row_i:
            ...
            output_offset = (
                output_offset_base
                + pre_i * self.indices_row
                + (indices_loop_offset + row_i)
            ) * self.params_row

            tik_instance.data_move(
                res_ub, self.x[params_offset], 0, 1, burst_len_row, 0, 0
            )
            tik_instance.data_move(
                self.y[output_offset], res_ub, 0, 1, burst_len_row, 0, 0
            )
        ...
        output_offset = (
            output_offset_base
            + pre_i * self.indices_row
            + (indices_loop_offset + loop_num - 1)
        ) * self.params_row
    ...
    def get_optimization_mode(
        ...
        cache_max_n_num,
        indices_simpling_ub,
        indices_index_float_ub,
    ):
        """
        compute for optimization mode simpling or cache n number
        """
        if (
            self.indices_dtype == Constant.DTYPE_INT32
            and tbe_platform_adapter.api_check_support(
                "tbe.dsl.vexp", Constant.DTYPE_FP32
            )
        ):
            # calc Simplingstep
            simpling_step.set_as(
                idx_num_cur_batch // Constant.TOPN_CACHE_ACT_SIMPLING_NUM
            )
            with self.tik_instance.if_scope(simpling_step != 0):
                with self.tik_instance.for_range(
                    0, Constant.TOPN_CACHE_ACT_SIMPLING_NUM
                ) as idx_simpling_i:
                    indices_simpling_ub[idx_simpling_i].set_as(
                        indices_index_float_ub[
                            idx_batch_i * self.row_num_once_ub
                            + idx_simpling_i * simpling_step
                        ]
                    )

                max_dup_data = self.tik_instance.Tensor(
                    Constant.DTYPE_FP32,
                    (Constant.CACHE_ACT_SIMPLING_BUFF_NUM,),
                    name="cache_simpling_data",
                    scope=tik.scope_ubuf,
                )
                sel_gt_ub = self.tik_instance.Tensor(
                    Constant.DTYPE_UINT32,
                    (Constant.CACHE_ACT_SIMPLING_BUFF_NUM,),
                    name="sel_gt_ub",
                    scope=tik.scope_ubuf,
                )
                cache_max_n_num_fp = self.tik_instance.Scalar(
                    dtype=Constant.DTYPE_FP32, name="cache_max_n_num_fp"
                )
                ...
                self.tik_instance.vec_cmpv_gt(
                    sel_gt_ub, indices_simpling_ub, max_dup_data, 1, 8, 8
                )
                sel_gt_flag = self.tik_instance.Scalar(
                    dtype=Constant.DTYPE_UINT32,
                    name="sel_gt_flag",
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def compute_for_simpling_cache_data(
        self,
        idx_batch_i,
        simpling_step,
        indices_ub,
        ...
        if tbe_platform_adapter.api_check_support("tbe.dsl.vexp", Constant.DTYPE_FP32):
            indices_value_dup_ub = self.tik_instance.Tensor(
                Constant.DTYPE_FP32,
                (Constant.CACHE_ACT_SIMPLING_BUFF_NUM,),
                name="indice_dup_ub",
                scope=tik.scope_ubuf,
            )
            idx_value_dup_ub = self.tik_instance.Tensor(
                Constant.DTYPE_FP32,
                (Constant.CACHE_ACT_SIMPLING_BUFF_NUM,),
                name="indice_dup_ub",
                scope=tik.scope_ubuf,
            )
            is_eq_ub = self.tik_instance.Tensor(
                Constant.DTYPE_UINT32,
                ((self.row_num_once_ub + 7) // 8,),
                name="is_eq_ub",
                scope=tik.scope_ubuf,
            )
            zero_dup_ub = self.tik_instance.Tensor(
                Constant.DTYPE_FP32,
                (Constant.CACHE_ACT_SIMPLING_BUFF_NUM,),
                name="zero_dup_ub",
                scope=tik.scope_ubuf,
            )
            invalid_dup_ub = self.tik_instance.Tensor(
                Constant.DTYPE_FP32,
                (Constant.CACHE_ACT_SIMPLING_BUFF_NUM,),
                name="invalid_dup_ub",
                scope=tik.scope_ubuf,
            )
            mask = self.tik_instance.Scalar(
                dtype=Constant.DTYPE_INT32, name="mask", init_value=constant.MASK64
            )

            self.tik_instance.vec_dup(mask, zero_dup_ub, 0.0, 1, 8)
            self.tik_instance.vec_dup(
                mask, invalid_dup_ub, Constant.TOPN_CACHE_INVALID_PARAMS_INDEX, 1, 8
            )
            indices_value_fp32 = self.tik_instance.Scalar(
                dtype=Constant.DTYPE_FP32, name="indices_value_fp32"
            )
            indices_index = self.tik_instance.Scalar(
                dtype=Constant.DTYPE_INT32, name="indices_index"
            )
            ...
            with self.tik_instance.if_scope(simpling_step != 0):
                ...
                with self.tik_instance.for_range(
                    ...
                    self.tik_instance.data_move(
                        cached_ub[(idx_simpling_i + 1) * row_size_align],
                        self.x[indices_index * self.params_row],
                        0,
                        1,
                        params_block_num,
                        0,
                ...
                repeat_times = self.tik_instance.Scalar(
                    dtype=Constant.DTYPE_INT32,
                    name="repeat_times",
                    init_value=(idx_num_cur_batch + mask - 1) // mask,
                )
                idx_num_cur_batch_tail = self.tik_instance.Scalar(
                    dtype=Constant.DTYPE_INT32, name="idx_num_cur_batch_tail"
                )
                idx_num_cur_batch_tail.set_as(
                    idx_num_cur_batch - idx_num_cur_batch // mask * mask
                )
                if tbe_platform.api_check_support(
                    "tik.data_move_pad", self.params_dtype
                ):
                    with self.tik_instance.for_range(
                        0, Constant.TOPN_CACHE_ACT_SIMPLING_NUM
                    ) as idx_simpling_i:
                        ...
                        with self.tik_instance.if_scope(
                            indices_value_fp32
                            != Constant.TOPN_CACHE_INVALID_PARAMS_INDEX
                        ):
                            self.tik_instance.vec_dup(
                                mask, indices_value_dup_ub, indices_value_fp32, 1, 8
                            )
                            self.tik_instance.vec_cmpv_eq(
                                is_eq_ub,
                                indices_value_dup_ub,
                                indices_simpling_ub,
                                1,
                                8,
                                8,
                            ...
                            self.tik_instance.vec_sel(
                                Constant.TOPN_CACHE_ACT_SIMPLING_NUM,
                                0,
                                indices_simpling_ub,
                                is_eq_ub,
                                invalid_dup_ub,
                            ...
                            idx_value = self.tik_instance.Scalar(
                                dtype=Constant.DTYPE_FP32,
                                name="idx_value",
                                init_value=-(idx_simpling_i + 1),
                            )
                            self.tik_instance.vec_dup(
                                mask, idx_value_dup_ub, idx_value, 1, 8
                            )
                    ...
                    self.tik_instance.data_move(
                        ...
                        1,
                        ceil_value(
                            idx_num_cur_batch * self.indices_dsize, constant.BLOCK_SIZE
                        ),
                        0,
                        0,
                    )
                    ...
                    self.tik_instance.h_cast(
                        indices_temp_int_ub, indices_temp_float_ub, "round"
                    )
                else:
                    with self.tik_instance.for_range(
                        0, Constant.TOPN_CACHE_ACT_SIMPLING_NUM
                    ...
                    with self.tik_instance.for_range(0, repeat_times) as idx_repeat:
                        self.tik_instance.vec_cmpv_gt(
                            is_eq_ub,
                            indices_temp_float_ub[mask * idx_repeat],
                            zero_dup_ub,
                            1,
                            8,
                            8,
    ...
    def trans(self, src_ub, dst_ub, trans_mode, length):
        ...
        """
        loop_times = (
            (length * Constant.FP16_ALIGN_NUM) // Constant.VECTOR_BLOCK_SIZE
        ) % Constant.MAX_REPEAT_TIME
        if trans_mode == Constant.LINE_TO_COL:
            src_list = [
                src_ub[Constant.FP16_ALIGN_NUM * i * loop_times]
                for i in range(Constant.FP16_ALIGN_NUM)
            ]
            dst_list = [
                dst_ub[Constant.FP16_ALIGN_NUM * i]
                for i in range(Constant.FP16_ALIGN_NUM)
            ]
            with self.tik_instance.if_scope(loop_times == 1):
                self.tik_instance.vnchwconv(
                    False, False, dst_list, src_list, loop_times, 0, 0
                )
            with self.tik_instance.else_scope():
                self.tik_instance.vnchwconv(
                    False,
            ...
            src_list = [
                src_ub[Constant.FP16_ALIGN_NUM * i]
                for i in range(Constant.FP16_ALIGN_NUM)
            ]
            dst_list = [
                dst_ub[Constant.FP16_ALIGN_NUM * i * loop_times]
                for i in range(Constant.FP16_ALIGN_NUM)
            ]
            with self.tik_instance.if_scope(loop_times == 1):
                self.tik_instance.vnchwconv(
    ...
    def transdata_and_move_out(
        ...
        """
        one_block_num = self.tik_instance.Scalar(
            dtype=Constant.DTYPE_INT32,
            name="one_block_num",
            init_value=Constant.ONE_BLOCK_FP32_NUM,
        )
        trans_type_to_fp16 = self.tik_instance.Scalar(
            dtype=Constant.DTYPE_INT32, name="trans_type_to_fp16", init_value=2
        )
        one_line_num = self.tik_instance.Scalar(
            dtype=Constant.DTYPE_INT32, name="one_line_num", init_value=8
        )
        one_repeat_num = self.tik_instance.Scalar(
            dtype=Constant.DTYPE_INT32, name="one_repeat_num", init_value=128
        )
        ...
        with self.tik_instance.if_scope(self.params_row < 64):
            ...
            with self.tik_instance.if_scope(loop_times != 0):
                ...
                self.trans(
                    src_fp16,
                    dst_fp16,
                    Constant.LINE_TO_COL,
                    one_line_num * trans_type_to_fp16 * loop_times * row_size_align,
                )
                block_num = ceil_value(
                    row_size * Constant.FP16_ALIGN_NUM * 2 * trans_type_to_fp16,
                    constant.BLOCK_SIZE,
                )
                src_stride = ceil_value(
                    (row_size_align - row_size)
                    * Constant.FP16_ALIGN_NUM
                    * 2
                    * trans_type_to_fp16,
                    constant.BLOCK_SIZE,
                ...
                y_offset.set_as(
                    y_pre_offset
                    + (
                        idx_loop_i * idx_num_per_loop
                        + idx_batch_i * self.row_num_once_ub
                    )
                    * self.params_row
                )
                self.tik_instance.data_move(
                    self.y[y_offset], res_ub[0], 0, 1, total_num // one_block_num, 0, 0
                )
                ...
                with self.tik_instance.if_scope(total_num % one_block_num != 0):
                    ...
                    self.tik_instance.data_move(
                        self.y[y_offset + base_offset], ub_xi[0], 0, 1, 1, 0, 0
                    )
            ...
            with self.tik_instance.if_scope(mod != 0):
                with self.tik_instance.if_scope(loop_times != 0):
                    self.tik_instance.data_move(
                        res_ub,
                        res_ub[one_repeat_num * loop_times * row_size_align],
                        0,
                        1,
                        ceil_value(
                            row_size_align * self.params_dsize * mod,
                            constant.BLOCK_SIZE,
                        ),
                        0,
                        0,
                ...
                self.trans(
                    ...
                    dst_fp16,
                    Constant.LINE_TO_COL,
                    one_line_num * trans_type_to_fp16 * row_size_align,
                )
                block_num = ceil_value(
                    row_size * Constant.FP16_ALIGN_NUM * 2 * trans_type_to_fp16,
                    constant.BLOCK_SIZE,
                ...
                self.tik_instance.data_move(
                    dst_fp16, dst_fp16, 0, one_line_num, block_num, src_stride, 0
                )
                self.trans(
                    dst_fp16,
                    src_fp16,
                    Constant.COL_TO_LINE,
                    one_line_num * trans_type_to_fp16 * row_size,
                )
                ...
                with self.tik_instance.else_scope():
                    ...
                    with self.tik_instance.for_range(0, total_num) as elem_i:
                        ub_xi[one_block_num - 1 - elem_i].set_as(
                            res_ub[total_num - 1 - elem_i]
                        )
                    self.tik_instance.data_move(
                        self.y[y_offset], ub_xi[0], 0, 1, 1, 0, 0
                    )
        ...
        with self.tik_instance.else_scope():
            ...
            with self.tik_instance.if_scope(mod_num == 0):
                with self.tik_instance.for_range(0, out_loop) as line_idx:
                    with self.tik_instance.if_scope(line_idx != out_loop - 1):
                        self.tik_instance.data_move(
                            self.y[y_offset + line_idx * one_line_num * row_size],
                            res_ub[line_idx * one_line_num * row_size_align],
                            0,
                            1,
                            ceil_value(one_line_num * row_size, one_block_num),
                            0,
                            0,
                        )
                    with self.tik_instance.else_scope():
                        self.tik_instance.data_move(
                            ...
                            0,
                            1,
                            one_line_num * row_size // one_block_num,
                            0,
                            0,
                        )
                        ...
                        with self.tik_instance.for_range(0, one_block_num) as elem_i:
                            ub_xi[elem_i].set_as(
                                res_ub[
                                    line_idx * one_line_num * row_size_align
                                    + last_data_offset
                                    + elem_i
                                ]
                            )
                        self.tik_instance.data_move(
                            self.y[
                                y_offset
                                + line_idx * one_line_num * row_size
                                + last_data_offset
                            ],
                            ub_xi,
                            0,
            ...
            with self.tik_instance.else_scope():
                ...
                with self.tik_instance.if_scope(mod_num * row_size >= one_block_num):
                    self.tik_instance.data_move(
                        self.y[y_offset + out_loop * one_line_num * row_size],
                        res_ub[out_loop * one_line_num * row_size_align],
                        0,
                        1,
                        mod_num * row_size // one_block_num,
                        0,
                        0,
                    )
                    ...
                    with self.tik_instance.for_range(0, one_block_num) as elem_i:
                        ub_xi[elem_i].set_as(
                            res_ub[
                                out_loop * one_line_num * row_size_align
                                + last_data_offset
                                + elem_i
                            ]
                        )
                    self.tik_instance.data_move(
                        self.y[
                            y_offset
                            + out_loop * one_line_num * row_size
                            + last_data_offset
                        ],
                        ub_xi,
                        0,
    ...
    def compute_mode_15_aixs_eq_0(self, row_offset_tuple, cur_idx_tuple):
        ...
        ub_tensor_size = floor_align(
            (self.ub_size - Constant.IMPL_MODE_INNER_LOOP_UB_8K) // 6,
            constant.BLOCK_SIZE,
        )

        res_ub = tik_inst.Tensor(
            self.params_dtype,
            (ub_tensor_size // self.params_dsize,),
            name="res_ub",
            scope=tik.scope_ubuf,
        )
        indices_ub = tik_inst.Tensor(
            self.indices_dtype,
            (ub_tensor_size // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        cached_ub = tik_inst.Tensor(
            self.params_dtype,
            (ub_tensor_size // self.params_dsize,),
            name="cached_ub",
            scope=tik.scope_ubuf,
        )
        cached_n_num_ub = tik_inst.Tensor(
            self.params_dtype,
            (ub_tensor_size // self.params_dsize,),
            name="cached_n_num_ub",
            scope=tik.scope_ubuf,
        )
        indices_index_float_ub = tik_inst.Tensor(
            Constant.DTYPE_FP32,
            (ub_tensor_size // self.indices_dsize,),
            name="indices_index_float_ub",
            scope=tik.scope_ubuf,
        )
        indices_simpling_ub = tik_inst.Tensor(
            Constant.DTYPE_FP32,
            (Constant.CACHE_ACT_SIMPLING_BUFF_NUM,),
        ...
        params_block_num = tik_inst.Scalar(
            dtype=Constant.DTYPE_INT32, name="params_block_num"
        )
        params_block_num.set_as(
            ceil_value(self.params_row * self.params_dsize, constant.BLOCK_SIZE)
        )

        simpling_step = tik_inst.Scalar(
            dtype=Constant.DTYPE_INT32, name="simpling_step"
        )
        cache_max_n_num = tik_inst.Scalar(
            dtype=Constant.DTYPE_INT32, name="cache_max_n_num"
        )
        cache_max_n_num.set_as(ub_tensor_size // (self.params_row * self.params_dsize))
        is_full_cache = tik_inst.Scalar(
            dtype=Constant.DTYPE_INT32, name="is_full_cache", init_value=0
        )
        ...
        tik_inst.data_move(
            ...
            1,
            ceil_value(
                cache_max_n_num * self.params_row * self.params_dsize,
                constant.BLOCK_SIZE,
            ),
            0,
            0,
        ...
        self.row_num_once_ub.set_as(
            floor_align(ub_tensor_size, row_size * constant.BLOCK_SIZE) // row_size
        )

        indices_temp_int_ub = tik_inst.Tensor(
            Constant.DTYPE_INT32,
            (self.row_num_once_ub,),
            name="indices_temp_int_ub",
            scope=tik.scope_ubuf,
        )
        indices_temp_float_ub = tik_inst.Tensor(
            Constant.DTYPE_FP32,
            (self.row_num_once_ub,),
        ...
        def compute_for_once_ub(idx_loop_i, idx_batch_i):
            ...
            compute for once ub
            """
            opt_mode = tik_inst.Scalar(
                dtype=Constant.DTYPE_INT32, name="cache_max_n_num"
            )
            indice_base_offset = tik_inst.Scalar(
                dtype=Constant.DTYPE_INT32,
                name="indice_base_offset",
                init_value=idx_batch_i * self.row_num_once_ub,
            )

            self.get_optimization_mode(
                idx_batch_i,
                opt_mode,
                simpling_step,
                idx_num_cur_batch,
            ...
            with self.tik_instance.if_scope(
                opt_mode == Constant.INPUT_LAST_AXIS_ALIGN_HP_OPT_CACHE_SIMPLING
            ):
                self.compute_for_simpling_cache_data(
                    idx_batch_i,
                    simpling_step,
                    indices_ub,
                    cached_ub,
                ...
                with self.tik_instance.for_range(0, idx_num_cur_batch) as idx_i:
                    ...
                    with self.tik_instance.if_scope(indices_value > 0):
                        tik_inst.data_move(
                            res_ub[idx_i * self.params_row],
                            cached_ub[indices_value * self.params_row],
                            0,
                            1,
                            params_block_num,
                            0,
            ...
            with self.tik_instance.else_scope():
                with tik_inst.if_scope(is_full_cache == 1):
                    unrool_num = 8
                    index_array = self.tik_instance.ScalarArray(
                        dtype=self.tiling_dtype, length=unrool_num, name="index_array"
                    )
                    with self.tik_instance.for_range(
                        0, idx_num_cur_batch // unrool_num
                    ) as idx_i:
                        with self.tik_instance.for_range(0, unrool_num) as unroll_idx:
                            index_array[unroll_idx].set_as(
                                indices_ub[
                                    indice_base_offset + idx_i * unrool_num + unroll_idx
                                ]
                            )

                        with self.tik_instance.for_range(0, unrool_num) as unroll_idx:
                            tik_inst.data_move(
                                res_ub[(idx_i * unrool_num + unroll_idx) * row_size],
                                cached_n_num_ub[index_array[unroll_idx] * row_size],
                                0,
                                1,
                                params_block_num,
                                0,
                    ...
                    with self.tik_instance.if_scope(idx_num_cur_batch % unrool_num > 0):
                        ...
                        with self.tik_instance.for_range(0, tail_num) as unroll_idx:
                            index_array[unroll_idx].set_as(
                                indices_ub[
                                    indice_base_offset + unrool_offset + unroll_idx
                                ]
                            )

                        with self.tik_instance.for_range(0, tail_num) as unroll_idx:
                            tik_inst.data_move(
                                res_ub[(unrool_offset + unroll_idx) * row_size],
                                cached_n_num_ub[index_array[unroll_idx] * row_size],
                                0,
                                1,
                                params_block_num,
                ...
                with self.tik_instance.else_scope():
                    with self.tik_instance.for_range(0, idx_num_cur_batch) as idx_i:
                        indices_value.set_as(indices_ub[indice_base_offset + idx_i])
                        with self.tik_instance.if_scope(
                            indices_value < cache_max_n_num
                        ):
                            tik_inst.data_move(
                                res_ub[idx_i * self.params_row],
                                cached_n_num_ub[indices_value * self.params_row],
                                0,
                                1,
                                params_block_num,
                                0,
    ...
    def compute_mode_16_aixs_eq_0(self, row_offset_tuple, cur_idx_tuple):
        ...
        ub_tensor_size = floor_align(
            (self.ub_size - Constant.RESERVED_UB_SIZE) // 6, constant.BLOCK_SIZE
        )
        ...
        row_size = tik_inst.Scalar(
            dtype=self.tiling_dtype, name="row_size", init_value=self.params_row
        )
        row_size_align = tik_inst.Scalar(dtype=self.tiling_dtype, name="row_size_align")
        row_size_align.set_as(
            align_value(self.params_row * self.params_dsize, constant.BLOCK_SIZE)
            // self.params_dsize
        )
        cache_max_n_num = tik_inst.Scalar(
            dtype=Constant.DTYPE_INT32, name="cache_max_n_num"
        ...
        self.row_num_once_ub.set_as(
            floor_align(
                ub_tensor_size,
                (row_size_align * Constant.FP16_ALIGN_NUM * self.params_dsize),
            )
        ...
        if tbe_platform.api_check_support("tik.data_move_pad", self.params_dtype):
            ...
            with self.tik_instance.for_range(0, cache_max_n_num) as idx:
                tik_inst.data_move(
                    cached_n_num_ub[idx * row_size_align],
                    self.x[idx * row_size],
                    0,
                    1,
                    ceil_value(
        ...
        def compute_for_once_ub(idx_loop_i, idx_batch_i):
            ...
            one_block_num = tik_inst.Scalar(
                dtype=Constant.DTYPE_INT32,
                name="one_block_num",
                init_value=Constant.ONE_BLOCK_FP32_NUM,
            )
            trans_type_to_fp16 = tik_inst.Scalar(
                dtype=Constant.DTYPE_INT32, name="trans_type_to_fp16", init_value=2
            )
            if self.params_dsize == 2:
                one_block_num.set_as(Constant.ONE_BLOCK_FP16_NUM)
            ...
            with self.tik_instance.if_scope(
                ...
                with self.tik_instance.for_range(0, idx_num_cur_batch) as idx_i:
                    ...
                    with self.tik_instance.if_scope(indices_value > 0):
                        tik_inst.data_move(
                            res_ub[idx_i * row_size_align],
                            cached_ub[indices_value * row_size_align],
                            0,
                            1,
                            params_block_num,
                            0,
                ...
                with tik_inst.if_scope(is_full_cache == 1):
                    ...
                    with self.tik_instance.for_range(
                        ...
                        with self.tik_instance.for_range(0, unrool_num) as unroll_idx:
                            tik_inst.data_move(
                                res_ub[
                                    (idx_i * unrool_num + unroll_idx) * row_size_align
                                ],
                                cached_n_num_ub[
                                    index_array[unroll_idx] * row_size_align
                                ],
                                0,
                                1,
                                params_block_num,
                    ...
                    with self.tik_instance.if_scope(idx_num_cur_batch % unrool_num > 0):
                        ...
                        with self.tik_instance.for_range(0, tail_num) as unroll_idx:
                            tik_inst.data_move(
                                res_ub[(unrool_offset + unroll_idx) * row_size_align],
                                cached_n_num_ub[
                                    index_array[unroll_idx] * row_size_align
                                ],
                                0,
                ...
                with self.tik_instance.else_scope():
                    with self.tik_instance.for_range(0, idx_num_cur_batch) as idx_i:
                        ...
                        with self.tik_instance.if_scope(
                            ...
                            tik_inst.data_move(
                                res_ub[idx_i * row_size_align],
                                cached_n_num_ub[indices_value * row_size_align],
                                0,
                                1,
                                params_block_num,
                                0,
    ...
    def create_pos_matrix(self, number, pos_matrix):
        ...
        with self.tik_instance.new_stmt_scope():
            add_offset = tik_inst.Tensor(
                Constant.DTYPE_INT32, (64,), name="add_offset", scope=tik.scope_ubuf
            )
            ...
            with tik_inst.if_scope(number % 64 != 0):
                tik_inst.vec_add(
                    number % 64,
                    pos_matrix[rept_time * 64],
                    pos_matrix[(rept_time - 1) * 64],
                    add_offset,
                    1,
                    8,
                    8,
    ...
    def date_move_use_unroll(
        ...
        unrool_num = 8
        index_array = self.tik_instance.ScalarArray(
            dtype=Constant.DTYPE_INT32, length=unrool_num, name="index_array"
        )
        index_pos_array = self.tik_instance.ScalarArray(
            dtype=Constant.DTYPE_INT32, length=unrool_num, name="index_pos_array"
        )
        with self.tik_instance.new_stmt_scope(disable_sync=True):
            with self.tik_instance.for_range(0, number // unrool_num) as idx_i:
                with self.tik_instance.for_range(0, unrool_num) as unroll_idx:
                    index_array[unroll_idx].set_as(
                        out_indices_cached[idx_i * unrool_num + unroll_idx]
                    )
                    index_pos_array[unroll_idx].set_as(
                        out_pos_indices_cached[idx_i * unrool_num + unroll_idx]
                    )
                with self.tik_instance.for_range(0, unrool_num) as unroll_idx:
                    tik_inst.data_move(
                        res_ub[index_pos_array[unroll_idx] * row_size_align],
                        src_ub[index_array[unroll_idx] * params_row],
                        0,
                        1,
                        params_block_num,
                        0,
            ...
            with self.tik_instance.if_scope(number % unrool_num > 0):
                ...
                with self.tik_instance.for_range(0, tail_num) as unroll_idx:
                    index_array[unroll_idx].set_as(
                        out_indices_cached[unrool_offset + unroll_idx]
                    )
                    index_pos_array[unroll_idx].set_as(
                        out_pos_indices_cached[unrool_offset + unroll_idx]
                    )
                with self.tik_instance.for_range(0, tail_num) as unroll_idx:
                    tik_inst.data_move(
                        res_ub[index_pos_array[unroll_idx] * row_size_align],
    ...
    # 'pylint: disable=too-many-arguments,huawei-too-many-arguments
    def calc_indices_for_move_date(
        self,
        indices_ub,
        indices_ub_fp,
        dup_cached_num,
        ...
        tik_inst = self.tik_instance
        with self.tik_instance.new_stmt_scope():
            ub_mask = tik_inst.Tensor(
                Constant.DTYPE_UINT32, (512,), name="ub_mask", scope=tik.scope_ubuf
            )

            rept_time = idx_num_cur_batch // 64 + 1
            tik_inst.vec_dup(64, ub_mask, 0, 8, 8)
            if cmp_mod == Constant.CMP_LESS_THAN:
                tik_inst.vec_cmpv_lt(
                    ub_mask, indices_ub_fp, dup_cached_num, rept_time, 8, 0
                )
            else:
                tik_inst.vec_cmpv_ge(
                    ub_mask, indices_ub_fp, dup_cached_num, rept_time, 8, 0
                )
    ...
    def compute_mode_17_aixs_eq_0(self, row_offset_tuple, cur_idx_tuple):
        ...
        ub_tensor_size = floor_align(
            (self.ub_size - Constant.RESERVED_UB_SIZE) // 4, constant.BLOCK_SIZE
        )
        res_ub = tik_inst.Tensor(
            self.params_dtype,
            (ub_tensor_size // self.params_dsize,),
        ...
        indices_ub = tik_inst.Tensor(
            self.indices_dtype,
            (indices_ub_size // self.indices_dsize + 64,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        indices_ub_fp = tik_inst.Tensor(
            Constant.DTYPE_FP32,
            (indices_ub_size // self.indices_dsize + 64,),
            name="indices_ub_fp",
            scope=tik.scope_ubuf,
        )
        pos_matrix = tik_inst.Tensor(
            self.indices_dtype,
            (indices_ub_size // self.indices_dsize,),
            name="pos_matrix",
            scope=tik.scope_ubuf,
        )
        out_indices = tik_inst.Tensor(
            self.indices_dtype,
            (indices_ub_size // self.indices_dsize,),
            name="out_indices_cached",
            scope=tik.scope_ubuf,
        )
        out_pos_indices = tik_inst.Tensor(
            self.indices_dtype,
            (indices_ub_size // self.indices_dsize,),
            name="out_pos_indices_cached",
            scope=tik.scope_ubuf,
        )
        dup_cached_num = tik_inst.Tensor(
            Constant.DTYPE_FP32, (64,), name="dup_cached_num", scope=tik.scope_ubuf
        )
        cached_ub = tik_inst.Tensor(
            self.params_dtype,
            (ub_tensor_size // self.params_dsize,),
        ...
        cached_n_number = tik_inst.Scalar(
            dtype=Constant.DTYPE_INT32, name="cached_n_number"
        )
        row_size = tik_inst.Scalar(
            dtype=self.tiling_dtype, name="row_size", init_value=self.params_row
        )
        ...
        with tik_inst.if_scope(cached_n_number > self.params_axis):
            ...
            is_full_cache.set_as(1)
        cached_n_number_fp = tik_inst.Scalar(
            dtype=Constant.DTYPE_FP32, name="cached_n_number_fp"
        )
        ...
        if (
            PlatformApi.get_soc_spec(PlatformApi.SHORT_SOC_VERSION) == "Ascend910B"
            and self.indices_dtype == Constant.DTYPE_INT32
        ):
        ...
        def compute_for_once_ub_not_align(idx_loop_i, idx_batch_i):
            ...
            with tik_inst.if_scope(is_full_cache == 1):
                ...
                with self.tik_instance.for_range(
                    ...
                    with self.tik_instance.for_range(0, unrool_num) as unroll_idx:
                        tik_inst.data_move(
                            res_ub[(idx_i * unrool_num + unroll_idx) * row_size_align],
                            cached_ub[index_array[unroll_idx] * row_size_align],
                            0,
                            1,
                            params_block_num,
                            0,
                ...
                with self.tik_instance.if_scope(idx_num_cur_batch % unrool_num > 0):
                    ...
                    with self.tik_instance.for_range(0, tail_num) as unroll_idx:
                        index_array[unroll_idx].set_as(
                            indices_ub[indice_base_offset + unrool_offset + unroll_idx]
                        )
            ...
            with tik_inst.else_scope():
                if (
                    PlatformApi.get_soc_spec(PlatformApi.SHORT_SOC_VERSION)
                    == "Ascend910B"
                    and self.indices_dtype == Constant.DTYPE_INT32
                ):
                    number = tik_inst.Scalar(dtype=Constant.DTYPE_UINT32, name="number")
                    self.calc_indices_for_move_date(
                        indices_ub[indice_base_offset],
                        indices_ub_fp[indice_base_offset],
                        dup_cached_num,
                        pos_matrix,
                        idx_num_cur_batch,
                        out_indices,
                        out_pos_indices,
                        Constant.CMP_LESS_THAN,
                        number,
                    )
                    self.date_move_use_unroll(
                        out_indices,
                        out_pos_indices,
                        cached_ub,
                        number,
        ...
        if tbe_platform.api_check_support("tik.data_move_pad", self.params_dtype):
            ...
            with tik_inst.for_range(0, cached_n_number) as i_index:
                self.tik_instance.data_move(
                    cached_ub[i_index * row_size_align],
                    self.x[i_index * row_size],
                    0,
                    1,
                    ceil_value(row_size * self.params_dsize, constant.BLOCK_SIZE),
                    0,
                    0,
                )
    ...
    def compute_mode_18_aixs_eq_0(self, row_offset_tuple, cur_idx_tuple):
        ...
        ub_tensor_size = floor_align(
            (self.ub_size - Constant.RESERVED_UB_SIZE) // 3, constant.BLOCK_SIZE
        )
        res_ub = tik_inst.Tensor(
            self.params_dtype,
            (ub_tensor_size // self.params_dsize,),
        ...
        indices_ub_size = (
            ub_tensor_size
            // Constant.CUT_SUB_UB_SIZE
            // Constant.SIZE_ALIGN_64
            * Constant.SIZE_ALIGN_64
        )
        indices_ub = tik_inst.Tensor(
            self.indices_dtype,
        ...
        row_size = tik_inst.Scalar(
            dtype=self.indices_dtype, name="row_size", init_value=self.params_row
        )
        row_size_align = tik_inst.Scalar(
            dtype=self.indices_dtype, name="row_size_align"
        )
        row_size_align.set_as(
            align_value(self.params_row * self.params_dsize, constant.BLOCK_SIZE)
            // self.params_dsize
        ...
        self.row_num_once_ub.set_as(
            floor_align(ub_tensor_size, (row_size_align * self.params_dsize))
            // (row_size_align * self.params_dsize)
        )
        self.row_num_once_ub.set_as(self.row_num_once_ub // 64 * 64)
        is_full_cache = tik_inst.Scalar(
        ...
        self.tik_instance.data_move(
            ...
            1,
            ceil_value(
                row_size * cached_n_number * self.params_dsize, constant.BLOCK_SIZE
            ),
            0,
            0,
        )
        ...
        with tik_instance.for_range(0, loop_num) as inner_loop_i:
            ...
            with tik_instance.new_stmt_scope(disable_sync=True):
                with tik_instance.for_range(
                    ...
                    indices_value.set_as(indices_ub[inner_indices_offset + row_i])
                    params_offset = (
                        params_offset_base + indices_value
                    ) * self.params_row
    ...
    def large_params_row(self, indices_loop_offset, batch_i, block_id, pre_i, is_last):
        ...
        with tik_instance.for_range(0, loop_num) as row_i:
            ...
            output_offset = (
                output_offset_base
                + pre_i * self.indices_row
                + (indices_loop_offset + inner_indices_offset + row_i)
            ) * self.params_row
            ...
            with tik_instance.for_range(0, self.one_row_loop) as row_inner_i:
                ...
                tik_instance.data_move(
                    res_ub,
                    self.x[params_offset + row_inner_i * half_ub_params_elem],
                    0,
                    1,
                    burst_len_sub_row,
                    0,
            ...
            with tik_instance.if_scope(self.one_row_tail > 0):
                ...
                tik_instance.data_move(
                    res_ub,
                    self.x[params_offset + (self.params_row - self.one_row_tail)],
                    0,
                    1,
                    burst_len_sub_row_last,
                    0,
    ...
    def small_indices_row(self, batch_i, block_id, is_last):
        ...
        output_offset_base = (
            batch_i * self.params_pre * self.indices_row
            + block_id * self.indices_num_each_core
        )
        indices_value = tik_instance.Scalar(
            dtype=self.indices_dtype, name="indices_value", init_value=0
        ...
        with tik_instance.for_range(0, loop_num) as inner_loop_i:
            output_offset = (
                output_offset_base + inner_loop_i * self.row_num_once_ub
            ) * self.params_row
            params_offset_base = (
                params_batch_diff * self.params_pre * self.params_axis
                + block_id * self.indices_num_each_core
            ...
            with tik_instance.for_range(0, indices_row_num, thread_num=2) as row_i:
                with tik_instance.for_range(0, self.indices_row) as ele_i:
                    ...
                    params_offset = (
                        params_offset_base + row_i * self.params_axis + indices_value
                    ) * self.params_row
                    ...
                    if (
                        ...
                        with tik_instance.for_range(0, self.params_row) as i:
                            res_ub[res_ub_offset + i].set_as(
                                params_ub[params_offset + i]
                            )
                    else:
                        temp_ub_1 = tik_instance.Tensor(
                            self.params_dtype,
        ...
        with tik_instance.if_scope(tail_num > 0):
            burst_len_res = ceil_value(
                tail_num * self.indices_row * self.params_row * self.params_dsize,
                constant.BLOCK_SIZE,
            )
            output_offset = (
                output_offset_base + loop_num * self.row_num_once_ub
            ) * self.params_row
            params_offset_base = (
                params_batch_diff * self.params_pre * self.params_axis
                + block_id * self.indices_num_each_core
            ...
            tail_elem = (
                tail_num * self.indices_row * self.params_row
            ) % self.block_elem
            with tik_instance.if_scope(tik.all(tail_elem != 0, burst_len_res > 1)):
                block_ub = tik_instance.Tensor(
                    self.params_dtype,
    ...
    def small_params_row(self, block_id, is_last):
        ...
        with tik_instance.for_range(0, loop_num) as inner_loop_i:
            ...
            with tik_instance.for_range(0, batch_num) as batch_i:
                params_offset_base = (
                    (params_batch_diff + inner_loop_i * batch_num + batch_i)
                    * self.params_pre
                    * self.params_axis
                )
                indices_num_offset = (
                    batch_base + inner_loop_i * batch_num + batch_i
                ) * self.indices_row
                tik_instance.data_move(
                    self.buffers.get("indices_ub"),
                    self.indices[indices_num_offset],
                    0,
                    1,
                    ceil_value(
                        self.indices_row * self.indices_dsize, constant.BLOCK_SIZE
                    ),
                    0,
                    0,
                )
                with tik_instance.for_range(0, self.params_pre) as pre_i:
                    with tik_instance.for_range(0, self.indices_row) as ele_i:
                        ...
                        res_ub_offset = (
                            (batch_i * self.params_pre + pre_i) * self.indices_row
                            + ele_i
                        ) * self.params_row
        ...
        with tik_instance.if_scope(tail_num > 0):
            ...
            with tik_instance.for_range(0, tail_num) as batch_i:
                params_offset_base = (
                    (params_batch_diff + loop_num * batch_num + batch_i)
                    * self.params_pre
                    * self.params_axis
                )
                indices_num_offset = (
                    batch_base + loop_num * batch_num + batch_i
                ) * self.indices_row
                tik_instance.data_move(
                    self.buffers.get("indices_ub"),
                    self.indices[indices_num_offset],
            ...
            tail_elem = (
                tail_num * self.params_pre * self.indices_row * self.params_row
            ) % self.block_elem
            with tik_instance.if_scope(tik.all(tail_elem != 0, burst_len_res > 1)):
                block_ub = tik_instance.Tensor(
                    self.params_dtype,
    ...
    def inner_loop_with_batch_dims(
        self, indices_loop_offset, batch_i, block_id, pre_i, is_last
    ):
        """
        ...
        if self.cached_types.get("aligned_types_params") == Constant.MODE_LESS_THAN_32B:
            self.params_row_less_than_32b(
                indices_loop_offset, batch_i, block_id, pre_i, is_last
            )
        elif self.cached_types.get("aligned_types_params") == Constant.MODE_ALIGNED:
            self.params_row_aligned(
                indices_loop_offset, batch_i, block_id, pre_i, is_last
            )
        elif (
            self.cached_types.get("aligned_types_params") == Constant.MODE_MORE_THAN_32B
        ):
            self.params_row_more_than_32b(
                indices_loop_offset, batch_i, block_id, pre_i, is_last
            )
        else:
            self.large_params_row(
                indices_loop_offset, batch_i, block_id, pre_i, is_last
            )
    ...
    def process_large_indices_row(self, indices_num_offset, batch_i, block_id, pre_i):
        ...
        with tik_instance.for_range(0, self.indices_loop_num) as indices_loop_i:
            ...
            self.inner_loop_with_batch_dims(
                indices_loop_offset, batch_i, block_id, pre_i, 0
            )
        with tik_instance.if_scope(self.indices_row_num_last > 0):
            ...
            self.inner_loop_with_batch_dims(
                indices_loop_offset, batch_i, block_id, pre_i, 1
            )
    ...
    def compute_with_batch_dims(self, avl_ub_size, block_id):
        ...
        """
        tik_instance = self.tik_instance
        self.buffers["indices_ub"] = tik_instance.Tensor(
            self.indices_dtype,
            ((avl_ub_size + constant.BLOCK_SIZE) // self.indices_dsize,),
            name="indices_ub",
            scope=tik.scope_ubuf,
        )
        self.buffers["res_ub"] = tik_instance.Tensor(
            self.params_dtype,
        ...
        if self.cached_types.get("cached_types_indices") == Constant.INDICES_CACHED_ALL:
            tik_instance.data_move(
                ...
                1,
                ceil_value(
                    self.indices_num_each_core * self.indices_dsize, constant.BLOCK_SIZE
                ),
                0,
                0,
            )
        ...
        if self.cached_types.get("cached_types_params") == Constant.PARAMS_CACHED_UB:
            self.buffers["params_ub"] = tik_instance.Tensor(
                self.params_dtype,
                (Constant.CACHE_UB_SIZE // self.params_dsize,),
                name="params_ub",
                scope=tik.scope_ubuf,
            ...
            if (
                self.cached_types.get("cached_types_indices")
                != Constant.INDICES_SMALL_ROW
            ):
                ...
                params_offset = 0
            tik_instance.data_move(
                self.buffers.get("params_ub"),
                self.x[params_offset],
                0,
                1,
                ceil_value(self.params_total, self.block_elem),
                0,
        ...
        if (
            self.cached_types.get("aligned_types_params")
            == Constant.MODE_SMALL_PARAMS_ROW
        ):
            with tik_instance.if_scope(block_id < self.tail_process_core):
                ...
                if (
                    ...
                    if (
                        ...
                        tik_instance.data_move(
                            ...
                            1,
                            ceil_value(
                                self.indices_row * self.indices_dsize,
                                constant.BLOCK_SIZE,
                            ),
                            0,
                            0,
                    ...
                    indices_num_offset = batch_i * self.indices_row
                    self.process_large_indices_row(
                        indices_num_offset, batch_i, block_id, pre_i
                    )
        with tik_instance.if_scope(
            tik.all(
                self.params_batch_remaining > 0, block_id < self.params_batch_remaining
            )
        ):
            self.is_remaining = 1
            ...
            if (
                ...
                params_offset = (
                    self.params_total * self.need_core_num + block_id * num_per_batch
                )
                tik_instance.data_move(
                    self.buffers.get("params_ub"),
                    self.x[params_offset],
                    0,
                    1,
                    ceil_value(num_per_batch, self.block_elem),
                    0,
                    0,
                )
            else:
    ...
    def add_gather_compile_info(self):
        ...
        add compile info
        """
        self.x = self.tik_instance.Tensor(
            self.params_dtype, self.x_shape, name="x", scope=tik.scope_gm
        )
        self.indices = self.tik_instance.Tensor(
            self.indices_dtype, self.indices_shape, name="indices", scope=tik.scope_gm
        )
        ...
        self.y = self.tik_instance.Tensor(
            self.y_dtype, shape=self.y_shape, name="y", scope=tik.scope_gm
        )
        ...
        if self.axis:
            self.tik_instance.set_tiling_params(
                [self.x, self.indices, self.axis], self.tiling_gm
            )
        else:
    ...
    def gather_v2_compute(self):
        ...
        compile info
        """
        self.axis = self.tik_instance.Tensor(
            self.axis_dtype, self.axis_shape, name="axis", scope=tik.scope_gm
        )
    ...
    def _set_mode_paras(
        self, aligned_types_params, cached_types_params, cached_types_indices
    ):
...
def gather_v2_tik(
    ...
    obj = GatherV2(
        x, indices, axis, y, batch_dims, is_preprocessed, kernel_name, impl_mode
    )
...
def gather_v2_compute(
    ...
    support_out_of_bound_index = (
        True if impl_mode == "support_out_of_bound_index" else False
    )

    res = tbe.gather(
        x, indices, axis, batch_dims, negative_index_support, support_out_of_bound_index
    )
...
def gather_v2_dsl(
    ...
    ins = classify(
        [x, indices, real_axis, batch_dims],
        OpPatternMode.GATHER,
        {"gather_type": "gather"},
    )
    schedules, tensors = [], []
    for shape_x, shape_indices, shape_axis, batch_dims_input in ins:
        with tbe.compute():
            x_var, indices_var, axis_dim, batch_dims = shape_util.variable_shape(
                [shape_x, shape_indices, shape_axis, batch_dims_input], "gather"
            )
            x_tensor = tvm.placeholder(x_var, name="x", dtype=dtype_x)
            indices_tensor = tvm.placeholder(
                indices_var, name="indices", dtype=dtype_indices
            )
            axis_tensor = tvm.placeholder([1], name="axis", dtype=dtype_axis)
            res = gather_v2_compute(
                x_tensor,
                indices_tensor,
                axis_dim,
                y,
...
def gather_v2(
    ...
    check_op_impl_mode(
        impl_mode,
        [
            OpImplMode.HIGH_PERFORMANCE,
            OpImplMode.HIGH_PRECISION,
            OpImplMode.SUPPORT_OUT_OF_BOUND_INDEX,
        ],
        kernel_name,
    )
    ...
    if x.get("dtype").lower() == "bfloat16":
        ...
        if (is_preprocessed is False and axis.__contains__("const_value") is False) or (
            is_preprocessed is False
            and axis.__contains__("const_value") is True
            and axis["const_value"][0] == 0
        ):
            gather_v2_tik(
                x, indices, axis, y, batch_dims, is_preprocessed, kernel_name, impl_mode
            )
