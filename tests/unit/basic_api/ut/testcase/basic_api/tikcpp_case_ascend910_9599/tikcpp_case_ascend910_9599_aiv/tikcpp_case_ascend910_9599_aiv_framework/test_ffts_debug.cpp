...
TEST_F(TestFfts, MIXFftsCaseMode0)
{
    ...
    AscendC::set_ffts_base_addr(reinterpret_cast<uint64_t>(ffts_addr));
    AscendC::CheckBlockdimForFfts(1);
    AscendC::ffts_cross_core_sync(static_cast<pipe_t>(0), 0x301);
    EXPECT_EQ(AscendC::g_syncCounterFfts[72][3], 0);
    EXPECT_EQ(AscendC::g_syncCounterEachcore[72][3], 1);
    AscendC::wait_flag_dev(PIPE_MTE3, 0x3l);
...
TEST_F(TestFfts, AICFftsCaseMode0)
{
    ...
    EXPECT_EQ(AscendC::g_syncCounterFfts[72][3], 0);
    EXPECT_EQ(AscendC::g_syncCounterEachcore[72][3], 1);
    AscendC::wait_flag_dev(PIPE_MTE3, static_cast<uint16_t>(0x3));
    EXPECT_EQ(AscendC::g_syncCounterEachcore[72][3], 0);
    AscendC::GmFree((void*)AscendC::g_syncCounterFfts);
...
TEST_F(TestFfts, FftsCaseMode1)
{
    ...
    g_coreType = 2;
    sub_block_idx = 0;
    AscendC::ffts_cross_core_sync(static_cast<pipe_t>(0), 0x311);
...
TEST_F(TestFfts, FftsCaseMode2)
{
    ...
    AscendC::CheckBlockdimForFfts(1);
    AscendC::ffts_cross_core_sync(static_cast<pipe_t>(0), 0x321);
    EXPECT_EQ(AscendC::g_syncCounterEachcore[0][3], 1);
    EXPECT_EQ(AscendC::g_syncCounterEachcore[1][3], 1);
    g_coreType = 2;
    sub_block_idx = 0;
