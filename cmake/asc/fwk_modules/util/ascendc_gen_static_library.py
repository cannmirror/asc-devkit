...
def args_parse():
    ...
    parser.add_argument("-p", "--package-name", default="", help="package name.")
...
def unpack_kernel_library(kernel_libs_dir, objects, tmp_obj_dir):
    kernel_libs_parts = []
    opregistry_pattern = kernel_libs_dir + "/**/libopregistry.a"
    opregistry_file_path = glob.glob(opregistry_pattern, recursive=True)
    if len(opregistry_file_path) != 1:
        raise RuntimeError(
            f"libopregistry.a not exist in {kernel_libs_dir}, please check and reset."
        )
    kernel_libs_parts.append(opregistry_file_path[0])

    kernels_pattern = kernel_libs_dir + "/**/libkernels.a"
    kernels_file_path = glob.glob(kernels_pattern, recursive=True)
    if len(kernels_file_path) != 1:
        raise RuntimeError(
            f"libkernels.a not exist in {kernel_libs_dir}, please check and reset."
        )
...
def pack_static_library(output_file, objects, tmp_obj_dir, package_name):
    ...
    for index in range(0, len(objects), step_size):
        tmp_file = os.path.join(
            output_dir_path, package_name + f"_ascendc_tmp{index}.o"
        )
        tmp_files.append(tmp_file)
        run_command(
            ["ld", "-r", "-o", tmp_file] + objects[index : index + step_size],
            cwd=tmp_obj_dir,
        )
...
def main():
    try:
        ...
        pack_static_library(
            args.output_file, objects, args.tmp_obj_dir, args.package_name
        )
