...
function install_patches() {
    ...
    if [ "${mode}" == "install" ];then
        ...
        if [ ! -d "${install_manifest_dir}" ];then
    ...
    # 获取源目录中所有文件和软连接
    local file_list=$(find ${src_dir} -mindepth 1 \( -type f -o -type l \) | sort)
    for file in ${file_list}; do
        ...
        if [ "${mode}" == "install" ];then
            ...
            if [ -e "${actual_file}" ];then
                if [ -d "${actual_file}" ]; then
                    chmod -R u+w "${actual_file}" 2>/dev/null
                else
                    chmod u+w "${actual_file}" 2>/dev/null
                fi
