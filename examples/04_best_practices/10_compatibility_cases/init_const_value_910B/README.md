...
## 目录结构介绍
```
├── init_const_value_910B
│   ├── scripts
│   │   ├── gen_data.py         // 输入数据和真值数据生成脚本
│   │   └── verify_result.py    // 验证输出数据和真值数据是否一致的验证脚本
│   ├── CMakeLists.txt          // 编译工程文件
│   └── init_const_value.asc    // AscendC算子实现 & 调用样例
```
## 算子描述
- 算子功能：  
...
- 算子规格：
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">init_const_value</td></tr>
  </table>
- 算子实现：  
  调用InitConstValue实现对L0A Buffer/L0B Buffer的初始化。
