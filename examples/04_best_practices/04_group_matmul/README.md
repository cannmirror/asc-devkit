...
本样例介绍QuantGroupMatmul算子的高性能实现，使用<<<>>>内核调用符来完成算子核函数在NPU侧运行验证的基础流程。
...
│   └── quant_group_matmul_custom.asc         // AscendC算子实现 & 调用样例
```
...
- 配置环境变量  
  ...
  ```bash
  cd ${git_clone_path}/examples/04_best_practices/04_group_matmul
  ```
