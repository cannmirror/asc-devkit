...
| [optimize_vf_fusion](./optimize_vf_fusion) | 本样例演示了SIMD场景下，基于RegBase编程范式，通过VF融合优化算子代码实现。样例中定义了两个VF函数，其中，DivVF函数实现对输入向量中的每个数取倒数操作，AddVF函数实现对输入向量的加一操作，DivVF和AddVF会被编译器融合为一个VF函数。 |
