## 概述
本样例在[10_matmul_frameworklaunch](../../../0_introduction/10_matmul_frameworklaunch)的单核场景样例的基础上，添加了printf调测功能，介绍单算子工程、单算子调用。

## 目录结构介绍
```
├── FrameworkLaunch                  // 使用框架调用的方式调用Matmul自定义算子，并添加printf调测功能。
...
创建完自定义算子工程后，开发者重点需要完成算子host和kernel文件的功能开发。为简化样例运行流程，本样例已在MatmulCustom目录中准备好了必要的算子实现，install.sh脚本会创建一个CustomOp目录，并将算子实现文件复制到对应目录下，再编译算子。
...
### 2. 生成自定义算子工程，复制host和kernel实现并编译算子<a name="operatorcompile"></a>
  - 切换到msOpGen脚本install.sh所在目录
    ...
    cd ${git_clone_path}/samples/operator/ascendc/1_utilities/0_printf/FrameworkLaunch
    ```
