# VectorAdd算子直调样例
## 概述
本样例介绍Add算子的核函数直调方法，支持的数据类型有：bfloat16_t/int8_t/float/half/int16_t/int32_t，算子支持多核运行、支持核间数据均分或不均分场景并且支持尾块处理。
## 支持的AI处理器
- Ascend 910D
## 目录结构介绍
```
...
- 配置环境变量  
  ...
  ```bash
  cd ${git_clone_path}/examples/00_introduction/11_vectoradd/add_tiling
  ```
