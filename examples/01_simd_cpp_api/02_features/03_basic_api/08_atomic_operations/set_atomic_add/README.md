...
本样例基于 `SetAtomicAdd` 原子操作接口，介绍数据从 VECOUT 向 GM 搬运时，原子累加的实现流程，且接口支持通过模板参数配置不同运算数据类型。需要注意，在调用 `SetAtomicAdd` 完成相关操作后，要调用 `DisableDmaAtomic()` 关闭原子模式，防止后续其他计算受影响。

> **接口提示：** 除本样例使用的 `SetAtomicAdd` 接口外，Ascend C 还提供了 `SetAtomicMin`、`SetAtomicMax` 接口用于配置 VECOUT 到 GM 的传输规则：在数据拷贝时与 GM 中已有数据执行原子比较，最终将两者的最小值/最大值写入 GM。`SetAtomicMin`、`SetAtomicMax` 的调用方式与 `SetAtomicAdd` 一致，只需替换函数名即可切换。
