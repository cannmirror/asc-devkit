...
This sample demonstrates creating a vector index with a specified starting value using the CreateVecIndex API in data filling scenarios, for generating consecutive integer sequences. The CreateVecIndex API supports creating incrementing index vectors starting from a specified value, commonly used in scenarios requiring sequence indices, such as generating subscripts for array elements or creating step sequences.
...
│   └── create_vec_index.asc    // Ascend C sample implementation & invocation example
```

## Sample Description

- Sample Function:  
  This sample demonstrates using the CreateVecIndex API to create a vector index with a specified starting value for generating consecutive integer sequences. The CreateVecIndex API is suitable for scenarios requiring sequence indices, such as generating subscripts for array elements, creating step sequences, or creating loop indices. The startIndex parameter specifies the starting value, and the count parameter specifies the number of elements to generate. For detailed API documentation, refer to [CreateVecIndex API Documentation](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/atlasascendc_api_07_0090.html).

- Sample Specifications:  
  <table border="2" align="center">
  <caption>Table 1: Sample Input/Output Specifications</caption>
  <tr><td rowspan="1" align="center">Sample Type</td><td colspan="4" align="center">CreateVecIndex</td></tr>
  ...
  <tr><td rowspan="1" align="center">Kernel Name</td><td colspan="4" align="center">testCreateVecIndex</td></tr>
  </table>

- Sample Implementation:  
  This sample implements a CreateVecIndex example with fixed shape: output y[1,128], generating 128 consecutive integer indices starting from 0 (0, 1, 2, ..., 127).
  
  CreateVecIndex API Parameters:
  - dst: Destination operand for storing the generated index vector
  - startIndex: Starting value of the index, 0 in this sample
  - count: Number of elements to compute, 128 in this sample

  - Kernel Implementation  
    - Call Duplicate interface to initialize UB (Unified Buffer) buffer to 0
    - Call CreateVecIndex interface to generate consecutive index vector starting from 0
