...
This sample demonstrates the assert functionality. If the condition inside assert evaluates to false, the assert condition is output along with the input information.
...
│   └── assert.asc              // Ascend C sample implementation & invocation sample
```
...
- Sample Function:
  ...
  This sample implements Matmul computation using high-level APIs with assert functionality added. Note that the assert interface has an impact on runtime performance.
...
- Sample Specifications:
  ...
  Sample parameters: M = 512, N = 1024, K = 512. Specifications are shown in the table below:
  <table>
