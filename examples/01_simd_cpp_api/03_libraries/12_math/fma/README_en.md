...
This example implements the function of calculating the sum of two inputs multiplied with a third input using the Fma high-level API.
...
- Example Function:
  Performs element-wise calculation of the result of multiplying two inputs and adding a third input.
...
- Example Implementation:
  This example implements the fma_custom example with a fixed shape of inputs src0[1, 128], src1[1, 128], src2[1, 128], and output dst[1, 128].

  - Kernel Implementation

    Uses the Fma high-level API to calculate src0 * src1 + src2. You can choose to use a temporary buffer.

  - Tiling Implementation

    On the host side, uses GetFmaMaxMinTmpSize to get the maximum and minimum temporary space required for the Fma interface calculation.
