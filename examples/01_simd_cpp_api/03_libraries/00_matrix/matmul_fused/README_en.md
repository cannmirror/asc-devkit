...
This example demonstrates AIC and AIV split-core fused programming implementation, mainly introducing the MIX mode of the Matmul high-level API, where the Matmul API internally automatically controls inter-core synchronization between AIC and AIV.
...
│   └── matmul_fused.asc        // Ascend C example implementation & call example
```

## Example Description

- Example Function:
  This example creates a MIX mode Matmul object, implements Matmul computation on the AIC core, and outputs intermediate results to VECIN (Unified Buffer). After the AIC core computation is complete, synchronization is controlled through Tque's EnQue/DeQue interfaces, then LeakyRelu computation is performed on the intermediate results in Unified Buffer on the AIV core.
