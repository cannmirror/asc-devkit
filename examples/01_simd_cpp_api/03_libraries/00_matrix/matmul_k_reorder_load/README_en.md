# Matmul K-axis Staggered Load Data Direct Call Example
## Overview
This is a Matmul example that enables the K-axis staggered load data feature to reduce the probability of multi-core Global Memory address access conflicts.

When executing Matmul computation, if multiple cores have the same left matrix or right matrix stored in Global Memory, multiple cores typically access the same address simultaneously to load matrix data, causing same-address access conflicts that affect performance. After enabling this parameter, during multi-core Matmul execution, different Global Memory addresses will be accessed at the same time as much as possible, reducing the probability of address access conflicts and improving performance.

It is recommended to enable this parameter when the K-axis is large (K-axis should be at least 4096) and neither the left nor right matrix is fully loaded.
...
│   └── matmul_k_reorder_load.asc              // Ascend C example implementation & call example
```
## Example Description
- Example Function:
  This example enables K-axis staggered load data by setting the MatmulConfig parameter enableKdimReorderLoad when calling the Matmul high-level API. This parameter feature only supports the MDL template.

- Example Specifications:
    In this example: M = 768, N = 2048, K = 6144.
  <table>
  ...
  <tr><td rowspan="1" align="center">Kernel Function Name</td><td colspan="5" align="center">matmul_k_reorder_load_custom</td></tr>
  </table>
- Example Implementation:
 
  - Kernel Key Steps
    - Create Matmul object: Call the GetMDLConfig interface to set the enableKdimReorderLoad parameter to true, obtain the custom template CFG_MDL_REORDER, and create a Matmul object by passing the template parameter.
      ```cpp
