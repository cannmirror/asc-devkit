# Gather算子直调样例
## 概述
本样例基于Gather实现对于给定的输入张量和一个地址偏移量，根据偏移地址将输入张量按元素收集到结果张量中。
本样例通过Ascend C编程语言实现了Gather算子，使用<<<>>>内核调用符来完成算子核函数在NPU侧运行验证的基础流程，给出了对应的端到端实现。
...
│   └── gather.asc              // Ascend C算子实现 & 调用样例
```

## 算子描述
- 算子功能：  
  Gather算子实现了对于给定的输入张量和一个地址偏移量，根据偏移地址将输入张量按元素收集到结果张量，并返回结果的功能。
...
- 算子实现：  
  本样例中实现的是固定shape为128的Gather算子。
  - kernel实现   
    计算逻辑是：Ascend C提供的矢量计算接口的操作元素都为LocalTensor，输入数据需要先搬运进片上存储，然后使用计算接口完成根据偏移地址将输入张量按元素收集到结果张量的操作，得到最终结果，再搬出到外部存储上。   

    Gather算子的实现流程分为3个基本任务：CopyIn，Compute，CopyOut。CopyIn任务负责将Global Memory上的输入Tensor xGm，yGm搬运至Local Memory，分别存储在xLocal，yLocal，Compute任务负责对xLocal，yLocal执行相关操作，计算结果存储在outLocal中，CopyOut任务负责将输出数据从outLocal搬运至Global Memory上的输出Tensor outGm中。
