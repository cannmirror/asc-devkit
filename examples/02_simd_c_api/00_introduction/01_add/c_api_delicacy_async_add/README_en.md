# Add Operator Sample Using C_API (Asynchronous Scenario, Manual Pipeline Synchronization Control)

## Summary

This sample uses C_API interfaces to implement the Add operator sample, based on asynchronous data movement, computation interfaces, and manually added synchronization instructions.
...
│   └── c_api_add.asc       // C_API operator implementation & call sample
```
...
- Operator Implementation: 
  ...
  C_API input data needs to be moved to on-chip storage first, then computation interfaces are used to complete the addition of two input parameters to obtain the final result, which is then moved to external storage. During computation, asc_sync_notify and asc_sync_wait are used to control synchronization between pipelines.
