# VectorAdd算子直调样例
## 概述
本样例介绍Add算子的核函数直调方法，算子支持单核运行，不同流水线之间使用VEC_LOAD和VEC_STORE同步。
## 支持的AI处理器
- Ascend 910D
## 目录结构介绍
```
...
- 算子实现：  
  ...
  使用SIMD VF函数编写不同流水的同步指令，微指令SIMD VF函数内不同流水线之间的同步指令使用LocalMemBar来表示，目的流水线将等待源流水线上所有指令完成才能进行执行。
...
- 配置环境变量  
  ...
  ```bash
  cd ${git_clone_path}/examples/00_features/04_add_simd
  ```
